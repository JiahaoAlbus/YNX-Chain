...
func (s *Server) routes() {
	...
	s.mux.HandleFunc("GET /trust/trace/{address}", s.handleProxy)
	s.mux.HandleFunc("POST /trust/labels", s.handleProxy)
	s.mux.HandleFunc("POST /trust/evidence", s.handleProxy)
	s.mux.HandleFunc("GET /trust/evidence/{id}", s.handleProxy)
	s.mux.HandleFunc("POST /trust/appeals", s.handleProxy)
	s.mux.HandleFunc("GET /trust/appeals/{id}", s.handleProxy)
	s.mux.HandleFunc("POST /trust/appeals/{id}/resolve", s.handleProxy)
	s.mux.HandleFunc("POST /trust/tracking-reviews", s.handleProxy)
	s.mux.HandleFunc("GET /trust/tracking-reviews/{id}", s.handleProxy)
	s.mux.HandleFunc("POST /governance/requests", s.handleProxy)
	s.mux.HandleFunc("GET /governance/requests/{id}", s.handleProxy)
	s.mux.HandleFunc("POST /governance/requests/{id}/review", s.handleProxy)
	s.mux.HandleFunc("POST /governance/requests/{id}/reject", s.handleProxy)
	s.mux.HandleFunc("GET /governance/request-validity-rules", s.handleProxy)
	s.mux.HandleFunc("GET /governance/transparency", s.handleProxy)
}
...
func (s *Server) handleMetrics(w http.ResponseWriter, r *http.Request) {
	...
	labels := `service="ynx-trustd",native_symbol="YNXT"`
	_, _ = fmt.Fprintf(w, "ynx_trust_gateway_requests_total{%s} %d\n", labels, health.Requests)
	_, _ = fmt.Fprintf(w, "ynx_trust_gateway_successes_total{%s} %d\n", labels, health.Successes)
	_, _ = fmt.Fprintf(w, "ynx_trust_gateway_denied_total{%s} %d\n", labels, health.Denied)
	_, _ = fmt.Fprintf(w, "ynx_trust_gateway_errors_total{%s} %d\n", labels, health.Errors)
	_, _ = fmt.Fprintf(w, "ynx_trust_gateway_audit_errors_total{%s} %d\n", labels, health.AuditErrors)
	_, _ = fmt.Fprintf(w, "ynx_trust_gateway_active_requests{%s} %d\n", labels, health.Active)
}
...
func (s *Server) handleProxy(w http.ResponseWriter, r *http.Request) {
	...
	if !s.service.Allow(r.RemoteAddr, accessKey, time.Now().UTC()) {
		...
		s.finish(w, r, requestID, nil, http.StatusTooManyRequests, "rate_limited", "Trust API rate limit exceeded")
		return
	}
	var body []byte
	...
	if err := s.service.Audit(entry); err != nil {
		s.service.RejectRequest()
		writeError(w, http.StatusInternalServerError, requestID, "Trust audit storage unavailable")
		return
	}
...
func (s *Server) authorize(w http.ResponseWriter, r *http.Request) (string, string, bool) {
	...
	accessKey := r.Header.Get("X-YNX-Trust-Key")
	if accessKey == "" {
	...
	if !s.service.Authorized(accessKey) {
		...
		writeError(w, http.StatusUnauthorized, requestID, "valid Trust API key required")
