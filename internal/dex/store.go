...
type storePayload struct {
	...
	LPProtectionEvents []LPProtectionEvent `json:"lpProtectionEvents,omitempty"`
}
...
func OpenStore(path string, secret []byte) (*Store, error) {
	...
	for _, event := range envelope.Payload.LPProtectionEvents {
		if err := event.Validate(); err != nil {
			return nil, fmt.Errorf("invalid persisted LP protection event: %w", err)
		}
	}
...
func (store *Store) Append(event Event) (bool, error) {
	...
	next.Sequence = uint64(len(next.Events) + len(next.FairFlowEvents) + len(next.LPProtectionEvents))
...
func (store *Store) AppendLPProtection(event LPProtectionEvent) (bool, error) {
	if err := event.Validate(); err != nil {
		return false, err
	}
	store.mu.Lock()
	defer store.mu.Unlock()
	for _, existing := range store.state.LPProtectionEvents {
		if existing.ID == event.ID {
			left, _ := json.Marshal(existing)
			right, _ := json.Marshal(event)
			if bytes.Equal(left, right) {
			...
			return false, errors.New("LP protection event replay conflicts with persisted event")
		}
		if existing.BlockNumber == event.BlockNumber && existing.LogIndex == event.LogIndex && existing.BlockHash != event.BlockHash {
			return false, errors.New("chain reorganization conflict requires explicit recovery")
		}
	...
	next := store.state
	next.LPProtectionEvents = append(append([]LPProtectionEvent(nil), store.state.LPProtectionEvents...), event)
	sort.Slice(next.LPProtectionEvents, func(i, j int) bool {
		if next.LPProtectionEvents[i].BlockNumber == next.LPProtectionEvents[j].BlockNumber {
			return next.LPProtectionEvents[i].LogIndex < next.LPProtectionEvents[j].LogIndex
		}
		return next.LPProtectionEvents[i].BlockNumber < next.LPProtectionEvents[j].BlockNumber
	})
	next.Sequence = uint64(len(next.Events) + len(next.FairFlowEvents) + len(next.LPProtectionEvents))
	if err := store.persist(next); err != nil {
		return false, err
...
func (store *Store) LPProtectionEvents(lpProtection, pool string) []LPProtectionEvent {
	store.mu.RLock()
	defer store.mu.RUnlock()
	result := make([]LPProtectionEvent, 0)
	for _, event := range store.state.LPProtectionEvents {
		if (lpProtection == "" || strings.EqualFold(event.LPProtection, lpProtection)) && (pool == "" || strings.EqualFold(event.Pool, pool)) {
			result = append(result, event)
		}
	}
	return result
...
func (store *Store) Rewind(fromBlock uint64) error {
	...
	next.LPProtectionEvents = make([]LPProtectionEvent, 0, len(store.state.LPProtectionEvents))
	for _, event := range store.state.LPProtectionEvents {
		if event.BlockNumber < fromBlock {
			next.LPProtectionEvents = append(next.LPProtectionEvents, event)
		}
	}
	next.Sequence = uint64(len(next.Events) + len(next.FairFlowEvents) + len(next.LPProtectionEvents))
	if err := store.persist(next); err != nil {
...
func (store *Store) Analytics() Analytics {
	...
	result.LPProtectionEvents = len(store.LPProtectionEvents("", ""))
	return result
}
