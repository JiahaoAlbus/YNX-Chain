  ...
  <script>
    ...
    function renderPortalRoute(route) {
      ...
      if (route === 'tokens') {
        ...
        set('Tokens','Only current 6423-native token facts and indexed transfer records are displayed. This portal does not invent market data or token contracts.','<div class="route-grid two">' + portalPanel('YNXT',body,'Native token') + portalPanel('Verified token registry','<div class="portal-list"><button type="button" data-search="YNXT">YNXT <small>Native asset · verified metadata</small></button></div>' + unavailable('No public verified token-list endpoint is configured, so unverified assets are not listed or promoted.'),'YNXT only') + '</div><section class="section"><div class="route-grid two">' + portalPanel('YNXT transfer records',transferTable,'Current indexed window') + portalPanel('YNXT holders',holders,'Unavailable') + '</div></section>'); return;
      }
      if (route === 'data') {
