...
<body>
  ...
  <div class="drawer-backdrop" id="detailBackdrop" aria-hidden="true">
    <aside class="drawer" id="detailDrawer" role="dialog" aria-modal="true" aria-labelledby="detailTitle">
      <div class="drawer-head"><div><div class="drawer-kicker" id="detailKicker">Chain detail</div><h2 id="detailTitle">Loading</h2></div><button class="icon-button" id="detailClose" type="button" data-i18n-aria="closeDetail" aria-label="Close detail panel">&times;</button></div>
  ...
  <script>
    ...
    const documentationUI = {
      ...
      'zh-CN':{using:'使用本门户',search:'搜索和浏览区块、交易与账户',identifiers:'使用已验证的测试网标识',downloads:'查看下载证据要求',policy:'状态政策',policyCopy:'没有已验证公开证据的产品、下载或服务会显示为暂不可用，而不会链接到占位页面。'},
      'zh-TW':{using:'使用此入口',search:'搜尋和瀏覽區塊、交易與帳戶',identifiers:'使用已驗證的測試網識別資料',downloads:'查看下載證據要求',policy:'狀態政策',policyCopy:'沒有已驗證公開證據的產品、下載或服務會顯示為暫時不可用，而不會連結到預留頁面。'},
    ...
    function renderIntelligence(validatorData, resources) {
      ...
      $('validatorsBody').innerHTML = validators.length ? validators.map(validator => {
        ...
        return '<tr><td><strong>' + escapeHTML(validator.moniker || compact(validator.address)) + '</strong><span class="mono hash muted" title="' + escapeHTML(validator.address) + '">' + escapeHTML(compact(validator.address,12,7)) + '</span></td><td>' + escapeHTML(validator.role || i('validator')) + '</td><td><span class="validator-state' + (ready ? '' : ' offline') + '">' + escapeHTML(status) + '</span></td><td class="mono">' + escapeHTML(number(validator.votingPower)) + '</td><td class="mono">' + escapeHTML(number(validator.latestHeight)) + '</td></tr>';
    ...
    function renderDashboard(summary, blocks, transactions, validatorData, resources, source = 'Live stream') {
      ...
      if (['tokens','data','developers'].includes(activeRoute)) renderPortalRoute(activeRoute);
    ...
    function renderPortalRoute(route) {
      ...
      const chainFacts = summary ? [[c('cosmos'),'ynx_6423-1'],[c('numeric'),String(summary.network?.chainId || 6423)],[c('evm'),String(summary.wallet?.chainIdHex || '0x1917')],[c('nativeAsset'),String(summary.nativeSymbol || 'YNXT')],[c('dataSource'),sourceTruth(summary.truthfulStatus)]] : [];
      if (route === 'blockchain') {
    ...
    $('metamaskButton').onclick = async () => {
      ...
      } catch (error) { $('resultPanel').classList.add('visible'); $('resultTitle').textContent = wm('requestDeclined'); $('resultBody').innerHTML = '<div class="result-error">' + escapeHTML(error.message) + '</div>'; }
    };
    ...
    $('walletConnectButton').onclick = async () => {
      ...
      try {
        ...
        updateWalletButton();
        showWalletSession();
      } catch (error) { showPortalNotice(wm('connectionNotApproved',{error:error?.message || wm('requestDeclined')})); }
    };
