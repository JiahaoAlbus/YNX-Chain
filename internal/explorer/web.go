  ...
  <script>
    ...
    function renderPortalRoute(route) {
      ...
      if (route === 'blockchain') {
        ...
        set(r('blockchain'),'Browse verified indexed blocks and transactions. Record details open in this tab; pagination never requests an unverified history source.','<div class="route-grid two">' + portalPanel(r('networkStatus'),summary ? facts(chainFacts) : unavailable('The 6423 RPC and indexer have not returned a verified snapshot yet.'),summary?.ok ? 'RPC + indexer backed' : r('unavailable')) + portalPanel(r('contracts'),contracts,r('unavailable')) + '</div><section class="section">' + portalPanel(r('blocks'),blockTable,'Verified local-index pagination') + '</section><section class="section">' + portalPanel(r('transactions'),txTable,'Verified local-index pagination') + '</section>');
      ...
      if (route === 'tokens') {
        ...
        set(r('tokens'),'Only current 6423-native token facts and indexed transfer records are displayed. This portal does not invent market data or token contracts.','<div class="route-grid two">' + portalPanel('YNXT',body,'Native token') + portalPanel(t('tokenRegistry'),'<div class="portal-list"><button type="button" data-search="YNXT">YNXT <small>Native asset · verified metadata</small></button></div>' + unavailable('No public verified token-list endpoint is configured, so unverified assets are not listed or promoted.'),'YNXT only') + '</div><section class="section"><div class="route-grid two">' + portalPanel('YNXT transfer records',transferTable,'Current indexed window') + portalPanel('YNXT holders',holders,r('unavailable')) + '</div></section>'); return;
      }
      if (route === 'data') {
