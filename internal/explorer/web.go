...
<body>
  ...
  <script>
    ...
    function renderPortalRoute(route) {
      ...
      const chainFacts = summary ? [[c('cosmos'),'ynx_6423-1'],[c('numeric'),String(summary.network?.chainId || 6423)],[c('evm'),String(summary.wallet?.chainIdHex || '0x1917')],[c('nativeAsset'),String(summary.nativeSymbol || 'YNXT')],[c('dataSource'),String(summary.truthfulStatus || 'unavailable')]] : [];
      if (route === 'blockchain') {
      ...
      if (route === 'tokens') {
        ...
        const body = summary ? '<p>' + escapeHTML(c('tokensIntro')) + '</p>' + facts([[c('symbol'),'YNXT'],[c('network'),'ynx_6423-1 / 6423 / 0x1917'],[c('decimals'),String(summary.wallet?.decimals ?? '—')],[c('contract'),c('nativeContract')],[c('source'),'native-token-from-rpc-status']]) : unavailable(c('tokenMetadataUnavailable'));
