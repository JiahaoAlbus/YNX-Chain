...
func TestAdmissionRateLimitAndBoundedQueue(t *testing.T) {
	...
	handler.ServeHTTP(first, httptest.NewRequest(http.MethodGet, "/api/summary", nil))
	second := httptest.NewRecorder()
	handler.ServeHTTP(second, httptest.NewRequest(http.MethodGet, "/api/summary", nil))
	...
	go func() {
		defer wg.Done()
		blocking.ServeHTTP(httptest.NewRecorder(), httptest.NewRequest(http.MethodGet, "/api/summary", nil))
	}()
	<-entered
	queued := httptest.NewRecorder()
	blocking.ServeHTTP(queued, httptest.NewRequest(http.MethodGet, "/api/summary", nil))
	close(release)
	wg.Wait()
