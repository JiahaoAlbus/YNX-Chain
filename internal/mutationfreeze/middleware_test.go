...
func TestRuntimeMutationFreezePreservesReadsAndRestoresWrites(t *testing.T) {
	...
	rootReadPayload := `{"jsonrpc":"2.0","id":3,"method":"eth_chainId","params":[]}`
	assertStatus(t, handler, http.MethodPost, "/", rootReadPayload, http.StatusNoContent)
	...
	for _, method := range []string{"eth_gasPrice", "eth_maxPriorityFeePerGas"} {
		...
		if receivedBody != feeReadPayload {
			t.Fatalf("fee read-only EVM body was not restored for %s: %q", method, receivedBody)
		}
	}
