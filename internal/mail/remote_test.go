...
func TestRemoteWalletVerifierUsesCentralContractAndRejectsTamper(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(VerifiedWalletSession{VerifierVersion: "wallet-auth-v1", SessionBinding: "binding", RequestDigest: "digest", ProductClientID: "ynx-mail-v1", BundleID: "com.ynxweb4.mail", Account: "ynx1account", Scopes: []string{RequiredScope}, IssuedAt: issued, ExpiresAt: expires})
	}))
	defer server.Close()
...
func TestRemoteAIUsesPrivatePostBodyAndParsesSSE(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		var body struct {
			...
			SelectedMessages []map[string]string `json:"selected_messages"`
		}
		if err := json.NewDecoder(r.Body).Decode(&body); err != nil || body.Session != "mail-approved-context" || body.Product != ProductID || body.Workflow != "summarize" || len(body.SelectedMessages) != 1 {
			t.Fatalf("invalid private AI body: %v %+v", err, body)
		}
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte("event: token\ndata: {\"text\":\"safe summary\"}\n\n"))
	}))
	defer server.Close()
	out, err := (RemoteAI{BaseURL: server.URL, Token: "server-only"}).Generate(context.Background(), "summarize", []Message{{ID: "m1", Subject: "private", Body: "selected only", SenderHandle: "@alice"}})
	if err != nil || out != "safe summary" {
