...
func TestHTTPStrictParsingScopeAndSmoke(t *testing.T) {
	...
	if err != nil || resp.StatusCode != http.StatusServiceUnavailable || resp.Header.Get("X-Request-ID") != "operator-check-0001" {
	...
	if json.NewDecoder(resp.Body).Decode(&readiness) != nil || readiness.Status != "degraded_single_host" || !readiness.StateIntegrity || readiness.DeployedPublic {
