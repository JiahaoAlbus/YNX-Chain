...
// Both Exchange PostgreSQL layouts have existed in released source. Detect the
// exact existing shape instead of letting CREATE TABLE IF NOT EXISTS silently
// accept an incompatible table and failing later at the first order write.
func detectExchangePostgresSchema(db *sql.DB) (string, error) {
	rows, err := db.Query(`SELECT column_name FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'ynx_exchange_state'`)
	if err != nil {
		return "", err
	}
	defer rows.Close()
	...
	return classifyExchangePostgresColumns(columns)
}

func classifyExchangePostgresColumns(columns map[string]bool) (string, error) {
	if columns["id"] && columns["revision"] && columns["payload"] && !columns["singleton"] {
		return "revision", nil
	}
	if columns["singleton"] && columns["schema_version"] && columns["integrity_hash"] && columns["state_json"] && !columns["id"] {
		return "integrity", nil
	}
	return "", errors.New("exchange PostgreSQL state table has unsupported layout; refusing migration")
}
...
func openStateRepository(path, databaseURL string) (stateRepository, error) {
	...
	mode, err := detectExchangePostgresSchema(db)
	if err != nil {
		db.Close()
		return nil, err
	}
	return &postgresStateRepository{db: db, bootstrapPath: path, schemaMode: mode}, nil
}
...
func (repository *postgresStateRepository) Load() (persistentState, bool, error) {
	...
	var revision int64
	var err error
	if repository.schemaMode == "revision" {
		err = repository.db.QueryRow(`SELECT revision, payload FROM ynx_exchange_state WHERE id = 'primary'`).Scan(&revision, &raw)
	} else {
...
func (repository *postgresStateRepository) Save(expectedIntegrity string, state *persistentState) error {
	...
	if repository.schemaMode == "revision" {
		if state.Revision < 0 {
			return errors.New("exchange database revision invalid")
		}
		if state.Revision == 0 && expectedIntegrity == "" {
			result, err = repository.db.ExecContext(ctx, `INSERT INTO ynx_exchange_state (id, revision, payload) VALUES ('primary', 1, $1::jsonb) ON CONFLICT (id) DO NOTHING`, string(raw))
		} else {
			result, err = repository.db.ExecContext(ctx, `UPDATE ynx_exchange_state SET revision = $1, payload = $2::jsonb, updated_at = NOW() WHERE id = 'primary' AND revision = $3 AND payload->>'integrityHash' = $4`, state.Revision+1, string(raw), state.Revision, expectedIntegrity)
		}
	} else if strings.TrimSpace(expectedIntegrity) == "" {
