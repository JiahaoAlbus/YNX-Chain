...
func TestPostgresStateLayoutsAreDetectedWithoutDestructiveMigration(t *testing.T) {
	for name, test := range map[string]struct {
		columns map[string]bool
		want    string
	}{
		"historical_integrity_cas": {map[string]bool{"singleton": true, "schema_version": true, "integrity_hash": true, "state_json": true}, "integrity"},
		"later_revision_cas":       {map[string]bool{"id": true, "revision": true, "payload": true}, "revision"},
		"mixed_table":              {map[string]bool{"id": true, "revision": true, "payload": true, "singleton": true}, ""},
		"incomplete_table":         {map[string]bool{"id": true, "revision": true}, ""},
	} {
		t.Run(name, func(t *testing.T) {
			got, err := classifyExchangePostgresColumns(test.columns)
			if got != test.want || (err != nil) != (test.want == "") {
				t.Fatalf("mode=%q error=%v want=%q", got, err, test.want)
			}
		})
	}
}
