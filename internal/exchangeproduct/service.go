...
// Close releases the optional multi-instance state backend. File snapshots do
// not hold a long-lived descriptor and need no close operation.
func (s *Service) Close() error {
	if repository, ok := s.stateRepository.(*postgresStateRepository); ok {
...
func (s *Service) StorageStatus() (backend string, multiInstance bool) {
	if s.stateRepository.Mode() == "postgres-cas-multi-instance" {
		return "postgresql", true
	}
	return "file_snapshot", false
}
...
func (s *Service) PlaceOrder(session WalletSession, req PlaceOrderRequest) (Order, error) {
	...
	if mulDiv(req.AmountMicro, req.PriceMicro, AmountScale) < 1 {
		return Order{}, ErrInvalid
	}
	d := digest(req)
	s.mu.Lock()
...
func (s *Service) AmendOrder(session WalletSession, orderID string, req AmendOrderRequest) (Order, error) {
	...
	if err := s.releaseOrderReserveLocked(&original); err != nil {
		s.state = before
		return Order{}, err
	}
	balance = s.balanceLocked(session.Account, asset)
...
func (s *Service) TickTWAP() (int, error) {
	...
	for _, item := range due {
		...
		if selfTrade {
			if err := s.releaseOrderReserveLocked(&child); err != nil {
				s.state = before
				return 0, err
			}
			...
			if err := s.matchLocked(child.ID); err != nil {
				s.state = before
				return 0, err
			}
...
func (s *Service) SweepDeadMan() (int, error) {
	...
	for _, account := range accounts {
		...
		for i := range orders {
			if err := s.releaseOrderReserveLocked(&orders[i]); err != nil {
				s.state = before
				return 0, err
			}
...
func (s *Service) matchLocked(incomingID string) error {
	for {
		...
		candidates := []Order{}
		for _, o := range s.state.Orders {
			if o.ID != incoming.ID && o.Market == incoming.Market && o.Side != incoming.Side && isOpenOrder(o) && executableRemaining(o) > 0 && crosses(incoming, o) && mulDiv(min64(executableRemaining(incoming), executableRemaining(o)), o.PriceMicro, AmountScale) >= 1 {
		...
		if !s.processingTriggers {
			...
			err := s.processConditionalLocked(maker.PriceMicro, s.state.Trades[len(s.state.Trades)-1].ID)
...
func (s *Service) processConditionalLocked(lastPrice int64, tradeID string) error {
	...
	for _, conditional := range pending {
		...
		if err := s.matchLocked(order.ID); err != nil {
			return err
		}
	}
	return nil
...
func (s *Service) executeTradeLocked(incoming, maker *Order, qty, price int64) error {
	...
	if buyer.Account == seller.Account || buyer.ReservedMicro < spend || seller.ReservedMicro < qty {
		return fmt.Errorf("%w: fill exceeds this order's reserved funds", ErrInsufficient)
	}
