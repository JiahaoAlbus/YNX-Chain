...
func TestNativeMobileOwnershipSessionIsBoundSeparatelyFromBrowserOrigin(t *testing.T) {
	...
	if review := protectedNativeRequest(t, server.URL, http.MethodPost, "/app/bridge/wallet-reviews", map[string]any{}, fixture.deviceID, session.Token); review.Code != http.StatusNotFound {
		t.Fatalf("non-Wallet native client reached Wallet review route: %d %s", review.Code, review.Body.String())
	}
...
func TestNativeWalletSessionForwardsBoundBridgeReviewContextOnly(t *testing.T) {
	...
	restarted, err := New(cfg)
	if err != nil {
		t.Fatalf("persisted Wallet session state rejected: %v", err)
	}
	if _, err := restarted.AuthenticateSession(nativeWalletBinding, session.Token, fixture.deviceID); err != nil {
		t.Fatalf("persisted Wallet session did not authenticate: %v", err)
	}
}
