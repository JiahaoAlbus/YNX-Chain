...
func (s *Server) routes() {
	...
	s.mux.HandleFunc("POST /pay/intents", s.handleProxy)
	...
	s.mux.HandleFunc("GET /pay/refunds/{id}", s.handleProxy)
	s.mux.HandleFunc("POST /pay/refunds/{id}/complete", s.handleProxy)
	...
	s.mux.HandleFunc("GET /pay/events", s.handleProxy)
	s.mux.HandleFunc("GET /pay/events/{id}", s.handleProxy)
}
...
func (s *Server) handleMetrics(w http.ResponseWriter, r *http.Request) {
	...
	labels := `service="ynx-payd",native_symbol="YNXT"`
	_, _ = fmt.Fprintf(w, "ynx_pay_gateway_requests_total{%s} %d\n", labels, health.Requests)
	_, _ = fmt.Fprintf(w, "ynx_pay_gateway_successes_total{%s} %d\n", labels, health.Successes)
	_, _ = fmt.Fprintf(w, "ynx_pay_gateway_denied_total{%s} %d\n", labels, health.Denied)
	_, _ = fmt.Fprintf(w, "ynx_pay_gateway_errors_total{%s} %d\n", labels, health.Errors)
	_, _ = fmt.Fprintf(w, "ynx_pay_gateway_audit_errors_total{%s} %d\n", labels, health.AuditErrors)
	_, _ = fmt.Fprintf(w, "ynx_pay_gateway_active_requests{%s} %d\n", labels, health.Active)
}
...
func (s *Server) handleProxy(w http.ResponseWriter, r *http.Request) {
	...
	if !s.service.Allow(r.RemoteAddr, accessKey, time.Now().UTC()) {
		...
		s.finish(w, r, requestID, nil, http.StatusTooManyRequests, "rate_limited", "Pay API rate limit exceeded")
		return
	}
	var body []byte
	if r.Method != http.MethodGet {
		...
		body, err = s.service.PrepareBody(r.URL.Path, body)
		if err != nil {
			s.service.RejectRequest()
			s.finish(w, r, requestID, body, http.StatusBadRequest, "invalid_request", err.Error())
			return
		}
	}
	accepted := AuditEntry{RequestID: requestID, At: time.Now().UTC(), RemoteIP: r.RemoteAddr, Method: r.Method, Path: r.URL.Path, BodyHash: BodyHash(body), Status: http.StatusAccepted, Outcome: "accepted"}
	if err := s.service.Audit(accepted); err != nil {
		s.service.RejectRequest()
		writeError(w, http.StatusInternalServerError, requestID, "Pay audit storage unavailable")
		return
	}
...
func (s *Server) authorize(w http.ResponseWriter, r *http.Request) (string, string, bool) {
	...
	if !s.service.Authorized(accessKey) {
		...
		writeError(w, http.StatusUnauthorized, requestID, "valid Pay API key required")
