...
func TestSnapshotFutureVersionRejectedWithoutMutation(t *testing.T) {
	for _, authenticated := range []bool{false, true} {
		t.Run(map[bool]string{false: "plain", true: "authenticated"}[authenticated], func(t *testing.T) {
			...
			snapshot.Version = CurrentPersistenceSchemaVersion + 1
			var key []byte
			if authenticated {
			...
			if err == nil || !strings.Contains(err.Error(), "unsupported commerce persistence schema version") {
...
func TestSnapshotRollbackExportV5PreservesRepresentableStateAndRestores(t *testing.T) {
	...
	if restored.s.Version != CurrentPersistenceSchemaVersion || restored.s.Stores["store_rollback"].ID == "" || restored.s.Stores["discarded"].ID != "" {
...
func TestSnapshotRollbackExportRefusesLossyOrDestructiveOperations(t *testing.T) {
	...
	for _, target := range []int{2, CurrentPersistenceSchemaVersion, CurrentPersistenceSchemaVersion + 1} {
...
func TestSnapshotRollbackExportTamperAndFutureBackupFailClosed(t *testing.T) {
	...
	if err := RestoreCommerceBackup(activePath, key); err == nil || !strings.Contains(err.Error(), "unsupported commerce persistence schema version") {
