...
| JavaScript SDK | recovered, extended and locally tested | `npm test --prefix sdk/dex` passes 13 deterministic, property, Vault approval, preflight risk, fee-semantics and receipt-reconciliation tests. | Final multi-pool/FairFlow API surface, live central integration and compatibility evidence. |
...
| FairFlow / solver competition / batch auction | local contract candidate implemented and tested; not indexed, integrated, deployed or audited | `YNXFairFlow.sol` at source commit `30c9e9d2577e02a13f3f91d0b71f4cce45dc008f`; `npm run dex:fairflow:test` passes 32 score differential vectors plus uniform-price settlement, verified surplus competition, CoW netting, exact external inventory, atomic rollback, cancellation, bond/reputation and objective slashing cases. Local observed deployment gas is 3825894 and two-Intent settlement gas is 252191. | Indexer/SDK/UI, Sybil/collusion controls, partial fills, attributable MEV evidence, external-route proof, cross-chain settlement, independent Solver, audit, verified Testnet bytecode and public batch receipts. |
...
- SDK deterministic/property/security tests: 13 pass.
