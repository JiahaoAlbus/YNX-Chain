...
| Governed jail, slashing and recovery candidate runtime | true | true | false | false | false | false | false | false | false | Threshold-signed action hashes, timelock, per-infraction caps, proportional exposure reconciliation, deterministic replay, restart validation and tamper rejection; no Chain Core activation, appeals authority or live telemetry |
...
| Canonical economics integration envelope and consumer adapter | true | true | false | false | false | false | false | false | false | Five canonical envelopes, eighteen fee/burn ledger entries, five candidate Explorer projections and fifteen Monitor checks reproduce bundle `sha256:0044010db3d8ea653fe5d7f15374919be14b5f28385f6a33d471c06a74882449`; no owner acceptance or shared-Testnet evidence |
| Durable idempotent integration Store and recovery | true | true | false | false | false | false | false | false | false | Atomic 0600 persistence, semantic deduplication, commit rebinding rejection, audit chain, restart/fresh-path restore and Store State `sha256:c4673098638660439cc69a5bbef21239e034c92a18d4b77c46ca9398022b41ed`; not accepted by Data Fabric |
...
- `make staking-risk-runtime-check` — pass; threshold Ed25519 authorization, governance timelock, slash/jail/recovery, proportional exposure reconciliation, deterministic replay, restart validation and tamper rejection.
- `make economics-integration-adapter-check` — pass; race-enabled canonical envelope, Billing Ledger, Explorer projection, Monitor mapping, release-truth and rehashed-tamper rejection coverage.
- `make economics-integration-store-check` — pass; first apply, idempotent replay, semantic deduplication, atomic 0600 persistence, restart/fresh-path restore, insecure-permission/symlink rejection and Store State evidence.
- `make economics-integration-contract-check` — pass; freezes release truth, supply/fee boundaries, canonical events and error codes, and replays economics, staking and integration vectors to exact state and bundle hashes.
...
- `go run ./cmd/ynx-economics-bench -source-commit 63afbdd82b001b1063332100796366f4423da3bc -requests 2000 -concurrency 16` — pass; direct values and limits are recorded in `evidence/performance/economics-local-benchmark.json`.
