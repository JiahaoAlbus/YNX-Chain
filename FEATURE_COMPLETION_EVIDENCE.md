...
| StableSwap | local contract, Indexer and SDK candidate implemented and tested; not integrated, deployed or audited | Source commit `575e7406885dc662d6b4aa3004fd172738bd5ba1`; `YNXStableFactory` creates governance-reviewed immutable two-asset pools with decimal normalization, A/fee bounds and proportional LP shares. `npm run dex:stable:test` passes 64 differential and 32 stateful invariant vectors plus multi-hop, exact-output, reentrancy, taxed-token, negative-rebase, overflow and delayed-governance cases. Schema/cursor v5 types Stable discovery/fees, SDK tests require fresh confirmed RPC state, and PWA tests prove Stable pools never enter CPMM quote math. | Dedicated PWA Stable transaction review, Strategy Vault routing, reviewed stable assets/rate providers, external depeg Oracle/pause, independent audit, verified Testnet bytecode, labelled pools/liquidity and public receipts. |
