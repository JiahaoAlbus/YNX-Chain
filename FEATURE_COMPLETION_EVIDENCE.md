...
Evidence baseline commit: `e8a70cbfbb07a5467d17251c3680dc01801c63b2`
...
| UltraLiquidity | partial, tested locally | Complete executable Native CLOB depth and committed consensus DEX v13 CPMM pools are quoted from authoritative state; exact-input/output math, cost/source evidence and settlement attestation are exposed. `ynx-ultraliquidity-execution-v1` binds the route and a separately Wallet-signed protected FOK order, executes the Native CLOB atomically, and restores idempotent results from persisted fills. A selected DEX route fails closed while its committed execution adapter is absent. | Committed DEX execution adapter; CLMM/StableSwap/RFQ/solver/batch/CoW/JIT/vault/POL/cross-chain adapters; measured fill/failure/oracle factors; public deployment and two-user route evidence |
