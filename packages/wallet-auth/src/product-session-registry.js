...
export function productPlatformBinding(registryInput, productId, platform) {
  ...
  if (product.retiredClients.some((item) => item.platform === platform)) fail("CLIENT_RETIRED", "The exact product client and callback are retired; use the registered replacement");
...
export function productPlatformStatus(registryInput, productId, platform) {
  ...
  if (retired) return Object.freeze({ status: "retired", code: "CLIENT_RETIRED", productId: product.productId, clientId: product.clientId, displayName: product.displayName, platform, applicationId: retired.applicationId, callback: retired.callback, retiredAt: retired.retiredAt, lastSupportedVersion: retired.lastSupportedVersion, replacementUrl: retired.replacementUrl, actions: Object.freeze(["open-replacement", "return-to-product"]), authority: "none", productSession: false });
  return Object.freeze({ status: "active", productId: product.productId, clientId: product.clientId, displayName: product.displayName, platform, actions: Object.freeze([]) });
}
...
function parseProduct(input) {
  ...
  if (new URL(webCallback).origin !== webOrigin) fail("INVALID_ROUTER_REGISTRY", "Web callback must remain on the registered Web origin");
...
function retiredClientList(input, product) {
  ...
  const result = input.map((item) => {
    ...
    const web = platform === "web", expectedApplicationId = web ? product.webApplicationId : product.applicationId, expectedCallback = web ? product.webCallback : product.nativeCallback;
...
function legacyRetirements(product) {
  ...
  if (product.clientId !== "ynx-shop-v1" || product.applicationId !== "com.ynxweb4.shop" || product.nativeCallback !== "ynxshop://wallet-auth/callback") fail("INVALID_ROUTER_REGISTRY", "legacy Shop client tuple cannot be retired safely");
  return [{ platform: "android", applicationId: "com.ynxweb4.shop", callback: "ynxshop://wallet-auth/callback", retiredAt: "2026-08-20T00:00:00.000Z", lastSupportedVersion: "0.3.0-testnet-preview", replacementUrl: "https://shop.ynxweb4.com/shop" }];
}
