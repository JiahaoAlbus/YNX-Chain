...
const FIELDS=["version","sessionBinding","productClientId","bundleId","productDeviceKey","origin","method","path","bodyDigest","nonce","issuedAt","expiresAt","signature"];
const CREATE_FIELDS=["method","path","bodyDigest","nonce","issuedAt","expiresAt"];
const EXPECTED_FIELDS=["method","path","bodyDigest","origin"];
const DOMAIN="YNX_PRODUCT_SESSION_HTTP_PROOF_V2";

// Retain the public device/bootstrap helpers used by existing consumers while
// emitting the current origin-bound v2 proof format.
...
export function createProductSessionProof(sessionInput,input,productDeviceSecret){
  ...
  if(session.verifierVersion!=="wallet-auth-v2")fail("SESSION_RETIRED","Product Session predates origin binding and must reconnect");
...
export function verifyProductSessionProof(proofInput,sessionInput,expectedInput,at=new Date()){
  ...
  const expected={method:method(expectedInput.method),path:path(expectedInput.path),bodyDigest:digest(expectedInput.bodyDigest,"bodyDigest"),origin:origin(expectedInput.origin)};
  for(const key of ["sessionBinding","productClientId","bundleId","productDeviceKey","origin"])if(proof[key]!==session[key])fail("SESSION_BINDING_MISMATCH",`Product Session proof ${key} does not match the session`);
...
function origin(value){const result=pattern(value,"origin",/^https:\/\/[A-Za-z0-9.-]+(?::[0-9]{1,5})?$/);let parsed;try{parsed=new URL(result)}catch{fail("INVALID_ORIGIN","Product Session proof origin is invalid")}if(parsed.protocol!=="https:"||parsed.username||parsed.password||parsed.pathname!=="/"||parsed.search||parsed.hash||parsed.toString()!==`${result}/`)fail("INVALID_ORIGIN","Product Session proof origin must be canonical HTTPS");return result}
