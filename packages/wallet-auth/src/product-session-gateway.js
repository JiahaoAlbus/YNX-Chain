...
export class ProductSessionGatewayKernel {
  ...
  constructor(registryInput, tokenFactory, snapshot) {
    ...
    this.#proofs = [...parsed.consumedProofs]; this.#idempotency = parsed.idempotency.filter((item) => !retiredCachedResponse(this.#registry, item)); this.#audit = [...parsed.audit];
  }
...
function retiredBinding(registry, item) { const product = registry.products.find((candidate) => candidate.productId === item.productId && candidate.clientId === item.clientId); return product?.retiredClients.some((client) => client.platform === item.platform && client.applicationId === item.applicationId && client.callback === item.callback) === true; }
function retiredCachedResponse(registry, item) { try { return retiredBinding(registry, JSON.parse(item.responseBody).result); } catch { return false; } }
function normalizeError(error) { if (!(error instanceof WalletAuthError)) return { status: 500, code: "INTERNAL", message: "Product Session Gateway failed closed" }; const forbidden = ["CROSS_PRODUCT_SESSION", "INVALID_DEVICE_PROOF", "SESSION_REVOKED", "SESSION_EXPIRED", "SCOPE_WIDENING", "PROOF_REQUIRED"]; const conflict = ["REPLAY", "ALREADY_REVOKED", "IDEMPOTENCY_CONFLICT"]; const status = error.code === "CLIENT_RETIRED" ? 410 : error.code === "NETWORK_UNAVAILABLE" ? 503 : error.code === "ROUTE_NOT_FOUND" || error.code === "SESSION_NOT_FOUND" ? 404 : conflict.includes(error.code) ? 409 : forbidden.includes(error.code) ? 403 : 400; return { status, code: error.code, message: error.message.length <= 300 ? error.message : "Product Session Gateway rejected the request" }; }
