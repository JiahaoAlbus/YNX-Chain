...
export const STANDARD_WALLET_PRIVATE_SERVICE = Object.freeze({
  NOT_REQUESTED: "not-requested", CONNECTING: "connecting", READY: "ready", DEGRADED: "degraded",
});
export const STANDARD_WALLET_RPC_PROBE = Object.freeze({ NOT_RUN: "not-run", READY: "ready", DEGRADED: "degraded" });
...
export function reduceStandardWalletConnectState(current, event) {
  ...
  switch (event.type) {
    ...
    case "PRIVATE_SESSION_CONNECTING": requireConnected(previous); return state({ ...previous, privateService: STANDARD_WALLET_PRIVATE_SERVICE.CONNECTING });
    ...
    case "RPC_PROBE_READY": requireConnected(previous); requireAcceptedRpcProbe(event); return state({ ...previous, rpcProbe: STANDARD_WALLET_RPC_PROBE.READY, rpcProbeCode: null });
    case "RPC_PROBE_DEGRADED": requireConnected(previous); requireAcceptedRpcProbe(event); return state({ ...previous, rpcProbe: STANDARD_WALLET_RPC_PROBE.DEGRADED, rpcProbeCode: safeCode(event.code), chooserOpen: false, chooserMode: "closed" });
    ...
    case "DISCONNECT": return disconnected(event.type === "PROVIDER_DISCONNECT" ? "provider-disconnect" : "user-disconnect");
    case "FAIL": return state({ status: STANDARD_WALLET_CONNECT_STATUS.FAILED, chooserOpen: true, chooserMode: "error", errorCode: safeCode(event.code), focusRestoreTarget: null });
    default: fail("INVALID_STANDARD_WALLET_EVENT", "Standard Wallet transition event is unknown");
  }
}

function connected(kind, approvedAccount, privateService) {
  return state({ status: STANDARD_WALLET_CONNECT_STATUS.CONNECTED, providerKind: kind, account: approvedAccount, chainId: STANDARD_WALLET_CHAIN_ID, chooserOpen: false, chooserMode: "closed", chooserActions: CONNECTED_ACTIONS, pendingIntent: null, focusRestoreTarget: "wallet-connect-trigger", privateService, standardPermissions: Object.freeze(["account:read", "chain:read"]), productAccess: "standard-wallet-connected" });
}
...
function state(input) {
  return Object.freeze({
    status: input.status, chooserOpen: input.chooserOpen ?? false, chooserMode: input.chooserMode ?? "closed",
    chooserActions: input.chooserActions ?? EMPTY, pendingIntent: input.pendingIntent ?? null,
    providerKind: input.providerKind ?? null, account: input.account ?? null, chainId: input.chainId ?? null,
    privateService: input.privateService ?? STANDARD_WALLET_PRIVATE_SERVICE.NOT_REQUESTED,
    privateServiceCode: input.privateServiceCode ?? null, rpcProbe: input.rpcProbe ?? STANDARD_WALLET_RPC_PROBE.NOT_RUN,
    rpcProbeCode: input.rpcProbeCode ?? null, standardPermissions: input.standardPermissions ?? EMPTY,
    productAccess: input.productAccess ?? "guest-or-public-only", focusRestoreTarget: input.focusRestoreTarget ?? null,
    errorCode: input.errorCode ?? null, disconnectReason: input.disconnectReason ?? null,
