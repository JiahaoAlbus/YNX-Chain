...
export const WALLET_PROVIDER_DISCOVERY_STATUS = Object.freeze({
  AVAILABLE: "available", NOT_INJECTED: "provider-not-injected", UNSUPPORTED: "unsupported-injected-provider",
  AMBIGUOUS: "ambiguous-provider", CONFLICTED: "conflicted-announcement",
});
export const WALLET_PROVIDER_NOT_INJECTED_POSSIBLE_CAUSES = Object.freeze([
  "extension-locked", "site-access-denied", "extension-disabled", "extension-not-installed",
]);
...
export function discoverInjectedWalletProviders(scope = globalThis) {
  ...
  const declared = Array.isArray(declaredProviders) ? declaredProviders.map((provider) => candidate(provider, safely(() => provider?.providerInfo), "legacy-injected")).filter(Boolean) : [];
  const root = ethereum === undefined ? [] : [candidate(ethereum, safely(() => ethereum?.providerInfo), "legacy-injected")].filter(Boolean);
  return selectWalletProviderCandidates(declared.length ? declared : root, 0, injectionDiagnostics(scope, { eip6963RequestDispatches: 0, domContentLoadedObserved: false }));
}
...
export async function discoverEip6963WalletProviders(scope = globalThis, waitMs = 160) {
  ...
  const afterDomContentLoaded = () => { domContentLoadedObserved = true; try { requestProviders(); } catch {} };
  try {
    ...
    if (safely(() => documentValue?.readyState) === "loading" && typeof documentAdd === "function" && typeof documentRemove === "function") {
      documentAdd.call(documentValue, "DOMContentLoaded", afterDomContentLoaded, { once: true }); domRegistered = true;
    }
    requestProviders();
    if (waitMs > 0) { const midpoint = Math.min(80, Math.max(1, Math.floor(waitMs / 2))); await delay(midpoint); requestProviders(); await delay(waitMs - midpoint); }
    requestProviders();
  } catch {
  ...
  return selectWalletProviderCandidates([...byUuid.values()], conflicted.size, injectionDiagnostics(scope, { eip6963RequestDispatches: requestDispatches, domContentLoadedObserved }));
}
...
export async function discoverWalletProviders(scope = globalThis, waitMs = 160) {
  ...
  ].filter(Boolean)), announced.conflictedAnnouncements + injected.conflictedAnnouncements, injectionDiagnostics(scope, { readyStateStart: initial.readyState, eip6963RequestDispatches: announced.diagnostics.eip6963RequestDispatches, domContentLoadedObserved: announced.diagnostics.domContentLoadedObserved }));
}
...
export function selectWalletProviderCandidates(input, conflictedAnnouncements = 0, diagnostics = null) {
  ...
  const status = conflictedAnnouncements > 0 ? WALLET_PROVIDER_DISCOVERY_STATUS.CONFLICTED
    : ambiguities.length > 0 ? WALLET_PROVIDER_DISCOVERY_STATUS.AMBIGUOUS
      : candidates.length > 0 ? WALLET_PROVIDER_DISCOVERY_STATUS.AVAILABLE
        : observed.injectedRootObserved || observed.injectedProvidersArrayObserved ? WALLET_PROVIDER_DISCOVERY_STATUS.UNSUPPORTED
...
function injectionSnapshot(scope) {
  ...
  return Object.freeze({ readyState: ["loading", "interactive", "complete"].includes(readyState) ? readyState : "unavailable", injectedRootObserved: ethereum !== undefined, injectedProvidersArrayObserved: Array.isArray(providers), injectedProviderCount: Array.isArray(providers) ? providers.length : ethereum === undefined ? 0 : 1 });
}
function injectionDiagnostics(scope, input = {}) {
  const snapshot = injectionSnapshot(scope);
  return Object.freeze({ readyStateStart: ["loading", "interactive", "complete", "unavailable"].includes(input.readyStateStart) ? input.readyStateStart : snapshot.readyState, readyStateEnd: snapshot.readyState, eip6963RequestDispatches: Number.isSafeInteger(input.eip6963RequestDispatches) && input.eip6963RequestDispatches >= 0 ? input.eip6963RequestDispatches : 0, domContentLoadedObserved: input.domContentLoadedObserved === true, injectedRootObserved: snapshot.injectedRootObserved, injectedProvidersArrayObserved: snapshot.injectedProvidersArrayObserved, injectedProviderCount: snapshot.injectedProviderCount, exactExtensionStateObservable: false });
}
