...
export class ProductSessionAuthority {
  #registry;
  #state;
  constructor(registryInput, snapshot = emptySnapshot()) { this.#registry = parseProductSessionRegistry(registryInput); this.#state = applyRegistryRetirements(this.#registry, parseSnapshot(snapshot)); }
...
function applyRegistryRetirements(registry, snapshot) {
  ...
  const retired = (item) => { const product = registry.products.find((candidate) => candidate.productId === item.productId && candidate.clientId === item.clientId); return product?.retiredClients.some((client) => client.platform === item.platform && client.applicationId === item.applicationId && client.callback === item.callback) === true; };
  for (const session of next.sessions.filter(retired)) { if (!next.revokedSessions.includes(session.sessionBinding)) next.revokedSessions.push(session.sessionBinding); if (!next.revokedDevices.includes(session.deviceBinding)) next.revokedDevices.push(session.deviceBinding); }
