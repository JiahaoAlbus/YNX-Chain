...
// tokens. Locale collation can disagree and reject a second pending challenge.
function sortSnapshot(value) { value.sessions.sort((a, b) => compareSnapshotKey(a.sessionBinding, b.sessionBinding)); value.issuedChallenges.sort((a, b) => compareSnapshotKey(a.challenge, b.challenge)); for (const field of ["consumedNonces", "consumedStates", "consumedRequests", "consumedChallenges", "revokedSessions", "revokedDevices"]) value[field].sort(); value.revokedAccounts.sort((a, b) => compareSnapshotKey(a.account, b.account)); }
function compareSnapshotKey(left, right) { return left < right ? -1 : left > right ? 1 : 0; }
