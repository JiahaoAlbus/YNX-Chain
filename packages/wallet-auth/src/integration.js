...
const SESSION_FIELDS = ["verifierVersion", "sessionBinding", "productClientId", "bundleId", "productDeviceAlgorithm", "requestDigest", "account", "scopes", "issuedAt", "expiresAt"];
...
export function assertCentralWalletSessionActive(session, input, at = new Date()) {
  ...
  if (requests.includes(parsed.requestDigest)) throw new WalletAuthError("REVOKED", "Wallet approval and all sessions derived from it have been revoked");
  return parsed;
}
