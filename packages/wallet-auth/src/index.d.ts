...
export declare function createProductWalletConnection(config:Readonly<{registry:unknown;productId:string;platform:ProductSessionPlatform;walletInstalled:()=>boolean|Promise<boolean>;schemeRegistered:()=>boolean|Promise<boolean>;gatewayTimeoutMs:number;storage:Readonly<Record<string,unknown>>;device:Readonly<Record<string,unknown>>;scope:unknown;discoveryWaitMs:number;openWallet:(input:Readonly<{url:string;request:Readonly<Record<string,unknown>>;requestId:string;automatic:boolean;productId:string;platform:ProductSessionPlatform}>)=>Readonly<{opened:true}|{opened:false;code:string}>|Promise<Readonly<{opened:true}|{opened:false;code:string}>>;openTimeoutMs:number}>):WalletConnectionCoordinator;
