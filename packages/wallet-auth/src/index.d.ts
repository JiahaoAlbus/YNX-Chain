...
export declare class StandardWalletProviderEngine { constructor(config: Readonly<{ origin: string; walletAccounts: readonly string[]; approveAccounts: (input: Readonly<{ origin: string; accounts: readonly string[] }>) => readonly string[] | Promise<readonly string[]>; permissionStorage?: StandardWalletPermissionStorage | null; rpcTransport?: (request: Readonly<{ method: string; params: unknown }>, network: typeof STANDARD_WALLET_NETWORK) => unknown | Promise<unknown>; signMessage?: (input: Readonly<Record<string, unknown>>) => string | Promise<string>; signTypedData?: (input: Readonly<Record<string, unknown>>) => string | Promise<string>; sendTransaction?: (input: Readonly<Record<string, unknown>>) => string | Promise<string> }>); readonly isYNXWallet: true; readonly chainId: "0x1917"; readonly selectedAddress: string | null; readonly authority: "standard-wallet-eip1193"; readonly state: Readonly<Record<string, unknown>>; on(name: string, listener: (payload: unknown) => void): this; once(name: string, listener: (payload: unknown) => void): this; removeListener(name: string, listener: (payload: unknown) => void): this; request(input: Readonly<{ method: string; params?: unknown }>): Promise<unknown>; restorePermissions(): Promise<Readonly<Record<string, unknown>>>; replaceWalletAccounts(accounts: readonly string[]): Promise<Readonly<Record<string, unknown>>>; setRpcStatus(status: "ready" | "degraded"): Readonly<Record<string, unknown>>; setPrivateServiceStatus(status: "not-requested" | "connecting" | "ready" | "degraded"): Readonly<Record<string, unknown>>; notifyChainChanged(chainId: string): Promise<Readonly<Record<string, unknown>>>; disconnect(): Promise<Readonly<Record<string, unknown>>>; }
...
export declare class StandardWalletWalletConnectSessionAdapter { constructor(config: Readonly<{ engine: StandardWalletProviderEngine; topic: string }>); readonly active: boolean; readonly topic: string; approve(proposal: Readonly<Record<string, unknown>>): Promise<Readonly<Record<string, unknown>>>; request(envelope: Readonly<Record<string, unknown>>): Promise<unknown>; disconnect(): Promise<Readonly<{ topic: string; active: false; authority: "walletconnect-session-terminated" }>>; }
