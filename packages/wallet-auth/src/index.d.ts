...
export declare function createProductWalletConnection(config:Readonly<{registry:unknown;productId:string;platform:ProductSessionPlatform;gatewayEndpoint:string;fetch:(url:string,init:Readonly<Record<string,unknown>>)=>Promise<unknown>;walletInstalled:()=>boolean|Promise<boolean>;schemeRegistered:()=>boolean|Promise<boolean>;gatewayTimeoutMs:number;storage:Readonly<Record<string,unknown>>;device:Readonly<Record<string,unknown>>;tokenFactory:()=>string;clock:()=>Date;scope:unknown;discoveryWaitMs:number;openWallet:(input:Readonly<{url:string;request:Readonly<Record<string,unknown>>;requestId:string;automatic:boolean;productId:string;platform:ProductSessionPlatform}>)=>Readonly<{opened:true}|{opened:false;code:string}>|Promise<Readonly<{opened:true}|{opened:false;code:string}>>;openTimeoutMs:number}>):WalletConnectionCoordinator;
