...
if (auditFacts.card?.auditState !== "P0_196_EMERGENCY_CANONICAL_TARGET_UNKNOWN" || auditFacts.card?.candidateArtifactHashesMatched !== 12 || auditFacts.card?.reportedDeploymentPrefix !== "dpl_8qCE..." || auditFacts.card?.rawResourcesMatched !== true || auditFacts.card?.browserGate !== "TIMED_OUT" || auditFacts.card?.rollbackCommand !== "EXIT_NONZERO_AFTER_CREATING_ALIAS" || auditFacts.card?.canonicalAliasTarget !== "UNKNOWN" || auditFacts.card?.retryAllowed !== false || auditFacts.card?.requiredCentralAction !== "READ_CARD_CANONICAL_ALIAS_THEN_RESTORE_EXACT_DPL_DQMU_IF_NEEDED" || auditFacts.card?.currentRollbackDeploymentId !== "dpl_DqMuEhno65bLFTNKBu9gs52soVri" || auditFacts.card?.allPromotionFrozen !== true) findings.push("Card P0-196 emergency alias-recovery audit drift");
