...
const required = ["reduceStandardWalletConnectState", "ACCOUNT_APPROVED", "CHAIN_CONFIRMED", "PRIVATE_SESSION_DEGRADED", "ACCOUNTS_CHANGED", "CHAIN_CHANGED", "PROVIDER_DISCONNECT", "RESTORE", "OPEN_CHOOSER", "RPC_PROBE_DEGRADED", "accepted-cors-safe"];
...
async function sourceFiles(directory) {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    else if ([".js", ".mjs", ".cjs", ".ts", ".tsx", ".jsx"].includes(extname(entry.name))) output.push(child);
  }
  return output;
}
