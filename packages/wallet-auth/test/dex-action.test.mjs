...
test("DEX Wallet signs exact-input, exact-output, add and remove liquidity actions", () => {
  ...
  for (const expected of cases) {
    const parsed = parseDexActionDeepLink(
        createDexActionDeepLink(expected, NOW),
        NOW,
      ),
      signed = signDexAction(
        parsed,
        { accountSecret: SECRET, account: expected.account },
        NOW,
      ),
      verified = parseDexActionResponse(signed, expected, NOW);
    assert.equal(verified.signedTransaction.action, expected.action);
    assert.deepEqual(verified.signedTransaction.payload, expected.payload);
    assert.equal(verified.signedTransaction.fee, 1);
    assert.match(verified.signedTransaction.signature, /^[0-9a-f]{136,144}$/);
    assert.match(
      createDexActionCallback(signed, expected, NOW),
      /^https:\/\/dex\.ynxweb4\.com\/wallet-action\/callback\?response=/,
    );
  }
});
...
test("DEX Wallet rejects action, route, session, quote, deadline, account and signature substitution", () => {
  ...
  assert.throws(
    () =>
      parseDexActionDeepLink(
        createDexActionDeepLink(expected, NOW).replace("dex-action", "action"),
        NOW,
      ),
    /route/,
  );
  assert.throws(
    () => createDexActionDeepLink({ ...expected, sessionBinding: "b" }, NOW),
    /sessionBinding/,
  );
  assert.throws(
    () =>
      createDexActionDeepLink(
        { ...expected, quote: { ...quote, asset0: "other" } },
        NOW,
      ),
    /quote/i,
  );
  assert.throws(
    () =>
      createDexActionDeepLink(
        { ...expected, quote: { ...quote, poolId: "dex_other_pool" } },
        NOW,
      ),
    /another pool/i,
  );
  assert.throws(
    () =>
      createDexActionDeepLink(
        { ...expected, payload: { ...expected.payload, deadlineUnix: 1 } },
        NOW,
      ),
    /deadline/,
  );
  assert.throws(
    () => signDexAction(expected, { accountSecret: "1".repeat(64) }, NOW),
    /account/,
  );
  assert.throws(
    () =>
      parseDexActionResponse(
        {
          ...
          signedTransaction: {
            ...
            payload: { ...signed.signedTransaction.payload, amountIn: 2000 },
          },
        },
        expected,
        NOW,
      ),
    /payload|encoding|hash|signature/i,
  );
  assert.throws(
    () =>
      parseDexActionResponse(
        { ...signed, transactionHash: `0x${"0".repeat(64)}` },
        expected,
        NOW,
      ),
    /hash|encoding/,
