...
test("Node host exposes truthful version, readiness, metrics and redacted structured events",async()=>{
  ...
  await serve(host,async(base)=>{
    ...
    assert.deepEqual(version.build,BUILD);assert.equal(version.gatewayHttpSchemaVersion,1);assert.equal(version.nodeStateSchemaVersion,1);assert.equal(version.observabilitySchemaVersion,1);assert.match(version.registrySha256,/^[0-9a-f]{64}$/);assert.deepEqual(version.enabledProductClientIds,["ynx-calendar-v1","ynx-developer-v1","ynx-dex-web-v1","ynx-exchange-v1","ynx-finance-v1","ynx-merchant-console-v1","ynx-quant-v1","ynx-shop-v1","ynx-social-v1","ynx-wallet-v1"]);
