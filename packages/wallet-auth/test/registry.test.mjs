...
test("central registry contains 26 unique, least-privilege products with DEX and reviewed financial clients approved", () => {
  ...
  assert.deepEqual(approved.map((product) => product.productId), ["dex", "exchange", "finance", "quant", "shop"]);
  ...
  assert.deepEqual(centralRegistrationByProduct(registry, "dex").callbacks, ["https://dex.ynxweb4.com/wallet-auth/callback"]);
});
