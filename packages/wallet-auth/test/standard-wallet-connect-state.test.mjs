...
import {
  createStandardWalletConnectState, reduceStandardWalletConnectState, STANDARD_WALLET_CHAIN_ID,
  STANDARD_WALLET_CONNECT_STATUS, STANDARD_WALLET_PRIVATE_SERVICE, STANDARD_WALLET_RPC_PROBE,
  STANDARD_WALLET_RPC_PROBE_TRANSPORT, WalletAuthError,
} from "../src/index.js";
...
test("refresh restore uses eth_accounts and chain outcome without reopening chooser", () => {
  ...
  assert.equal(reduce(initial, "RESTORE", { providerKind: "metamask", accounts: [], chainId: "0x1917" }).status, STANDARD_WALLET_CONNECT_STATUS.DISCONNECTED);
});

test("Product Session degradation never blocks or reopens Standard Wallet success", () => {
  const connecting = reduce(connected(), "PRIVATE_SESSION_CONNECTING");
...
test("account, chain and provider events are the only connection-invalidating transitions", () => {
  ...
  assert.equal(reduce(standard, "ACCOUNTS_CHANGED", { accounts: [] }).disconnectReason, "accounts-empty");
  const wrong = reduce(standard, "CHAIN_CHANGED", { chainId: "0x1" });
  assert.equal(wrong.status, STANDARD_WALLET_CONNECT_STATUS.WRONG_CHAIN);
  assert.deepEqual(wrong.standardPermissions, []);
  assert.equal(reduce(wrong, "CHAIN_CHANGED", { chainId: "0x1917" }).status, STANDARD_WALLET_CONNECT_STATUS.CONNECTED);
