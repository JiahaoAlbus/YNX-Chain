...
import { canonicalJSON, createProductSessionRequest, parseProductSessionRegistry, signProductSessionApproval } from "../src/index.js";
...
async function serveGet(host,path){const server=createServer(host.handler());await new Promise((resolve,reject)=>{server.once("error",reject);server.listen(0,"127.0.0.1",resolve)});try{const address=server.address(),response=await fetch(`http://127.0.0.1:${address.port}${path}`);return{status:response.status,body:await response.text()}}finally{await new Promise(resolve=>server.close(resolve))}}
