...
test("deployment lifecycle verifier proves idempotency, replay rejection and revoke through the persistent host", async (context) => {
  ...
  let challenge = 0;
  const host = new PersistentProductSessionGatewayNodeHost(registry, { now: () => new Date(NOW), statePath: join(directory, "state.json"), tokenFactory: () => token(`deployment-lifecycle-${challenge++}`) });
...
test("deployment lifecycle verifier floors live proof time at the server-issued Session time", async (context) => {
  ...
  context.after(() => rmSync(directory, { force: true, recursive: true }));
  let challenge = 0;
  const host = new PersistentProductSessionGatewayNodeHost(registry, { now: () => new Date(), statePath: join(directory, "state.json"), tokenFactory: () => token(`deployment-live-clock-${challenge++}`) });
  const server = createServer(host.handler());
  await new Promise((resolve, reject) => { server.once("error", reject); server.listen(0, "127.0.0.1", resolve); });
  context.after(() => new Promise((resolve) => server.close(resolve)));
  const address = server.address();
