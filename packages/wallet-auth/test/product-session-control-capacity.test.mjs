...
test("capabilities keeps exact 15-product CORS and read-only method/request-ID boundaries",async t=>{
  assert.equal(registry.products.length,15);
  const directory=fs.mkdtempSync(join(tmpdir(),"ynx-capability-registry-"));fs.chmodSync(directory,0o700);
  const statePath=join(directory,"state.json");initializeProductSessionControlState(statePath,gateway().snapshot());
  const host=new ProductSessionControlNodeHost(registry,{statePath,now:()=>at(),tokenFactory:()=>token(requestId()),capacityPolicy:POLICY});
  ...
  const target=`http://127.0.0.1:${server.address().port}/v2/product-sessions/capabilities`,before=fs.readFileSync(statePath);
  for(const origin of [...registry.products.map(p=>p.webOrigin),"https://wallet.ynxweb4.com"]){
    const response=await fetch(target,{headers:{origin,"x-request-id":requestId()}});
    ...
    const body=await response.json();assert.equal(body.result.productSessionStateVersion,3);assert.equal(body.result.controlIntentCapacity.maxOwners,2);
    assert.deepEqual(Object.keys(body.result).sort(),["controlIntentCapacity","productSessionStateVersion"]);
    const preflight=await fetch(target,{method:"OPTIONS",headers:{origin,"access-control-request-method":"GET","access-control-request-headers":"x-request-id"}});
    assert.equal(preflight.status,204);assert.equal(preflight.headers.get("access-control-allow-methods"),"GET");
  }
  const cases=[
    {options:{headers:{origin:"https://evil.example","x-request-id":requestId()}},code:403},
    {options:{headers:{origin:"https://wallet.ynxweb4.com"}},code:400},
    {options:{method:"POST",headers:{origin:"https://wallet.ynxweb4.com","x-request-id":requestId()}},code:405},
    {options:{method:"OPTIONS",headers:{origin:"https://wallet.ynxweb4.com","access-control-request-method":"POST","access-control-request-headers":"x-request-id"}},code:405},
    {options:{method:"OPTIONS",headers:{origin:"https://wallet.ynxweb4.com","access-control-request-method":"GET","access-control-request-headers":"authorization"}},code:400},
  ];
  for(const c of cases){const response=await fetch(target,c.options);assert.equal(response.status,c.code);await response.text();}
  const query=await fetch(target+"?owner=ignored",{headers:{"x-request-id":requestId()}});assert.equal(query.status,400);await query.text();
  assert.deepEqual(fs.readFileSync(statePath),before);
});

test("actual daemon consumes configured capacity ENV and inspection CLI returns no owner data",async t=>{
  const directory=fs.mkdtempSync(join(tmpdir(),"ynx-capacity-daemon-"));fs.chmodSync(directory,0o700);
  const statePath=join(directory,"v3.json"), seed=gateway().snapshot();initializeProductSessionControlState(statePath,seed);
  const before=fs.readFileSync(statePath),base=fileURLToPath(new URL("..",import.meta.url));
  const probe=createServer();await new Promise(r=>probe.listen(0,"127.0.0.1",r));const port=probe.address().port;await new Promise(r=>probe.close(r));
  const env={YNX_WALLET_GATEWAY_HTTP_ADDR:"127.0.0.1",YNX_WALLET_GATEWAY_HTTP_PORT:String(port),YNX_WALLET_GATEWAY_STATE_PATH:join(directory,"v1.json"),YNX_PRODUCT_SESSION_GATEWAY_STATE_PATH:statePath,YNX_PRODUCT_SESSION_GATEWAY_STATE_VERSION:"3",YNX_PRODUCT_SESSION_CONTROL_CAPACITY_POLICY:JSON.stringify(POLICY),YNX_WALLET_GATEWAY_REMOTE_DEPLOYED:"false"};
  const child=spawn(process.execPath,[join(base,"scripts/ynx-wallet-gatewayd.mjs")],{env,cwd:base});
  t.after(async()=>{if(child.exitCode===null){await new Promise(resolve=>{const timeout=setTimeout(()=>{child.kill("SIGKILL");},2000);child.once("exit",()=>{clearTimeout(timeout);resolve();});child.kill("SIGTERM");});}fs.rmSync(directory,{recursive:true,force:true});});
  let output="";
  await new Promise((resolve,reject)=>{
    const timer=setTimeout(()=>reject(new Error(`daemon startup timeout: ${output}`)),5000);
    const read=chunk=>{output+=chunk;if(output.includes('"event":"listening"')){clearTimeout(timer);resolve();}};
    child.stdout.on("data",read);child.stderr.on("data",read);child.once("exit",code=>{clearTimeout(timer);reject(new Error(`daemon exited ${code}: ${output}`));});
  });
  const response=await fetch(`http://127.0.0.1:${port}/v2/product-sessions/capabilities`,{headers:{origin:"https://wallet.ynxweb4.com","x-request-id":requestId()}});
  assert.equal(response.status,200);const body=await response.json();
  assert.equal(body.result.controlIntentCapacity.maxOwners,2);assert.equal(body.result.controlIntentCapacity.intentsPerOwner,2);assert.equal(body.result.controlIntentCapacity.productionConcurrencyValidated,false);
  assert.deepEqual(fs.readFileSync(statePath),before);
  const inspect=spawnSync(process.execPath,[join(base,"scripts/ynx-wallet-control-inspect.mjs"),"--state",statePath],{env:{YNX_PRODUCT_SESSION_CONTROL_CAPACITY_POLICY:JSON.stringify(POLICY)},encoding:"utf8"});
  assert.equal(inspect.status,0,inspect.stderr);const report=JSON.parse(inspect.stdout);
  assert.equal(report.readOnly,true);assert.equal(report.policy.maxOwners,2);assert.equal(report.admittedOwners,0);assert.equal(report.sourceEnvelopeVersion,2);assert.equal(report.snapshotSchemaVersion,3);assert.equal(inspect.stdout.includes("ynx1"),false);
  assert.deepEqual(fs.readFileSync(statePath),before);
});
