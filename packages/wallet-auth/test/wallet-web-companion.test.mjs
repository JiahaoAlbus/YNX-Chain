...
test("Web companion freeze keeps every public and device-visible gate false", () => {
  ...
  assert.equal(Object.values(contract.truthBoundary).filter(Boolean).length, 11);
  ...
  assert.equal(contract.truthBoundary.networkEpochCandidate, true);
  assert.equal(contract.truthBoundary.completionNetworkTransitionProtectedCandidate, true);
  assert.equal(contract.reconciliation.consumedRouterSource, "48ca620105d07a78b1abb8bd02648ed7b0fe4772");
  assert.equal(contract.reconciliation.secureSignerReconciliationPending, true);
