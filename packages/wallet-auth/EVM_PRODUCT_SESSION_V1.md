...
| Revoke proof | Same signed HTTP proof schema, fixed `method=POST` and `target=/api/wallet-login/revoke` | The request's actual body digest and one-time nonce are signed. It only revokes the bound session and grants no private read or write. |
...
For revoke, use `createEvmProductSessionRevokeProof` (or `...With` for a WebCrypto signer) and `verifyAndConsumeEvmProductSessionRevokeProof`. The server supplies the exact raw request body SHA-256 and origin, loads the authoritative session, and atomically marks it revoked while consuming `(sessionId, nonce)` in `revokeAndConsume`. This fixed POST route is separate from the GET-only read verifier. It deliberately does not require the browser to remain connected or on chain 6423: those are precisely the events that trigger revocation. A failed or offline revoke leaves the short expiry as the backstop.
