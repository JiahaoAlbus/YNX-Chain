...
export function parseStablePoolState(value,{now=new Date(),maxAgeMs=15_000}={}) {
  exactObject(value,["address","amplification","asOf","blockNumber","chainId","confidence","contractVersion","coverage","failure","feeBps","precisionMultiplier0","precisionMultiplier1","reserve0","reserve1","source","token0","token1","version"]);
  if(value.chainId!==6423||value.contractVersion!=="ynx-stableswap-v1"||value.source!=="YNX Testnet EVM RPC"||value.version!=="ynx-stable-pool-state-v1"||value.confidence!=="confirmed-on-chain"||!bounded(value.coverage,20,500)||value.failure!==null||![value.address,value.token0,value.token1].every(item=>ADDRESS.test(item)&&item.toLowerCase()!==ZERO_ADDRESS)||value.token0.toLowerCase()>=value.token1.toLowerCase()||!Number.isSafeInteger(value.blockNumber)||value.blockNumber<1||!Number.isInteger(value.amplification)||value.amplification<10||value.amplification>10_000||!Number.isInteger(value.feeBps)||value.feeBps<1||value.feeBps>100)fail("INVALID_STABLE_POOL","invalid stable pool identity or parameters");
  const reserve0=positiveBigInt(value.reserve0),reserve1=positiveBigInt(value.reserve1),precisionMultiplier0=positiveBigInt(value.precisionMultiplier0),precisionMultiplier1=positiveBigInt(value.precisionMultiplier1);const asOf=new Date(value.asOf);const age=now.valueOf()-asOf.valueOf();
  if(!Number.isFinite(now.valueOf())||!Number.isFinite(asOf.valueOf())||!Number.isInteger(maxAgeMs)||maxAgeMs<1||age<0||age>maxAgeMs||!isPowerOfTen(precisionMultiplier0)||!isPowerOfTen(precisionMultiplier1)||reserve0*precisionMultiplier0>10n**36n||reserve1*precisionMultiplier1>10n**36n)fail("INVALID_STABLE_POOL","stale or unsafe stable pool state");
  return Object.freeze({...value,address:value.address.toLowerCase(),token0:value.token0.toLowerCase(),token1:value.token1.toLowerCase(),reserve0,reserve1,precisionMultiplier0,precisionMultiplier1,asOf:asOf.toISOString()});
}

export function stableAmountOut({pool,tokenIn,amountIn}) {
  pool=parseStablePoolState(pool,{now:new Date(pool.asOf)});const input=positiveBigInt(amountIn);if(!ADDRESS.test(tokenIn))fail("INVALID_STABLE_POOL","token is outside stable pool");const normalizedToken=tokenIn.toLowerCase(),zeroForOne=normalizedToken===pool.token0;if(!zeroForOne&&normalizedToken!==pool.token1)fail("INVALID_STABLE_POOL","token is outside stable pool");
  const reserveIn=zeroForOne?pool.reserve0:pool.reserve1,reserveOut=zeroForOne?pool.reserve1:pool.reserve0,multiplierIn=zeroForOne?pool.precisionMultiplier0:pool.precisionMultiplier1,multiplierOut=zeroForOne?pool.precisionMultiplier1:pool.precisionMultiplier0;const x=reserveIn*multiplierIn,y=reserveOut*multiplierOut,d=stableInvariant(x,y,BigInt(pool.amplification));const effective=input*(BPS-BigInt(pool.feeBps))/BPS;const after=stableGetY(x+effective*multiplierIn,d,BigInt(pool.amplification));if(after>=y+1n)fail("INSUFFICIENT_LIQUIDITY","stable quote has no output");const output=(y-after-1n)/multiplierOut;if(output<=0n||output>=reserveOut)fail("INSUFFICIENT_LIQUIDITY","stable quote is not executable");return output;
}

export function stableAmountIn({pool,tokenIn,amountOut}) {
  pool=parseStablePoolState(pool,{now:new Date(pool.asOf)});const output=positiveBigInt(amountOut);if(!ADDRESS.test(tokenIn))fail("INVALID_STABLE_POOL","token is outside stable pool");const normalizedToken=tokenIn.toLowerCase(),zeroForOne=normalizedToken===pool.token0;if(!zeroForOne&&normalizedToken!==pool.token1)fail("INVALID_STABLE_POOL","token is outside stable pool");
  const reserveIn=zeroForOne?pool.reserve0:pool.reserve1,reserveOut=zeroForOne?pool.reserve1:pool.reserve0,multiplierIn=zeroForOne?pool.precisionMultiplier0:pool.precisionMultiplier1,multiplierOut=zeroForOne?pool.precisionMultiplier1:pool.precisionMultiplier0;if(output>=reserveOut)fail("INSUFFICIENT_LIQUIDITY","stable output consumes reserve");const x=reserveIn*multiplierIn,yAfter=(reserveOut-output)*multiplierOut,d=stableInvariant(pool.reserve0*pool.precisionMultiplier0,pool.reserve1*pool.precisionMultiplier1,BigInt(pool.amplification));const xAfter=stableGetY(yAfter,d,BigInt(pool.amplification));if(xAfter<=x)fail("INSUFFICIENT_LIQUIDITY","stable inverse has no input");const effective=ceilDiv(xAfter-x,multiplierIn);let input=ceilDiv(effective*BPS,BPS-BigInt(pool.feeBps));while(stableAmountOut({pool,tokenIn,amountIn:input})<output)input++;while(input>1n&&stableAmountOut({pool,tokenIn,amountIn:input-1n})>=output)input--;return input;
}

export function quoteStableExactInput({amountIn,tokenIn,tokenOut,pools,maxHops=MAX_HOPS,now=new Date()}) {
  const parsed=pools.map(pool=>parseStablePoolState(pool,{now}));const routes=enumerateStableRoutes(tokenIn,tokenOut,parsed,maxHops);if(routes.length===0)fail("NO_ROUTE","no supported StableSwap route");const input=positiveBigInt(amountIn);const quotes=routes.map(route=>stableRouteExactInput(input,tokenIn,route));quotes.sort((a,b)=>a.amountOut===b.amountOut?routeKey(a).localeCompare(routeKey(b)):a.amountOut>b.amountOut?-1:1);return Object.freeze(quotes[0]);
}

export function quoteStableExactOutput({amountOut,tokenIn,tokenOut,pools,maxHops=MAX_HOPS,now=new Date()}) {
  const parsed=pools.map(pool=>parseStablePoolState(pool,{now}));const routes=enumerateStableRoutes(tokenIn,tokenOut,parsed,maxHops);if(routes.length===0)fail("NO_ROUTE","no supported StableSwap route");const output=positiveBigInt(amountOut);const quotes=routes.map(route=>stableRouteExactOutput(output,tokenOut,route));quotes.sort((a,b)=>a.amountIn===b.amountIn?routeKey(a).localeCompare(routeKey(b)):a.amountIn<b.amountIn?-1:1);return Object.freeze(quotes[0]);
}
...
function stableInvariant(x,y,amplification){const sum=x+y;if(sum===0n)return 0n;let invariant=sum;const ann=amplification*2n;for(let index=0;index<255;index++){const previous=invariant;let product=invariant*invariant/(x*2n);product=product*invariant/(y*2n);invariant=(ann*sum+product*2n)*invariant/((ann-1n)*invariant+product*3n);if(bigDifference(invariant,previous)<=1n)return invariant;}fail("STABLE_INVARIANT","StableSwap invariant did not converge");}
function stableGetY(known,invariant,amplification){const ann=amplification*2n;let c=invariant*invariant/(known*2n);c=c*invariant/(ann*2n);const b=known+invariant/ann;let y=invariant;for(let index=0;index<255;index++){const previous=y;y=(y*y+c)/(2n*y+b-invariant);if(bigDifference(y,previous)<=1n)return y;}fail("STABLE_INVARIANT","StableSwap balance solve did not converge");}
function stableRouteExactInput(input,tokenIn,route){let current=tokenIn.toLowerCase(),value=input;const path=[current],steps=[];let quotedAt=route[0].asOf;for(const pool of route){const zeroForOne=pool.token0===current,next=zeroForOne?pool.token1:pool.token0,reserveIn=zeroForOne?pool.reserve0:pool.reserve1,reserveOut=zeroForOne?pool.reserve1:pool.reserve0;const output=stableAmountOut({pool,tokenIn:current,amountIn:value});steps.push(Object.freeze({pool:pool.address,contractVersion:pool.contractVersion,tokenIn:current,tokenOut:next,amountIn:value,amountOut:output,reserveIn,reserveOut,feeBps:pool.feeBps,amplification:pool.amplification}));value=output;current=next;path.push(next);if(pool.asOf<quotedAt)quotedAt=pool.asOf;}return {kind:"exact-input",amountIn:input,amountOut:value,path:Object.freeze(path),steps:Object.freeze(steps),quotedAt,source:"fresh confirmed YNX Testnet StableSwap RPC state",version:"ynx-stable-route-quote-v1",confidence:"deterministic-preflight",failure:null};}
function stableRouteExactOutput(output,tokenOut,route){let current=tokenOut.toLowerCase(),value=output;const path=[current],steps=[];let quotedAt=route[0].asOf;for(let index=route.length-1;index>=0;index--){const pool=route[index],zeroForOne=pool.token1===current,previous=zeroForOne?pool.token0:pool.token1,reserveIn=zeroForOne?pool.reserve0:pool.reserve1,reserveOut=zeroForOne?pool.reserve1:pool.reserve0;const input=stableAmountIn({pool,tokenIn:previous,amountOut:value});steps.push(Object.freeze({pool:pool.address,contractVersion:pool.contractVersion,tokenIn:previous,tokenOut:current,amountIn:input,amountOut:value,reserveIn,reserveOut,feeBps:pool.feeBps,amplification:pool.amplification}));value=input;current=previous;path.push(previous);if(pool.asOf<quotedAt)quotedAt=pool.asOf;}return {kind:"exact-output",amountIn:value,amountOut:output,path:Object.freeze(path.reverse()),steps:Object.freeze(steps.reverse()),quotedAt,source:"fresh confirmed YNX Testnet StableSwap RPC state",version:"ynx-stable-route-quote-v1",confidence:"deterministic-preflight",failure:null};}
function enumerateStableRoutes(tokenIn,tokenOut,pools,maxHops){if(!ADDRESS.test(tokenIn)||!ADDRESS.test(tokenOut)||tokenIn.toLowerCase()===tokenOut.toLowerCase()||!Number.isInteger(maxHops)||maxHops<1||maxHops>MAX_HOPS)fail("INVALID_ROUTE","invalid StableSwap route bounds");const target=tokenOut.toLowerCase(),routes=[];const walk=(current,route,seen)=>{if(route.length>=maxHops)return;for(const pool of pools){if(route.some(item=>item.address===pool.address))continue;let next;if(pool.token0===current)next=pool.token1;else if(pool.token1===current)next=pool.token0;else continue;if(seen.has(next))continue;const candidate=[...route,pool];if(next===target)routes.push(candidate);else walk(next,candidate,new Set([...seen,next]));}};walk(tokenIn.toLowerCase(),[],new Set([tokenIn.toLowerCase()]));return routes;}
function isPowerOfTen(value){if(value<1n||value>10n**18n)return false;while(value>1n){if(value%10n!==0n)return false;value/=10n;}return true;}
function ceilDiv(a,b){return a===0n?0n:(a-1n)/b+1n;}
function bigDifference(a,b){return a>=b?a-b:b-a;}
