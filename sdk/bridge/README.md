...
The client exposes `getHealth()`, `getVersion()`, `getStateMachine()`, and `getProviders()` so consumers can reject stale runtime contracts and distinguish a configured provider identity from an available executable route. An empty incident list is accepted only when `incidentHistoryComplete=false`. Version `0.3.0` targets runtime schema `7` and state machine `ynx.bridge.lifecycle.v1`.
