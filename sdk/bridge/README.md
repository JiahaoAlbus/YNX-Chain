...
`@ynx-chain/bridge-sdk` is a dependency-free, read-only JavaScript client with TypeScript declarations for the public Bridge health, version, state-machine, transparency, route-catalog, provider-registry, asset-registry, and product-status surfaces. It validates source, runtime schema, state-machine version, timestamps, route and asset shape, dependency status, and fail-closed execution boundaries.
...
The client exposes `getHealth()`, `getVersion()`, `getStateMachine()`, and `getProviders()` so consumers can reject stale runtime contracts and distinguish a connected Provider API from an available executable route. An empty incident list is accepted only when `incidentHistoryComplete=false`. Version `0.3.1` accepts evidence-bound Circle CCTP V2 Provider/contract metadata while still rejecting external submission, asset availability, or public-route overclaims; it targets runtime schema `7` and state machine `ynx.bridge.lifecycle.v1`.
