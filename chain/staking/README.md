...
Committed state v10 implements signer-owned delegation to an active migration validator, a disclosed 10% candidate commission field, explicit reward source, seven-day unbonding liabilities, maturity-gated withdrawal, fee events, restart-safe state, and validator concentration summaries. Signed staking actions preserve the existing application envelope, nonce, fee, replay, and fail-closed chain-ID checks.
