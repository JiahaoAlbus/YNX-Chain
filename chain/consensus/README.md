...
`CONSENSUS_CANDIDATE_KEY_CEREMONY_APPROVED=yes ENV_FILE=.env.deploy make consensus-candidate-key-ceremony` performs that generation directly on the four assigned servers. It preflights every role before mutation, refuses existing key directories, writes mode-restricted host-local files under `/etc/ynx/consensus-candidate/<role>`, and retrieves only public ceremony records into ignored `tmp/consensus-candidate-key-ceremony/public`. If a network interruption leaves a partial completed ceremony, rerun with `CONSENSUS_CANDIDATE_KEY_CEREMONY_RESUME=yes`; inspect mode derives the same public records without rotating or replacing keys.

Cross-region RFC1918 reachability must be proven rather than assumed. When direct private routing is unavailable, `CONSENSUS_OVERLAY_KEY_CEREMONY_APPROVED=yes ENV_FILE=.env.deploy make consensus-overlay-key-ceremony` installs only the required WireGuard userspace tools when absent, generates one host-local mode-`0600` overlay key per role, and retrieves public overlay records for `10.77.42.1/32` through `10.77.42.4/32`. This command does not create an interface or change routes; overlay configuration, firewall approval, and peer reachability are separate gates.

`CONSENSUS_OVERLAY_DEPLOY_APPROVED=yes ENV_FILE=.env.deploy make deploy-consensus-overlay` builds a public-key-only full-mesh package, installs the `ynxwg0` oneshot service, and verifies three encrypted handshakes plus three overlay pings on every role. The interface carries only the four `/32` candidate routes. If verification fails, all overlay services are stopped; every path also checks that authoritative `ynx-chaind` remains active. Cloud security groups must permit UDP 51820 only between the four approved public server addresses.
