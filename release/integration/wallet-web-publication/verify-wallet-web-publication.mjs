...
async function artifactCheck(fetcher,origin,artifact){
  const url=`${origin}${artifact.officialPath}`,record={origin,name:artifact.name,url,status:null,contentType:null,contentDisposition:null,contentLength:null,downloadedBytes:null,downloadedSha256:null,passed:false,error:null};
  try{
    const response=await fetcher(url,{redirect:"error",signal:AbortSignal.timeout(30000)});record.status=response.status;record.contentType=response.headers.get("content-type");record.contentDisposition=response.headers.get("content-disposition");const length=response.headers.get("content-length");record.contentLength=length===null?null:Number(length);
