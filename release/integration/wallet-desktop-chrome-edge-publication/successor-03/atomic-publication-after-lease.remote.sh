...
chmod_refresh_identity() {
  local label="$1" expected_mode="$2" path actual_dev actual_ino actual_uid actual_gid actual_mode actual_nlink
  assert_identity "$label" || return 1
  path="${ID_PATH[$label]}"
  chmod "$expected_mode" "$path" || return 1
  test ! -L "$path" || return 1
  read -r actual_dev actual_ino actual_uid actual_gid actual_mode actual_nlink < <(stat_identity "$path") || return 1
  test "$actual_dev:$actual_ino:$actual_uid:$actual_gid:$actual_nlink" = "${ID_DEV[$label]}:${ID_INO[$label]}:${ID_UID[$label]}:${ID_GID[$label]}:${ID_NLINK[$label]}" || return 1
  test "$actual_mode" = "${expected_mode#0}" || return 1
  ID_MODE[$label]="$actual_mode"
...
self_test() {
  local root original_inode readable_zip_inode readable_config_inode
  root="$(mktemp -d)"
  ...
  chmod 0755 "$root"
  mkdir -m 0755 "$root/service-user-case"
  printf 'zip-payload' > "$root/service-user-case/upload.zip"
  printf 'candidate-config' > "$root/service-user-case/upload.caddy"
  chmod 0600 "$root/service-user-case/upload.zip" "$root/service-user-case/upload.caddy"
  capture_identity service_zip "$root/service-user-case/upload.zip" file
  capture_identity service_config "$root/service-user-case/upload.caddy" file
  readable_zip_inode="${ID_INO[service_zip]}"
  readable_config_inode="${ID_INO[service_config]}"
  if setpriv --reuid=65534 --regid=65534 --clear-groups test -r "$root/service-user-case/upload.zip"; then return 83; fi
  if setpriv --reuid=65534 --regid=65534 --clear-groups test -r "$root/service-user-case/upload.caddy"; then return 84; fi
  chmod_refresh_identity service_zip 0644
  chmod_refresh_identity service_config 0644
  test "${ID_INO[service_zip]}:${ID_MODE[service_zip]}" = "$readable_zip_inode:644"
  test "${ID_INO[service_config]}:${ID_MODE[service_config]}" = "$readable_config_inode:644"
  ln "$root/service-user-case/upload.zip" "$root/service-user-case/target.zip"
  ln "$root/service-user-case/upload.caddy" "$root/service-user-case/live.caddy"
  capture_identity service_target "$root/service-user-case/target.zip" file
  capture_identity service_live "$root/service-user-case/live.caddy" file
  unlink "$root/service-user-case/upload.zip"
  unlink "$root/service-user-case/upload.caddy"
  refresh_identity service_target
  refresh_identity service_live
  test "${ID_INO[service_target]}:${ID_MODE[service_target]}" = "$readable_zip_inode:644"
  test "${ID_INO[service_live]}:${ID_MODE[service_live]}" = "$readable_config_inode:644"
  setpriv --reuid=65534 --regid=65534 --clear-groups test -r "$root/service-user-case/target.zip"
  setpriv --reuid=65534 --regid=65534 --clear-groups test -r "$root/service-user-case/live.caddy"
  ...
  printf 'terminal_finalizer_self_test=pass scenarios=prepare-empty,upload-partial,publish-success,rollback,symlink-preserved,inode-substitution-preserved,service-user-0600-rejected,service-user-0644-readable,post-chmod-identity-refreshed\n'
}
...
restore_prior_config() {
  ...
  /usr/bin/sudo -n -u caddy test -r "$live" || return 1
  /usr/bin/caddy validate --config "$entry" --adapter caddyfile || return 1
  /usr/bin/sudo -n -u caddy /usr/bin/caddy validate --config "$entry" --adapter caddyfile || return 1
...
receipt_append() {
  ...
  chmod_refresh_identity receipt 0600 || return 1
  printf '%s %s\n' "$1" "$2" >> "$receipt" || return 1
  chmod_refresh_identity receipt 0400 || return 1
}
...
chmod_refresh_identity receipt 0400
...
chmod_refresh_identity uploaded_zip 0644
chmod_refresh_identity uploaded_caddy 0644
test "${ID_MODE[uploaded_zip]}" = 644
test "${ID_MODE[uploaded_caddy]}" = 644
receipt_append POST_CHMOD_UPLOAD_ZIP "$(stat_identity "$uploaded_zip") $(stat_bytes "$uploaded_zip") $(sha256_file "$uploaded_zip")"
receipt_append POST_CHMOD_UPLOAD_CADDY "$(stat_identity "$uploaded_caddy") $(stat_bytes "$uploaded_caddy") $(sha256_file "$uploaded_caddy")"
...
test "${ID_MODE[target]}" = 644
/usr/bin/sudo -n -u caddy test -r "$target"
...
test "${ID_MODE[live_candidate]}" = 644
/usr/bin/sudo -n -u caddy test -r "$live"
/usr/bin/caddy validate --config "$entry" --adapter caddyfile
/usr/bin/sudo -n -u caddy /usr/bin/caddy validate --config "$entry" --adapter caddyfile
