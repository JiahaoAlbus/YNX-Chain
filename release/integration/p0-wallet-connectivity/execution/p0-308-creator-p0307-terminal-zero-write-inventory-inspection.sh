...
inventory() {
  local label="$1" root="$2" entry rel encoded kind digest
  printf '%sRootTuple=' "$label"
  stat_tuple "$root"
  if [[ ! -d "$root" || -L "$root" ]]; then
    printf '%sInventory=UNAVAILABLE\n' "$label"
    return
  fi
  printf '%sInventoryBegin=true\n' "$label"
  while IFS= read -r -d '' entry; do
    rel="${entry#"$root"/}"
    encoded="$(printf '%s' "$rel" | /usr/bin/base64 | /usr/bin/tr -d '\n')"
    [[ -n "$encoded" ]] || encoded=-
    kind="$(/usr/bin/stat -c '%F' -- "$entry")"
    digest=-
    if [[ -f "$entry" && ! -L "$entry" ]]; then
      digest="$(/usr/bin/sha256sum -- "$entry" | /usr/bin/cut -d' ' -f1)"
    fi
    printf 'INVENTORY\t%s\t%s\t%s\t%s\t%s\n' "$label" "$encoded" "$kind" "$(stat_tuple "$entry")" "$digest"
  done < <(/usr/bin/find "$root" -mindepth 1 -print0 | /usr/bin/sort -z)
  printf '%sInventoryEnd=true\n' "$label"
}

printf 'inspection=CREATOR_P0307_TERMINAL_ZERO_WRITE_INVENTORY_FREEZE\n'
for entry in \
...
for parent in \
  /opt/ynx-release-control-plane \
  /opt/ynx-creator-studio-wallet/releases \
  /opt/ynx-creator-studio-wallet/receipts; do
  printf 'parent:%s=' "$(printf '%s' "$parent" | /usr/bin/base64 | /usr/bin/tr -d '\n')"
  stat_tuple "$parent"
done
...
inventory candidateInventory /opt/ynx-creator-studio-wallet/releases/creator-studio-3f97a13d
inventory controlInventory /opt/ynx-release-control-plane/creator-studio-3f97a13d
