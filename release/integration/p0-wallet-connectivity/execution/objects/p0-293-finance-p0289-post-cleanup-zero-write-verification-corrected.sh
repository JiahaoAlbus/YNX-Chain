...
absent(){ [[ ! -e "$1" && ! -L "$1" ]]; }
assert_direct_file(){ local p=$1 t=$2 b=$3 s=$4; [[ -f "$p" && ! -L "$p" && "$(realpath -e -- "$p")" == "$p" && "$(tuple_l "$p")" == "$t" && "$(bytes_file "$p")" == "$b" && "$(sha_file "$p")" == "$s" ]]; }
http_exact(){
...
printf 'verification=FINANCE_P0289_POST_CLEANUP_ZERO_WRITE_CORRECTED\n'
for path in \
...
RELEASE=/opt/ynx/releases/finance/ynx-finance-3b2383f5c18a
CURRENT_BINARY=$CURRENT/ynx-finance
RESOLVED_BINARY=$RELEASE/ynx-finance
[[ -L "$CURRENT" && "$(tuple_l "$CURRENT")" == '64770:1312291:0:0:777:1:50:symbolic link' ]]
[[ "$(readlink -- "$CURRENT")" == "$RELEASE" && "$(realpath -e -- "$CURRENT")" == "$RELEASE" ]]
[[ -f "$CURRENT_BINARY" && ! -L "$CURRENT_BINARY" ]]
[[ "$(realpath -e -- "$CURRENT_BINARY")" == "$RESOLVED_BINARY" ]]
[[ "$(tuple_l "$CURRENT_BINARY")" == '64770:1860512:0:0:755:1:8573112:regular file' ]]
[[ "$(bytes_file "$CURRENT_BINARY")" == 8573112 && "$(sha_file "$CURRENT_BINARY")" == 0cc43c8a77c12975a0fcbada65971f08f2bc3a52345d547ea194dd3ccd60d83f ]]
assert_direct_file /etc/ynx/finance.env '64770:1324724:0:986:640:1:1545:regular file' 1545 854e7f1077e8fa4d5a4741918e25d04b0c1a109f2eb9c716b72dae918aa5f252
assert_direct_file /etc/systemd/system/ynx-finance.service '64770:161304:0:0:644:1:515:regular file' 515 2e72cdad422a3a714c46d074ea97b725233576cf726dbbfd43e82e99c2c2975b
assert_direct_file /etc/caddy/conf.d/ynx-finance.caddy '64770:1051759:0:0:644:1:255:regular file' 255 dcf75a7aed315c54632321b8bb80e44c0abc22f6700fadfbfa9a7da21b88a282
absent /var/lib/ynx/finance/state.json
printf 'currentTarget=%s\nresolvedBinary=%s\noldBinarySha256=0cc43c8a77c12975a0fcbada65971f08f2bc3a52345d547ea194dd3ccd60d83f\nenvSha256=854e7f1077e8fa4d5a4741918e25d04b0c1a109f2eb9c716b72dae918aa5f252\nunitSha256=2e72cdad422a3a714c46d074ea97b725233576cf726dbbfd43e82e99c2c2975b\ncaddySha256=dcf75a7aed315c54632321b8bb80e44c0abc22f6700fadfbfa9a7da21b88a282\nstate=absent\n' "$RELEASE" "$RESOLVED_BINARY"
...
[[ "$(systemctl show ynx-finance.service -p WorkingDirectory --value)" == "$CURRENT" ]]
[[ "$(systemctl show ynx-finance.service -p User --value)" == ynx ]]
[[ "$(systemctl show ynx-finance.service -p Group --value)" == ynx ]]
printf 'service=loaded:active:running:2241003:0:/opt/ynx/finance-current:ynx:ynx\n'
