...
## 2026-08-31 ecosystem evidence-state follow-up

- Ecosystem cards now use the required evidence-bound state vocabulary. Developer and Infrastructure are shown as `Testnet` only because this local portal can verify their 6423 Testnet path; every other product remains `Unavailable` without a verified public product route, documentation route, or download artifact.
- The 6423 support field no longer says a vague endpoint-dependent state. The two local Testnet cards explicitly say they are verified only in this local 6423 Testnet portal; all other cards state unavailable.
- Browser evidence: the Chinese full directory rendered two Testnet cards and eight unavailable cards. Each unavailable card retained three disabled Open/Docs/Download controls with their reason, while Status stayed actionable and opened the in-portal availability notice. At 390 x 844 CSS px, cards were 351 px wide and the document had no horizontal overflow.

## 2026-08-31 wallet-download-governance boundary audit
