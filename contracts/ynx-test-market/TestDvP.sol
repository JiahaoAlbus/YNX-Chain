...
contract TestDvP {
    ...
    constructor(address stock_, address tusd_, address feeRecipient_, uint256 feeBps_) {
        if (block.chainid != 6423 || stock_ == address(0) || tusd_ == address(0) || stock_ == tusd_ ||
            feeRecipient_ == address(0) || feeBps_ > 500) revert Invalid();
        ...
        if (stock.TEST_MARKER() != bytes32("YNX-6423-TEST-ONLY") || tusd.TEST_MARKER() != bytes32("YNX-6423-TEST-ONLY") ||
            stock.decimals() != 6 || tusd.decimals() != 6) revert Invalid();
    }
