...
const stockCodeHash = ethers.keccak256(await ethers.provider.getCode(await stock.getAddress()));
const cashCodeHash = ethers.keccak256(await ethers.provider.getCode(await cash.getAddress()));
const settlement = await DvP.deploy(await stock.getAddress(), await cash.getAddress(), stockCodeHash, cashCodeHash,
  treasury.address, catalog.settlement.feeBps);
...
const manifest = {
  ...
  constructorInputs: {
    ...
    settlement: { stock: "DEPLOYED_TEST_AAPL", cash: "DEPLOYED_TUSD", stockCodeHash, cashCodeHash,
      feeRecipient: "CONFIGURE_QA_FEE_RECIPIENT", feeBps: catalog.settlement.feeBps }
  },
  creationBytecodeKeccak256: {
    TestAsset: ethers.keccak256(Asset.bytecode),
    TestDvP: ethers.keccak256(DvP.bytecode)
  },
  approvedAssetRuntimeCodeHash: { TestAAPL: stockCodeHash, tUSD: cashCodeHash },
