...
func main() {
	...
	defer stop()
	srv := &http.Server{Addr: *httpAddr, Handler: mutationfreeze.FromEnv(resourcegateway.NewServerWithBuild(service, currentBuildInfo()).Handler()), ReadHeaderTimeout: 5 * time.Second}
	go func() {
		<-ctx.Done()
