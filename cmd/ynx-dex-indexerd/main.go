...
func main() {
	...
	if factory := strings.TrimSpace(os.Getenv("DEX_FACTORY_ADDRESS")); factory != "" {
		...
		poller, err := dex.NewEVMPoller(store, dex.EVMPollerConfig{RPCURL: env("YNX_EVM_RPC_URL", "https://evm.ynxweb4.com"), Factory: factory, StrategyVault: strings.TrimSpace(os.Getenv("DEX_STRATEGY_VAULT_ADDRESS")), StartBlock: startBlock, Confirmations: confirmations, CursorPath: env("YNX_DEX_CURSOR_PATH", "tmp/dex/evm-cursor.json"), CursorSecret: stateSecret})
		if err != nil {
			log.Fatal(err)
		}
		go func() {
