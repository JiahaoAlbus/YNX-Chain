import {
  ...
  walletChoiceNeedsResolution,
  walletCandidatesFromError,
...
let walletConnectState = createStandardWalletConnectState();
...
function resetWallet(message = "Wallet disconnected. Guest playback remains available.") {
  currentWallet = null;
  walletConnectState = reduceStandardWalletConnectState(walletConnectState, {type: "DISCONNECT"});
...
function renderWalletState(state) {
  walletConnectState = state.connectState || walletConnectState;
  ...
  storageState.write({
    ...
    providerRdns: state.providerInfo?.rdns || "",
  });
}
...
function bindWalletEvents(walletState) {
  ...
  const onAccountsChanged = (accounts) => {
    walletConnectState = reduceStandardWalletConnectState(walletConnectState, {type: "ACCOUNTS_CHANGED", accounts});
    if (walletConnectState.status === STANDARD_WALLET_CONNECT_STATUS.DISCONNECTED) return resetWallet("Wallet account permission was removed. Reconnect to continue.");
    currentWallet = {...currentWallet, account: walletConnectState.account, connectState: walletConnectState};
    renderWalletState(currentWallet);
    setConnectionMessage(`Account changed. Reconnected as ${maskAccount(walletConnectState.account)}.`);
  };

  const onChainChanged = (chainId) => {
    walletConnectState = reduceStandardWalletConnectState(walletConnectState, {type: "CHAIN_CHANGED", chainId: String(chainId)});
    if (walletConnectState.status !== STANDARD_WALLET_CONNECT_STATUS.CONNECTED) {
      setConnectionMessage("Wallet chain changed away from 0x1917; reconnect or switch back in your Wallet to continue.");
      return;
    }
    currentWallet = {...currentWallet, chainId: walletConnectState.chainId, connectState: walletConnectState};
    renderWalletState(currentWallet);
    setConnectionMessage(`Wallet switched back to ${currentWallet.chainId}.`);
  };

  const onDisconnect = () => {
    walletConnectState = reduceStandardWalletConnectState(walletConnectState, {type: "PROVIDER_DISCONNECT"});
    resetWallet("Provider disconnected. Reconnect to continue.");
  };
...
function renderWalletChoices(candidates) {
  ...
  $("#wallet-chooser-title").textContent = "Select a wallet";
  $("#wallet-choices").replaceChildren();

  for (const candidate of candidates) {
    const walletId = candidate.id || candidate.info?.uuid || candidate.uuid || candidate.providerId || "";
    ...
    const iconMarkup = icon ? `<img src="${esc(icon)}" alt="${esc(candidate.label)} logo" />` : `<span class="wallet-choice-fallback">${candidate.isMetaMask ? "MM" : "YNX"}</span>`;
    ...
    item.innerHTML = `<span class="wallet-choice-logo ${role}">${iconMarkup}</span><span><strong>${esc(label)}</strong><small>${esc(candidate.rdns || candidate.info?.rdns || "Provider identity unavailable")}</small></span>`;
    item.onclick = async () => {
      chooser.close();
...
function showConnectionDetails() {
  ...
  const chooser = $("#wallet-chooser");
  const info = currentWallet.providerInfo || {};
  const icon = info.icon ? `<img src="${esc(info.icon)}" alt="${esc(getWalletName(currentWallet))} logo">` : `<span class="wallet-choice-fallback">${info.isMetaMask ? "MM" : "YNX"}</span>`;
  $("#wallet-chooser-title").textContent = "Wallet connection details";
  $("#wallet-choices").innerHTML = `<section class="wallet-details"><div class="wallet-details-identity"><span class="wallet-choice-logo">${icon}</span><div><strong>${esc(getWalletName(currentWallet))}</strong><small>${esc(info.rdns || "Provider identity unavailable")}</small></div></div><dl><div><dt>Account</dt><dd>${esc(currentWallet.account)}</dd></div><div><dt>Network</dt><dd>YNX Testnet · ${esc(currentWallet.chainId)}</dd></div></dl><div class="wallet-detail-actions"><button id="wallet-switch-account" type="button">Switch account</button><button id="wallet-disconnect" class="outline" type="button">Disconnect</button></div></section>`;
  $("#wallet-switch-account").onclick = async () => {
    try {
      const account = await requestWalletAccountSwitch(currentWallet.provider);
      walletConnectState = reduceStandardWalletConnectState(walletConnectState, {type: "ACCOUNTS_CHANGED", accounts: [account]});
      currentWallet = {...currentWallet, account, connectState: walletConnectState};
      renderWalletState(currentWallet);
      chooser.close();
      setConnectionMessage(`Account changed. Reconnected as ${maskAccount(account)}.`);
    } catch (error) { notice(error.message || "Account selection failed", true); }
  };
  $("#wallet-disconnect").onclick = async () => { chooser.close(); await revokeWallet("user-requested"); };
  $("#wallet-chooser-close").onclick = () => chooser.close();
  chooser.showModal();
}
...
async function connectVideoWalletInteractive(walletId = null) {
  try {
    ...
    const result = await connectVideoWallet(window, {timeoutMs: 1600, walletId});
    currentWallet = {...result, walletKind: result.walletKind};
    renderWalletState(currentWallet);
...
async function restoreWalletFromSession() {
  ...
  if (!state?.walletId || !state?.account) return null;
  try {
    const restored = await restoreVideoWallet(window, state);
    if (!restored || restored.account.toLowerCase() !== state.account.toLowerCase()) return resetWallet("Refresh found no approved Wallet session. Choose a provider to continue.");
...
async function revokeWallet(reason = "user") {
  if (!currentWallet) return;
  await revokeWalletPermissions(currentWallet.provider);
...
$("#signin").onclick = async () => {
  if (currentWallet) return showConnectionDetails();
  try {
    const candidates = await discoverWalletCandidates(window, {timeoutMs: 1600});
    ...
    if ((ynxCount >= 1 && mmCount >= 1) || candidates.length > 1) {
      walletChoiceOptions(null, candidates);
      return;
    }
...
clearLegacySession();
getWalletProviderRegistry(window);
