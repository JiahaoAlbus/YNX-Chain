...
set -Eeuo pipefail
[[ $EUID == 0 ]]
source=29896f10c5e59fc17590dd3697d6e7af3d34a809
mode=creator
[[ ${1:-creator} == creator ]]
case "$mode" in
 api)
  ...
  config=/etc/systemd/system/$unit.d/20260906-v3-channel-id.conf
 ...
 creator)
  ...
  artifact=/var/tmp/ynx-creator-29896f10c-runtime.tar.gz
  artifact_sha=e8ec4bf7584a3811015980a5b71912a9e2a0bbc755f09731590fc8975b883481
  ...
  [[ $(systemctl show "$unit" -p DropInPaths --value) == /etc/systemd/system/$unit.d/20260906-v2-consumer.conf ]]
  [[ $(sha256sum /etc/systemd/system/$unit.d/20260906-v2-consumer.conf | cut -d' ' -f1) == 07a65496a21a81a367d65e9e18e9db4d1703b794c3f1608e0f2de92c66b75ccd ]]
  ;;
 *) exit 2;;
esac
[[ ! -e $release && ! -e $config ]]
...
if [[ $mode == api ]]; then
 ...
 curl -fsS --max-time 10 http://127.0.0.1:6495/creator-studio.manifest.json | python3 -c 'import json,sys; v=json.load(sys.stdin); assert v["sourceCommit"]=="29896f10c5e59fc17590dd3697d6e7af3d34a809"; print("CREATOR_RUNTIME_SOURCE="+v["sourceCommit"])'
