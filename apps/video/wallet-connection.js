...
export function reduceStandardWalletConnectState(state = REDUCE_STATES.RESTORE, action, payload = null) {
  ...
  if (action === REDUCE_ACTIONS.PRIVATE_SERVICE_DEGRADED) return Object.freeze({state: REDUCE_STATES.PRIVATE_SERVICE_DEGRADED, ...payload});
  if (action === REDUCE_ACTIONS.PRIVATE_SESSION_DEGRADED) return Object.freeze({state: REDUCE_STATES.PRIVATE_SESSION_DEGRADED, ...payload});
