...
test("connection approval persists before response and restores the exact origin-bound session",async()=>{
  ...
  const second=setup(first.storage);const restored=await second.controller.restore(identity.account,"https://fallback.example");assert.equal(restored.activeOrigin,origin);assert.equal(second.controller.pendingConnection()?.id,1);assert.equal(second.controller.canApprovePending(),false);await assert.rejects(second.controller.approveConnection(identity.account,origin),/exact request replay/);await assert.rejects(second.controller.receive(origin,"Wrong DApp",message(101,"eth_requestAccounts")),/differs/);await second.controller.receive(origin,"External DApp",message(101,"eth_requestAccounts"));assert.equal(second.controller.canApprovePending(),true);assert.equal(second.controller.pendingConnection()?.id,101);assert.equal(state(first.storage).pending.value.id,101);
  await second.controller.approveConnection(identity.account,origin);assert.equal(state(first.storage).pending,null);assert.equal(state(first.storage).sessions[0].account,account);assert.deepEqual(second.responses,[{id:101,jsonrpc:"2.0",result:[account]}]);
...
test("pending signing request survives process reconstruction and remains single",async()=>{
  ...
  const second=setup(first.storage);const restored=await second.controller.restore(identity.account,origin);assert.equal(restored.restoredPending,true);assert.equal(second.controller.pendingRequest()?.method,"personal_sign");assert.equal(second.controller.canApprovePending(),false);await assert.rejects(second.controller.receive(origin,"External DApp",message(4,"personal_sign",["0x00",account])),/differs/);await second.controller.receive(origin,"External DApp",message(202,"personal_sign",["0x6869",account]));assert.equal(second.controller.canApprovePending(),true);assert.equal(second.controller.pendingRequest()?.requestId,"n:202");assert.equal(state(first.storage).pending.value.requestId,"n:202");
  const raced=await Promise.allSettled([second.controller.receive(origin,"External DApp",message(3,"eth_chainId")),second.controller.receive(origin,"External DApp",message(4,"personal_sign",["0x00",account]))]);assert.equal(raced[0].status,"rejected");assert.equal(raced[1].status,"rejected");assert.equal(second.controller.pendingRequest()?.requestId,"n:202");
});

test("reconstructed request responds only to the newly rebound live DApp promise",async()=>{
  const first=await restoreAndConnect();await first.controller.receive(origin,"External DApp",message(2,"personal_sign",["0x6869",account]));const second=setup(first.storage);await second.controller.restore(identity.account,origin);await second.controller.receive(origin,"External DApp",message(77,"personal_sign",["0x6869",account]));await second.controller.rejectRequest();assert.deepEqual(second.responses,[{id:77,jsonrpc:"2.0",error:{code:4001,message:"User rejected DApp request"}}]);assert.equal(state(first.storage).pending,null);
});
