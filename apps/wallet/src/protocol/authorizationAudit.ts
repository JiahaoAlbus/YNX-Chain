...
export class AuthorizationAuditStore {
  ...
  private async appendExclusive(request:AuthorizationRequest, input:{action:AuthorizationAuditAction;account:string;at:string}, assertActive:()=>void):Promise<AuthorizationAuditRecord> {
    ...
    return this.appendFromRecords(records,request,input,assertActive);
  }

  private async appendFromRecords(records:readonly AuthorizationAuditRecord[],request:AuthorizationRequest, input:{action:AuthorizationAuditAction;account:string;at:string}, assertActive:()=>void):Promise<AuthorizationAuditRecord> {
