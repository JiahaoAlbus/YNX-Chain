...
import { createProductionFaucetTransport, type FaucetEndpointRoute, type NativeFaucetTransport, type FaucetHttpRequest, type FaucetHttpResponse } from "../../modules/ynx-faucet-transport";
...
const endpoints = Object.freeze({
  primary: Object.freeze({ admit: NATIVE_FAUCET_AUTHORITY + "/request", rpc: NATIVE_FAUCET_CHAIN_ORIGIN }),
  legacy: Object.freeze({ admit: LEGACY_FAUCET_AUTHORITY + "/request", rpc: LEGACY_FAUCET_CHAIN_ORIGIN + "/evm" }),
});
...
export class FaucetNativeSession {
  ...
  constructor(private readonly host: NativeFaucetTransport, private readonly signal: AbortSignal, private readonly route: FaucetEndpointRoute = "primary") {
    const endpoint = endpoints[route];
    this.rpc = Object.freeze({ origin: endpoint.rpc, request: (method: FaucetReadMethod, params: readonly string[]) => this.read(method, params) });
  ...
  private exchange(purpose: "admit" | "rpc", input: (taskId: string) => FaucetHttpRequest): Promise<FaucetHttpResponse> {
    const endpoint = endpoints[this.route];
...
/** Production availability is owned by the compiled native module. JavaScript
 * cannot supply an endpoint or turn the bridge on with a runtime flag. */
export function createProductionFaucetSession(signal: AbortSignal, route: FaucetEndpointRoute = "primary"): FaucetNativeSession | null {
  const host = createProductionFaucetTransport(route); return host ? new FaucetNativeSession(host, signal, route) : null;
}
