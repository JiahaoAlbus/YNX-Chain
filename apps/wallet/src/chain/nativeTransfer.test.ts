...
test("REST account-not-found preserves factual nonzero RPC balance and nonce",async()=>{
...
test("RPC transport timeout retries once and never converts failure to zero",async()=>{
  let rpcCalls=0;
  const client=new NativeChainClient(undefined,undefined,async(url,init)=>{
    if(url.startsWith(DEFAULT_CHAIN_API))return response({error:"account not found"},404);
    rpcCalls+=1;
  ...
  await assert.rejects(()=>client.account(account),/timed out/);
  assert.equal(rpcCalls,2,"only the single bounded transport retry is allowed");
});
