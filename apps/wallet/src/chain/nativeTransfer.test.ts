...
test("native client loads exact account/activity and broadcasts only matching signed result",async()=>{
  ...
  assert.ok(calls.every(({url})=>url.startsWith(`${DEFAULT_CHAIN_API}/`)),"native REST calls must never be sent to the JSON-RPC host");
});

test("native client rejects mismatched authoritative identity and broadcast",async()=>{
  const mismatch=new NativeChainClient("https://rest.ynxweb4.com",async()=>response({account:{address:"0x"+"1".repeat(40),balance:1,nonce:0}}));
  await assert.rejects(()=>mismatch.account(account),/identity/);
  const broadcast=new NativeChainClient("https://rest.ynxweb4.com",async()=>response({transaction:{hash:"0x"+"0".repeat(64),from:signed.transaction.from,to:signed.transaction.to,amount:25,fee:1,nonce:7},replayed:false,truthfulStatus:"signature-verified-authoritative-native-transfer"}));
