...
export type WalletConnectBroadcastBinding=Readonly<{topic:string;requestId:number;sessionBinding:string;requestDigest:string}>;
export type BroadcastRecord = Readonly<{ version: 2|3; account: string; rawTransaction: string; transactionHash: string; status: "broadcasting" | "acknowledged" | "uncertain" | "confirmed" | "rejected" | "cancelled"; attempt: number; unknownHistory: boolean; createdAt: string;binding?:WalletConnectBroadcastBinding }>;
...
export class WalletConnectBroadcastJournal {
  ...
  async broadcast(account: string, signed: SignedEvmTransaction, rpc: Rpc = ynxEvmRpc): Promise<string> { return this.#broadcast(account,signed,rpc,()=>{},null); }
  async broadcastAuthorized(account: string, signed: SignedEvmTransaction, assertCurrent:()=>void,binding:WalletConnectBroadcastBinding, rpc: Rpc = ynxEvmRpc): Promise<string> { return this.#broadcast(account,signed,rpc,assertCurrent,binding); }
  async #broadcast(account: string, signed: SignedEvmTransaction, rpc: Rpc, assertCurrent:()=>void,binding:WalletConnectBroadcastBinding|null): Promise<string> {
    assertCurrent();const normalized = address(account); if (await this.read(normalized)) fail("TRANSACTION_PENDING", "A prior WalletConnect transaction requires status resolution.");assertCurrent();
    const record: BroadcastRecord = frozen({ version: binding?3:2, account: normalized, rawTransaction: signed.rawTransaction, transactionHash: signed.transactionHash, status: "broadcasting", attempt: 1, unknownHistory: false, createdAt: new Date().toISOString(),...(binding?{binding}:{}) }); await this.#write(record);assertCurrent();
    await requireDurability(rpc,assertCurrent);assertCurrent();return this.#dispatch(record, rpc,assertCurrent);
  }
  async retryOriginal(account: string, rpc: Rpc = ynxEvmRpc): Promise<string> {return this.#retryOriginal(account,rpc,()=>{});}
  async retryOriginalAuthorized(account: string,assertCurrent:()=>void,rpc: Rpc = ynxEvmRpc): Promise<string> {return this.#retryOriginal(account,rpc,assertCurrent);}
  async #retryOriginal(account: string,rpc:Rpc,assertCurrent:()=>void): Promise<string> {assertCurrent();const record = await this.read(account);assertCurrent(); if (!record || !["broadcasting","acknowledged","uncertain"].includes(record.status)) fail("TRANSACTION_NOT_PENDING", "No original signed transaction is pending."); await requireDurability(rpc,assertCurrent);assertCurrent(); const next = frozen({ ...record, status: "broadcasting" as const, attempt: record.attempt + 1, unknownHistory: true }); await this.#write(next);assertCurrent(); return this.#dispatch(next, rpc,assertCurrent); }
  ...
  async #dispatch(record: BroadcastRecord, rpc: Rpc,assertCurrent:()=>void=()=>{}): Promise<string> { try { assertCurrent();const hash = await rpc("eth_sendRawTransaction", [record.rawTransaction]);assertCurrent(); if (typeof hash !== "string" || hash.toLowerCase() !== record.transactionHash) fail("TRANSACTION_HASH_MISMATCH", "RPC acknowledged a different transaction hash."); await this.#write({ ...record, status: "acknowledged", unknownHistory: true });assertCurrent(); return record.transactionHash; } catch (error: any) { if (!record.unknownHistory && record.attempt === 1 && error?.code === -32003 && error?.rpcResponseValidated === true) { await this.#write({ ...record, status: "rejected" }); throw error; } await this.#write({ ...record, status: "uncertain", unknownHistory: true }); throw Object.assign(new Error("Broadcast outcome is uncertain. The original signed transaction is saved; check or resend only these bytes."), { code: "BROADCAST_UNKNOWN", cause: error }); } }
...
function parseRecord(value: any, account: string): BroadcastRecord { try { const tx = Transaction.from(value.rawTransaction),binding:any=value.binding;if (![2,3].includes(value.version) || value.version===2&&binding!==undefined || value.version===3&&(!binding||typeof binding!=="object"||Array.isArray(binding)||Object.getPrototypeOf(binding)!==Object.prototype||!/^([0-9a-f]{64})$/.test(binding.topic)||!Number.isSafeInteger(binding.requestId)||binding.requestId<1||!/^([0-9a-f]{64})$/.test(binding.sessionBinding)||!/^([0-9a-f]{64})$/.test(binding.requestDigest)) || value.account !== account || tx.from?.toLowerCase() !== account || tx.hash?.toLowerCase() !== value.transactionHash || tx.chainId !== 6423n || !Number.isSafeInteger(value.attempt) || value.attempt < 1 || typeof value.unknownHistory !== "boolean" || !["broadcasting", "acknowledged", "uncertain", "confirmed", "rejected", "cancelled"].includes(value.status) || !Number.isFinite(Date.parse(value.createdAt))) throw new Error(); return frozen(value); } catch { fail("BROADCAST_RECORD_INVALID", "Stored WalletConnect transaction is invalid; sending remains disabled."); } }
async function requireDurability(rpc: Rpc,assertCurrent:()=>void=()=>{}): Promise<typeof DURABILITY_MODEL> {assertCurrent();const chain=await rpc("eth_chainId", []);assertCurrent();if (chain !== EVM_CHAIN_HEX) fail("WRONG_NETWORK", "RPC did not prove YNX Testnet chain 6423.");const value: any = await rpc("ynx_getDurabilityModel", []);assertCurrent(); if (!plain(value) || Object.keys(value).length !== Object.keys(DURABILITY_MODEL).length || Object.entries(DURABILITY_MODEL).some(([key, expected]) => value[key] !== expected)) fail("DURABILITY_UNCONFIRMED", "RPC did not prove the required local durability model."); return DURABILITY_MODEL; }
