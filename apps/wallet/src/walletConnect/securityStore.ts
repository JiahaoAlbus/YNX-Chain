...
type State=Readonly<{version:2;sessions:readonly WalletConnectSessionApproval[];replay:ReplaySnapshot;outbox:readonly WalletConnectResponseRecord[]}>;
...
export class WalletConnectSecurityStore{
  ...
  load():Promise<{sessions:readonly WalletConnectSessionApproval[];replayStore:WalletConnectRequestReplayStore}>{return this.#enqueue(async()=>{const state=await this.#read();return{sessions:state.sessions,replayStore:new WalletConnectRequestReplayStore(state.replay)}})}
  session(topic:string):Promise<WalletConnectSessionApproval|null>{return this.#enqueue(async()=>(await this.#read()).sessions.find(item=>item.topic===topic)??null)}
  outbox():Promise<readonly WalletConnectResponseRecord[]>{return this.#enqueue(async()=>(await this.#read()).outbox)}
  saveSession(approval:WalletConnectSessionApproval):Promise<void>{return this.#enqueue(async()=>{const state=await this.#read(),previous=state.sessions.find(item=>item.topic===approval.topic),sessions=[...state.sessions.filter(item=>item.topic!==approval.topic),approval];if(sessions.length>50)throw new Error("WalletConnect session limit reached");const changed=previous&&(previous.sessionBinding!==approval.sessionBinding||previous.account!==approval.account);await this.#write({...state,sessions,outbox:changed?quarantineTopic(state.outbox,approval.topic,new Date()):state.outbox})})}
  removeSession(topic:string):Promise<void>{return this.#enqueue(async()=>{const state=await this.#read();await this.#write({...state,sessions:state.sessions.filter(item=>item.topic!==topic),outbox:quarantineTopic(state.outbox,topic,new Date())})})}
  reconcileSession(topic:string,namespaces:SessionTypes.Namespaces,account:string):Promise<"current"|"missing"|"changed">{return this.#enqueue(async()=>{const approval=(await this.#read()).sessions.find(item=>item.topic===topic);if(!approval)return"missing";const next=normalizedNamespaces(namespaces),expected=normalizedNamespaces(approval.namespaces as unknown as SessionTypes.Namespaces);return approval.account===account&&JSON.stringify(next)===JSON.stringify(expected)?"current":"changed"})}
  reconcileActiveSessions(activeSessions:readonly ActiveSession[],account:string):Promise<WalletConnectSessionReconciliation>{
    ...
    return this.#enqueue(async()=>{const state=await this.#read(),retained=state.sessions.filter(approval=>{const active=activeByTopic.get(approval.topic);return Boolean(active&&approval.account===account&&sameNamespaces(active.namespaces,approval.namespaces as unknown as SessionTypes.Namespaces))}),retainedTopics=new Set(retained.map(approval=>approval.topic)),disconnectTopics=activeSessions.filter(session=>!retainedTopics.has(session.topic)).map(session=>session.topic),prunedTopics=state.sessions.filter(approval=>!retainedTopics.has(approval.topic)).map(approval=>approval.topic);if(retained.length!==state.sessions.length){const removed=new Set(prunedTopics);await this.#write({...state,sessions:retained,outbox:state.outbox.map(item=>removed.has(item.topic)?quarantined(item,new Date()):item)})}return Object.freeze({disconnectTopics:Object.freeze(disconnectTopics),prunedTopics:Object.freeze(prunedTopics)})});
  }
  ...
  async #read():Promise<State>{
    ...
    if(value.version===1&&Object.keys(value).sort().join(",")==="replay,sessions,version")return Object.freeze({version:2,sessions,replay:Object.freeze(value.replay),outbox:Object.freeze([])});
    if(value.version!==2||!Array.isArray(value.outbox)||Object.keys(value).sort().join(",")!=="outbox,replay,sessions,version"||value.outbox.length>MAX_OUTBOX_RECORDS)throw new Error("WalletConnect security state is invalid");
    const outbox=Object.freeze((value.outbox as unknown[]).map(parseOutboxRecord));if(new Set(outbox.map(item=>item.key)).size!==outbox.length)throw new Error("WalletConnect response outbox contains duplicates");validateStateBindings(sessions,value.replay,outbox);return Object.freeze({version:2,sessions,replay:Object.freeze(value.replay),outbox});
  }
