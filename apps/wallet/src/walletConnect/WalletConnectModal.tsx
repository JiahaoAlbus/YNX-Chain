...
function WalletConnectSheet({ account,withAccountSecret,operations,inbound,clearInbound,close }: { account: WalletAccount;withAccountSecret:SecretAccess;operations:WalletOperationLifecycle;inbound: string | null; clearInbound: () => void; close: () => void }) {
  ...
  const retrySessionCleanup = async (topic:string) => {setBusy(true);setError(null);try{await retryQuarantinedWalletConnectSession(walletConnectRuntime,securityStore,topic)}catch(caught){setError(message(caught))}finally{setBusy(false)}};
  ...
  return <Modal visible transparent animationType="slide" onRequestClose={closeReviewSheet}><View style={s.backdrop}><ScrollView contentContainerStyle={s.sheet} keyboardShouldPersistTaps="handled"><View style={s.header}><Text style={s.title}>WalletConnect</Text><Pressable accessibilityRole="button" accessibilityLabel="Close WalletConnect" disabled={busy} onPress={closeReviewSheet}><Text style={s.close}>Close</Text></Pressable></View>
    ...
    <Text style={s.section}>Active sessions</Text>{walletConnectRuntime.quarantinedTopics().map(topic=><View key={topic} style={s.card}><Text style={s.cardTitle}>Session cleanup required</Text><Text style={s.body}>This session cannot approve requests. Retry removal of the local grant and Relay session.</Text><Text style={s.caption}>{topic.slice(0,12)}…{topic.slice(-8)}</Text><Pressable accessibilityRole="button" accessibilityLabel={`Retry WalletConnect session cleanup ${topic}`} disabled={busy} onPress={()=>void retrySessionCleanup(topic)}><Text style={s.danger}>Retry session cleanup</Text></Pressable></View>)}{snapshot.sessions.length === 0 ? <Text style={s.body}>No dApp sessions are connected.</Text> : snapshot.sessions.map(session => <View key={session.topic} style={s.card}><Text style={s.cardTitle}>{session.peer.metadata.name}</Text><Text style={s.caption}>{session.peer.metadata.url}{"\n"}{session.topic.slice(0, 12)}…{session.topic.slice(-8)}</Text><Pressable accessibilityRole="button" accessibilityLabel={`Disconnect ${session.peer.metadata.name}`} disabled={busy} onPress={() => void disconnect(session.topic)}><Text style={s.danger}>Disconnect</Text></Pressable></View>)}
