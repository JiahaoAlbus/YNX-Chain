...
test("critical Wallet controls expose accessibility roles, names and state",async()=>{
  ...
  for(const label of ["Lock Wallet","Switch Wallet account","YNX Wallet recovery key","Approve","Reject","AI security explanation","Remove local account","Recovery authorization failed","Import authorization failed"])assert.ok(`${source}\n${i18n}`.includes(label),`missing accessibility contract for ${label}`);
