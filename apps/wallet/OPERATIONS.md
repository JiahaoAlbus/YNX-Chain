...
Engineering-only Android and iOS Simulator downloads are recorded by `artifact-manifest.json`. GitHub prerelease assets can be replaced by the publisher, so verify the manifest's exact URL, byte size and SHA-256 after every download. Do not label these builds production-signed or store-released. No public product deployment or update service exists.
