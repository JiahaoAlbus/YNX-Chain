...
for(const replayStore of [nonceReplayStore,actionReplayStore])for(const required of ["assertActive:()=>void=()=>{}","assertActive();","await this.storage.setItem("])assert.ok(replayStore.includes(required),`replay consumption must revalidate lifecycle at the storage linearization point through ${required}`);
