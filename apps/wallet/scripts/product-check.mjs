...
for(const required of ["assertDecisionTransition(records,digest,action)",'action==="intent-approved"||action==="request-rejected"',"Wallet authorization request is already decided","Wallet authorization callback was already returned"])assert.ok(audit.includes(required),`authorization decision audit must linearize approve/reject through ${required}`);
