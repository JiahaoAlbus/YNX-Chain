...
const rejectionProof=JSON.parse(await readFile(new URL("../proof/canonical-authorization-rejection-local-2026-08-14.json",import.meta.url),"utf8"));
...
assert.equal(historicalGatewayProof.registry.products,release.centralIntegration.registryProducts);
assert.equal(historicalGatewayProof.registry.enabledProducts,release.centralIntegration.registryEnabledProducts);
assert.equal(historicalGatewayProof.bridgeProductSessionE2E.postRevocationError,"REVOKED");
assert.equal(historicalGatewayProof.claims.walletNativeApplicationDeployedPublic,false);
assert.equal(historicalGatewayProof.secretMaterialRecorded,false);
assert.equal(deploymentTopologyProof.deployment.sourceCommit,release.centralIntegration.deployedSourceCommit);
assert.equal(deploymentTopologyProof.deployment.release,release.centralIntegration.deployedRelease);
assert.equal(deploymentTopologyProof.deployment.registryRuntimeSha256,release.capabilityStates.canonicalGatewayAdapter.registryRuntimeSha256);
assert.equal(deploymentTopologyProof.deployment.deploymentResult,"passed");
assert.equal(deploymentTopologyProof.postDeploymentPublicLifecycle.freshPublicLifecycleVerified,true);
assert.equal(deploymentTopologyProof.postDeploymentPublicLifecycle.postRevocationCode,"REVOKED");
assert.equal(deploymentTopologyProof.postDeploymentPublicLifecycle.requestIdCompleteness,false);
assert.equal(deploymentTopologyProof.postDeploymentPublicLifecycle.traceIdCompleteness,false);
assert.equal(deploymentTopologyProof.postDeploymentPublicLifecycle.errorIdCompleteness,false);
...
assert.equal(rejectionProof.sourceCommit,release.capabilityStates.canonicalAuthorizationRejection.evidenceSourceCommit);
assert.equal(rejectionProof.gatewayResult.code,"AUTHORIZATION_REJECTED");
assert.equal(rejectionProof.gatewayResult.mutated,false);
assert.equal(rejectionProof.gatewayResult.sessionsCreated,0);
assert.equal(rejectionProof.gatewayResult.noncesConsumed,0);
assert.equal(rejectionProof.gatewayResult.productProofsConsumed,0);
assert.equal(rejectionProof.releaseClaims.integratedCentral,false);
assert.equal(rejectionProof.releaseClaims.deployedPublic,false);
assert.equal(rejectionProof.releaseClaims.publicRouteVerified,false);
