...
for(const required of ["resetCorruptStorage(assertActive","assertActive();await this.storage.deleteItem(secretKey(item.account))","assertActive();\n    await this.storage.deleteItem(MANIFEST_KEY)"])assert.ok(walletRepository.includes(required),`unreadable Wallet reset must revalidate lifecycle through ${required}`);
