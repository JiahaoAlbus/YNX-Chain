...
- macOS native companion: an arm64 engineering application is locally implemented and installed for launch, second-launch and fail-closed custom-URL verification. Its recovery vault uses system randomness, device-only Keychain accessibility and current-biometric-set access control. The test host directly verifies Keychain but has no available biometric policy, so recovery access remains fail closed. It exposes no account derivation, signing, send, Gateway or callback-success claim until the frozen Core Wallet/Auth native bridge is integrated. The local bundle is ad-hoc signed, not Developer ID/notarized/store/production signed, and is not hosted.
