...
Recovery branch run [31698140178](https://github.com/JiahaoAlbus/YNX-Chain/actions/runs/31698140178) verified the immutable action pins and rebuilt the frozen Solidity fixture, then passed 112/113 Wallet/Auth tests. The sole failure occurred in the Developer deployment test that launches a Go verifier subprocess, before CocoaPods or Xcode build steps. The Wallet CI now runs the complete 113-test set serially to remove concurrent toolchain contention; no test is skipped and no iOS success is claimed for that failed run.
