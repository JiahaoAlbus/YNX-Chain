...
Recovery branch runs [31698140178](https://github.com/JiahaoAlbus/YNX-Chain/actions/runs/31698140178) and [31700203479](https://github.com/JiahaoAlbus/YNX-Chain/actions/runs/31700203479) verified the immutable action pins and rebuilt the frozen Solidity fixture, then passed 112/113 Wallet/Auth tests. The exact failing assertion observed `spawnSync("go", …).status === null`, proving the macOS runner had no Go executable; both runs stopped before CocoaPods or Xcode build. Wallet CI now installs the repository-required Go 1.25 through an immutable `setup-go` commit pin and runs all 113 tests serially. No test is skipped and no iOS success is claimed for either failed run.
