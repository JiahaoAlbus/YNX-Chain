...
The 2026-08-13 multi-user loopback drill exercised 32 distinct Wallet accounts and 32 distinct P-256 devices concurrently through the actual Node HTTP host and mode-0600 persisted state. The recorded completion latency was p50 298.799 ms, p95 725.398 ms, p99/max 752.327 ms with zero failed completions or introspections; replay and revoke remained fail-closed after host reconstruction. This single-host local sample is regression evidence, not a 32-user public capacity claim, and it excludes ingress, TLS, external load balancing, production storage, multi-region recovery and provider latency. Machine-readable evidence is `proof/gateway-multi-user-loopback-2026-08-13.json`.
