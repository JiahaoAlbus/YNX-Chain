...
function SendModal({visible,account,close,onSent}:{visible:boolean;account:WalletAccount;close:()=>void;onSent:()=>void}){
  ...
  const act=async(mode:"new"|"retry"|"check")=>{let lease:WalletOperationLease|undefined;setBusy(true);setError(null);const request=Object.freeze({account:account.account,accountPublicKey:account.accountPublicKey,to,amount:Number(amount)});
    try{lease=scope.begin({account:request.account});const activeLease=lease,client=stored&&mode!=="new"?storedChainClient(stored.origin):chainClient();let result:NativeTransferOutboxEntry;
      ...
      activeLease.assert();setStored(result.phase==="done"?null:result);if(result.phase==="observed"||result.phase==="accepted"||result.phase==="done")onSent();if(result.phase==="done"){cancelInput();setTo("");setAmount("");setReview(false)}
