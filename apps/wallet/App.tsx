...
function SendModal({visible,account,close,onSent}:{visible:boolean;account:WalletAccount;close:()=>void;onSent:()=>void}){
  ...
  const dismiss=()=>{scope.cancel();setBusy(false);close()};
  useEffect(()=>{scope.cancel();setTo("");setAmount("");setReview(false);setBusy(false);setError(null);setStored(null);setLoaded(false);if(!visible)return;let current=true;
    void nativeOutbox.read(account.account).then(value=>{if(current){setStored(value?.phase==="done"?null:value);setLoaded(true)}}).catch(caught=>{if(current)setError(message(caught))});return()=>{current=false}
  },[visible,account.account,scope,reload]);
