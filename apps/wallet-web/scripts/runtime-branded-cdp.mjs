...
const artifact=await readFile(join(root,"artifacts","ynx-wallet-chrome-edge-0.1.1.zip"));
const result={schemaVersion:2,sourceCommit,generatedAt:new Date().toISOString(),browserId,browserName:browserSpec.name,executablePath:browserSpec.path,browserVersion:null,loadMode:"temporary-unpacked-isolated-profile",launchMode,profileClass:"disposable-isolated-profile-reused-only-for-second-launch",runtimePresentation:`${headed?"headed":"headless-new"} branded binary automation; not installedLocal evidence`,browserCapability:browserId==="chrome"?{unpackedAutomation:"manual-required",automationUnsupported:true,reason:"Branded Chrome rejects command-line unpacked-extension automation in this environment; Chrome for Testing provides the automated path."}:{unpackedAutomation:"supported",automationUnsupported:false,reason:null},extension:{path:extensionPath,derivedId:extensionId,actualId:null},artifact:{name:"ynx-wallet-chrome-edge-0.1.1.zip",bytes:artifact.length,sha256:createHash("sha256").update(artifact).digest("hex"),signingClass:"unsigned-unpacked-extension"},launchArgs:{removedDisableExtensions:true,headlessNew:!headed,gpuDisabled:true,remoteAllowOrigins:true,remoteDebuggingPort:"ephemeral",dailyProfileTouched:false},launches:[],passed:false,temporaryUnpackedRuntimeProved:false,firstOpenProved:false,secondOpenProved:false,serviceWorkerRestartProved:false,visibleBrowserAcceptance:false,installedLocal:false,providerSuccessClaimed:false,accountAuthorized:false,messageSigned:false,transactionSubmitted:false,downloadHosted:false,deployedPublic:false,productionSigned:false,storeReleased:false};
