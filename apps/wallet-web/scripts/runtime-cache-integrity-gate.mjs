...
try{
  ...
  const expectedApp=createHash("sha256").update(await readFile(join(dist,"app.js"))).digest("hex");result.passed=result.legacyClient.controlled&&result.legacyClient.cacheNames.includes("ynx-wallet-web-v7")&&result.updateProbe.worker===true&&result.initial.controlled&&result.initial.cacheNames.includes(PWA_CACHE)&&!result.initial.cacheNames.includes("ynx-wallet-web-v7")&&result.initial.statusText==="Status: Not connected"&&!result.initial.session&&result.initial.signDisabled&&result.initial.sendDisabled&&result.tamper.status===503&&!result.tamper.body.includes("__YNX_TAMPERED__")&&!result.tamper.tamperedExecuted&&!result.tamper.session&&result.tamper.signDisabled&&result.tamper.sendDisabled&&result.rollback.status===503&&!result.rollback.body.includes("ROLLBACK_FAKE_CONNECTED")&&!result.rollback.legacyPresent&&!result.rollback.session&&result.recovery.appStatus===200&&result.recovery.indexStatus===200&&result.recovery.appSha256===expectedApp&&!result.recovery.indexContainsFake&&!result.recovery.session;
