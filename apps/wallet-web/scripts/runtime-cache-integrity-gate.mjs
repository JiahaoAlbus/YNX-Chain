...
const root=resolve(dirname(fileURLToPath(import.meta.url)),".."),repository=resolve(root,"..",".."),dist=join(root,"dist","pwa"),legacyCommit="4152af9061b77a619a8f09522dc150c6a6b8d622",legacyArtifact=`${legacyCommit}:apps/wallet-web/artifacts/ynx-wallet-web-pwa-0.1.0.zip`,temp=await mkdtemp(join(tmpdir(),"ynx-pwa-v6-to-v8-")),legacyZip=join(temp,"legacy-v6.zip"),legacyDist=join(temp,"legacy-v6"),profile=join(temp,"profile"),evidencePath=join(root,"evidence","runtime","pwa-v6-to-v8-upgrade-20260821.json"),screenshotPath=join(root,"evidence","browser","pwa-v6-to-v8-upgrade-20260821.png");
...
try {
  ...
  generation="v8";
  const recoveryRedirect=page.waitForURL(/pwa-recovered=8/u,{timeout:8000});
  ...
  await bounded(recoveryRedirect,9000,"v8 recovery redirect");
  ...
  for(let attempt=0;attempt<40;attempt+=1){
    ...
    if(names.includes("ynx-wallet-web-v8")&&!names.some(name=>name.startsWith("ynx-wallet-web-v")&&name!=="ynx-wallet-web-v8")){upgraded=true;break;}
  ...
  if(!upgraded) {
    ...
    throw Object.assign(new Error("v8 activation and cache purge did not complete"),{code:"UPGRADE_INCOMPLETE"});
  }
  if(!await page.evaluate(()=>Boolean(navigator.serviceWorker.controller))) { await page.reload({waitUntil:"domcontentloaded"}); await page.waitForFunction(()=>Boolean(navigator.serviceWorker.controller)); }
  await page.waitForFunction(()=>/NO_PROVIDER/.test(document.querySelector("#status")?.textContent||""));
  ...
  result.upgrade.oldCacheRemoved=!result.upgrade.after.caches.some(name=>name.startsWith("ynx-wallet-web-v")&&name!=="ynx-wallet-web-v8");
