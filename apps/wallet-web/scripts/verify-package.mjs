...
for (const artifact of manifest.artifacts) {
  ...
  if (artifact.browsers.includes("PWA")) {
    for (const required of ["manifest.webmanifest", "preferences.js", "mobile-wallet-routing.js", "core-auth-consumer.js", "wallet-web-companion-lifecycle.js", "core-auth-binding.js", "sw.js", "sw-v7.js", "sw-v8.js", "service-worker-policy.js", "asset-integrity.js", "pwa-upgrade.html", "pwa-upgrade.js", "ynx-icon-192.png", "ynx-icon-512.png", "ynx-icon-maskable-512.png"]) if (!entries.includes(required)) throw new Error(`Missing ${required}: ${artifact.name}`);
    ...
    const integrity=JSON.parse(match[1]),expected=["./","./index.html","./styles.css","./accessibility.css","./app.js","./provider.js","./public-endpoint-consumer.js","./i18n.js","./preferences.js","./mobile-wallet-routing.js","./core-auth-consumer.js","./wallet-web-companion-lifecycle.js","./core-auth-binding.js","./ynx-logo.png","./ynx-icon-192.png","./ynx-icon-512.png","./ynx-icon-maskable-512.png","./manifest.webmanifest"];
  ...
  const chromium=artifact.browsers.includes("Chrome");
  const expectedHosts=chromium?["https://rpc.ynxweb4.com/*","https://*.ynxweb4.com/*"]:["https://rpc.ynxweb4.com/*"];
  if (JSON.stringify(extension.host_permissions) !== JSON.stringify(expectedHosts)) throw new Error(`Invalid host permissions: ${artifact.name}`);
  const expectedScripts=chromium?[
  ...
  if(JSON.stringify(extension.content_scripts)!==JSON.stringify(expectedScripts)) throw new Error(`Invalid YNX DApp injection scope: ${artifact.name}`);
