...
function apiState(){
  ...
  const api={runtime:{onMessage:{addListener(listener){state.listener=listener}}},permissions:{async getAll(){return{origins:["https://evm.ynxweb4.com/*"]}},async remove(){return true}},tabs:{async query(){state.tabCalls.push("query");return[{id:7,url:`${ORIGIN}/`}]} ,async get(id){state.tabCalls.push(["get",id]);return{id,url:`${ORIGIN}/`}},sendMessage:async()=>null},scripting:{async getRegisteredContentScripts(){return[]},async unregisterContentScripts(){},async executeScript(plan){state.scriptingCalls.push({files:plan.files||null,world:plan.world||null,hasFunction:typeof plan.func==="function"});if(plan.func)return[{result:{ynx:false,metamask:false}}];return[]}},storage:{local:{async set(value){Object.assign(state.storage,value)}},session:{async get(key){return{[key]:state.storage[key]}},async set(value){Object.assign(state.storage,value)}}}};
  return{api,state};
}
