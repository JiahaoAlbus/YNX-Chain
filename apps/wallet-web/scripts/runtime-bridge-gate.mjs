...
if(!browsers[browserId]||!["injection","public-injection","accounts","lifecycle","wrong-chain","chain-id","rpc","add-chain","switch-chain","tamper"].includes(gate))throw new Error("Unsupported YNX_BROWSER or YNX_GATE");
...
const fixtureUrl=gate==="public-injection"?"https://wallet.ynxweb4.com/?verify=extension-eip6963":`http://127.0.0.1:${server.address().port}/`,profile=await mkdtemp(join(tmpdir(),`ynx-${browserId}-${gate}-`));let context;
...
try{
  ...
  result.preInjection=await page.evaluate(()=>({url:location.href,readyState:document.readyState,ethereumType:typeof globalThis.ethereum,companionPresent:Boolean(globalThis.ethereum?.__ynxCompanion||globalThis.ethereum?.providers?.some((provider)=>provider?.__ynxCompanion))}));
  await bounded(page.waitForFunction(()=>Boolean(globalThis.ethereum?.__ynxCompanion||globalThis.ethereum?.providers?.some((provider)=>provider?.__ynxCompanion===true)),null,{timeout:5000}),6000,"provider injection");
  result.provider=await page.evaluate(()=>{const provider=globalThis.ethereum?.__ynxCompanion?globalThis.ethereum:globalThis.ethereum?.providers?.find((item)=>item?.__ynxCompanion===true);return{injected:Boolean(provider),isYNXWallet:provider?.isYNXWallet===true,rdns:provider?.providerInfo?.rdns,requestType:typeof provider?.request}});
  if(gate==="injection"||gate==="public-injection")result.passed=result.provider.injected&&result.provider.isYNXWallet&&result.provider.rdns==="com.ynx.wallet.companion"&&result.provider.requestType==="function";
  if(gate==="accounts"){
  ...
  if(gate==="add-chain"||gate==="switch-chain"){
    result.outcome=await bounded(page.evaluate(async(gateName)=>{const provider=globalThis.ethereum.providers.find((item)=>item?.__ynxCompanion===true),method=gateName==="add-chain"?"wallet_addEthereumChain":"wallet_switchEthereumChain",params=gateName==="add-chain"?[{chainId:"0x1917",chainName:"YNX Testnet",nativeCurrency:{name:"YNX Testnet",symbol:"YNXT",decimals:18},rpcUrls:["https://rpc.ynxweb4.com/evm"],blockExplorerUrls:["https://explorer.ynxweb4.com"]}]:[{chainId:"0x1917"}];try{await provider.request({method,params});return{success:true,chainId:await provider.request({method:"eth_chainId"}),calls:globalThis.__YNX_FIXTURE_CALLS__}}catch(error){return{success:false,error:{code:error?.code,message:error?.message},calls:globalThis.__YNX_FIXTURE_CALLS__}}},gate),20000,`${gate} bridge`);
