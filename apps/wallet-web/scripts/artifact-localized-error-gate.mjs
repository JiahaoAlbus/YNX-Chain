...
for(const name of variants){
  ...
  try{
    ...
    const checks={allLocalesDirect:localeChecks.every(item=>item.untranslated.length===0),allErrorsLocalized:localeChecks.every(item=>typeof item.requestFailed==="string"&&item.requestFailed.length>10),arabicExact:i18n.catalog("ar").requestFailed==="تعذر إكمال الطلب. تحقق من محفظتك قبل المحاولة مجددًا.",stableCodePreserved:/return `\$\{code\}: \$\{text\("requestFailed"\)\}`/u.test(app),rawProviderMessageNotRendered:!/error\?\.message \|\|/u.test(app)};
