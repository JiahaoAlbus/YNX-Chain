...
for(const contract of centralMobileContracts){
  ...
  centralMobileBytes.push(bytes);
}
if(JSON.stringify(derivePublicEndpointBinding(JSON.parse(centralMobileBytes[0])))!==JSON.stringify(PUBLIC_ENDPOINT_BINDING))throw new Error("Runtime endpoint projection does not match Central's immutable matrix");
...
// The Worker and its integrity map must always reach the browser network
// update path. Putting either into its own versioned shell cache can trap an
// older Worker behind a digest it can no longer validate.
...
for (const [name, manifest] of variants) {
  ...
  for (const file of ["provider.js", "public-endpoint-consumer.js", "i18n.js", "preferences.js", "mobile-wallet-routing.js", "wallet-web-companion-lifecycle.js"]) await cp(join(root, "src", file), join(target, file));
