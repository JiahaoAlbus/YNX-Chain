...
const server=createServer({key:fixtureKey,cert:fixtureCert},async(request,response)=>{
  const url=new URL(request.url||"/","https://ynx.fixture"),relative=url.pathname==="/"?"dapp-eip6963.html":url.pathname.slice(1),path=resolve(fixtureRoot,relative);
  ...
  try{
    ...
    try {
      ...
      const bridge=await page.evaluate(async()=>{
        ...
        return{announcementCount:announcements.length,matchingCount:matching.length,info:item?.info||null,injected:Boolean(provider),isYNXWallet:provider?.isYNXWallet===true,isMetaMask:provider?.isMetaMask===true,requestType:typeof provider?.request,chainId,chainError};
      });
      ...
      result.ynxPriority={passed:bridge.matchingCount===1&&bridge.info?.rdns==="com.ynx.wallet"&&bridge.isYNXWallet&&bridge.isMetaMask===false&&bridge.requestType==="function"};
