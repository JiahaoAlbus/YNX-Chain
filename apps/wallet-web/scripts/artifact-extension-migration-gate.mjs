...
async function serviceWorkerFailureGate(target,module,platform){
  ...
  const dappUrl="https://dapp.example/";
  globalThis.chrome={runtime:{id:"fixture",getURL:path=>`chrome-extension://fixture/${path}`,onMessage:{addListener(listener){state.listener=listener}}},permissions:{async getAll(){return{origins:[...state.origins]}},async remove(){return false}},scripting:{async getRegisteredContentScripts(){return[]},async unregisterContentScripts(){},async executeScript(){state.scriptingCalls+=1;return[]}},storage:{local:{async set(value){Object.assign(state.local,value)}},session:{async get(key){return{[key]:state.session[key]}},async set(value){Object.assign(state.session,value)}}},tabs:{async query(){return[{id:7,url:dappUrl,incognito:false}]},async get(){return{id:7,url:dappUrl,incognito:false}},async sendMessage(){}}};
  await import(`${pathToFileURL(join(target,"service-worker.js")).href}?migration-failure=${platform}-${Date.now()}`);
  const discovery=await send(state.listener,{type:"YNX_WALLET_DISCOVER"}),sensitive=await send(state.listener,{type:"YNX_DAPP_REQUEST_V1",version:1,requestId:"ynx-11111111-1111-4111-8111-111111111111",origin:"https://dapp.example",method:"eth_requestAccounts",params:[],deadlineAt:Date.now()+18000,documentNonce:"a".repeat(64)},{tab:{id:7,url:dappUrl,incognito:false},url:dappUrl,frameId:0,documentId:"FF2F212A00379D284FE8558A23819E2D",documentLifecycle:"active"});
