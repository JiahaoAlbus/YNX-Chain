...
function fake(module,{alarmsApi,removeWorks=true,rpc=true}={}){
  const state={origins:[...(rpc?[module.REQUIRED_RPC_ORIGIN]:[]),...module.LEGACY_ORIGIN_GRANTS],scripts:[{id:"legacy-static-shadow"}],alarms:alarmsApi?[{name:"legacy-refresh"}]:null,local:{},account:{sentinel:"untouched"},calls:[]};
  const api={
...
async function serviceWorkerFailureGate(target,module,platform){
  const state={listener:null,origins:[module.REQUIRED_RPC_ORIGIN,module.LEGACY_ORIGIN_GRANTS[0]],scriptingCalls:0,local:{},session:{}};
...
try{
  for(const variant of variants){
    ...
    const messageFailure=await serviceWorkerFailureGate(target,module,variant.platform),bytes=await readFile(archive),info=await stat(archive),passed=upgrade.state.origins.length===1&&upgrade.state.origins[0]===module.REQUIRED_RPC_ORIGIN&&upgrade.state.scripts.length===0&&(!variant.alarmsApi||upgrade.state.alarms.length===0)&&upgrade.state.account.sentinel==="untouched"&&first.accountStateTouched===false&&second.legacyOriginsRemoved.length===0&&second.dynamicScriptIdsRemoved.length===0&&cleanupFailure.code==="MIGRATION_ORIGIN_REMAINS"&&rpcFailure.code==="REQUIRED_RPC_PERMISSION_MISSING"&&messageFailure.passed&&!manifest.permissions.includes("alarms");
