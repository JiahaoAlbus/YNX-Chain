...
for(const name of variants){
  ...
  try{
    ...
    const app=await readFile(join(temp,"app.js"),"utf8"),styles=await readFile(join(temp,"styles.css"),"utf8"),identity=JSON.parse(await readFile(join(temp,"build-identity.json"),"utf8")),bytes=await readFile(archive),info=await stat(archive);
    ...
    const checks={
      ...
      exactSourceIdentity:/^[0-9a-f]{40}$/u.test(identity.sourceCommit)&&(!process.env.YNX_WALLET_WEB_SOURCE_COMMIT||identity.sourceCommit===process.env.YNX_WALLET_WEB_SOURCE_COMMIT),
    ...
    result.artifacts.push({name,bytes:info.size,sha256:createHash("sha256").update(bytes).digest("hex"),sourceCommit:identity.sourceCommit,matrix,checks,passed:Object.values(checks).every(Boolean)});
