...
For a release candidate, set `YNX_WALLET_EXTENSION_ARCHIVE` to the exact Chromium ZIP, `YNX_WALLET_WEB_SOURCE_COMMIT` to its frozen commit, and `YNX_WALLET_WEB_EVIDENCE_DIR` to a new isolated receipt directory before running `npm run test:runtime:private-v2` with `YNX_BROWSER=chromium` or `edge`. The fixture extracts that ZIP, checks its source identity, and records ZIP bytes and SHA-256. Without the archive variable it tests the local `dist/chromium` directory only.
