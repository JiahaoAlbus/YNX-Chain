...
**Data before you install:** The extension keeps an encrypted private key, public account information, approved site origins and transaction recovery records in this browser profile. It sends public account queries and approved signed transactions to `https://rpc-testnet.ynxweb4.com`; safe read-only queries may use the previous `https://evm.ynxweb4.com` authority only when the canonical service is unreachable, while signed transactions are never automatically replayed there. It returns approved account information and signatures to the requesting DApp. Signing messages may authenticate you or authorize actions. DApp request/response data is handled for these wallet features. RPC operators receive ordinary connection information, including your IP address. The local vault password and raw private key are not uploaded by the extension. Read the publisher's completed privacy policy before installation: **[PRIVACY_POLICY_HTTPS_URL — pending]**.
...
Keep your recovery key offline. There is no publisher password reset or recovery-key escrow in this extension. Browser storage and local password encryption do not establish hardware-backed protection. Private browsing is disabled. In Firefox, each container must approve its own site connection, while the encrypted vault and account-level transaction journal remain shared by the extension profile. Minimum package targets are Chrome/Edge 120 and Firefox 142 desktop. Store approval and public transfer availability must be verified separately before release.
