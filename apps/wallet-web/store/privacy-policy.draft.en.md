...
Wallet queries send account addresses, hashes and other allowlisted request parameters to `https://rpc-testnet.ynxweb4.com`. If that service is unreachable, safe read-only queries may fall back to the previous `https://evm.ynxweb4.com` authority. DApps can request public read-only RPC information without an account-connection or signing prompt. Transactions require separate review and password authorization. Approved signed raw transactions are sent once to the canonical RPC and are never automatically replayed to the fallback authority. Transaction data may be distributed through the Testnet and viewed by other participants or explorers. RPC operators receive ordinary network information such as your IP address. Extension RPC requests omit cookies and use HTTPS; this does not mean the servers keep no logs.
