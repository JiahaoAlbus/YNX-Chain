...
`public-channel-manifest.json` records the current mixed public set: PWA and Firefox remain unsigned `0.1.1` from `c93e16be81beddc957ef5f27b7bbcdfa89c28db3`; Chrome/Edge is unsigned `0.1.2` from `40c2814d6e87e19ce2762737356c4f9d1128a69d`. Each channel has its own filename, byte count, SHA-256, source and URL. `artifact-manifest.json` is the unchanged three-package `0.1.2` build receipt; its PWA and Firefox packages are not promoted to the public channel by that fact. `npm run test:public-artifacts` reads the channel ledger and product download matrix, then checks the exact official URLs. Firefox runtime evidence remains bound to the public Firefox `0.1.1` tuple. Future `npm run package` output goes under `candidate-artifacts/<version>/<sourceCommit>` and never overwrites the public receipt or old ZIPs.
...
The machine-readable earlier candidate receipt in `release-readiness.json` binds source commit `c5df9a9a1cc2a22ba7623ba2b85da138b66c3b5f`, all three unsigned package hashes, the reviewer-source ZIP hash and a clean extracted rebuild with 110/110 output files matching byte for byte. The exact reviewer-source ZIP is committed under `store/reviewer-archives`; `npm run verify:store` checks its bytes and SHA-256. Isolated Edge and Chrome for Testing profiles loaded that earlier local ZIP, created disposable accounts, approved a DApp connection, recovered `personal_sign` signatures and denied signing after permission revocation. Playwright Firefox loaded that earlier candidate as a temporary add-on; branded Mozilla Firefox, installed-candidate persistence, AMO signing and store distribution remain unverified. Local ZIP loading is not a hosted download, as recorded in `extension-candidate-local-20260925.json`.

The revocation follow-up is separately bound to source `ab82417086b6f067512e30507813af3c22ee07e5`. Its unsigned PWA, Chromium and Firefox ZIPs are local artifacts recorded in `revocation-followup-artifact-manifest.json`. The committed `ynx-wallet-reviewer-source-ab82417.zip` was extracted into a fresh directory, dependencies were installed there, and all 249 source and 110 output files passed the exact rebuild check. `npm run verify:store-candidate` checks the latest local ZIPs against this follow-up manifest. The older Edge/Chrome loading evidence above does not prove installation of the follow-up ZIPs; their installed, public, signed and store-released flags remain false.
