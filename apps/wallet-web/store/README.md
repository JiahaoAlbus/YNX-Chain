...
Both extension manifests disallow private browsing because the current vault, permissions and transaction journal are persistent profile storage. This is a browser policy declaration; an installed-browser verification remains pending. Normal Firefox containers are a separate boundary: permissions currently key by origin/account rather than container. Do not advertise container isolation. [MDN incognito](https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/manifest.json/incognito)
...
4. Correct the existing 1254 px logo referenced as a 128 px manifest icon with an approved faithful asset, then inspect the built bytes. Review the four existing Firefox `innerHTML` lint warnings with source context; do not suppress them or label lint warning-free.
...
6. Resolve or explicitly constrain Firefox container identity behavior before asserting AMO privacy compliance. Preserve the known queued-old-document request review boundary; the existing epoch tests do not prove every browser document-delivery ordering.
7. Attach a self-contained reviewer source archive and complete an extracted-archive clean rebuild/byte comparison after the root commits this candidate. A source directory lacking the historical Git objects required by the current build is insufficient.
