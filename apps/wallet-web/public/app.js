...
function platformDownloads(){return Object.entries(WALLET_DOWNLOAD_MATRIX).filter(([platform])=>platform!=="android").map(([,item])=>item.hosted===true&&item.url?`<div class="package-download"><a href="${escape(item.url)}" rel="noreferrer">${escape(item.label)} · ${text("download")}</a><details class="package-details"><summary>${text("packageDetails")}</summary><p class="download-meta mono">${item.bytes.toLocaleString("en-US")} Bytes · SHA-256 ${escape(item.sha256)} · ${escape(item.signingClass)} · productionSigned=${String(item.productionSigned===true)}</p></details></div>`:`<button type="button" disabled aria-disabled="true" data-permanent-disabled="true">${escape(item.label)} · ${text("otherDownloads")}</button>`).join("")}
...
async function act(work, success) {
  ...
  const epoch=state.epoch,provider=state.provider,current=()=>state.epoch===epoch&&state.provider===provider;
  state.busy = true;
  setStatus(text("working"));
  for (const button of document.querySelectorAll("button")) button.disabled = true;
  try { const result = await work();if(!current())return null;setStatus(success(result)); return result; }
  catch (error) {
...
function invalidateConnectedState() {
  state.providerRevision++;state.discoveryRevision++;state.unsubscribeProvider?.();state.unsubscribeProvider=null;state.provider=null;state.wallet=null;
...
function bindProviderLifecycle(provider) {
  ...
  const revision=state.providerRevision,current=()=>state.wallet==="ynx"&&provider===state.provider&&revision===state.providerRevision;
  state.unsubscribeProvider = subscribeProviderLifecycle(provider, {
    ...
    disconnect() { if(current())clearConnectedSession(); },
  });
}

function selectProvider(wallet) {
  if(wallet!=="ynx")throw Object.assign(new Error("Only YNX Wallet can open this session."),{code:"WALLET_PROVIDER_UNSUPPORTED"});
  state.providerRevision++;state.epoch++;
...
async function connect(wallet) {
  ...
  const assertCurrent=()=>{if(state.wallet!=="ynx"||state.provider!==provider||state.epoch!==attemptEpoch)throw Object.assign(new Error("Wallet session changed."),{code:"ACCOUNT_CHANGED"})};
  const pendingIntent=`connect_${crypto.randomUUID().replaceAll("-","")}`;
  const result = await act(() => connectStandardWallet(provider,wallet,{pendingIntent,assertCurrent}), (value) => `${text("connected")} · ${toYNXAddress(value.session.account)}`);
  if (!result) return;
...
function presentAvailability(availability) {
  ...
  document.querySelector("#download").classList.toggle("primary", !presentation.ynxPresent);
  document.querySelector("#download").classList.toggle("secondary", presentation.ynxPresent);
  document.querySelector("#detected").textContent = !state.discoveryReady?text("checkingWallets"):presentation.ynxPresent ? text("detected") : presentation.errorKey==="providerNotInjected"?text("noWalletHelp"):text(presentation.errorKey);
}

async function detect({preserveConnection=false}={}) {
  const revision=++state.discoveryRevision,epoch=state.epoch;
  ...
  if (wallet) {
    ...
    const assertCurrent=()=>{if(revision!==state.discoveryRevision||restoreEpoch!==state.epoch||state.provider!==provider||state.wallet!=="ynx")throw Object.assign(new Error("Wallet session changed."),{code:"ACCOUNT_CHANGED"})};
    let restored;try{restored=await restoreTestnetSession(provider,localStorage,{assertCurrent})}catch(error){if(revision!==state.discoveryRevision||restoreEpoch!==state.epoch||state.provider!==provider||state.wallet!=="ynx")return;throw error}
    if(revision!==state.discoveryRevision||restoreEpoch!==state.epoch||state.provider!==provider||state.wallet!=="ynx")return;
    if (restored) { state.connectState=reduceStandardWalletConnectState(createStandardWalletConnectState(),{type:"RESTORE",providerKind:wallet==="ynx"?"ynx-wallet":wallet,accounts:[restored.account],chainId:restored.chainId});state.account = restored.account; state.chainId = restored.chainId;state.errorCode=null;render();setStatus(`${text("connected")} · ${toYNXAddress(restored.account)}`); }
  }
  applyActionGates();
}
