...
function render() {
  ...
  app.innerHTML = `<div class="shell">
    ...
    <section class="card" aria-label="${text("walletConnection")}"><div id="detected" class="eyebrow">${text("unavailable")}</div>
      ...
      <p id="download-meta" class="download-meta">${text("testnet")}</p>
      <details id="platforms" class="platforms"><summary>${text("download")}</summary><a id="android-download" href="${WALLET_DOWNLOAD_MATRIX.android.url}" rel="noreferrer">Android · ${text("download")}</a><p class="download-meta mono">${escape(WALLET_DOWNLOAD_MATRIX.android.label)} · ${WALLET_DOWNLOAD_MATRIX.android.bytes.toLocaleString("en-US")} Bytes · SHA-256 ${escape(WALLET_DOWNLOAD_MATRIX.android.sha256)} · ${escape(WALLET_DOWNLOAD_MATRIX.android.signingClass)} · productionSigned=false</p><div class="platform-grid">${unavailablePlatforms()}</div></details>
