...
export function parseFeeModel(value){
  ...
  return Object.freeze({...NATIVE_FEE_MODEL,enabled:value.enabled});
}
...
export function validateNativeTransferInput(input,model){
  ...
  if(!input.to||input.from?.toLowerCase()===input.to.toLowerCase()||amount<=0n||amount%NATIVE_UNIT!==0n||(input.data??"0x")!=="0x"||input.type!==undefined&&![0,"0x0"].includes(input.type)||input.accessList!==undefined||input.maxFeePerGas!==undefined||input.maxPriorityFeePerGas!==undefined)capabilityError("UNSUPPORTED_NATIVE_TRANSFER","Current YNX RPC supports positive whole-YNXT plain native transfers only. Full EVM calls and typed transactions are not enabled.");
}
