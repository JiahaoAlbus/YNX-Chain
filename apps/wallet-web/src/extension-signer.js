...
import {NATIVE_FEE_MODEL,readNativeTransferCapability,validateNativeTransferInput} from "./extension-fee-model.js";
...
async function prepareTransaction(expectedAccount,input,rpc){
  ...
  const {feeModel:model}=await readNativeTransferCapability(rpc),value=quantity(input.value??"0x0","Value");
  ...
  await readNativeTransferCapability(rpc);
...
export async function signExtensionRequest({secretHex,expectedAccount,prepared,rpc,assertAuthorized}){
  ...
  await assertAuthorized();const{method,params}=prepared;let result;
  if(method==="eth_sendTransaction"){
    if(typeof rpc!=="function")fail("RPC_UNAVAILABLE","YNX Testnet RPC is unavailable.");await readNativeTransferCapability(rpc);
    ...
    await readNativeTransferCapability(rpc);await assertAuthorized();const fields=signingFields(params[0]),expected=Transaction.from(fields).unsignedSerialized,rawTransaction=await new Wallet(`0x${secretHex}`).signTransaction(fields),parsed=Transaction.from(rawTransaction);
  ...
  }else if(method==="personal_sign")result=await new Wallet(`0x${secretHex}`).signMessage(getBytes(params[0]));
  else{const typed=JSON.parse(params[1]),types={...typed.types};delete types.EIP712Domain;result=await new Wallet(`0x${secretHex}`).signTypedData(typed.domain,types,typed.message)}
