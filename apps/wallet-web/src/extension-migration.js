...
export const REQUIRED_PROVIDER_ORIGIN="https://*/*";
...
export async function runExtensionMigration(api,options={}){
  ...
  if(!afterOrigins.includes(REQUIRED_PROVIDER_ORIGIN))fail("REQUIRED_PROVIDER_PERMISSION_MISSING","Required HTTPS DApp provider permission is missing after migration.");
  ...
  const report=Object.freeze({schemaVersion:1,migrationVersion:EXTENSION_MIGRATION_VERSION,legacyOriginsRemoved:Object.freeze([...legacyOrigins]),dynamicScriptIdsRemoved:Object.freeze([...scriptIds]),alarmNamesRemoved:Object.freeze([...alarmNames]),alarmCleanup,httpsProviderOriginRetained:true,accountStateTouched:false,completedAt:new Date(options.now??Date.now()).toISOString()});
