...
export const REQUIRED_DAPP_ORIGINS=Object.freeze(["http://*/*","https://*/*"]);
export const LEGACY_ORIGIN_GRANTS=Object.freeze(["http://localhost/*","http://127.0.0.1/*"]);
...
export async function runExtensionMigration(api,options={}){
  ...
  if(!afterOrigins.includes(REQUIRED_RPC_ORIGIN)&&!afterOrigins.includes("https://*/*"))fail("REQUIRED_RPC_PERMISSION_MISSING","Frozen RPC origin permission is missing after migration.");
  if(REQUIRED_DAPP_ORIGINS.some(origin=>!afterOrigins.includes(origin)))fail("REQUIRED_DAPP_PERMISSION_MISSING","Declared HTTP(S) DApp permission is missing after migration.");
