...
export async function inspectOfficialArtifact(spec,fetchImpl=fetch){
  const record={name:spec.name,url:spec.url,expectedBytes:spec.bytes,expectedSha256:spec.sha256,status:null,contentType:null,contentDisposition:null,contentLength:null,downloadedBytes:null,downloadedSha256:null,hosted:false,errorCode:null,errorMessage:null};
  try{
    ...
    if(record.contentType?.split(";",1)[0].trim().toLowerCase()!=="application/zip")throw Object.assign(new Error("Artifact is not application/zip."),{code:"ARTIFACT_CONTENT_TYPE"});
