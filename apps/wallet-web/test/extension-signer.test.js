...
const rpc=async(method)=>({eth_chainId:"0x1917",eth_getTransactionCount:"0x1",eth_estimateGas:"0x5208",eth_gasPrice:"0x3b9aca00",eth_maxPriorityFeePerGas:"0x3b9aca"})[method];
...
test("vault signer preserves supplied transaction bounds and supports EIP-1559",async()=>{
  const result=await signExtensionRequest({secretHex:SECRET,expectedAccount:ACCOUNT,method:"eth_sendTransaction",params:[{from:ACCOUNT,to:TO,nonce:"0x2",gas:"0x6000",maxFeePerGas:"0x77359400",maxPriorityFeePerGas:"0x3b9aca00",type:"0x2",chainId:"0x1917"}],rpc}),tx=Transaction.from(result.rawTransaction);
  assert.equal(tx.type,2);assert.equal(tx.nonce,2);assert.equal(tx.gasLimit,24576n);assert.equal(tx.maxFeePerGas,2000000000n);assert.equal(tx.maxPriorityFeePerGas,1000000000n);assert.equal(tx.chainId,6423n);
});
