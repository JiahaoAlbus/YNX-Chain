...
async function fixture(t,{permitted=true,existingLocal=null,existingSession=null,firefox=false,browserContext="firefox-container-1"}={}){
  ...
  const state={documentNonce:"a".repeat(64),documentId:"11111111-1111-4111-8111-111111111111",probes:0,tabId:1,incognito:false,cookieStoreId:firefox?browserContext:undefined,contextByTab:{},url:`${ORIGIN}/request`,nonce:"0x1",chain:"0x1917",calls:[],signCalls:0,broadcasts:0,unlocks:0,opened:[],closed:[],events:[]};
...
test("actual old content requests queued after navigation cannot acquire the new document epoch on Chrome or Firefox 140",async t=>{
  for(const firefox of[false,true])await t.test(firefox?"Firefox without documentId":"Chrome document target",async t=>{
    const f=await fixture(t,{firefox}),old=await attachContentDocument(f,t,{firefox,queue:true}),packet=old.request("eth_sendTransaction",[{from:ACCOUNT,to:TO,value:toQuantity(10n**18n)}]);old.event("pagehide");f.state.tabUpdated(1,{status:"loading"});f.state.documentId="22222222-2222-4222-8222-222222222222";
