...
test("actual content bridge returns the original external ID while worker review and replay use the full internal ID",async t=>{
  ...
  const context=vm.createContext({window:pageWindow,location:{origin:ORIGIN},chrome:{runtime:{sendMessage:message=>f.rawSend(message,{frameId:0,url:f.state.url,tab:{id:1,url:f.state.url,incognito:false}}),onMessage:{addListener(){}}}},setTimeout:(fn,ms)=>{const id=setTimeout(fn,ms);timers.add(id);return id},clearTimeout:id=>{clearTimeout(id);timers.delete(id)},Date,Set});vm.runInContext(content,context);t.after(()=>{for(const id of timers)clearTimeout(id)});
