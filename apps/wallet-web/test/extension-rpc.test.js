...
import {READ_ONLY_RPC_METHODS,RPC_REQUEST_ID,RPC_TIMEOUT_MS,YNX_CHAIN_ID,YNX_LEGACY_RPC_URL,YNX_RPC_URL,YNX_RPC_URLS,broadcastExtensionTransaction,forwardExtensionRpc,verifyExtensionRpc} from "../src/extension-rpc.js";

const response = (body, options={}) => ({ok:options.ok??true,redirected:false,url:options.url??"https://rpc-testnet.ynxweb4.com/",status:options.status??200,json:async()=>body});
...
test("extension RPC rejects wrong chain, malformed envelope, errors and unavailable transport",async()=>{
  ...
  await assert.rejects(()=>verifyExtensionRpc(async url=>response({jsonrpc:"2.0",id:6423,result:null}, {ok:false,status:503,url:`${url}/`})),error=>error.code==="RPC_UNAVAILABLE");
});

test("read-only RPC forwards only the frozen allowlist and exact JSON-RPC envelope",async()=>{
  let observed;const result=await forwardExtensionRpc("eth_getTransactionCount",["0x1111111111111111111111111111111111111111","latest"],async(url,options)=>{const body=JSON.parse(options.body);observed={url,body};return response({jsonrpc:"2.0",id:RPC_REQUEST_ID,result:body.method==="eth_chainId"?"0x1917":"0x0"})});
...
test("legacy balance exposes whole-YNXT explicitly and cannot masquerade as standard wei",async()=>{
  for(const enabled of[false,true]){
    ...
    assert.equal(details.amountYNXT,"100");assert.equal(details.rawUnit,enabled?"wei":"whole-YNXT");assert.equal(details.fullEVM,false);assert.deepEqual(calls,["eth_chainId","eth_chainId","ynx_getFeeModel","eth_getBalance","eth_chainId","ynx_getFeeModel"]);
...
test("safe reads use canonical first and legacy only after reachability failure; broadcast never auto-replays",async()=>{
  assert.deepEqual(YNX_RPC_URLS,[YNX_RPC_URL,YNX_LEGACY_RPC_URL]);
  const calls=[];
  const fetcher=async(url,options)=>{const body=JSON.parse(options.body);calls.push([url,body.method]);if(url===YNX_RPC_URL)throw new TypeError("offline");return response({jsonrpc:"2.0",id:6423,result:body.method==="eth_chainId"?"0x1917":"0x2a"},{url:`${url}/`})};
  assert.equal(await forwardExtensionRpc("eth_blockNumber",[],fetcher),"0x2a");
  assert.deepEqual(calls,[[YNX_RPC_URL,"eth_chainId"],[YNX_LEGACY_RPC_URL,"eth_chainId"],[YNX_LEGACY_RPC_URL,"eth_blockNumber"]]);
  let broadcasts=0;
  await assert.rejects(broadcastExtensionTransaction("0x01",async url=>{broadcasts++;assert.equal(url,YNX_RPC_URL);throw new TypeError("unknown outcome")}),{code:"RPC_UNAVAILABLE"});
  assert.equal(broadcasts,1);
  let wrongChainCalls=0;
  await assert.rejects(verifyExtensionRpc(async url=>{wrongChainCalls++;return response({jsonrpc:"2.0",id:6423,result:"0x1"},{url:`${url}/`})}),{code:"WRONG_NETWORK"});
  assert.equal(wrongChainCalls,1);
});


test("RPC deadline includes a response body that never completes",async t=>{
  t.mock.timers.enable({apis:["setTimeout"]});
  const pending=broadcastExtensionTransaction("0x01",async()=>({ok:true,redirected:false,url:"https://rpc-testnet.ynxweb4.com/",text:async()=>new Promise(()=>{})}));
