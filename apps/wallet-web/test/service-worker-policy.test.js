...
import {PWA_CACHE, PWA_CACHE_PREFIX, allPwaCaches, assetKeyForRequest, cacheableResponse, cleanPwaNavigationUrl, obsoletePwaCaches, recoveryNavigationUrl, responseMatchesIntegrity, serviceWorkerRoute, upgradeNavigationUrl} from "../src/service-worker-policy.js";
...
test("PWA routes only same-origin GET navigation and assets into cache strategies", () => {
  assert.equal(PWA_CACHE_PREFIX,"ynx-wallet-shell-build-");
...
test("version drift recovery and upgrade navigation are single-attempt and URL preserving",()=>{
  ...
  assert.equal(recovery,`${origin}/?lang=en&source=3651&ynx-sw-recovery=${PWA_CACHE}#connect`);
  assert.equal(recoveryNavigationUrl(recovery),null);
  const upgrade=upgradeNavigationUrl(original);
  assert.equal(upgrade,`${origin}/?lang=en&source=3651&ynx-sw-upgrade=${PWA_CACHE}#connect`);
...
test("built worker derives cache keys from its registration scope",async()=>{
  ...
  assert.doesNotMatch(worker,/self\.registration\.unregister\(\)/u);
  assert.match(worker,/self\.registration\.update\(\)/u);
  assert.match(worker,/upgradeNavigationUrl\(client\.url\)/u);
  assert.match(worker,/reloadingClients\.has\(event\.clientId\)/u);
  ...
  assert.doesNotMatch(worker,/await caches\.delete\(PWA_CACHE\);\s+try/u);
  assert.match(worker,/if \(!await currentCacheReady\(\)\) throw new Error\("PWA shell integrity cache is incomplete"\)/u);
  assert.match(worker,/if \(!existing\) await caches\.delete\(PWA_CACHE\)/u);
...
test("PWA bootstrap compares the generated build identity and permits one bounded reload",async()=>{
  ...
  assert.match(app,/registration\.active\?\.state==="activated"/u);
  assert.match(app,/await waitForActivatedWorker\(registration\)/u);
  assert.match(app,/const startingControllerVersion=await workerVersion\(navigator\.serviceWorker\.controller\)/u);
  assert.match(app,/if\(active\.cache!==PWA_CACHE\)/u);
  assert.match(app,/reloadForPwaBuild\(active\.cache\)/u);
  assert.match(app,/return \{reloading:true\}/u);
  assert.match(app,/if\(!await waitForBuildController\(\)\)throw Object\.assign/u);
  ...
  assert.match(app,/const reloadUrl=upgradeNavigationUrl\(initialPwaNavigationUrl,cache\)/u);
  assert.match(app,/location\.replace\(reloadUrl\)/u);
  assert.match(app,/addEventListener\("controllerchange"/u);
  assert.match(app,/if\(!result\.reloading&&!pwaReloadStarted\)\{document\.documentElement\.dataset\.pwa="ready"/u);
