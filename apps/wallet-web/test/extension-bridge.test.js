...
test("content and page scripts enforce source, origin, timeout, duplicate-id and companion exclusion guards",async()=>{
  ...
  assert.match(page,/event\.source!==window\|\|event\.origin!==expectedOrigin/);assert.match(page,/crypto\.randomUUID\(\)/);assert.match(page,/__ynxCompanion:true/);assert.match(page,/__YNX_COMPANION_PROVIDER_V1__/);assert.match(page,/name:"YNX Wallet"/);assert.match(page,/rdns:"com\.ynx\.wallet"/);
