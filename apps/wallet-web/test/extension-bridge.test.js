...
test("content and page scripts enforce source, origin, timeout, duplicate-id and companion exclusion guards",async()=>{
  ...
  assert.match(worker,/provider\?\.__ynxCompanion!==true/);assert.match(worker,/requestAccountApproval\(tabId,origin,requestId,deadlineAt,documentLease\)/);assert.match(worker,/PROVIDER_ACCOUNT_UNAVAILABLE/);assert.match(worker,/forwardExtensionRpc\(method,params\)/);
  assert.match(worker,/requireVaultPage\(sender\)/);assert.match(worker,/providerAccountFromVault\(vault\)/);assert.match(worker,/recoverMissingProviderAccount\(extensionApi\.storage\.local/);assert.match(worker,/\[PROVIDER_PERMISSIONS_KEY\]:\{\}/);
