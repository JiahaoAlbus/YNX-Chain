...
test("extension packages expose truthful install metadata without hosted-update claims", () => {
  for (const manifest of [chromiumManifest, firefoxManifest]) {
    ...
    assert.equal(manifest.content_security_policy.extension_pages,"script-src 'self'; object-src 'self'; connect-src https://rpc-testnet.ynxweb4.com https://evm.ynxweb4.com");
