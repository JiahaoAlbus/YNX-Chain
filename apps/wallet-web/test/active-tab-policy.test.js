...
test("activeTab accepts only an exact HTTPS top-level DApp context",()=>{
  assert.deepEqual(requireActiveDappTab({id:7,url:"https://dapp.example/path"}),{tabId:7,origin:"https://dapp.example"});
  for(const tab of [null,{id:7},{id:"7",url:"https://dapp.example"},{id:7,url:"http://dapp.example"},{id:7,url:"file:///tmp/app"},{id:7,url:"chrome://extensions"},{id:7,url:"https://user:secret@dapp.example"}])assert.throws(()=>requireActiveDappTab(tab),error=>error.code==="ACTIVE_TAB_REQUIRED");
});
