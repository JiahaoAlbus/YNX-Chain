...
async function fixture(transaction){
 ...
 vm.runInContext(source,vm.createContext({document:{querySelector:element},chrome:{runtime:{sendMessage:runtime}},crypto:webcrypto,Date,console,createEncryptedVault:async()=>{throw new Error("No key operation expected")},generateExtensionSecret:()=>{throw new Error("No signing expected")},confirm:()=>false,addEventListener:(name,fn)=>events.set(name,fn)}));await tick();return{element,calls,state,events};
}
