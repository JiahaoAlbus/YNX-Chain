...
import { inspectWalletTransportURL } from "./wallet-transport-policy.js";
import { PermissionStore, canonicalOrigin, originPartition, reviewTransaction, securitySummary, selectAiContext } from "@ynx/web4-permissions";

app.commandLine.appendSwitch("site-per-process");
app.commandLine.appendSwitch("enable-features", "StrictOriginIsolation");

let window; let activeView; let activeTab; const views = new Map(); const aiRuns = new Map();
...
function interceptWalletTransport(url) { const result = inspectWalletTransportURL(url); if (result.intercept) void emitState({ warning: { kind: "wallet-transport", code: result.code, message: result.message } }); return result.intercept; }
...
async function makeView(tab) {
  ...
  view.webContents.setWindowOpenHandler(({ url }) => { if (!interceptWalletTransport(url)) void openTab(url, { privateMode: tab.privateMode }); return { action: "deny" }; });
  const guardNavigation = (event, url) => { if (interceptWalletTransport(url)) { event.preventDefault(); return; } const result = phishing.check(url); if (result.action === "warn") { event.preventDefault(); emitState({ warning: { kind: "phishing", url, ...result } }); } };
