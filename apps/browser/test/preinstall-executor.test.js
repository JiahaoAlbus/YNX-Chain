...
function fixture() {
  ...
  const target = join(temp, "isolated", "YNX Browser Testnet Preview-ad890f0a2fe5-aaed312ef608.app");
  ...
  return {temp, controls, oldCopies, oldHandler, target, receipt, env};
}
...
test("forward and rollback only register candidate and restore exact old handler", () => {
  ...
  for (let index = 0; index < 11; index += 1) {
    ...
    oldFiles.set(path, readFileSync(path));
  }
  const oldPid = readFileSync(join(state.controls, "old-pid"));
  ...
  for (const [path, bytes] of oldFiles) assert.deepEqual(readFileSync(path), bytes);
  assert.deepEqual(readFileSync(join(state.controls, "old-pid")), oldPid);
});
