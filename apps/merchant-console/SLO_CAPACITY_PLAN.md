...
Apple M2, darwin/arm64, 2026-07-29, repeatable in-process HTTP capacity
gate (`TestMerchantHTTPReadCapacity`, 1,000 authenticated reads at concurrency
25):

| Requests | Failures | Throughput | p50 | p95 | p99 |
|---:|---:|---:|---:|---:|---:|
| 1,000 | 0 | 25,948.0 req/s | 745 µs | 1.951375 ms | 3.0495 ms |

This is a local handler and file-store baseline, not a public-network,
multi-instance, provider, or production capacity claim. The gate records all
percentiles from the same request population and fails on any non-2xx response.
...
These measurements establish a bounded HTTP throughput/concurrency baseline,
but do not prove public-network, multi-instance, long-soak or provider capacity.
Before staging release, run 1/10/50/100 concurrency levels for at least 30
minutes each and record p50/p95/p99, throughput, error rate, CPU, RSS,
file-store growth, queue depth and provider latency.
