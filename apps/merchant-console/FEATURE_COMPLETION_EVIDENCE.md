...
| Supply-chain inventory and verification | partial | yes-local+CI | Frontend/backend CycloneDX, deterministic Go generator, exact vendor manifest, locale-safe placeholder/credential scanner, input/hash/path tests and two-run identical bundle evidence | Resolve Wallet Auth provenance/license; approved independent scanners, hermetic build, signed provenance and hosted immutable artifacts |
...
| Branch CI | yes-remote | verified | GitHub Actions run `30276842541` for `c9eb7e4`: frontend/backend success, 14 frontend tests, audit, source scan, build, Go test/vet and three fuzz gates | Workflow uploads no Merchant Console artifact; no visible latest GitHub Release |
| Git recovery checkpoint | yes-remote | verified | Branch synchronized through `c9eb7e41054fa4f88a3c0f2cc5352a3d187f4504`; prior verified local bundle remains recovery history | Keep each subsequent verified checkpoint synchronized |
