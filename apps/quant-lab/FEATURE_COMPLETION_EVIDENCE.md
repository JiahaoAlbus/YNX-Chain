...
| canonical Gateway/Wallet integration | deployed public Testnet | the approved exact Quant client/bundle/HTTPS callback/scopes are live; the browser creates and restores a device-bound session and generates fresh one-time proofs; public Wallet → Quant → Exchange E2E proves completion, exact Quant-scoped introspection, account-public-key binding and proof replay rejection; evidence: `evidence/public-wallet-quant-exchange-20260810.json` |
| real Exchange/DEX Testnet | Exchange path deployed public; DEX absent | public E2E registers the exact mandate, signs and submits the exact Testnet order, receives `submitted_testnet`, and rejects proof replay with 401; DEX owner transport, vault receipt, revoke propagation and emergency-exit receipt remain absent |
| public web/download | web runtime deployed; desktop downloads remain older candidates | `https://quant.ynxweb4.com/` runs source `5fb7a35e658594719bd1b687ecbd3d28076a17e5` with byte-identical Wallet Auth web asset; existing macOS/Windows archives remain the older `70382c37` candidates and are not promoted as this runtime build |
