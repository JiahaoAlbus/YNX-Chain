...
| canonical Gateway/Wallet integration | tested local, public cutover pending | the Wallet registry approves the exact Quant client/bundle/callback/scopes; the browser creates and restores a device-bound session and generates fresh one-time proofs; local three-service E2E proves Wallet completion, exact Quant-scoped Exchange introspection, proof replay rejection and account-public-key binding; no public receipt is claimed before cutover |
| real Exchange/DEX Testnet | Exchange path tested local; DEX absent | local Wallet → Quant → Exchange E2E registers an exact mandate, signs and submits an exact Testnet order, receives the authoritative owner receipt and rejects proof replay; DEX owner transport, vault receipt, revoke propagation and emergency-exit receipt remain absent |
