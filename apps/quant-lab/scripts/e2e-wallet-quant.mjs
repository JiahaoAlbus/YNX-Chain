...
if(response.status!==201||order.status!=="submitted_testnet"||!order.brokerProof||!order.venueOrderId||!["open","partially_filled","filled"].includes(order.venueStatus)||!/^[0-9a-f]{64}$/.test(order.authorizationDigest??""))throw new Error(`Quant authoritative Exchange receipt failed closed (${response.status})`);
...
process.stdout.write(`${canonicalJSON({account:identity.account,accountPublicKeyBound:session.accountPublicKey===identity.accountPublicKey,mandateDigest:registered.Digest,ok:true,quantOrderId:order.id,quantOrderStatus:order.status,venueOrderId:order.venueOrderId,venueStatus:order.venueStatus,authorizationDigest:order.authorizationDigest,replayStatus:response.status})}\n`);
