...
run("go", ["build", "-trimpath", "-buildvcs=false", "-ldflags", `-s -w -X github.com/JiahaoAlbus/YNX-Chain/internal/quantlab.BuildCommit=${args.commit}`, "-o", binary, "./apps/quant-lab/server"], root, { ...process.env, GOOS: "linux", GOARCH: "amd64", CGO_ENABLED: "0" });
...
function run(command, argv, cwd = root, env = process.env) { execFileSync(command, argv, { cwd, env, stdio: "inherit" }); }
