...
Wallet Web reproducibility continuation: the prior 168,865-byte bundle (`a69ce909...`) was generated before the local `@ynx-chain/sdk` dependency added Endpoint Authority v2 and explicit trusted-clock validation. Because the current Finance entry imports `validateEndpointAuthority` from the SDK package root, that committed bundle no longer represented the current tracked dependency graph. The authoritative runtime is now the deterministic esbuild 0.25.9 output from the current tracked entry, private Product Session adapter, versioned Standard Wallet authority, Product Session vendor/registry, endpoint pin and current `sdk/js` source: 179,986 bytes / `fbebce55d813f1acbbec7738d171bb2a3142eb415d2c513bae48862b4ecd1967`.

Two independent clean dependency installs and rebuilds produced the same bytes, and both match the committed `wallet-auth.js`. The verifier reads every reviewed input once, rebuilds twice from that in-memory snapshot and requires both rebuilds and the committed bundle to be byte-identical. Its independently pinned manifest now covers the package lock and complete build input graph. Missing inputs, legacy launchers, byte tampering, coordinated bundle/manifest edits, nondeterministic output, rebuild mismatch and first-read TOCTOU tampering remain fail closed. No legacy launcher or product-local Wallet protocol was restored.

Exact implementation checkpoint: `5dc8f2f6f1d7f5e7d43b60fa0913d1ea30c452a9` / tree `1f15fb30b58a6ee829e48e15698b58583b9d4c5e`. Evidence: `apps/finance/evidence/finance-wallet-bundle-reproducibility-20260921.json`.

Validation passed `npm run verify:wallet`, all 64 Finance browser/contract tests, the 242-file Finance security gate, Wallet migration-evidence evaluation, Finance smoke, scoped Finance Go tests and race tests, vet, and builds for all Finance Go commands. This source correction does not deploy, change Wallet authorization, alter RPC/shared authority, or promote provider/transaction/public-runtime state.
