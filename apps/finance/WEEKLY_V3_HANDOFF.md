...
- Activation readiness reports approval-pending, approved-but-not-consumed, rejected/revoked, execution, reconcile, terminal and inconsistent states separately. Readiness requires one globally eligible lifecycle across request and worker queues, while terminal orders require matching outbox/provider correlation. Unknown or contradictory combinations remain fail-closed and are never mislabeled terminal or ready.
