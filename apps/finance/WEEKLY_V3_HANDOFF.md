...
Wallet Web verifier continuation: the verifier now derives the reviewed entry/bundle from the exact `build:wallet` manifest and binds the current entry, private entry, versioned Standard Wallet browser authority, HTML, candidate runtime manifest, candidate legacy guard and committed bundle by byte count and SHA-256. The verifier manifest is itself pinned by an independent digest in verifier code, so coordinated bundle-plus-manifest edits fail closed without changing the reviewed verifier authority. Missing, legacy and byte-tampered bundles fail closed. A clean esbuild 0.25.9 rebuild of the current entry produced 179,986 bytes / `fbebce55d813f1acbbec7738d171bb2a3142eb415d2c513bae48862b4ecd1967`, while the existing committed runtime is 168,865 bytes / `a69ce9095f1b6ebb5929fe55d6a0948cf7600fa712be4ea80c7b50c35595942e`; source-to-bundle byte reproducibility therefore remains `NOT_VERIFIED_AS_REPRODUCIBLE`. This verifier-only repair does not replace Finance runtime, change Wallet authorization, alter RPC authority, deploy, or promote provider verification state.

Verifier validation passed `npm run verify:wallet`, all 60 Finance browser/contract tests, the 241-file Finance security gate, Wallet migration-evidence evaluation, Finance smoke, scoped Finance Go tests and race tests, and builds for all Finance Go commands. Repository-wide `go test ./...` remains outside this verifier result: the unchanged baseline still has duplicate `main` declarations in `apps/quant-lab/scripts` and a missing generated BFT gateway contract artifact.
