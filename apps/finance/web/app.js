...
function renderBrokerWorkspace(workspace){
  ...
  $('#broker-local-orders').innerHTML=orders.length?orders.map(record=>{const queued=outbox.get(record.order.orderId),cancelable=['submitted','partially_filled'].includes(record.state),executable=state.brokerSubmissionEnabled&&record.approvalState==='consumed'&&queued?.status==='pending_unwired';return `<div class="row"><div class="row-main"><strong>${esc(record.order.side)} ${esc(record.order.qty)} ${esc(record.order.symbol)}</strong><small>${esc(record.approvalState)} · ${esc(record.state)} · request ${esc(short(record.requestId))}</small><small>${queued?`outbox ${esc(queued.status)} · attempts ${esc(queued.attempts)}`:'no provider outbox'}</small></div><div class="row-value">max ${esc(record.order.maxCost)} simulated USD<div class="wallet-choice"><button type="button" data-broker-order-refresh="${esc(record.order.orderId)}">Refresh</button>${executable?`<button type="button" class="primary" data-broker-order-execute="${esc(record.order.orderId)}">Request controlled execution</button>`:''}${cancelable?`<button type="button" class="danger" data-broker-order-cancel="${esc(record.order.orderId)}">Request cancellation</button>`:''}</div></div></div>`}).join(''):'<div class="empty compact">No local Sandbox order drafts.</div>';
...
async function refreshBrokerWorkspace(){
  ...
  try{const result=await api('/api/broker/orders');if(result?.schema!=='ynx-finance-broker-workspace-v1'||typeof result.workspace?.serverTime!=='string')throw new Error('Broker workspace response is invalid.');renderBrokerWorkspace(result.workspace);return result.workspace}catch(error){$('#broker-local-orders').innerHTML=`<div class="empty compact">${esc(error.message)} No order state was substituted.</div>`;return null}
}
...
async function load(){await window.YNXFinanceWallet.ready;state.connected=window.YNXFinanceWallet.connected();if(!state.connected){renderSignedOut();return}try{$('#source-pill').textContent='Checking sources';const data=await api('/api/overview');state.overview=data;reconcileOpaqueBrokerOwner();render(data)}catch(error){if(error.status===401||error.status===403){window.YNXFinanceWallet.reportPrivateFailure();clearPrivateView();notify('Private Finance access needs reauthorization. Standard Wallet is unchanged.',true)}else notify(error.message,true)}}
