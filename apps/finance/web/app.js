...
function budgetProgressRow(b,progress){
  ...
  return `<div class="row"><div class="row-main"><strong>${esc(b.name)}</strong><small>${esc(financeText(b.period==='weekly'?'weeklyLabel':'monthlyLabel'))} · ${esc(financeText('planningOnly'))} · UTC</small><small>${esc(financeText('periodStart'))}: ${esc(period)} · ${esc(financeText('countFrom'))}: ${esc(effective)}</small><small>${esc(status)}</small><small>${esc(financeText('noCompletePeriodHistory'))}</small></div><div class="row-value">${esc(financeText('budgetLimit'))}: ${budgetAmount(b.limitYnxt)}<small>${esc(financeText('observedSpending'))}: ${observed}</small><small>${esc(financeText('fullPeriodSpending'))}: ${esc(financeText('unknown'))}</small><small>${esc(financeText('remainingBudget'))}: ${esc(financeText('unknown'))}</small></div></div>`;
}
...
$('#statement-form').addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.currentTarget);try{const from=new Date(`${f.get('from')}T00:00:00Z`).toISOString(),toDate=new Date(`${f.get('to')}T00:00:00Z`);toDate.setUTCDate(toDate.getUTCDate()+1);state.statement=await api(`/api/statements?from=${encodeURIComponent(from)}&to=${encodeURIComponent(toDate.toISOString())}`);renderStatement(state.statement)}catch(error){notifyFailure(error,'unavailable')}});
