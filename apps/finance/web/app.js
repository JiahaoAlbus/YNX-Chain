...
async function refreshBrokerQuote(){
  ...
  const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),5000);
  try{const response=await fetch(`/api/broker/quote?symbol=${encodeURIComponent(symbol)}`,{cache:'no-store',credentials:'omit',redirect:'error',signal:controller.signal}),result=await response.json();if(!response.ok||result?.schema!=='ynx-finance-broker-quote-v1')throw new Error(result?.error||'Sandbox quote is unavailable.');const quote=result.quote;$('#broker-quote-status').textContent=`${quote.symbol} · bid ${quote.bidPrice} / ask ${quote.askPrice} simulated USD · IEX · ${date(quote.timestamp)}. This does not set the order limit automatically.`}catch(error){$('#broker-quote-status').textContent=`${error.message||'Sandbox quote is unavailable.'} No price was substituted.`}finally{clearTimeout(timer)}
}
