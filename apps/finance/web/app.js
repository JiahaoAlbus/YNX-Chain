const state={connected:false,overview:null,statement:null,aiJob:null,aiTimer:null,context:0,brokerAssets:new Map(),brokerWatchlist:new Map(),brokerSelectedAsset:null,brokerSubmissionEnabled:false};
...
function renderReceipts(items,status){const el=$('#recent-receipts');if(!status.available){el.innerHTML=`<div class="empty compact">${esc(status.error)}. ${esc(financeText('noReceiptPlaceholders'))}</div>`;return}el.innerHTML=items.length?items.slice(0,5).map(r=>`<div class="row"><div class="row-main"><strong>${esc(r.status||financeText('payRecord'))}</strong><small>${esc(date(r.createdAt))} · ${esc(short(r.transactionHash||r.id))}</small></div><div class="row-value">${fmt(r.amountYnxt)} YNXT${r.disputeUrl?`<small><a href="${esc(r.disputeUrl)}" rel="noreferrer">${esc(financeText('disputeLink'))}</a></small>`:''}</div></div>`).join(''):`<div class="empty compact">${esc(financeText('noOwnedPayReceipts'))}</div>`}
...
function budgetProgressRow(b,progress){
  ...
  return `<div class="row"><div class="row-main"><strong>${esc(b.name)}</strong><small>${esc(b.period)} · ${esc(financeText('planningOnly'))} · UTC</small><small>${esc(financeText('periodStart'))}: ${esc(period)} · ${esc(financeText('countFrom'))}: ${esc(effective)}</small><small>${esc(status)}</small><small>${esc(p?.coverage||financeText('noCompletePeriodHistory'))}</small></div><div class="row-value">${esc(financeText('budgetLimit'))}: ${budgetAmount(b.limitYnxt)}<small>${esc(financeText('observedSpending'))}: ${observed}</small><small>${esc(financeText('fullPeriodSpending'))}: ${esc(financeText('unknown'))}</small><small>${esc(financeText('remainingBudget'))}: ${esc(financeText('unknown'))}</small></div></div>`;
}
...
$('#statement-form').addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.currentTarget);try{const from=new Date(`${f.get('from')}T00:00:00Z`).toISOString(),toDate=new Date(`${f.get('to')}T00:00:00Z`);toDate.setUTCDate(toDate.getUTCDate()+1);state.statement=await api(`/api/statements?from=${encodeURIComponent(from)}&to=${encodeURIComponent(toDate.toISOString())}`);renderStatement(state.statement)}catch(error){notify(error.message,true)}});
...
async function startAI(){const button=$('#ai-start'),idleLabel=button.textContent;if(button.disabled)return;button.disabled=true;button.textContent=financeText('aiRequesting');try{const recordIds=$$('#ai-records input:checked').map(x=>x.value),kind=$('#ai-kind').value,consent=$('#ai-consent').checked;if(kind!=='draft_broker_order'&&recordIds.length<1)throw new Error(financeText('aiSelectOwned'));if(!consent)throw new Error(financeText('aiConsentRequired'));const payload={kind,recordIds,contextClasses:recordIds.length?['owned_activity']:[],consent};if(kind==='draft_broker_order'){const form=$('#ai-order-intent');if(!(form instanceof HTMLFormElement))throw new Error('Broker order intent form is unavailable.');const fields=new FormData(form),intent={symbol:String(fields.get('symbol')||'').trim().toUpperCase(),side:String(fields.get('side')||''),qty:String(fields.get('qty')||'').trim(),limitPrice:String(fields.get('limitPrice')||'').trim()};if(!/^[A-Z][A-Z0-9.]{0,11}$/.test(intent.symbol))throw new Error('Enter a canonical Broker symbol.');if(!['buy','sell'].includes(intent.side))throw new Error('Choose buy or sell.');if(!/^(?:[1-9][0-9]{0,5}|1000000)$/.test(intent.qty))throw new Error('Quantity must be a whole-share value from 1 through 1000000.');if(!/^(?:0\.[0-9]{0,3}[1-9]|[1-9][0-9]{0,8}(?:\.[0-9]{0,3}[1-9])?)$/.test(intent.limitPrice))throw new Error('Limit price must be a canonical fixed decimal from 0.0001 through 999999999.9999.');payload.securitiesOrderIntent=intent}state.aiJob=await api('/api/ai/jobs',{method:'POST',body:JSON.stringify(payload)});renderAIJob();pollAI()}catch(error){notify(error.message||financeText('aiDraftFailed'),true)}finally{button.disabled=false;button.textContent=idleLabel}}
