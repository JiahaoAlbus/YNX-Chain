...
export function createBrowserAuthorityCheckpointStore({indexedDB=globalThis.indexedDB,localStorage=globalThis.localStorage,anchor,clock,onCommit=()=>{}}={}){
  ...
  return Object.freeze({
    ...
    async compareAndSwap(previous,next){localStorage.setItem(MARKER,'yes');const {transaction,store,value}=await state('readwrite'),actual=value?.checkpoint??anchor,at=assertClock(clock,value?.trustedClockHighWaterMs??0);if(canonical(actual)!==canonical(previous)){transaction.abort();try{await transactionDone(transaction);}catch{}return false;}store.put({schemaVersion:SCHEMA,checkpoint:clone(next),trustedClockHighWaterMs:at},KEY);await transactionDone(transaction);onCommit(clone(next));return true;},
