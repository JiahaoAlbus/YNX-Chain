(()=>{
  ...
  const cardDetails=(source)=>{
    if(source?.id!=='card'||!source?.status?.available||!source?.envelope?.payload)return '';
    const payload=source.envelope.payload;
    if(payload.product!=='card'||payload.providerEnvironment!=='TEST'||payload.spendableBalance!==null||payload.balanceAuthority!=='none'||payload.simulationAndProviderFundsSeparated!==true)return '';
    const cards=Array.isArray(payload.cards)?payload.cards:[];
    const activities=Array.isArray(payload.activities)?payload.activities:[];
    const transactions=Array.isArray(payload.transactions)?payload.transactions:[];
    const capabilities=Array.isArray(source.envelope.capabilities)?source.envelope.capabilities:[];
    const activityAllowed=capabilities.includes('card.provider-activity.read');
    const transactionAllowed=capabilities.includes('card.provider-transactions.read');
    const amount=(minor,digits,currency)=>Number.isInteger(digits)&&digits>=0&&digits<=9?`${scaled(minor,10n**BigInt(digits),digits)??'—'} ${esc(currency||'')}`:'—';
    const cardRows=cards.slice(-5).reverse().map(item=>`<li><span>${esc(item?.provider||'—')} · ${esc(item?.productCardId||'—')}</span><strong>${esc(item?.status||'—')} · ${esc(item?.environment||'TEST')}</strong></li>`).join('');
    const activityRows=activityAllowed?activities.slice(-5).reverse().map(item=>`<li><span>${esc(item?.type||'—')} · ${esc(item?.status||'—')} · ${esc(observedAt(item?.occurredAt))}</span><strong>${esc(item?.productCardId||'—')} · ${item?.unit?.kind==='CARD_ACCOUNT'?amount(item?.amount,item?.unit?.minorUnitDigits,item?.unit?.currency):label('Amount not inferred','不推断金额')}</strong></li>`).join(''):'';
    const transactionRows=transactionAllowed?transactions.slice(-5).reverse().map(item=>`<li><span>${esc(item?.paymentType||'—')} · ${esc(item?.status||'—')} · ${esc(observedAt(item?.occurredAt||item?.processedAt))}</span><strong>${amount(item?.amountMinor,item?.minorUnitDigits,item?.currency)} · ${esc(item?.reconciliation||'—')}</strong></li>`).join(''):'';
    return `<div class="source-details" aria-label="${label('Owner-consented Card TEST records','本人同意共享的 Card 测试记录')}"><div class="source-metrics"><div><small>${label('Card TEST records','Card 测试记录')}</small><strong>${cards.length}</strong><span>${label('Provider references; not issued-card or balance proof','服务商引用；不是已发卡或余额证明')}</span></div><div><small>${label('Provider activity','服务商活动')}</small><strong>${activityAllowed?activities.length:'—'}</strong><span>${activityAllowed?label('Owner-consented metadata only','仅本人同意共享的元数据'):label('Owner consent required','需要本人同意')}</span></div><div><small>${label('Provider transaction readbacks','服务商交易回读')}</small><strong>${transactionAllowed?transactions.length:'—'}</strong><span>${transactionAllowed?label('Recorded TEST readbacks; not settlement','已记录测试回读；不是结算证明'):label('Owner consent required','需要本人同意')}</span></div></div><div class="source-evidence-grid"><div><h4>${label('Card references','卡片引用')}</h4><ul>${cardRows||`<li class="quiet">${label('No card reference returned.','未返回卡片引用。')}</li>`}</ul></div><div><h4>${label('Recent activity','近期活动')}</h4><ul>${activityRows||`<li class="quiet">${activityAllowed?label('No activity returned.','未返回活动。'):label('Owner consent required.','需要本人同意。')}</li>`}</ul></div><div><h4>${label('Transaction readbacks','交易回读')}</h4><ul>${transactionRows||`<li class="quiet">${transactionAllowed?label('No transaction readback returned.','未返回交易回读。'):label('Owner consent required.','需要本人同意。')}</li>`}</ul></div></div><small class="source-provenance">${label('TEST provider records only; spendable balance unknown. No real payment or custody inferred.','仅测试服务商记录；可用余额未知。不推断真实支付或托管。')} ${esc(source.envelope.coverage||'')} · ${label('as of','截至')} ${esc(source.envelope.asOf||'—')}</small></div>`;
  };
  ...
  const renderReadSources=(sources)=>{
    ...
    if(target)target.innerHTML=values.length?values.map(source=>{
      ...
      const stateText=({
        ...
        'owner-consent-required':label('Card owner consent is required. No card records or balance were read.','需要 Card 账户本人同意；未读取卡片记录或余额。'),
        'owner-integration-unconfigured':label('Card read service is not configured. No data substituted.','Card 只读服务尚未配置；不填入替代数据。'),
