...
const REVIEWED_VERIFIER_MANIFEST_SHA256='13a8700cbd779a3398648427b9464272afc800e0611725bb79da9b5c8ef551cd';
...
export async function verifyFinanceWalletBundle(options={}){
  ...
  if(binding.sourceBundleRelation?.byteReproducible!==true||binding.sourceBundleRelation?.status!=='VERIFIED_REPRODUCIBLE'||binding.sourceBundleRelation?.cleanBuildCount!==2||binding.sourceBundleRelation?.bytes!==181663||binding.sourceBundleRelation?.sha256!=='308714f5873469d528a49ce1df993fa9e3617ecabc60c7edaca7db063316893a'||Object.keys(binding.sourceBundleRelation).sort().join(',')!=='byteReproducible,bytes,cleanBuildCount,sha256,status')fail('FINANCE_WALLET_VERIFIER_MANIFEST_INVALID','source/bundle reproducibility binding is invalid');
