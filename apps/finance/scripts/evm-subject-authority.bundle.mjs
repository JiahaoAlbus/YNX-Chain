// apps/finance/scripts/evm-subject-authority.mjs
...
// packages/wallet-auth/src/finance-evm-common.js
...
function evmAccountType(value) {
  return pattern(value, "accountType", /^(eoa|contract)$/);
}
...
function subjectId(value) {
  return pattern(value, "subjectId", /^evm_subject_[0-9a-f]{64}$/);
}
...
function authorityTime(value) {
  if (!(value instanceof Date) || !Number.isFinite(value.getTime())) fail2("INVALID_TIME", "Trusted authority time is required");
...
function activeWindow(issuedAt, expiresAt, maxLifetime, at) {
  ...
  if (expires <= issued || expires - issued > maxLifetime) fail2("INVALID_EXPIRY", "Lifetime is invalid");
  if (issued > now + 3e4) fail2("ISSUED_IN_FUTURE", "Issued time is outside the trusted clock window");
  if (expires <= now) fail2("EXPIRED", "Authorization expired");
}
...
function verifyDevice(signature, message, publicKey) {
  let valid = false;
  try {
    valid = p256.verify(decodeBase64url(signature, "deviceSignature"), utf8ToBytes(message), decodeBase64url(publicKey, "deviceKey"), { format: "der", lowS: false });
  } catch {
    valid = false;
  }
  if (!valid) fail2("INVALID_DEVICE_PROOF", "P-256 device proof differs from the bound key");
}
function walletSignature(value, accountType) {
  const text = pattern(value, "walletSignature", /^0x(?:[0-9a-fA-F]{2}){1,2048}$/);
  if (accountType === "eoa" && !/^0x[0-9a-fA-F]{130}$/.test(text)) fail2("INVALID_SIGNATURE", "EOA signature must be 65 bytes");
  return text;
}
async function verifyWalletSignature(message, signature, account, accountType, verifyContractSignature) {
  const digestBytes = ethereumPersonalMessageDigest(message);
  if (accountType === "eoa") {
    if (recoverEthereumAddress(signature, digestBytes) !== account) fail2("INVALID_SIGNATURE", "EVM signature does not match account");
  } else {
    if (typeof verifyContractSignature !== "function") fail2("CONTRACT_ACCOUNT_UNSUPPORTED", "EIP-1271 verifier is unavailable");
    ...
    if (valid !== true) fail2("INVALID_SIGNATURE", "Contract account rejected the signature");
  }
}
...
var CHALLENGE = ["version", "productId", "subjectNamespace", "origin", "callback", "chainId", "account", "accountType", "scope", "deviceId", "deviceAlgorithm", "deviceKey", "nonce", "state", "requestId", "issuedAt", "expiresAt"];
var SESSION = ["version", "sessionId", "challengeDigest", "subjectId", "nativeAccount", "productId", "subjectNamespace", "origin", "chainId", "account", "accountType", "scope", "deviceId", "deviceAlgorithm", "deviceKey", "issuedAt", "expiresAt"];
...
var same = (a, b, code) => {
  if (canonicalJSON(a) !== canonicalJSON(b)) fail2(code, "Proof differs from server authority");
};
function parseFinanceEvmSubjectChallenge(input) {
  ...
  const v = Object.freeze({
    ...
    subjectNamespace: literal(input.subjectNamespace, "evm"),
    origin: literal(input.origin, FINANCE_EVM_ORIGIN),
    callback: literal(input.callback, FINANCE_EVM_CALLBACK),
    chainId: literal(input.chainId, FINANCE_EVM_CHAIN_ID),
    account: evmAccount(input.account),
    accountType: evmAccountType(input.accountType),
    scope: literal(input.scope, FINANCE_EVM_SUBJECT_SCOPE),
  ...
  if (Date.parse(v.expiresAt) <= Date.parse(v.issuedAt) || Date.parse(v.expiresAt) - Date.parse(v.issuedAt) > FINANCE_EVM_MAX_LIFETIME_MS) fail2("INVALID_EXPIRY", "Challenge lifetime invalid");
  return v;
}
...
function parseFinanceEvmSubjectLoginProof(input) {
  ...
  if (input.message !== message) fail2("MESSAGE_MISMATCH", "Wallet message changed");
...
function parseFinanceEvmSubjectSession(input) {
  ...
  const v = Object.freeze({
    ...
    nativeAccount: literal(input.nativeAccount, null),
    productId: literal(input.productId, "finance"),
    subjectNamespace: literal(input.subjectNamespace, "evm"),
    origin: literal(input.origin, FINANCE_EVM_ORIGIN),
    chainId: literal(input.chainId, FINANCE_EVM_CHAIN_ID),
  ...
  if (Date.parse(v.expiresAt) <= Date.parse(v.issuedAt) || Date.parse(v.expiresAt) - Date.parse(v.issuedAt) > FINANCE_EVM_MAX_LIFETIME_MS) fail2("INVALID_EXPIRY", "Session lifetime invalid");
  return v;
}
async function issueFinanceEvmSubjectSession(proofInput, expectedChallenge, issue, commit2, at, verifyContractSignature) {
  if (typeof commit2 !== "function") fail2("AUTHORITY_STORE_REQUIRED", "Atomic subject issuance required");
  ...
  if (Date.parse(session.expiresAt) > Date.parse(challenge.expiresAt)) fail2("INVALID_EXPIRY", "Session extends challenge");
  if (await commit2(Object.freeze({ challengeDigest: verified.challengeDigest, nonce: challenge.nonce, requestId: challenge.requestId, state: challenge.state, session })) !== true) fail2("REPLAY_OR_STORE_FAILURE", "Challenge consumed or session store failed");
  return session;
}
function parseUnsignedHttp(input) {
  ...
  const v = Object.freeze({
    ...
    origin: literal(input.origin, FINANCE_EVM_ORIGIN),
    scope: literal(input.scope, FINANCE_EVM_SUBJECT_SCOPE),
    method: pattern(input.method, "method", /^(GET|POST)$/),
  ...
  if (Date.parse(v.expiresAt) <= Date.parse(v.issuedAt) || Date.parse(v.expiresAt) - Date.parse(v.issuedAt) > FINANCE_EVM_PROOF_MAX_LIFETIME_MS) fail2("INVALID_EXPIRY", "HTTP proof lifetime invalid");
  return v;
}
...
function verifyHttp(proof, session, request, at) {
  if (request.origin !== session.origin || proof.origin !== session.origin) fail2("ORIGIN_MISMATCH", "Origin changed");
  for (const field of ["sessionId", "challengeDigest", "subjectId", "account", "scope"]) if (proof[field] !== session[field]) fail2("SESSION_BINDING_MISMATCH", "Session proof changed");
  for (const field of ["method", "target", "bodyDigest"]) if (proof[field] !== request[field]) fail2("HTTP_BINDING_MISMATCH", "Request changed");
  activeWindow(proof.issuedAt, proof.expiresAt, FINANCE_EVM_PROOF_MAX_LIFETIME_MS, at);
  const now = authorityTime(at);
  if (Date.parse(session.expiresAt) <= now || Date.parse(proof.issuedAt) < Date.parse(session.issuedAt) || Date.parse(proof.expiresAt) > Date.parse(session.expiresAt)) fail2("SESSION_EXPIRED", "Session is inactive");
...
async function verifyAndConsumeFinanceEvmSubjectRead(proofInput, loadSession, request, consumeActiveProof, at) {
  if (typeof loadSession !== "function" || typeof consumeActiveProof !== "function") fail2("AUTHORITY_STORE_REQUIRED", "Authoritative active session transaction required");
  ...
  if (stored == null) fail2("SESSION_NOT_FOUND", "Session absent");
  const session = parseFinanceEvmSubjectSession(stored);
  if (request.method !== "GET" || proof.method !== "GET" || request.requiredScope !== FINANCE_EVM_SUBJECT_SCOPE) fail2("SCOPE_DENIED", "Private read scope only");
  const path = rawTarget(request.target).split("?")[0];
  if (!Array.isArray(request.allowedTargets) || request.allowedTargets.length === 0 || request.allowedTargets.length > 32 || request.allowedTargets.some((v) => typeof v !== "string" || v.includes("?") || rawTarget(v) !== v) || !request.allowedTargets.includes(path)) fail2("ROUTE_DENIED", "Target not server allowlisted");
  verifyHttp(proof, session, request, at);
  if (await consumeActiveProof(Object.freeze({ sessionId: session.sessionId, subjectId: session.subjectId, chainId: session.chainId, account: session.account, challengeDigest: session.challengeDigest, nonce: proof.nonce, expiresAt: proof.expiresAt, asOf: new Date(authorityTime(at)).toISOString() })) !== true) fail2("REPLAY_OR_REVOKED", "Proof replayed or session revoked");
...
async function verifyAndConsumeFinanceEvmSubjectRevoke(proofInput, loadSession, request, revokeAndConsume, at) {
  if (typeof loadSession !== "function" || typeof revokeAndConsume !== "function") fail2("AUTHORITY_STORE_REQUIRED", "Authoritative revocation transaction required");
  exactFields(request, ["origin", "method", "target", "bodyDigest"], "Finance EVM revoke context");
  const proof = parseFinanceEvmSubjectHttpProof(proofInput), stored = await loadSession(proof.sessionId);
  if (stored == null) fail2("SESSION_NOT_FOUND", "Session absent");
  const session = parseFinanceEvmSubjectSession(stored);
  if (request.method !== "POST" || proof.method !== "POST" || request.target !== FINANCE_EVM_SUBJECT_REVOKE_TARGET) fail2("REVOKE_ROUTE_MISMATCH", "Revoke route changed");
  verifyHttp(proof, session, request, at);
  if (await revokeAndConsume(Object.freeze({ sessionId: session.sessionId, subjectId: session.subjectId, account: session.account, nonce: proof.nonce, asOf: new Date(authorityTime(at)).toISOString() })) !== true) fail2("REPLAY_OR_REVOKED", "Session already revoked or nonce consumed");
