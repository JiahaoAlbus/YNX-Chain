...
trap - EXIT
printf 'releaseContainer=%s\nreleaseContainerIdentityTuple=%s\nreleaseContainerEmptyTuple=%s\nreleaseContainerTuple=%s\nrelease=%s\nreleaseTuple=%s\nreleaseInventorySha256=%s\nstageParentTuple=%s\nbackupParentTuple=%s\nreleaseParentTuple=%s\n' "$release_container" "$release_container_identity_tuple" "$release_container_empty_tuple" "$release_container_tuple" "$release" "$release_tuple" "$release_inventory" "$(stat -Lc '%u:%g:%a:%h' "$(get '.paths.parents.stage.path')")" "$(stat -Lc '%u:%g:%a:%h' "$(get '.paths.parents.backup.path')")" "$(stat -Lc '%u:%g:%a:%h' "$(get '.paths.parents.release.path')")"
