...
set -euo pipefail
if [[ $# -ne 23 ]]; then exit 64; fi
stdin=$1; stdout=$2; stderr=$3; receipt=$4; bootstrap=$5; bootstrap_bytes=$6; bootstrap_sha=$7; shift 7
id=$1; shift
bootstrap_args=("$@")
...
remote_launcher='bootstrap_source=$(printf "%s" "$1" | base64 -d) || exit 65; shift; exec /bin/bash -c "$bootstrap_source" finance-phase3 "$@"'
ssh_bin=/usr/bin/ssh; sudo_bin=/usr/bin/sudo
if [[ "${FINANCE_PHASE3_TRANSPORT_TEST_ROOT:-}" = 1 ]]; then ssh_bin=${FINANCE_PHASE3_SSH_BIN:?}; sudo_bin=${FINANCE_PHASE3_SUDO_BIN:?}; fi
remote_quote(){ printf "'"; printf '%s' "$1" | sed "s/'/'\\\\''/g"; printf "'"; }
remote_command=exec
...
terminal_receipt_valid(){
  ...
  test "$(grep -Fxc 'phase=3' "$stdout")" = 1
  for key in deployParent deployParentTuple executor executorTuple executorBytes executorSha256 lease leaseTuple leaseBytes leaseSha256 rollbackArgv0 rollbackArgv1 rollbackArgv2; do test "$(grep -c "^${key}=" "$stdout")" = 1; done
}
