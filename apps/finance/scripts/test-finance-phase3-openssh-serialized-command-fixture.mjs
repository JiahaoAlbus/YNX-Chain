...
function make({silent=false, malformed=false}={}){
  ...
  const stdin=join(root,'lease.json'),stdout=join(root,'stdout'),stderr=join(root,'stderr'),receipt=join(root,'receipt'),bootstrap=join(root,'bootstrap.sh'),executorB64=join(root,'apps/finance/evidence/finance-nonregressive-phase3-executor.base64');writeFileSync(stdin,'{"lease":{"signed":true}}\n');mkdirSync(join(root,'apps/finance/evidence'),{recursive:true});writeFileSync(executorB64,'ZXhlY3V0b3I=\n');
  ...
  const args=[stdin,stdout,stderr,receipt,bootstrap,String(n(bootstrap)),sha(bootstrap),executorB64,String(n(executorB64)),sha(executorB64),id,'/carrier','root','leases','carrier','archiveTuple','archiveSha','archiveBytes','envTuple','envSha','envBytes','executorBytes','executorSha','leaseBytes','leaseSha'];
...
{const x=make();x.args[10]='wrong-phase3-namespace';const r=run(x);assert.equal(r.status,65);assert.equal(existsSync(x.counter),false,'wrong transport namespace must not reach SSH');rmSync(x.root,{recursive:true,force:true});}
{const x=make();x.args[9]='0'.repeat(64);const r=run(x);assert.equal(r.status,65);assert.equal(existsSync(x.counter),false,'executor carrier mismatch must not reach SSH');rmSync(x.root,{recursive:true,force:true});}
