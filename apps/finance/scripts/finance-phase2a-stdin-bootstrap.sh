...
cleanup(){ for e in "${created[@]}"; do IFS='|' read -r p h <<<"$e"; test -f "$p" && test ! -L "$p" && test "$(sha256sum "$p" | awk '{print $1}')" = "$h" && rm -f -- "$p"; done; }; trap cleanup EXIT
for i in 0 1 2; do read -r tag name mode size hash; [[ "$tag" = FRAME && "$name" = "${names[$i]}" && "$mode" = "${modes[$i]}" && "$size" = "${size//[!0-9]/}" && "$hash" = "${hash//[!0-9a-f]/}" ]] || exit 65; read -r data; target="$lease_parent/$id.$name"; [[ ! -e "$target" && ! -L "$target" ]] || exit 65; printf %s "$data" | base64 -d > "$target"; chmod "$mode" "$target"; created+=("$target|$(sha256sum "$target" | awk '{print $1}')"); [[ "$(wc -c < "$target" | tr -d ' ')" = "$size" && "$(sha256sum "$target" | awk '{print $1}')" = "$hash" ]] || exit 65; done
read -r end; test "$end" = END; test -z "$(cat)"; for e in "${created[@]}"; do IFS='|' read -r p h <<<"$e"; stat -Lc '%n:%d:%i:%u:%g:%a:%h:%s:%F' "$p"; sha256sum "$p"; done; created=(); trap - EXIT
