...
const selected=tracked.filter(path=>(exact.has(path)||evidence.has(path)||scripts.has(path)||prefixes.some(prefix=>path.startsWith(prefix)))&&!path.startsWith('apps/finance/evidence/release-candidates/')).sort();
...
const manifest={
  ...
  recovery:{checkout:`git checkout ${source}`,verify:`node apps/finance/scripts/verify-finance-v3-acceptance-recovery.mjs --archive ${prefix}.tar.gz --source ${source}`,cleanExtractionVerify:`node apps/finance/scripts/verify-finance-v3-acceptance-recovery.mjs --archive ${prefix}.tar.gz --source ${source} --execute-clean`,tests:['go test -race ./internal/finance/... ./apps/finance/cmd/...','go vet ./internal/finance/... ./apps/finance/cmd/...','go build ./apps/finance/cmd/...','npm --prefix apps/finance test','npm --prefix apps/finance run security','npm --prefix apps/finance run smoke'],secretsIncluded:false},
  files:inventory,
};
