...
const manifest={
  ...
  recovery:{checkout:`git checkout ${source}`,verify:`node apps/finance/scripts/verify-finance-v3-acceptance-recovery.mjs --archive ${prefix}.tar.gz --source ${source}`,tests:['go test -race ./internal/finance/... ./apps/finance/cmd/...','go vet ./internal/finance/... ./apps/finance/cmd/...','go build ./apps/finance/cmd/...','npm --prefix apps/finance test','npm --prefix apps/finance run security','npm --prefix apps/finance run smoke'],secretsIncluded:false},
  files:inventory,
};
