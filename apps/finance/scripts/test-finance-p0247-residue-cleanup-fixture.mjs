...
function setup() {
  ...
  return { root, stageParent, backupParent, stage, backup, current, old, env, unit, caddy, lease, mkLease, serviceLog, removalLog, curlLog };
}
...
{ const f = setup(); const before = Object.fromEntries([f.env, f.unit, f.caddy].map(p => [p, sha(readFileSync(p))])); const r = run(f, { PATH: `${join(f.root, 'bin')}:${process.env.PATH}` }); assert.equal(r.status, 0, `${r.stdout}\n${r.stderr}`); assert.equal(existsSync(f.stage), false); assert.equal(existsSync(f.backup), false); assert.equal(readFileSync(f.removalLog, 'utf8').split('\n').filter(Boolean).every(line => line.endsWith(f.stage) || line.endsWith(f.backup)), true, 'only exact residues are removed'); assert.equal(readFileSync(f.curlLog, 'utf8').includes(' -o - '), true, 'response stays streamed'); unchanged(f, before); cleanup(f); }
