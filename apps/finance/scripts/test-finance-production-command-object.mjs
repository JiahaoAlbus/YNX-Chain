...
for(const x of ['lease.signed','LEASE_ID','http_check','path.url','%{http_code}','verify_old_live','verify_candidate_live','verify_restored','verify_local_assets "$candidate"','relativePath','paths.basenames.$name','fresh.state.tuple','fresh.state.restoredTuple','candidate-state-sha256','newpid','MainPID','NRestarts','assert_exact_child archive','assert_exact_child newEnv','realpath -e','ELF 64-bit.*x86-64','$(bytes "$candidate/ynx-finance")','restore(){','trap \'restore\' EXIT','systemctl restart','current.next'])assert.ok(s.includes(x),x);
