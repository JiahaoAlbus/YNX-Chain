#!/usr/bin/env bash
# One-invocation P0-275 transport. Two signed frames are materialized only as
...
cleanup() {
  if [[ "$lease_created" = true ]] && exact "$lease" "$lease_bytes" "$lease_sha" "$lease_tuple"; then rm -- "$lease"; fi
  if [[ "$executor_created" = true ]] && exact "$executor" "$executor_bytes" "$executor_sha" "$executor_tuple"; then rm -- "$executor"; fi
...
read_frame() {
  local expected_name=$1 path=$2 expected_bytes=$3 expected_sha=$4 expected_mode=$5 created_flag=$6 line name fbytes fsha fmode payload extra
...
read_frame executor "$executor" "$executor_bytes" "$executor_sha" 700 executor_created || exit 65; executor_tuple=$frame_tuple
read_frame signedLease "$lease" "$lease_bytes" "$lease_sha" 600 lease_created || exit 65; lease_tuple=$frame_tuple
IFS= read -r terminal && test "$terminal" = END
if IFS= read -r extra; then exit 65; fi
test "$(jq -r '.lease.signed' "$lease")" = true
test "$(jq -r '.lease.kind' "$lease")" = FINANCE_P0272_CONTROL_CLEANUP_ONLY
...
set -e
test "$rc" = 0
test "$(printf '%s\n' "$receipt" | grep -Fxc 'cleanup=P0272_CONTROL_FILES_REMOVED')" = 1
