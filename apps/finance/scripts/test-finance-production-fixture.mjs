...
function makeFixture(failure, { absent = false, id = 'fixture-lease' } = {}) {
  ...
  const parents = { stage: join(ynx, 'stage', 'finance'), backup: join(ynx, 'backups', 'finance'), release: join(ynx, 'releases', 'finance') }; for (const parent of Object.values(parents)) mkdirSync(parent, { recursive: true });
  const carrier = join(ynx, 'stage', 'finance', 'finance-combined-4f7fba323a89-20260831t041500z');
  const carrierId = basename(carrier); const namespaces = { stage: `${carrierId}-deploy`, backup: carrierId, release: carrierId };
  const releaseBasename = 'ynx-finance-4f7fba323a89';
  const p = { parents, namespaces, carrier, old: join(ynx, 'releases', 'old'), current: join(ynx, 'finance-current'), env: join(ynx, 'etc', 'finance.env'), unit: join(ynx, 'etc', 'finance.service'), caddy: join(ynx, 'etc', 'Caddyfile'), state: join(ynx, 'var', 'state.json'), archive: join(carrier, 'candidate.tgz'), newEnv: join(carrier, 'finance.env'), targetWeb: join(parents.release, namespaces.release, releaseBasename, 'web'), stageContainer: join(parents.stage, namespaces.stage), stage: join(parents.stage, namespaces.stage, 'stage'), backupContainer: join(parents.backup, namespaces.backup), backup: join(parents.backup, namespaces.backup, 'backup'), releaseContainer: join(parents.release, namespaces.release), release: join(parents.release, namespaces.release, releaseBasename), lease: join(ynx, 'leases', 'finance', 'fixture.json'), responses: join(dir, 'responses'), service: join(dir, 'service.state') };
...
function crossStagePhase3Fixture() {
  ...
  assert.ok(p.stage.includes(`/${p.namespaces.stage}/`) && p.backup.includes(`/${p.namespaces.backup}/`) && p.release.includes(`/${p.namespaces.release}/`), 'signed namespaces own only mutable paths');
  assert.notEqual(p.stageContainer, p.carrier, 'deployment stage namespace cannot collide with retained carrier');
