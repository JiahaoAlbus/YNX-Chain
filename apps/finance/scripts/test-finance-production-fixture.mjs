...
function writeLease(fixture, failure, absent, id = 'fixture-lease', releaseContainerGid = null) {
...
function releaseOwnershipTransitionFixture() {
  const currentGid = Number(shell('/usr/bin/id', ['-g']));
  const alternateGid = shell('/usr/bin/id', ['-G']).split(/\s+/).map(Number).find(gid => gid !== currentGid);
  assert.ok(Number.isInteger(alternateGid), 'fixture requires a supplementary group to exercise an actual release-container gid transition');
  const fixture = makeFixture(false, { releaseContainerGid: alternateGid }); const p = fixture.paths;
  ...
  assert.equal(Number(tuple(p.releaseContainer).split(':')[1]), alternateGid, 'signed release-container group transition is accepted after physical identity recheck');
  assert.equal(readlinkSync(p.current), p.release, 'gid transition reaches the candidate switch only after release materialization succeeds');
}
