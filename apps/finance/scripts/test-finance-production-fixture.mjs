...
function crossStagePhase3Fixture() {
  ...
  assert.ok(result.stdout.includes('phase=3') && result.stdout.includes('releaseContainerTuple=') && result.stdout.includes(`lease=${join(leasesParent, 'finance', `${id}.json`)}`), 'exact bootstrap invokes real executor and returns release-container receipt');
