...
const walletSource = (await read('mobile/src/wallet.ts')).toString('utf8');
const apiSource = (await read('mobile/src/api.ts')).toString('utf8');
const appSource = (await read('mobile/App.tsx')).toString('utf8');
...
assert(hash(JSON.stringify(payload)) === expectedManifestHash, 'accepted endpoint manifest payload hash mismatch');
...
const report = {
  ...
  endpointManifest: {
    ...
    payloadSha256: expectedManifestHash,
