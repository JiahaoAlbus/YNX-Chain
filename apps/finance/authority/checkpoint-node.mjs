...
async function withLock(file,action){
  ...
  for(let attempt=0;attempt<80;attempt++){
    let handle;
    try{handle=await fs.open(lock,'wx',0o600);}catch(error){if(error?.code==='EEXIST'){await wait(10);continue;}throw error;}
    try{return await action();}finally{await handle.close();await fs.unlink(lock).catch(error=>{if(error?.code!=='ENOENT')throw error;});}
  }
