...
export type Budget={id:string;name:string;categoryId:string;limitYnxt:number;period:string;startsAt:string};
...
export type Overview={portfolio:{account:string;network:string;symbol:string;balanceYnxt:number;stakedYnxt:number;activity:Activity[];payReceipts:PayReceipt[];explorerStatus:SourceStatus;payStatus:SourceStatus;asOf:string;readOnly:boolean};profile:{categories:Category[];budgets:Budget[];reminders:Reminder[];privacy:Privacy;aiJobs:AIJob[]};alerts:Array<Record<string,unknown>>;support:Support;boundaries:Record<string,unknown>};
