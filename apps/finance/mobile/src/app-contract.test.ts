...
test('native client exposes the complete read-only Finance workflow',()=>{
  for(const path of ['/api/overview','/api/activity/','/api/statements','/api/export?format=','/api/privacy','/api/audit','/api/ai/jobs'])assert.ok(api.includes(path),path);
  for(const copy of ['Pay receipts','Open dispute evidence','Security alerts','Copy CSV activity','Support, disputes and audit','Recovery: retry live sources'])assert.ok(app.includes(copy),copy);
});
...
test('Wallet request is exact and the client carries no fake finance claims',()=>{
  ...
  assert.ok(app.includes('no fiat conversion inferred'));
