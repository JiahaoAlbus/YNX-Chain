...
- Adds a bounded tenant-bound parser for the documented v2 trade-event SSE shape. It accepts additive provider fields and the legal `at`, `event_id`, `extended_hours`, `created_at` and `updated_at` fields while strictly validating the identity, event, order, status and timestamps Finance relies on. Official `at` publication time and `timestamp` occurrence time may differ; Finance persists the latter. Duplicate cursors, tenant mismatch, truncation and oversized batches fail closed. Network streaming remains unactivated.
...
- `npm test --prefix apps/finance` — 59/59 PASS, including real Chrome bundle time parsing, approve-delivery-failure/reload/revoke recovery, pending unsigned challenge revocation, and the provider-search/watchlist/reconcile/cancellation product-entry contract.
- Exact Wallet authority package test at `bd977cd2d` — 660/660 PASS.
- Cross-product integration checkpoint `c66b498c723cdc7ce0dad1d7ac26022b01a91c94` (tree `24953b3918537a6fa015de8fa9d1b0740257a0ec`) — PASS with Finance `a3e5d21b91443e6db4d9cad2d38e112773e9a57f`, Wallet `bd977cd2d382c4c43a435c8e415e698205b8a102`, and `diagnosticDateAdapter=false`. The immutable report SHA-256 is `9707d6a7e4b84cf82cd3473f12186f2fe258766a7d022676f52e62b16f32f6b9`. It covers manual/AI approve, reject and revoke; account/asset/quote/positions preflight before provider POST; lost acknowledgement, restart/query, partial fill, duplicate-event non-mutation, cancel and reconciliation. The fixture pins provider sockets to loopback TLS and is not official Sandbox, installed-client, public deployment or production evidence.
