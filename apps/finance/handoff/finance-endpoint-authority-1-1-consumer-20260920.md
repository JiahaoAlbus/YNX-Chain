...
Finance now consumes the shared authority merged at `98a5774ae229acd1a41f7647d95b858d31b73ad2`. Both the mobile package and the Web order-approval bundle import `@ynx-chain/sdk` directly from the repository workspace; neither copies the shared validator, canonicalization, manifest, or bundle.
...
The authority deliberately leaves Wallet Gateway and `products.finance` at `PENDING`. Accordingly, Finance Wallet start, callback completion, Product Session proof, private API access, and order submission fail closed with `PRIVATE_SERVICE_DEGRADED`. The Web gate runs before challenge POST, callback POST, execution-request POST, order pending-state creation, or confirmation UI. Browser regression evidence records zero challenge/callback/execution network calls and zero order pending writes while the authority is pending. The canonical approval protocol remains covered only by an isolated fixture and is not represented as publicly active. This does not disable the separate Web Standard Wallet connection or guest/public surfaces.
