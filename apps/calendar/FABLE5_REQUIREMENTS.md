...
| Web / Android / iOS / macOS | Web is deployed; Android source now exposes a real Calendar workspace with supplied proportional YNX asset, date/view navigation, week strip, guest agenda, reviewed event editor, reconnect state and Wallet boundary; iOS/macOS source remains earlier compact UI | Web proven; Android debug APK builds and was installed/cold-launched on a disposable API 36 AVD, but the AVD repeatedly produced system/Launcher ANRs so this run is diagnostic evidence, not an accepted native install gate | Healthy-emulator Android interaction evidence plus current-source iOS/macOS install and cold launch remain |
