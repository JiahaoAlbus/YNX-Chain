...
| Web / Android / iOS / macOS | Web is deployed; Android and iOS source now expose a real Calendar workspace with the supplied proportional YNX asset, date/view navigation, week strip, guest agenda, reviewed event editor and Wallet boundary | Web proven; Android debug APK builds and rendered after install, but the AVD repeatedly produced system/Launcher ANRs so the run is diagnostic; iOS source parses and project/plist validate, but this host has Command Line Tools only and no Xcode/Simulator | Healthy-emulator Android interaction evidence plus Xcode-hosted current-source iOS and macOS install/cold launch remain |
