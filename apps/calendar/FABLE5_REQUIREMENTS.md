...
| --- | --- | --- | --- |
| Day / Week / Month | Web implemented | Browser screenshots | Public build `9abb1616` verified with ComputerControl |
| Agenda | Web implemented | Browser recurring/search proof | Public build `9abb1616`; direct interactive public proof remains |
...
| Title / description / location / link | Service + Web implemented | Round-trip and browser proof | Public build `9abb1616` exposes all fields |
| Start / end / all-day / time zone | Service + Web implemented | Round-trip, invalid-zone and DST tests | Public build `9abb1616` exposes all fields |
...
| Search | Web occurrence search implemented | Browser exact/no-result proof | Public build `9abb1616`; server-side full-text indexing is not implemented |
...
| Invitation update / cancellation / organizer / attendee | Persistent event lifecycle, immutable attributed comments, and account activity notifications implemented | Update/cancel/revert/comment/notification authorization and read-state tests | Public build `9abb1616`; external Mail/push delivery remains |
...
| Daily / weekly / monthly / yearly recurrence | Versioned schema v1 implemented | Unit recurrence tests | Public build `9abb1616`; public series-management walkthrough remains |
...
| Conflict overlap / recurrence / time zone / buffer / attendee availability | Owner overlap plus per-event preparation/travel buffers and explicitly shared attendee availability implemented; attendee details redact to Busy | Unit owner/recurrence/buffer/authorization/privacy tests; browser buffer round-trip | Public build `9abb1616` plus ComputerControl control proof |
| Alternative-time suggestions | Deterministic weekday conflict-free drafts are generated for single timed events; selecting one returns to the editor and never auto-moves the event; AI adapter remains optional | Unit no-auto-move test and browser controls | Exact public alternative-selection evidence pending; AI provider success is not proved |
...
| Notification / Mail integration | In-app invitation, RSVP, comment, calendar permission and revoke notifications implemented with unread state; Mail remains a contract dependency | Multi-user notification lifecycle, availability privacy, and browser notification-center tests | Public build `9abb1616` plus ComputerControl fail-closed proof; external Mail/push and product 25 integration remain |
