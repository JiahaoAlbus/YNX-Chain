...
function ynxAnnouncement(detail) {
...
export async function connectCalendarWallet(windowLike = window, {timeoutMs = 250} = {}) {
  const announced = await discoverEIP6963(windowLike, {timeoutMs});
  const ynxProviders = announced.filter(ynxAnnouncement);
  if (ynxProviders.length > 1) throw new DAppConnectError("AMBIGUOUS_YNX_WALLET", "More than one YNX Wallet provider was announced. Disable duplicate Wallet installations and retry.");
  const selected = ynxProviders[0] ?? (announced.length === 1 ? announced[0] : injectedFallback(windowLike));
