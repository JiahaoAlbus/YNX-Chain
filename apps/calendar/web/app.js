...
async function rsvp(event, response) {
  ...
  const comment = prompt("Optional RSVP note for the organizer (up to 1000 characters)", "");
  if (comment === null) return;
  try {
    await api(`/v1/events/${event.id}/rsvp`, {
      method: "POST",
      body: JSON.stringify({ response, comment: comment.trim(), client_mutation_id: mutationID(), base_version: event.version }),
    });
    toast("RSVP updated");
    $("#event-detail").close();
    loadEvents();
