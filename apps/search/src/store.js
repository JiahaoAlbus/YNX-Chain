...
export const SEARCH_SNIPPET_STORAGE_LIMIT = SEARCH_SOURCE_USE_POLICY.snippetStorageLimit;
const EMPTY = { version: 4, revision: 0, sources: {}, documents: {}, cases: {}, aiAudit: [], privacyAudit: [], walletChallenges: {} };
function sourceRightsError(source, documents = []) {
  if (!source?.dataRights || !source?.terms) return null;
  const use = source.terms.permittedUse;
  if (!SEARCH_SOURCE_USE_POLICY.permittedUses.includes(use)) return "source permitted use is invalid";
  if (source.dataRights.aiRetrieval === true && (source.dataRights.storage !== true || source.dataRights.snippets !== true || use === "metadata-only")) return "AI retrieval requires stored, snippet-authorized index content";
  if (use === "index-fulltext-link" && source.dataRights.storage !== true) return "full-text indexing requires storage rights";
  if (use === "index-snippet-link" && (source.dataRights.storage !== true || source.dataRights.snippets !== true)) return "snippet indexing requires storage and snippet rights";
  if (source.dataRights.storage !== true && documents.length) return "stored documents exceed source storage rights";
  if (use === "metadata-only" && documents.some(document => String(document.text ?? "").length > 0)) return "stored document body exceeds metadata-only use";
  if (use === "index-snippet-link" && documents.some(document => String(document.text ?? "").length > SEARCH_SNIPPET_STORAGE_LIMIT)) return "stored document exceeds snippet limit";
  return null;
}
function migrateDatabase(input) {
  ...
  for (const source of Object.values(db.sources)) {
    if (source.migrationRequired) continue;
    const error = sourceRightsError(source, Object.values(db.documents).filter(document => document.sourceId === source.id));
    if (error) {
      source.enabled = false;
      source.indexingStatus = "disabled";
      source.migrationRequired = "source-data-rights-v1";
...
export function validateSource(input) {
  ...
  if (!SEARCH_SOURCE_USE_POLICY.permittedUses.includes(permittedUse)) throw new Error("invalid permitted use");
  const storageRight = input.storageRight !== false;
  const snippetRight = input.snippetRight !== false;
  const aiRetrievalRight = input.aiRetrievalRight === true;
  if (permittedUse === "index-fulltext-link" && !storageRight) throw new Error("full-text indexing requires storage rights");
  if (permittedUse === "index-snippet-link" && (!storageRight || !snippetRight)) throw new Error("snippet indexing requires storage and snippet rights");
  if (aiRetrievalRight && (!storageRight || !snippetRight || permittedUse === "metadata-only")) throw new Error("AI retrieval requires stored, snippet-authorized index content");
  ...
  return {
    ...
    dataRights: {
      storage: storageRight,
      snippets: snippetRight,
      aiRetrieval: aiRetrievalRight,
    },
    dataPolicy: {
...
export class SearchStore {
  ...
  async indexDocument(sourceId,input){
    return this.mutate(db=>{
      ...
      if(source.dataRights.storage!==true)throw new Error("source storage right required");
      const permittedUse=source.terms.permittedUse;
      const rawText=String(input.text??"").replace(/\s+/g," ").trim();
      if(rawText.length>2_000_000||(permittedUse!=="metadata-only"&&rawText.length<20))throw new Error("document text size invalid");
      const title=String(input.title??url.pathname).slice(0,300);
      const language=input.language??source.languages[0]??null;
      if(!source.languages.includes(language))throw new Error("document language is outside source policy");
      ...
      const text=permittedUse==="metadata-only"?"":permittedUse==="index-snippet-link"?rawText.slice(0,SEARCH_SNIPPET_STORAGE_LIMIT):rawText;
      ...
      db.documents[id]={id,sourceId,url:url.href,title,text,dataClass,language,contentType:input.contentType??"text/html",publishedAt:input.publishedAt??null,fetchedAt,indexedAt,contentDigest,indexReceipt:{...receipt,digest:canonicalDigest("YNX_INDEX_RECEIPT_V1",receipt)},terms:words(`${title} ${text}`)};
  ...
  async search(query,{page=1,pageSize=10,sourceId=null,freshnessDays=null,contentType=null,language=null,dataClass=null,aiRetrievalOnly=false}={}){
    ...
    const candidates=Object.values(db.documents).filter(doc=>{const source=db.sources[doc.sourceId];return source?.enabled===true&&(!aiRetrievalOnly||(source.dataRights?.aiRetrieval===true&&source.dataRights?.snippets===true&&source.terms?.permittedUse!=="metadata-only"))&&(!sourceId||doc.sourceId===sourceId)&&(!contentType||doc.contentType===contentType)&&(!language||doc.language===language||source.languages?.includes(language))&&(!dataClassFilter||doc.dataClass===dataClassFilter)&&(!cutoff||Date.parse(doc.publishedAt??doc.fetchedAt)>=cutoff)}).map(doc=>{
    ...
    return{
      ...
      results:candidates.slice(start,start+pageSize).map(({doc,source,factors,score})=>({
        ...
        sourceUse:{permittedUse:source.terms?.permittedUse??null,storage:source.dataRights?.storage===true,snippets:source.dataRights?.snippets===true,aiRetrieval:source.dataRights?.aiRetrieval===true},
        ...
        snippet:source.dataRights?.snippets===true&&doc.text?snippet(doc.text,terms):null
      })),
  ...
  async updateCase(id,status,note,actor="operator"){if(!['under-review','accepted','rejected','corrected','removed'].includes(status))throw new Error("invalid case status");return this.mutate(db=>{const item=db.cases[id];if(!item)throw new Error("case not found");item.status=status;item.updatedAt=this.clock();item.audit.push({event:status,note:String(note??"").slice(0,1000),actor,at:this.clock()});if(status==='removed'){const affectedSources=new Set();for(const [docId,doc] of Object.entries(db.documents))if(doc.url===item.sourceUrl){affectedSources.add(doc.sourceId);delete db.documents[docId]}for(const sourceId of affectedSources)if(db.sources[sourceId])db.sources[sourceId].documentCount=Object.values(db.documents).filter(document=>document.sourceId===sourceId).length}return item})}
