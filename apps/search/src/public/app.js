...
let aiSources = [];
let locale = resolve(localStorage.getItem("ynx-search-locale") || "en");
...
let standardAccount="",standardChainId="",standardConnection=null,standardProviderName="";
const privateServiceAvailable=()=>!["UNAVAILABLE","PENDING"].includes(globalThis.__YNX_SEARCH_RUNTIME__?.endpoints?.appGateway?.status);
function renderWalletState(){if(standardAccount){$("#wallet-button").textContent=`${standardProviderName||"Wallet"} · ${standardAccount.slice(0,6)}…${standardAccount.slice(-4)}`;$("#private-wallet-button").disabled=false}else{$("#wallet-button").textContent="Connect Wallet";$("#private-wallet-button").disabled=true}$("#private-wallet-button").textContent=privateServiceAvailable()?"Connect private Search service":"Private service degraded"}
$("#wallet-button").onclick = async () => {
  try {const result=await connectStandardWallet();standardAccount=result.account;standardChainId=result.chainId;standardConnection=result.connection;standardProviderName=result.providerName;result.connection.on("accountsChanged",accounts=>{standardAccount=accounts[0]||"";renderWalletState()});result.connection.on("chainChanged",chain=>{standardChainId=chain;renderWalletState()});renderWalletState();showNotice(`Standard Wallet connected on ${standardChainId}. Private Search service is ${privateServiceAvailable()?"available for separate approval":"degraded"}; public Search remains available.`)}
  catch(error){$("#wallet-recovery").hidden=false;showNotice(`No compatible Wallet is connected: ${error.message}`)}
};
$("#private-wallet-button").onclick = async () => {
