...
export async function createDocsBrowserSession({sdk, registry, environment = globalThis, walletInstalled = () => false}) {
  ...
  return sdk.createBrowserProductSessionClient({
    ...
    scopes: [...docsSessionBinding.scopes],
    purpose: 'Read only explicitly authorized YNX Docs documents.',
    gateway, environment,
  });
}

export async function loadDocsBrowserSession({environment = globalThis, walletInstalled} = {}) {
  ...
  return createDocsBrowserSession({sdk, registry, environment, walletInstalled});
}
