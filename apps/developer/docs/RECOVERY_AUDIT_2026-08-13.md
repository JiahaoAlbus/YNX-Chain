...
| Debug Adapter Protocol | Authenticated, bounded DAP bridge for C/C++ via LLDB and Python via pinned debugpy; Python requires the selected owner/project-bound LXD lease, locks it for the session, keeps external networking disabled and removes the temporary debug workspace on exit | Mock routing gates pass for both adapter families; real debugpy 1.8.21 reaches a source breakpoint, stack frame, local scope and `value = 7`; the protected live-container gate repeats the same evidence inside the published LXD image | Capture the protected public live-container DAP evidence; installed LLDB-DAP remains unavailable on this macOS verifier, and Node, Go and Rust adapter acceptance remains open |
...
- Debug: 3 passed, 1 skipped (real debugpy breakpoint/stack/locals passed; reviewed LLDB-DAP absent on this verifier).
- Git: 1 passed.
