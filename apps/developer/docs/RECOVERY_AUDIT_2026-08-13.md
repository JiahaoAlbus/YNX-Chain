...
| Dependency installation | Exact-version npm reconciliation and exact Python wheel installation in an owner/project LXD store with one-time review | `runtime-profile-service` rejects ranges, malformed locks and cross-owner access; npm disables lifecycle scripts; Python rejects source builds, atomically persists a project venv and returns `requirements.ynx.lock`; both bound storage, remove temporary egress and link only the project store into later tasks | Configure and firewall the production package-egress network, then capture public npm/Python install, restart and import evidence; Cargo, Go, Maven/Gradle and Solidity-framework adapters remain open |
...
- Runtime profiles: 14 passed, including packaged Java, reviewed npm and exact Python wheel installation, atomic project venv reuse and capability-scoped loopback preview boundaries.
