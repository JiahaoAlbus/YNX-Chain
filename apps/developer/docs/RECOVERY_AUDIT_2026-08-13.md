...
| Real build/run/test | Approved sandbox runners for C, C++, JavaScript, TypeScript, Python, Go, Java and Solidity plus separately reviewed project tests | `workspace-agent` direct gates cover real C/C++/JS/TS/Python/Go/Java/Solidity build/run and JS/Python/Go/C/C++/JUnit project tests; JUnit is compiled and executed with a SHA-pinned standalone runner and no network; project environment resolves before spawn and live activity remains owner-scoped and redacted | Rust build is skipped where the reviewed host toolchain is absent; Cargo and Solidity-framework project-test adapters plus the revised cloud image still need public gates |
