...
| Debug Adapter Protocol | Authenticated, bounded DAP bridge for C/C++ via LLDB, Python via pinned debugpy and Rust via pinned Ubuntu LLDB 18; Python/Rust require the selected owner/project-bound LXD lease, lock it for the session, keep external networking disabled and remove the temporary debug workspace on exit | Mock routing gates cover Python and Rust container paths; real debugpy 1.8.21 reaches a source breakpoint, stack frame, local scope and `value = 7`; the protected live-container gate requires equivalent Python evidence plus a compiled Rust breakpoint at line 3 and `value = 9` | Execute and capture the protected public Python/Rust live-container DAP evidence; LLDB-DAP is unavailable on this macOS verifier, and Node and Go adapter acceptance remains open |
...
- Debug: 3 passed, 1 skipped (real debugpy breakpoint/stack/locals and mock Python/Rust container routing passed; reviewed LLDB-DAP is absent on this verifier, while the protected Rust live gate is pending).
- Git: 1 passed.
