...
| Real build/run/test | Approved sandbox runners for C, C++, JavaScript, TypeScript, Python, Go, Java and Solidity plus separately reviewed project tests | `workspace-agent` direct gates cover real C/C++/JS/TS/Python/Go/Java/Solidity build/run and JS/Python/Go/C/C++/Cargo/JUnit project tests; Cargo is dependency-free, offline and locked; JUnit is compiled and executed with a SHA-pinned standalone runner; project environment resolves before spawn and live activity remains owner-scoped and redacted | Rust gates skip where the reviewed host toolchain is absent; Solidity-framework project tests plus the revised cloud image still need public gates |
