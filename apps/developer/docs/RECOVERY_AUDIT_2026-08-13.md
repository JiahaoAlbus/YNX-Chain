...
| Web/PWA | React/TypeScript/Monaco workbench behind same-origin gateway with real persisted editor settings and project-wide literal replace | Settings control Monaco font/minimap/wrap and revision-safe auto/manual save; replace has exact preview/confirm/dirty/recovery policy; frontend and protocol TypeScript checks plus gateway 2/2 passed | Re-run browser E2E/current accessibility against the current release service |
