...
| Debug Adapter Protocol | Authenticated, bounded DAP bridge for C/C++ via LLDB, Python via pinned debugpy, Rust via pinned Ubuntu LLDB 18 and Go via pinned Delve 1.25.2; Python/Rust/Go require the selected owner/project-bound LXD lease, lock it for the session, keep external networking disabled and remove the temporary debug workspace on exit | Mock routing gates cover Python, Rust and Go container paths; real debugpy 1.8.21 reaches a source breakpoint, stack frame, local scope and `value = 7`; the protected live-container gate requires equivalent Python evidence, a compiled Rust breakpoint at line 3 with `value = 9`, and a Go breakpoint at line 5 with `value = 11` | Execute and capture the protected public Python/Rust/Go live-container DAP evidence; LLDB-DAP and Delve are unavailable on this macOS verifier, and Node adapter acceptance remains open |
...
- Debug: local suites cover real debugpy breakpoint/stack/locals and mock Python/Rust/Go container routing; reviewed LLDB-DAP and Delve are absent on this verifier, while the protected Rust/Go live gates remain pending.
- Git: 1 passed.
