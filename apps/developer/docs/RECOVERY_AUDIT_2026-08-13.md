...
| AI Software Engineer | Persisted Planner/context/Coder/Reviewer/Tester flow, exact-path file create/edit, digest-bound deployment review and visible graded permission matrix | `agent-orchestrator` 4/4 and AI Web boundary 4/4 passed; create collisions and approval replay are rejected, grants/denials are hash-chained, model usage is retained when reported and cost remains explicitly unreported | Delete, package, Agent Git, browser-network, secret-reference and actual deployment adapters plus accepted public provider/cost evidence remain open |
