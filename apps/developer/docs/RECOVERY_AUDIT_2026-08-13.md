...
- Workspace agent: 14 passed, 1 skipped (Rust toolchain absent); project environment injection, redacted owner-scoped queued/running task activity, pre-spawn queued cancellation and process-group cancellation with cross-owner rejection join real packaged Java compile/run and reviewed JS/Python/Go/C++ project tests through the network-disabled local sandbox.
