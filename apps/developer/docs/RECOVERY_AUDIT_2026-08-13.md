...
| Real build/run/test | Approved sandbox runners for C++, JavaScript, TypeScript, Python, Go, Java and Solidity plus separately reviewed project tests | `workspace-agent` 13 passed/1 skipped; real C++/JS/TS/Python/Go/Java/Solidity build/run and JS/Python/Go/C++ project-test cases passed with wrong-approval/missing-test rejection; project environment is resolved before spawn and the live owner-scoped activity record redacts source/commands/values | Rust build skipped because the reviewed host toolchain is absent; Cargo, JUnit and Solidity-framework project-test adapters, task cancellation and the revised Java cloud image still need direct gates |
...
- Workspace agent: 13 passed, 1 skipped (Rust toolchain absent); project environment injection and redacted owner-scoped task activity join real packaged Java compile/run and reviewed JS/Python/Go/C++ project tests through the network-disabled local sandbox.
