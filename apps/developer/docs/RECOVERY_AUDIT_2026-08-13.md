...
| AI Software Engineer | Persisted Planner/context/Coder/Reviewer/Tester flow plus a digest-bound, non-executable deployment-review artifact | `agent-orchestrator` 4/4 and deployment-review Web boundary 2/2 passed; model usage is hash-chained when reported and cost remains explicitly unreported | Actual deployment/signing/publishing, the full tool permission matrix and accepted public provider/cost evidence remain open |
