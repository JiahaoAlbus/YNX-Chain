...
| AI Software Engineer | Persisted Planner/context/Coder/Reviewer/Tester flow, exact-path create/edit/recoverable-delete, digest-bound deployment review and visible graded permission matrix | `agent-orchestrator` 4/4 and AI Web boundary 4/4 passed; create collisions, stale/replayed approvals and unapproved deletes are rejected; deleted content restores only under a new one-time grant and revision | Irreversible delete, package, Agent Git, browser-network, secret-reference and actual deployment adapters plus accepted public provider/cost evidence remain open |
