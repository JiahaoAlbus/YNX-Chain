...
| Real build/run/test | Approved sandbox runners for C++, JavaScript, TypeScript, Python, Go, Java and Solidity plus separately reviewed project tests | `workspace-agent` 11 passed/1 skipped; real C++/JS/TS/Python/Go/Java/Solidity build/run and JS/Python/Go/C++ project-test cases passed with wrong-approval/missing-test rejection | Rust build skipped because the reviewed host toolchain is absent; Cargo, JUnit and Solidity-framework project-test adapters and the revised Java cloud image still need direct gates |
| Dependency installation | Exact-version npm reconciliation in an owner/project LXD store with one-time review | `runtime-profile-service` rejects ranges and cross-owner access, disables lifecycle scripts, bounds the store, always removes temporary egress and links persisted modules into later tasks; React UI returns manifest/lockfile through workspace limits | Configure and firewall the production package-egress network, then capture public install/restart/import evidence; pip, Cargo, Go, Maven/Gradle and Solidity-framework adapters remain open |
| LSP, Outline and Problems | Bounded language-service bridge plus content-bound navigable document symbols and Problems records | 8 passed: C++, Python, Solidity and TypeScript completion/definition/diagnostics; real TypeScript `documentSymbol` returns `add`; Outline preserves hierarchy and location while Problems renders severity/location/source/code; both reject stale-content entries | `gopls` and `rust-analyzer` skipped because reviewed binaries are absent; Java LSP, unopened-file whole-project analysis and formatting/rename/reference acceptance remain open |
| Terminal and long processes | Authenticated PTY/WebSocket broker with cloud and SSH profiles; detached sessions retain a bounded 256 KiB output tail and can be reattached only by the same owner/project/runtime before idle or hard expiry | `terminal-service` 6/6 covers a real PTY idle timeout, output after detach, owner-bound reconnect/replay, active-client replacement, cross-owner 403, explicit stop and revision-safe workspace synchronization | Record the same stop/timeout/reconnect sequence against the accepted public release; detached sessions are intentionally server-memory-local and do not survive a gateway restart |
...
| Container/remote workspace | Owner-bound LXD leases and reviewed-host-key SSH | `runtime-profile-service` 9/9 passed, including packaged Java command and reviewed npm install boundaries; text workspaces have separate server-local revision history | Build and deploy the revised eight-language image; Kubernetes lifecycle, sleep/wake, container-volume/object-store backup and restore, and public resource-limit evidence remain open |
...
| Web/PWA | React/TypeScript/Monaco workbench behind same-origin gateway with settings, project-wide literal replace and bounded text-project transfer | Settings control Monaco font/minimap/wrap and revision-safe save; replace has preview/confirm/recovery; directory/versioned-JSON import and JSON export enforce safe UTF-8 256-file/2 MiB limits; frontend/protocol checks and gateway 2/2 passed | Re-run browser E2E/current accessibility against the current release service; binary/symlink/permission-preserving archive import is not claimed |
...
- Terminal: 6 passed, including real PTY timeout enforcement and a long-running shell process that continued after WebSocket detach, replayed output on an owner-bound reconnect, replaced a stale client, rejected a second owner, stopped explicitly and synchronized the workspace.
...
- Runtime profiles: 9 passed, including packaged Java and reviewed npm install boundaries.
