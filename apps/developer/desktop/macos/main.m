...
- (void)userContentController:(WKUserContentController *)controller didReceiveScriptMessage:(WKScriptMessage *)message {
    ...
    if ([message.name isEqualToString:@"wallet"]) {
        ...
        BOOL exact=parts && [parts.scheme isEqualToString:@"ynxwallet"] && [parts.host isEqualToString:@"authorize"] && parts.path.length==0 && !parts.fragment.length && items.count==1 && [items.firstObject.name isEqualToString:@"request"] && items.firstObject.value.length>32;
        BOOL opened=exact && [NSWorkspace.sharedWorkspace openURL:parts.URL]; NSString *event=YNXJSON(@{@"id":job,@"ok":@(opened),@"message":opened?@"YNX Wallet review opened.":@"YNX Wallet is not installed or rejected the exact authorization route."});
...
- (void)application:(NSApplication *)application openURLs:(NSArray<NSURL *> *)urls { for(NSURL *url in urls){NSURLComponents *parts=[NSURLComponents componentsWithURL:url resolvingAgainstBaseURL:NO];NSArray<NSURLQueryItem *> *items=parts.queryItems;BOOL exact=[parts.scheme isEqualToString:@"ynxdeveloper"]&&[parts.host isEqualToString:@"wallet-auth"]&&[parts.path isEqualToString:@"/callback"]&&!parts.fragment.length&&items.count==1&&[items.firstObject.name isEqualToString:@"response"]&&items.firstObject.value.length>32;if(!exact)continue;NSString *encoded=YNXJSON(url.absoluteString);[_webView evaluateJavaScript:[NSString stringWithFormat:@"window.dispatchEvent(new CustomEvent('ynx-wallet-callback',{detail:%@}))",encoded?:@"null"] completionHandler:nil];} }
