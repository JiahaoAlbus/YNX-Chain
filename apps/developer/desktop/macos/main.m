...
static NSDictionary *YNXWalletAvailability(void) {
    NSURL *probe=[NSURL URLWithString:@"ynxwallet://authorize?request=probe"];
    BOOL available=probe && [NSWorkspace.sharedWorkspace URLForApplicationToOpenURL:probe]!=nil;
    return @{ @"installed":@(available), @"schemeRegistered":@(available) };
}
