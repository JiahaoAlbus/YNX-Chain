...
static NSDictionary *YNXWalletAvailability(void) {
    // Availability lookup never opens Wallet and therefore never impersonates an
    // authorization request. The actual opener below accepts only a populated
    // canonical request generated by the accepted Product Session root factory.
    NSURLComponents *components=[NSURLComponents new]; components.scheme=@"ynxwallet"; components.host=@"authorize";
    components.queryItems=@[[NSURLQueryItem queryItemWithName:@"request" value:@"availability_probe_not_opened"]];
    NSURL *probe=components.URL;
