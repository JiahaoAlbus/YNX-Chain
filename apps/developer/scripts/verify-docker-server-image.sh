...
node -e 'const value=JSON.parse(process.argv[1]);if(value.ok!==true||value.service!=="ynx-code-gateway"||value.sandboxReady!==true)throw new Error(`invalid Docker health: ${JSON.stringify(value)}`)' "$health"
compile=$(curl --fail --silent --show-error -X POST "http://127.0.0.1:$port/runtime/tasks" -H 'content-type: application/json' --data '{"protocolVersion":"ynx-code/v1","task":"build-run-active","approval":"execute-once","activePath":"hello.cpp","projectId":"docker-image-cpp","files":{"hello.cpp":"#include <iostream>\nint main(){std::cout << \"YNX-DOCKER-CPP\";return 0;}"}}')
