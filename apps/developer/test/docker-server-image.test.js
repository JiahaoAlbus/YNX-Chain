...
test("Docker server image is source-bound and requires an unprivileged real compile proof", async () => {
  ...
  for (const value of ["--read-only", "--cap-drop=ALL", "no-new-privileges", "seccomp=unconfined", "uid=10001,gid=10001,mode=0700", "YNX_CODE_WORKSPACE_SESSION_KEY", "/runtime/health", "--cookie-jar", "sandboxReady", "YNX-DOCKER-CPP", "realCppCompile", "outerContainerSeccomp", "registryPublished:false"]) assert.match(verifier, new RegExp(value));
