...
test("localized UI and native desktop sources preserve language, permission and release boundaries", async () => {
  ...
  assert.match(mac,/New File/); assert.match(mac,/ynxDesktopWallet/); assert.match(mac,/open-authorization/); assert.match(mac,/developer-deploy/); assert.match(mac,/ynx-wallet-callback/); assert.match(mac,/ynx-deployment-callback/); assert.match(plist,/com\.ynxweb4\.developer\.testnetpreview\.wallet-auth/); assert.match(plist,/>ynxdeveloper</); assert.match(windows,/owner-signed manifest/); assert.doesNotMatch(mac+windows,/production release is signed/i);
...
test("Windows proof requires a real Windows build, MSIX installation and cold launch",async()=>{
  ...
  assert.match(verify,/--self-test/);assert.match(verify,/Invoke-YNXWindowsUIAcceptance/);assert.match(verify,/realCppCompile/);assert.match(verify,/runtime\/tasks/);assert.match(verify,/sameProfileProjectRestored/);assert.match(verify,/YNX_DEVELOPER_EXPECTED_SOURCE_COMMIT/);
  ...
  assert.match(installerVerify,/Add-AppxPackage/);assert.match(installerVerify,/Remove-AppxPackage/);assert.match(installerVerify,/startMenuActivation = \$false/);assert.match(installerVerify,/installedExecutable/);assert.match(installerVerify,/test-self-signed-not-production/);assert.match(installerVerify,/sameProfileProjectRestored/);
