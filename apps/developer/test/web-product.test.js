...
test("Windows proof requires a real Windows build, MSIX installation and cold launch",async()=>{
  ...
  assert.match(packageScript,/MakeAppx\.exe/);assert.match(packageScript,/Windows Kits\\10\\bin/);assert.match(packageScript,/\.msix/);assert.match(packageScript,/test-self-signed-not-production/);assert.match(packageScript,/New-SelfSignedCertificate/);
  ...
  assert.match(installerVerify,/Add-AppxPackage/);assert.match(installerVerify,/Remove-AppxPackage/);assert.match(installerVerify,/explorer\.exe/);assert.match(installerVerify,/test-self-signed-not-production/);assert.match(installerVerify,/second launch/);
