export type ProjectState = { id:string; name:string; revision:number; files:Record<string,string>; folders:string[]; open:string[]; active:string };
const KEY="ynx-code-project-v1";
const initial:ProjectState={id:crypto.randomUUID(),name:"YNX C++ Starter",revision:1,open:["src/main.cpp"],active:"src/main.cpp",folders:["src"],files:{"src/main.cpp":"#include <iostream>\n\nint main() {\n  std::cout << \"Hello from YNX Code\" << std::endl;\n  return 0;\n}\n","README.md":"# YNX C++ Starter\n\nRun the active C++ file through an isolated YNX Code workspace runtime.\n"}};

export function loadProject():ProjectState { try { const value=JSON.parse(localStorage.getItem(KEY)||""); if(value&&value.files&&value.active)return {...value,folders:Array.isArray(value.folders)?value.folders:foldersFromFiles(Object.keys(value.files))}; } catch {} return initial; }
