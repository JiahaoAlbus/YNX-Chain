...
const workspaceStore=createWorkspaceStore({filename:join(stateDir,"workspaces.sqlite")}),runtime=createWorkspaceRuntime({workspaceStore,languageRequest:runCppLanguageRequest});
const server=createServer(createGateway({staticRoot,runtime}));
const terminalService=createTerminalService({workspaceStore,ownerForRequest:request=>runtime.ownerForRequest(request)});
server.on("upgrade",terminalService.handleUpgrade);
server.listen(port,host,()=>console.log(`YNX Code Gateway http://${host}:${port}`));
for(const signal of ["SIGINT","SIGTERM"])process.once(signal,()=>server.close(async()=>{await terminalService.close();workspaceStore.close();process.exit(0)}));
