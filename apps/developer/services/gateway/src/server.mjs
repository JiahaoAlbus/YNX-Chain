...
const debugService=createDebugService({workspaceStore,ownerForRequest:request=>runtime.ownerForRequest(request)});
server.on("upgrade",(request,socket,head)=>{if(terminalService.handleUpgrade(request,socket,head)||debugService.handleUpgrade(request,socket,head))return;socket.write("HTTP/1.1 404 Not Found\r\nConnection: close\r\nContent-Length: 0\r\n\r\n");socket.destroy()});
server.listen(port,host,()=>console.log(`YNX Code Gateway http://${host}:${port}`));
for(const signal of ["SIGINT","SIGTERM"])process.once(signal,()=>server.close(async()=>{await terminalService.close();await debugService.close();workspaceStore.close();process.exit(0)}));
