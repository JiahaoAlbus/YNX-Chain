...
test("agent plan, context, coder, reviewer and one-time write are persisted and hash chained", async (t) => {
  ...
  const base=`http://127.0.0.1:${server.address().port}`, key="request-only-secret-never-store", call=(path,body)=>fetch(`${base}${path}`,{method:"POST",headers:{"x-owner":"owner-a","content-type":"application/json"},body:JSON.stringify({protocolVersion:"ynx-code-agent/v1",approvalId:randomUUID(),ownerId:"forged-quota-owner",...body})});
  ...
  assert.ok(seen.every(input => input.ownerId === "owner-a"), "all model roles and retries use the authenticated owner, ignoring body ownerId");
});
