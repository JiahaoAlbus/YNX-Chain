...
async function fixture(t){const root=await mkdtemp(join(tmpdir(),"ynx-code-gateway-"));await mkdir(join(root,"assets"));await writeFile(join(root,"index.html"),"<!doctype html><title>YNX Code</title>");await writeFile(join(root,"assets","app-abcdef.js"),"globalThis.YNX=true");const runtime=createWorkspaceRuntime({root:join(root,"runtime"),sessionKey:Buffer.alloc(32,5)}),server=createServer(createGateway({staticRoot:root,runtime,version:"test"}));await new Promise(resolve=>server.listen(0,"127.0.0.1",resolve));t.after(async()=>{await new Promise(resolve=>server.close(resolve));await rm(root,{recursive:true,force:true})});return`http://127.0.0.1:${server.address().port}`}
...
test("gateway rejects traversal and routes runtime health through the same origin",async t=>{const url=await fixture(t),traversal=await fetch(`${url}/%5c..%5csecret`),health=await fetch(`${url}/runtime/health`),gateway=await fetch(`${url}/healthz`);assert.equal(traversal.status,400);assert.equal((await health.json()).protocolVersion,"ynx-code/v1");assert.equal((await gateway.json()).version,"test")});
