...
SOURCE='62f6109c163fcf98ca7a3cc58e40bd8cb91467c2'
OLD='d736a87831fe83249ce7adfeae7a21efbc283b3e'
RUN='exchange-schema10-hotfix-62f6109c163f-20260912T114000Z'
ARCHIVE=Path('/tmp/'+RUN+'.tar.gz')
ARCHIVE_SHA='f809aa3df4b23121d15bc34a7845581b1f9aa5001208499d0e9f898db9d4d119'
...
HASHES={
 '/etc/systemd/system/ynx-exchange.service.d/zz-exchange-schema10-d736a87831fe-20260912T113000Z.conf':'831ec6a3971561c93698e3ac84835dd24ffe30ed945c1cd8939d9eff2f762874',
 '/opt/ynx/exchange-releases/exchange-schema10-d736a87831fe-20260912T113000Z/ynx-exchanged':'01b9398202aec44dc1f9872378728720a6158216f264a07e738f17a8781f3918',str(ENV):'409983db6acf881430e64f8a2a767ab6f63ac625f7c0e5413a41c02b88153bc0',
...
def verify_baseline():
 ...
 original_links()
 s=service();assert s['ActiveState']=='active' and s['SubState']=='running' and s['MainPID']=='1696915' and s['NRestarts']=='0' and s['User']==s['Group']=='ynx'
 assert service('ynx-quant-exchange.service')['MainPID']=='2275763'
 assert s['WorkingDirectory']=='/opt/ynx/exchange-releases/exchange-schema10-d736a87831fe-20260912T113000Z' and process_executable(s['MainPID'])==s['WorkingDirectory']+'/ynx-exchanged'
 ...
 for base in ['http://127.0.0.1:18446','https://exchange.ynxweb4.com']:
  http(base+'/api/version',OLD);http(base+'/api/health',digest='f7b2947c118b79fb9c2d6f6609a73e8d8bc6c9eca2b47d37ac6789af3c89557b');http(base+'/',digest='6b0dc758aa3ff2b9414d2c22e398c86d507d0cb5f1e6318a4735d6c0456503bb')
 return {'service':s,'state':r,'config':[regular(p,d) for p,d in HASHES.items()],'quant':service('ynx-quant-exchange.service')}
def archive_files(p):
 assert regular(p,ARCHIVE_SHA)['bytes']==4232069
 ...
 binary=data['ynx-exchanged'];assert binary[:6]==b'\x7fELF\x02\x01' and binary[18:20]==b'\x3e\x00' and len(binary)==9568440 and sha(binary)=='660088fa81755cd4a3bc8af307eedcb27ee25381cf1454539a927a0afd59ad00'
...
def deploy():
 ...
 before=verify_baseline();absent(DROPIN);directory(DROPIN.parent);parent=receipt(DROPIN.parent);old_names=set(p.name for p in DROPIN.parent.iterdir());assert old_names=={'public.conf','quant-product-session.conf','zz-exchange-schema10-d736a87831fe-20260912T113000Z.conf'}
 ...
 try:
  ...
  if stopped:
   ...
   for p,d in HASHES.items():regular(p,d)
   for base in ['http://127.0.0.1:18446','https://exchange.ynxweb4.com']:http(base+'/api/version',OLD);http(base+'/api/health',digest='f7b2947c118b79fb9c2d6f6609a73e8d8bc6c9eca2b47d37ac6789af3c89557b');http(base+'/',digest='6b0dc758aa3ff2b9414d2c22e398c86d507d0cb5f1e6318a4735d6c0456503bb')
