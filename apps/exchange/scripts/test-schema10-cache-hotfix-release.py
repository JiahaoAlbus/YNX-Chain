...
spec=importlib.util.spec_from_file_location('release',Path(__file__).with_name('release-schema10-cache-hotfix-62f6109c.py'))
module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)

class ReleaseBoundaries(unittest.TestCase):
 def test_frozen_real_archive_inventory(self):
  archive=Path('/tmp/ynx-exchange-schema10-runtime-20260912.S33fLQ/ynx-exchange-schema10-62f6109c163f-linux-amd64.tar.gz').resolve()
  data,manifest=module.archive_files(archive)
  self.assertEqual(manifest['stateSchema'],10);self.assertEqual(len(data),7)
  self.assertEqual(module.sha(data['ynx-exchanged']),'660088fa81755cd4a3bc8af307eedcb27ee25381cf1454539a927a0afd59ad00')
  ...
  try:
   for failed in [False,True]:
    with tempfile.TemporaryDirectory() as temp:
     ...
     for name in ['public.conf','quant-product-session.conf','zz-exchange-schema10-d736a87831fe-20260912T113000Z.conf']:(control/name).write_bytes(name.encode())
     ...
     if failed:
      ...
      self.assertEqual(actions,['stop','start','stop','start']);self.assertFalse(module.DROPIN.exists());self.assertEqual(set(p.name for p in control.iterdir()),{'public.conf','quant-product-session.conf','zz-exchange-schema10-d736a87831fe-20260912T113000Z.conf'})
     else:
