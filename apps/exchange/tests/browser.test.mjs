...
test('explicit installed Wallet fallback remains in Exchange with truthful downloads',async()=>{const page=await browser.newPage({viewport:{width:1024,height:800}});await page.goto(base,{waitUntil:'domcontentloaded'});await page.getByRole('button',{name:'Connect YNX Wallet'}).click();assert.equal(await page.locator('#wallet-dialog').evaluate(el=>el.open),true);await page.locator('#connect-ynx-wallet').click();await page.getByText('Injected YNX Wallet was not available.',{exact:false}).waitFor();assert.equal(await page.url(),base+'/');assert.equal(await page.locator('#wallet-fallback a',{hasText:'Download YNX Wallet'}).count(),1);assert.equal(await page.locator('#wallet-fallback a',{hasText:'Use MetaMask'}).count(),1);await page.screenshot({path:path.join(evidence,'wallet-provider-fallback.png'),fullPage:true});await page.close()});
