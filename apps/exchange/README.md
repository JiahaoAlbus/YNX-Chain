...
`YNX_EXCHANGE_DATABASE_URL` is required by the Exchange server command. The service creates its own `ynx_exchange_state` table, refreshes the durable snapshot at the API boundary, and saves mutations with revision compare-and-swap. A concurrent update returns a conflict instead of silently overwriting an order or idempotency record. The command also creates `ynx_exchange_admission_windows`: a PostgreSQL-backed, atomic fixed-window admission limit whose client keys are SHA-256 digests, not raw addresses. A database admission error fails closed with HTTP 503; the process does not silently fall back to an instance-local production limiter.

The database endpoint and credentials are runtime-only configuration and must never be placed in Web assets, release evidence, or logs. See [distributed-admission handoff](EXCHANGE_DISTRIBUTED_ADMISSION_LIMITER_20260831.md) for the source-only checkpoint and its remaining deployment evidence gates.
