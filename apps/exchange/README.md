...
The native client uses the exact Task 1 Wallet Auth v1 protocol and a P-256 product-device key stored in platform secure storage. The exact `ynx-exchange-v1` client, callback, bundle and five least-privilege scopes are approved for public-Testnet Wallet sessions. Protected order, cancel and withdrawal actions still fail closed until the canonical action-verification route is deployed and attested. The Web companion remains a public read-only terminal and never accepts pasted or browser-stored bearer tokens.
