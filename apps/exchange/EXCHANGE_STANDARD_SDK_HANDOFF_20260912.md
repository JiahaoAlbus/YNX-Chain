...
The main agent supplied an exact private ESM; its registry and source `packages/wallet-auth/integration/browser-product-session-v2.md` were read for the next boundary (private ESM is not consumed in this checkpoint). Registry Exchange is `ynx-exchange-v1` / `com.ynxweb4.exchange` / `https://exchange.ynxweb4.com`, with read/trade scopes. The documented `createBrowserProductSessionClient` uses IndexedDB non-extractable WebCrypto, persists pending before Wallet approval, handles exact callback URL, and returns a fresh `X-YNX-Product-Session-Proof-V2` per API attempt.
