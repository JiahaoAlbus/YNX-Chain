...
@MainActor final class ShopModel:ObservableObject {
    ...
    func add(_ product:Product,_ variant:Variant){if let index=cart.firstIndex(where:{$0.variantID==variant.variantID}){cart[index].quantity+=1}else{cart.append(.init(productID:product.productID,variantID:variant.variantID,quantity:1))}}
    ...
    func checkout(recipient:String,address:String,city:String,country:String){guard signedIn else{signIn();return};let id=UUID().uuidString.lowercased();let items=cart.map{["ProductID":$0.productID,"VariantID":$0.variantID,"Quantity":$0.quantity] as [String:Any]};guard let store=products.first(where:{$0.productID==cart.first?.productID})?.storeID else{state="cart_empty";return};let json:[String:Any]=["StoreID":store,"Items":items,"Address":["Recipient":recipient,"Line1":address,"City":city,"Country":country],"IdempotencyKey":id];Task{do{_ = try await api.raw("orders",json:json);cart=[];await loadOrders()}catch{do{try queue.append(method:"POST",path:"orders",body:try CanonicalJSON.data(json));state="queued_offline"}catch{state="security_error"}}}}
    func transition(_ order:Order,_ action:String,reason:String){let json:[String:Any]=["Action":action,"Reason":reason,"Explanation":reason,"Body":reason,"Rating":action=="reviewed" ? 5:0,"IdempotencyKey":UUID().uuidString.lowercased()];Task{do{_ = try await api.raw("orders/\(order.orderID)/transition",json:json);await loadOrders()}catch{do{try queue.append(method:"POST",path:"orders/\(order.orderID)/transition",body:try CanonicalJSON.data(json));state="queued_offline"}catch{state="security_error"}}}}
    func pay(_ order:Order){Task{do{let r=try await api.raw("orders/\(order.orderID)/pay-handoff",json:["IdempotencyKey":UUID().uuidString.lowercased()]);if let text=r["deepLink"] as? String,let url=URL(string:text){await UIApplication.shared.open(url)}else{state="unavailable"}}catch{state="unavailable"}}}
