...
public final class MainActivity extends Activity {
    ...
    private void showAccount(){
        ...
        if(hasSession()){recipient=input(getString(R.string.recipient));address=input(getString(R.string.address));city=input(getString(R.string.city));country=input(getString(R.string.country));for(EditText f:List.of(recipient,address,city,country))content.addView(f);Button save=primary(getString(R.string.save_profile));save.setOnClickListener(v->saveProfile());content.addView(save);content.addView(text(getString(R.string.privacy_data_title),18,true));content.addView(text(getString(R.string.privacy_data_description),13,false));Button export=button(getString(R.string.privacy_export_button));export.setOnClickListener(v->exportPrivacyData());content.addView(export);Button delete=button(getString(R.string.privacy_delete_button));delete.setTextColor(Color.rgb(180,35,24));delete.setOnClickListener(v->confirmPrivacyDeletion());content.addView(delete);}
    }
    ...
    private void exportPrivacyData(){api.request("GET","/privacy/export",null,(value,error)->runOnUiThread(()->{if(error!=null){status.setText(getString(R.string.privacy_export_unavailable));return;}pendingPrivacyExport=value.toString();Intent intent=new Intent(Intent.ACTION_CREATE_DOCUMENT).addCategory(Intent.CATEGORY_OPENABLE).setType("application/json").putExtra(Intent.EXTRA_TITLE,"ynx-shop-data-"+LocalDate.now()+".json");startActivityForResult(intent,CREATE_PRIVACY_EXPORT);}));}
    private void confirmPrivacyDeletion(){EditText confirmation=input("DELETE_MY_SHOP_DATA");new AlertDialog.Builder(this).setTitle(getString(R.string.privacy_delete_button)).setMessage(getString(R.string.privacy_delete_warning)).setView(confirmation).setNegativeButton(getString(R.string.cancel),null).setPositiveButton(getString(R.string.privacy_delete_action),(dialog,which)->{if(!"DELETE_MY_SHOP_DATA".equals(confirmation.getText().toString())){status.setText(getString(R.string.privacy_exact_required));return;}api.request("POST","/privacy/delete",json("confirmation","DELETE_MY_SHOP_DATA"),(value,error)->runOnUiThread(()->{if(error!=null){status.setText(getString(R.string.privacy_delete_failed));return;}try{secure.put("cart","");cart=new JSONArray();cartBadge.setText("0");status.setText(getString(R.string.privacy_deleted)+" "+value.optString("receiptId"));showAccount();}catch(Exception e){status.setText(getString(R.string.privacy_delete_failed));}}));}).show();}
