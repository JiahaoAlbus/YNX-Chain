...
function parseSignedApplicationAction(input) {
  ...
  const fields2 = dataFields(value, SIGNED_FIELDS, "Signed application action");
  const unsigned = unsignedAction(fields2);
  const signature = exactString(fields2.signature, "signature", /^30(?:[0-9a-f]{2}){7,71}$/);
  ...
  let valid = false;
  try {
    valid = evmAddressFromYNX(walletIdentityFromPublicKey(unsigned.publicKey)) === unsigned.signer && secp256k1.verify(hexToBytes(signature), sha256(utf8ToBytes(signJSON(unsigned))), hexToBytes(unsigned.publicKey), { prehash: false, format: "der", lowS: true });
  } catch {
    valid = false;
  }
...
function verifySignedApplicationAction(input, expected) {
  ...
  if (transaction.signer !== account || transaction.action !== action || transaction.nonce !== nonce || JSON.stringify(transaction.payload) !== JSON.stringify(payload)) {
...
function applicationActionPayloadHash(action, payload) {
...
function businessPayload(action, input) {
  const fields2 = dataFields(input, PAYLOAD_FIELDS[action], "DEX business payload");
  const result = {};
  for (const key of PAYLOAD_FIELDS[action]) {
    const value = fields2[key];
...
function dataFields(value, expected, label) {
  ...
  for (const key of expected) {
    ...
    snapshot2[key] = descriptor.value;
  }
  return snapshot2;
}
...
function exactString(value, label, pattern2) {
  if (typeof value !== "string" || value.trim() !== value || !pattern2.test(value)) throw invalid(`${label} is invalid`);
  return value;
}
...
var REQUEST_FIELDS = ["version", "chainId", "productId", "platform", "applicationId", "origin", "callback", "account", "action", "payload", "nonce", "requestId", "state", "issuedAt", "expiresAt"];
...
function createApplicationActionRequest(registry, input, at = /* @__PURE__ */ new Date()) {
  ...
  return parseApplicationActionRequest(registry, {
    ...
    expiresAt: new Date(time.getTime() + MAX_LIFETIME).toISOString()
  }, time);
}
function parseApplicationActionRequest(registry, input, at = /* @__PURE__ */ new Date()) {
  ...
  if (request.productId !== "dex" || request.version !== "1" || request.chainId !== "ynx_6423-1" || !binding.callback || ["applicationId", "origin", "callback"].some((key) => request[key] !== binding[key])) {
...
function parseApplicationActionWalletURL(registry, url, at = /* @__PURE__ */ new Date()) {
...
function parseApplicationActionReturnURL(registry, url, input, at = /* @__PURE__ */ new Date()) {
  const request = parseApplicationActionRequest(registry, input, at);
...
function decodeRoute(value, target, key, limit) {
  if (typeof value !== "string" || value.length > limit * 2 || !value.startsWith(`${target}?${key}=`)) fail2("INVALID_APPLICATION_ACTION_ROUTE", "Application action route is not registered");
