...
describe('DEX Wallet Product Session v2 adapter',()=>{
  ...
  it('fails closed when a Product Session capability is absent',async()=>{await expect(beginWalletAuthorization()).rejects.toMatchObject({code:'PRODUCT_SESSION_UNAVAILABLE'});});
  it('accepts only a capability surface without endpoint, callback, origin or session injection',()=>{expect(typeof configureDexPrivateConnection).toBe('function');expect(dexProductSessionGatewayOrigin()).toBe('https://wallet-auth.ynxweb4.com');expect(new WalletRequestError('x','x').code).toBe('x');});
});
