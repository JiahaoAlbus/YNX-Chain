...
declare module '*standard-wallet-connect-state.js' { export const STANDARD_WALLET_CONNECT_STATUS:Readonly<{CONNECTED:'connected'}>; export const STANDARD_WALLET_RPC_PROBE_TRANSPORT:'accepted-cors-safe'; export type StandardWalletConnectState=Readonly<{status:string;account:string|null;chainId:string|null;providerKind:string|null;chooserOpen:boolean;pendingIntent:string|null}>; export function createStandardWalletConnectState():StandardWalletConnectState; export function reduceStandardWalletConnectState(state:StandardWalletConnectState,event:Record<string,unknown>):StandardWalletConnectState; }
declare module '@ynx-chain/wallet-auth/src/crypto.js' { export function evmAddressFromYNX(account:string):string; export function ynxAddressFromEVM(account:string):string; }
