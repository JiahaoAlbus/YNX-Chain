...
export function PortfolioPanel({account,locale,onConnect}:{account:string;locale:Locale;onConnect:()=>void}) {
  ...
  return <section className="portfolio-panel" aria-label={t.title}>
    ...
    {!account?<><p>{t.connectHint}</p><button className="primary" onClick={onConnect}>{t.connect}</button></>:<>
      ...
      {error?<p role="alert">{error==='UNAVAILABLE'?t.unavailable:t.invalid}</p>:!data?<p role="status">{t.loading}</p>:<>
        ...
        <p className="portfolio-provenance">{t.source}: {data.source} · {data.version}<br/>{t.readAt}: <time dateTime={data.readAt}>{new Intl.DateTimeFormat(locale,{dateStyle:'medium',timeStyle:'medium'}).format(new Date(data.readAt))}</time></p>
      </>}
    </>}
