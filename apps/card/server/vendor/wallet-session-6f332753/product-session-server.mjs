...
function parseProduct(input) {
  ...
  const scopes3 = stringList(input.scopes, "scopes", 1, 8, (value) => pattern(value, "scope", /^[a-z][a-z0-9._:-]{1,63}$/));
  if (scopes3.some((scope) => scope.includes("*"))) fail("INVALID_ROUTER_REGISTRY", "Wildcard Product Session scope is forbidden");
...
function parseProductSessionProofV2(input) {
  ...
  if (bytes.length < 68 || bytes.length > 72 || encodeBase64url(bytes) !== signature) fail3("INVALID_DEVICE_PROOF", "Product Session proof signature is invalid");
...
function parseUnsigned(input) {
  ...
  if (value.bundleId !== null && value.bundleId !== value.applicationId || value.packageId !== null && value.packageId !== value.applicationId || value.bundleId !== null && value.packageId !== null) fail3("INVALID_FIELD", "Product Session proof application identity is inconsistent");
  if (value.expiresAt <= value.issuedAt || Date.parse(value.expiresAt) - Date.parse(value.issuedAt) > 6e4) fail3("INVALID_EXPIRY", "Product Session proof lifetime must be at most sixty seconds");
  return value;
}
function url(value, label) {
  ...
  try {
    ...
    fail3("INVALID_FIELD", `${label} is invalid`);
  }
  const canonical = label === "origin" && parsed.protocol === "https:" ? parsed.origin === normalized : parsed.toString() === normalized;
  if (!canonical || ["http:", "file:", "javascript:", "data:"].includes(parsed.protocol)) fail3("INVALID_FIELD", `${label} is unsafe`);
  return normalized;
}
...
function path(value) {
  ...
  if (normalized.includes("//") || normalized.endsWith("/") || normalized.includes("?") || normalized.includes("#")) fail3("INVALID_PATH", "Product Session proof path is non-canonical");
  return normalized;
}
...
var ProductSessionGatewayFetchAdapter = class {
  ...
  constructor(config) {
    ...
    if (typeof config.fetch !== "function" || typeof config.walletInstalled !== "function" || typeof config.schemeRegistered !== "function") fail5("INVALID_GATEWAY", "Product Session Gateway fetch adapter dependencies are invalid");
    if (!Number.isInteger(config.timeoutMs) || config.timeoutMs < 1e3 || config.timeoutMs > 3e4) fail5("INVALID_GATEWAY", "Product Session Gateway timeout must be between one and thirty seconds");
  ...
  async currentTime(input) {
    ...
    try {
      ...
      if (typeof result.serverTime !== "string" || !Number.isFinite(now.getTime()) || now.toISOString() !== result.serverTime) fail5("INVALID_GATEWAY_RESPONSE", "Product Session Gateway time is invalid");
      return now;
    } catch (error) {
      if (error instanceof WalletAuthError && error.code === "NETWORK_UNAVAILABLE") throw error;
      fail5("CLOCK_UNAVAILABLE", "Product Session authority time could not be verified; Retry when Auth is available");
    }
  }
  ...
  async introspect(input) {
    ...
    if (proof.sessionBinding !== input.sessionBinding) fail5("CROSS_PRODUCT_SESSION", "Product Session proof does not match the requested session binding");
  ...
  async #request(requestId, path2, body, proof, method2 = "POST") {
    if (typeof requestId !== "string" || !/^req_[A-Za-z0-9_-]{12,80}$/.test(requestId)) fail5("INVALID_REQUEST_ID", "Product Session Gateway request ID is invalid");
    ...
    try {
      ...
      fail5("NETWORK_UNAVAILABLE", "Product Session Gateway is unavailable; no local response was substituted");
    }
    try {
      if (!response || typeof response.status !== "number" || !response.headers || typeof response.headers.get !== "function" || typeof response.text !== "function") fail5("INVALID_GATEWAY_RESPONSE", "Product Session Gateway response is invalid");
      const contentType = response.headers.get("content-type") ?? "";
      const responseRequestId = response.headers.get("x-request-id");
      const cacheControl = response.headers.get("cache-control") ?? "";
      const contentLength = response.headers.get("content-length");
      if (!/^application\/json(?:;\s*charset=utf-8)?$/i.test(contentType) || responseRequestId !== requestId || !/(^|,)\s*no-store\s*(,|$)/i.test(cacheControl)) fail5("INVALID_GATEWAY_RESPONSE", "Product Session Gateway response headers are invalid");
      if (contentLength !== null && (!/^\d+$/.test(contentLength) || Number(contentLength) > MAX_RESPONSE_BYTES)) fail5("INVALID_GATEWAY_RESPONSE", "Product Session Gateway response exceeds policy");
      let text4;
      try {
        text4 = await response.text();
      } catch {
        fail5("NETWORK_UNAVAILABLE", "Product Session Gateway response stream was interrupted; no local response was substituted");
      }
      if (new TextEncoder().encode(text4).length > MAX_RESPONSE_BYTES) fail5("INVALID_GATEWAY_RESPONSE", "Product Session Gateway response exceeds policy");
      let payload;
      try {
        payload = JSON.parse(text4);
      } catch {
        fail5("INVALID_GATEWAY_RESPONSE", "Product Session Gateway response is not JSON");
      }
      if (canonicalJSON(payload) !== text4) fail5("INVALID_GATEWAY_RESPONSE", "Product Session Gateway response is not canonical JSON");
      if (response.status >= 200 && response.status < 300) {
        exactFields(payload, ["ok", "requestId", "result", "schemaVersion"], "Product Session Gateway success response");
        if (payload.ok !== true || payload.requestId !== requestId || payload.schemaVersion !== PRODUCT_SESSION_GATEWAY_SCHEMA_VERSION) fail5("INVALID_GATEWAY_RESPONSE", "Product Session Gateway success response binding is invalid");
      ...
      if (payload.ok !== false || payload.requestId !== requestId || payload.schemaVersion !== PRODUCT_SESSION_GATEWAY_SCHEMA_VERSION || typeof payload.error.code !== "string" || !/^[A-Z][A-Z0-9_]{2,63}$/.test(payload.error.code) || typeof payload.error.message !== "string" || payload.error.message.length > 300) fail5("INVALID_GATEWAY_RESPONSE", "Product Session Gateway error response binding is invalid");
...
function decodeProductSessionGatewayProofHeaderV2(value) {
  if (typeof value !== "string" || value.length > 16384) fail5("INVALID_PROOF_HEADER", "Product Session proof header is invalid");
  let parsed;
  try {
    parsed = JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(decodeBase64url(value, "Product Session proof header")));
  } catch {
    fail5("INVALID_PROOF_HEADER", "Product Session proof header is invalid");
  }
...
function encodeProductSessionGatewayProofHeaderV2(value) {
  ...
  if (encoded.length > 16384) fail5("INVALID_PROOF_HEADER", "Product Session proof header exceeds policy");
  return encoded;
}
function endpoint(value) {
  if (typeof value !== "string" || value.length > 512) fail5("INVALID_GATEWAY", "Product Session Gateway endpoint is invalid");
  let parsed;
  try {
    parsed = new URL(value);
  } catch {
    fail5("INVALID_GATEWAY", "Product Session Gateway endpoint is invalid");
  }
  if (parsed.protocol !== "https:" || parsed.username || parsed.password || parsed.port || parsed.search || parsed.hash || parsed.pathname !== "/" || value !== parsed.origin) fail5("INVALID_GATEWAY", "Product Session Gateway endpoint must be a canonical HTTPS origin");
  return parsed.origin;
}
function capability(value, label) {
  if (typeof value !== "boolean") fail5("INVALID_GATEWAY", `${label} must return a boolean`);
  return value;
}
...
var ProductSessionServerAuthorizer = class {
  ...
  constructor(config) {
    ...
    if (typeof this.#clock !== "function") fail6("INVALID_SERVER_POLICY", "Server time source is invalid");
  ...
  async authorize(input) {
    ...
    if (input.origin !== this.#binding.origin && !(input.origin === null && (this.#binding.platform !== "web" || input.method === "GET"))) fail6("ORIGIN_MISMATCH", "Request origin does not match the configured product");
    if (typeof input.method !== "string" || !/^(GET|POST|PUT|PATCH|DELETE)$/.test(input.method) || typeof input.path !== "string" || !/^\/[A-Za-z0-9._~!$&'()*+,;=:@\/-]{1,255}$/.test(input.path) || input.path.includes("//")) fail6("INVALID_ROUTE_POLICY", "Server route metadata is invalid");
    const requiredScopes = scopes2(input.requiredScopes, this.#binding.scopes);
    const proof = decodeProductSessionGatewayProofHeaderV2(input.proofHeader);
    if (TUPLE.some((key) => proof[key] !== this.#binding[key])) fail6("CROSS_PRODUCT_SESSION", "Proof does not match the fixed server product identity");
    const body = canonicalJSON({ requiredScopes });
    if (proof.method !== "POST" || proof.path !== INTROSPECT || proof.bodyDigest !== httpBodyDigest(body)) fail6("HTTP_BINDING_MISMATCH", "Proof must bind the exact route-required introspection scopes");
    ...
    if (result.active !== true) fail6("SESSION_INACTIVE", "The authority did not confirm an active session");
    const session = parseProductSession(result.session);
    if (session.chainId !== this.#binding.chainId || session.platform !== this.#binding.platform || TUPLE.some((key) => session[key] !== this.#binding[key]) || SENDER.some((key) => session[key] !== proof[key])) fail6("SESSION_BINDING_MISMATCH", "Authority response changed the requested product or sender");
    const after = instant(this.#clock());
    if (after < before) fail6("CLOCK_UNAVAILABLE", "Server clock moved backwards during authorization");
    active(proof, after);
    active(session, after);
    if (proof.issuedAt < session.issuedAt || proof.expiresAt > session.expiresAt) fail6("SESSION_EXPIRED", "Proof does not fit within the live session lifetime");
    if (session.scopes.some((scope) => !this.#binding.scopes.includes(scope)) || requiredScopes.some((scope) => !session.scopes.includes(scope))) fail6("SCOPE_WIDENING", "The live session does not grant the required scopes");
    return session;
  }
};
function scopes2(value, allowed) {
  if (!Array.isArray(value)) fail6("INVALID_ROUTE_POLICY", "Route scopes must be an array");
  const snapshot = [...value];
  if (!snapshot.length || snapshot.length > 32 || snapshot.some((scope) => typeof scope !== "string" || !allowed.includes(scope)) || new Set(snapshot).size !== snapshot.length || snapshot.some((scope, index) => index > 0 && snapshot[index - 1] >= scope)) fail6("INVALID_ROUTE_POLICY", "Route scopes must be a nonempty canonical subset of the configured product");
  return Object.freeze(snapshot);
}
function active(value, now) {
  if (Date.parse(value.issuedAt) > now || Date.parse(value.expiresAt) <= now) fail6("SESSION_EXPIRED", "Session proof is not currently valid");
}
function instant(value) {
  if (!(value instanceof Date) || !Number.isFinite(value.getTime())) fail6("CLOCK_UNAVAILABLE", "Valid server time is required");
  return value.getTime();
}
function randomRequestId() {
  if (typeof globalThis.crypto?.getRandomValues !== "function") fail6("RANDOM_UNAVAILABLE", "Server cryptographic random source is unavailable");
...
var CHALLENGE_FIELDS = ["id", "applicationId", "owner", "chainId", "purpose", "payloadHash", "nonce", "issuedAt", "expiresAt"];
var DETAILS_FIELDS = ["nickname", "useCase", "limitWei", "riskAccepted", "termsVersion"];
var PROOF_FIELDS2 = ["version", "productId", "challenge", "details", "account", "accountPublicKey", "issuedAt", "expiresAt", "signature"];
...
function parseSignedCardApplicationApproval(input) {
  ...
  if (raw !== null) {
    ...
    try {
      ...
      fail7("INVALID_CARD_APPROVAL", "Card approval JSON is invalid");
    }
  }
  const fields = record(value, PROOF_FIELDS2, "Signed Card approval");
  if (fields.version !== "1" || fields.productId !== "card") fail7("INVALID_CARD_APPROVAL", "Unsupported Card approval version or product");
  const challenge = parseChallenge(fields.challenge), details = parseDetails(fields.details);
  assertDetailsHash(challenge, details);
  const account = text3(fields.account, "account", /^ynx1[023456789acdefghjklmnpqrstuvwxyz]{38}$/);
  evmAddressFromYNX(account);
  const accountPublicKey = text3(fields.accountPublicKey, "accountPublicKey", /^(02|03)[0-9a-f]{64}$/);
  const issuedAt = timestamp(fields.issuedAt), expiresAt = timestamp(fields.expiresAt);
  if (issuedAt < challenge.issuedAt || expiresAt > challenge.expiresAt || expiresAt <= issuedAt || Date.parse(expiresAt) - Date.parse(issuedAt) > 3e5) fail7("INVALID_CARD_APPROVAL_TIME", "Card approval must fit inside its challenge lifetime");
  const unsigned = { version: "1", productId: "card", challenge, details, account, accountPublicKey, issuedAt, expiresAt };
  const signature = text3(fields.signature, "signature", /^[0-9a-f]{128}$/);
  const proof = Object.freeze({ ...unsigned, signature });
  const encoded = canonicalJSON(proof);
  boundedBytes(encoded);
  if (raw !== null && raw !== encoded) fail7("INVALID_CARD_APPROVAL", "Card approval JSON must be canonical");
  let valid = false;
  try {
  ...
  if (!valid) fail7("INVALID_CARD_APPROVAL_SIGNATURE", "Card approval signature, account or owner is invalid");
  return proof;
}
function verifySignedCardApplicationApproval(input, expected, at = /* @__PURE__ */ new Date()) {
  ...
  if (canonicalJSON(proof.challenge) !== canonicalJSON(challenge) || canonicalJSON(proof.details) !== canonicalJSON(details) || evmAddressFromYNX(proof.account) !== accountAddress || ownerAddress(challenge.owner) !== accountAddress) {
    fail7("BINDING_MISMATCH", "Card approval does not match the current challenge, full details and authenticated account");
  }
...
function parseChallenge(input) {
  ...
  ownerAddress(owner);
  if (fields.chainId !== "0x1917" || fields.purpose !== "create-testnet-card") fail7("INVALID_CARD_CHALLENGE", "Card challenge chain or purpose is invalid");
  const issuedAt = timestamp(fields.issuedAt), expiresAt = timestamp(fields.expiresAt);
  if (expiresAt <= issuedAt || Date.parse(expiresAt) - Date.parse(issuedAt) > 3e5) fail7("INVALID_CARD_APPROVAL_TIME", "Card challenge lifetime must be positive and at most 300 seconds");
  return Object.freeze({
    id: text3(fields.id, "challenge id", new RegExp(`^challenge_${UUID}$`)),
    applicationId: text3(fields.applicationId, "application id", new RegExp(`^application_${UUID}$`)),
    owner,
    chainId: "0x1917",
    purpose: "create-testnet-card",
    payloadHash: text3(fields.payloadHash, "payloadHash", /^[0-9a-f]{64}$/),
    nonce: text3(fields.nonce, "nonce", new RegExp(`^${UUID}$`)),
    issuedAt,
    expiresAt
  });
}
function parseDetails(input) {
  ...
  const limitWei = text3(fields.limitWei, "limitWei", /^[1-9][0-9]{0,77}$/);
  if (BigInt(limitWei) > 2n ** 256n - 1n) fail7("INVALID_CARD_DETAILS", "Card YNXT limit exceeds uint256");
  if (fields.riskAccepted !== true || fields.termsVersion !== "card-testnet-v1") fail7("INVALID_CARD_DETAILS", "Explicit Testnet risk acceptance and current terms are required");
...
function record(value, fields, label) {
  ...
  if (Reflect.ownKeys(value).length !== fields.length) fail7("INVALID_CARD_APPROVAL", `${label} contains hidden fields`);
  const out = {};
  for (const key of fields) {
    const property = Object.getOwnPropertyDescriptor(value, key);
    if (!property?.enumerable || !Object.hasOwn(property, "value")) fail7("INVALID_CARD_APPROVAL", `${label} must contain plain data fields`);
...
function timestamp(value) {
  ...
  if (!Number.isFinite(date.getTime()) || date.toISOString() !== value) fail7("INVALID_CARD_APPROVAL_TIME", "Card approval timestamp is invalid");
  return value;
}
function instant2(value) {
  if (!(value instanceof Date) || !Number.isFinite(value.getTime())) fail7("INVALID_CARD_APPROVAL_TIME", "Card approval requires valid authority time");
  return value;
}
function assertActive(issuedAt, expiresAt, at) {
  if (Date.parse(issuedAt) > at.getTime() || Date.parse(expiresAt) <= at.getTime()) fail7("CARD_APPROVAL_EXPIRED", "Card approval is not currently valid");
}
function assertDetailsHash(challenge, details) {
  if (hash(canonicalJSON(details)) !== challenge.payloadHash) fail7("BINDING_MISMATCH", "Card challenge hash does not match the exact business details");
}
function boundedText(value, label, min, max) {
  if (typeof value !== "string" || value.trim() !== value || value.length < min || value.length > max) fail7("INVALID_CARD_DETAILS", `Card ${label} is invalid`);
  return value;
}
function text3(value, label, pattern4) {
  if (typeof value !== "string" || value.trim() !== value || !pattern4.test(value)) fail7("INVALID_CARD_APPROVAL", `Card ${label} is invalid`);
  return value;
}
function boundedBytes(raw) {
  if (!raw.length || raw.length > MAX_BYTES || utf8ToBytes(raw).length > MAX_BYTES) fail7("INVALID_CARD_APPROVAL", "Card approval exceeds its byte limit");
}
function hash(value) {
  return bytesToHex(sha256(utf8ToBytes(value)));
}
function signDigest(unsigned) {
