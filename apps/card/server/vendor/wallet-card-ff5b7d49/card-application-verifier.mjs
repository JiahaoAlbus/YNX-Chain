...
function parseSignedCardApplicationApproval(input) {
  ...
  if (raw !== null) {
    ...
    try {
      ...
      fail("INVALID_CARD_APPROVAL", "Card approval JSON is invalid");
    }
  }
  ...
  if (issuedAt < challenge.issuedAt || expiresAt > challenge.expiresAt || expiresAt <= issuedAt || Date.parse(expiresAt) - Date.parse(issuedAt) > 3e5) fail("INVALID_CARD_APPROVAL_TIME", "Card approval must fit inside its challenge lifetime");
...
function parseChallenge(input) {
  ...
  const issuedAt = timestamp(fields.issuedAt), expiresAt = timestamp(fields.expiresAt);
  if (expiresAt <= issuedAt || Date.parse(expiresAt) - Date.parse(issuedAt) > 3e5) fail("INVALID_CARD_APPROVAL_TIME", "Card challenge lifetime must be positive and at most 300 seconds");
  return Object.freeze({
    ...
    nonce: text(fields.nonce, "nonce", new RegExp(`^${UUID}$`)),
    issuedAt,
    expiresAt
  });
}
...
function instant(value) {
  if (!(value instanceof Date) || !Number.isFinite(value.getTime())) fail("INVALID_CARD_APPROVAL_TIME", "Card approval requires valid authority time");
  return value;
}
function assertActive(issuedAt, expiresAt, at) {
  if (Date.parse(issuedAt) > at.getTime() || Date.parse(expiresAt) <= at.getTime()) fail("CARD_APPROVAL_EXPIRED", "Card approval is not currently valid");
}
function assertDetailsHash(challenge, details) {
  if (hash(canonicalJSON(details)) !== challenge.payloadHash) fail("BINDING_MISMATCH", "Card challenge hash does not match the exact business details");
}
function boundedText(value, label, min, max) {
  if (typeof value !== "string" || value.trim() !== value || value.length < min || value.length > max) fail("INVALID_CARD_DETAILS", `Card ${label} is invalid`);
  return value;
}
function text(value, label, pattern) {
  if (typeof value !== "string" || value.trim() !== value || !pattern.test(value)) fail("INVALID_CARD_APPROVAL", `Card ${label} is invalid`);
  return value;
}
function boundedBytes(raw) {
  if (!raw.length || raw.length > MAX_BYTES || utf8ToBytes(raw).length > MAX_BYTES) fail("INVALID_CARD_APPROVAL", "Card approval exceeds its byte limit");
}
function hash(value) {
