...
import {CardProviderRegistry,type CardProvider} from './providerRegistry.ts';
...
export type ProviderApplication={id:string;productCardId:string;owner:string;provider:CardProvider;programId:string;providerEnvironment:'TEST';nickname:string;useCase:string;testSpendingLimitMinor:string;cardAccountCurrency:string;minorUnitDigits:number;termsVersion:string;termsHash:string;feeDisclosureHash:string;riskAccepted:boolean;status:ProviderApplicationStatus;hostedKycSessionId:string|null;hostedKycSourceAsOf:string|null;upstreamApplicationId:string|null;upstreamCardId:string|null;upstreamCancellationConfirmed:false;walletApprovalVerified:boolean;createdAt:string;updatedAt:string;audit:{eventId:string;type:string;at:string}[]};
...
export class CardProviderApplications{
  constructor(private readonly store:CardStore,private readonly hostedKyc?:HostedKycAdapter,private readonly verifier?:HostedKycVerifier,private readonly allowedHostedOrigins:readonly string[]=[],private readonly programs:readonly ProviderProgramTerms[]=[],private readonly registry?:CardProviderRegistry){}
  ...
  createDraft(principal:Principal,input:{provider:CardProvider;programId:string;nickname:string;useCase:string;testSpendingLimitMinor:string;cardAccountCurrency:string;minorUnitDigits:number;termsVersion:string;termsHash:string;feeDisclosureHash:string;riskAccepted:boolean},idempotencyKey:string,now=new Date().toISOString()){
    ...
    const application=this.store.transaction(key(principal.owner),empty,state=>{const prior=state.idempotency[idempotencyKey];if(prior){if(prior.digest!==inputDigest)throw new CardError('APPLICATION_IDEMPOTENCY_CONFLICT');return safe(state.applications[prior.applicationId]!)}
      const id='application_'+randomUUID(),productCardId='card-'+randomUUID(),application:ProviderApplication={id,productCardId,owner:principal.owner,...fields,providerEnvironment:'TEST',status:'DRAFT',hostedKycSessionId:null,hostedKycSourceAsOf:null,upstreamApplicationId:null,upstreamCardId:null,upstreamCancellationConfirmed:false,walletApprovalVerified:false,createdAt:now,updatedAt:now,audit:[{eventId:randomUUID(),type:'DRAFT_CREATED_LOCAL',at:now}]};state.applications[id]=application;state.idempotency[idempotencyKey]={digest:inputDigest,applicationId:id};return application});
    this.registry?.plan(principal,{productCardId:application.productCardId,provider:p,programId,environment:'TEST'},now);return application;
  }
  ...
  async acceptVerifiedKyc(principal:Principal,raw:Uint8Array,headers:Readonly<Record<string,string>>,now=new Date().toISOString()){
    ...
    return this.store.transaction(key(principal.owner),empty,state=>{const app=state.applications[event.applicationId];if(!app||app.provider!==event.provider||app.programId!==event.programId||app.hostedKycSessionId!==event.sessionId)throw new CardError('HOSTED_KYC_BINDING_MISMATCH',403);const prior=state.kycEventIds[event.eventId];if(prior){if(prior!==digest(event))throw new CardError('HOSTED_KYC_REPLAY_CONFLICT');return app}if(!['KYC_PENDING','KYC_REVIEW'].includes(app.status))throw new CardError('PROVIDER_APPLICATION_STATE_CONFLICT');
      if(app.hostedKycSourceAsOf&&Date.parse(event.sourceAsOf)<=Date.parse(app.hostedKycSourceAsOf))throw new CardError('HOSTED_KYC_EVENT_STALE',409);
      if(app.status==='KYC_REVIEW'&&event.status==='PENDING')throw new CardError('HOSTED_KYC_STATE_REGRESSION',409);
  ...
  cancelLocal(principal:Principal,applicationId:string,now=new Date().toISOString()){
    requireOwner(principal);ref(applicationId,'APPLICATION_ID');iso(now);return this.store.transaction(key(principal.owner),empty,state=>{const app=state.applications[applicationId];if(!app)throw new CardError('PROVIDER_APPLICATION_NOT_FOUND',404);if(app.status==='CANCELLED')return app;if(app.walletApprovalVerified||app.upstreamCardId||app.status==='ACTIVE_SANDBOX'||app.status==='PROVIDER_PENDING')throw new CardError('PROVIDER_APPLICATION_STATE_CONFLICT');app.status='CANCELLED';app.updatedAt=now;app.audit.push({eventId:randomUUID(),type:'CANCELLED_LOCAL_UPSTREAM_UNCONFIRMED',at:now});return app});
  }
  /** Only the Card-owned lifecycle may commit a verified Wallet approval. */
  recordProviderApproval(principal:Principal,applicationId:string,approvalId:string,now=new Date().toISOString()){
    requireOwner(principal);ref(approvalId,'APPROVAL_ID');iso(now);
    return this.store.transaction(key(principal.owner),empty,state=>{const app=state.applications[ref(applicationId,'APPLICATION_ID')];if(!app)throw new CardError('PROVIDER_APPLICATION_NOT_FOUND',404);if(app.status!=='APPROVAL_REQUIRED')throw new CardError('PROVIDER_APPLICATION_STATE_CONFLICT');if(app.walletApprovalVerified)return app;app.walletApprovalVerified=true;app.updatedAt=now;app.audit.push({eventId:approvalId,type:'WALLET_PROVIDER_APPROVAL_VERIFIED',at:now});return app});
  }
  recordProviderSubmission(principal:Principal,applicationId:string,cardId:string,now=new Date().toISOString()){
    requireOwner(principal);ref(cardId,'EXTERNAL_CARD_ID');iso(now);
    return this.store.transaction(key(principal.owner),empty,state=>{const app=state.applications[ref(applicationId,'APPLICATION_ID')];if(!app)throw new CardError('PROVIDER_APPLICATION_NOT_FOUND',404);if(!app.walletApprovalVerified||!['APPROVAL_REQUIRED','PROVIDER_PENDING'].includes(app.status))throw new CardError('PROVIDER_APPLICATION_STATE_CONFLICT');if(app.upstreamCardId&&app.upstreamCardId!==cardId)throw new CardError('PROVIDER_CARD_CONFLICT');app.upstreamCardId=cardId;app.status='PROVIDER_PENDING';app.updatedAt=now;app.audit.push({eventId:randomUUID(),type:'PROVIDER_CARD_REFERENCE_RECORDED',at:now});return app});
  }
  recordProviderReadback(principal:Principal,applicationId:string,cardId:string,status:string,now=new Date().toISOString()){
    requireOwner(principal);ref(cardId,'EXTERNAL_CARD_ID');ref(status,'PROVIDER_STATUS');iso(now);
    return this.store.transaction(key(principal.owner),empty,state=>{const app=state.applications[ref(applicationId,'APPLICATION_ID')];if(!app)throw new CardError('PROVIDER_APPLICATION_NOT_FOUND',404);if(app.upstreamCardId!==cardId||!app.walletApprovalVerified||!['PROVIDER_PENDING','ACTIVE_SANDBOX'].includes(app.status))throw new CardError('PROVIDER_APPLICATION_STATE_CONFLICT');app.status=status==='active'||status==='frozen'&&app.status==='ACTIVE_SANDBOX'?'ACTIVE_SANDBOX':'PROVIDER_PENDING';app.updatedAt=now;app.audit.push({eventId:randomUUID(),type:'PROVIDER_CARD_READBACK_'+status,at:now});return app});
  }
  doctor(){return {schemaVersion:2,configuredTestPrograms:this.programs.filter(program=>program.enabled&&program.environment==='TEST').length,hostedKycConfigured:Boolean(this.hostedKyc&&this.allowedHostedOrigins.length),hostedKycVerifierConfigured:Boolean(this.verifier),unknownSessionRecovery:'BLOCKED_PROVIDER_SPEC_SESSION_LOOKUP',providerSubmissionConfigured:false,providerActivationConfigured:false,productionIssuingEnabled:false} as const}
}
