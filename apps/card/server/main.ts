...
async function main(){
  ...
  if(adapter){const module=await import(pathToFileURL(resolve(adapter)).href);wallet=await module.createWalletAuthority({origin:'https://wallet-auth.ynxweb4.com'});if(typeof wallet.authenticate!=='function'||typeof wallet.approve!=='function')throw Error('Invalid accepted Wallet server adapter')}
