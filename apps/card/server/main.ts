...
async function main(){
  ...
  const server=createCardServer({service,wallet,providerRegistry:new CardProviderRegistry(store),sourceCommit:process.env.YNX_CARD_SOURCE_COMMIT??'unbound-development',allowedOrigin:process.env.YNX_CARD_ALLOWED_ORIGIN??'https://card.ynxweb4.com',configurationReady:Boolean(adapter&&rpc&&recipient)});
