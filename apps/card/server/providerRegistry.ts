...
export type FundingUnit=
  |{kind:'YNXT_TESTNET';assetId:'YNXT';network:'0x1917';decimals:18;tokenContract:string|null}
  |{kind:'TUSD_TESTNET';assetId:'tUSD';network:'0x1917';decimals:6;tokenContract:string|null}
  |{kind:'PROVIDER_TEST_ASSET';assetId:string;provider:CardProvider;environment:'TEST';decimals:number}
  |{kind:'CARD_ACCOUNT';currency:string;provider:CardProvider;environment:ProviderEnvironment;minorUnitDigits:number};
...
export type ProviderReadConnector={provider:CardProvider;environment:'TEST';getCardStatus(accountId:string,cardId:string):Promise<{externalCardId:string;status:string;sourceAsOf:string;[key:string]:unknown}>};
...
function assertUnit(unit:FundingUnit):FundingUnit{
  if(unit.kind==='YNXT_TESTNET'){if(unit.assetId!=='YNXT'||unit.network!=='0x1917'||unit.decimals!==18||unit.tokenContract!==null&&!/^0x[0-9a-fA-F]{40}$/.test(unit.tokenContract))throw new CardError('FUNDING_UNIT_INVALID',400)}
  else if(unit.kind==='TUSD_TESTNET'){if(unit.assetId!=='tUSD'||unit.network!=='0x1917'||unit.decimals!==6||unit.tokenContract!==null&&!/^0x[0-9a-fA-F]{40}$/.test(unit.tokenContract))throw new CardError('FUNDING_UNIT_INVALID',400)}
  else if(unit.kind==='PROVIDER_TEST_ASSET'){provider(unit.provider);id(unit.assetId,'ASSET_ID');if(unit.environment!=='TEST'||!Number.isInteger(unit.decimals)||unit.decimals<0||unit.decimals>30)throw new CardError('FUNDING_UNIT_INVALID',400)}
  else if(unit.kind==='CARD_ACCOUNT'){provider(unit.provider);if(!/^[A-Z]{3}$/.test(unit.currency)||!Number.isInteger(unit.minorUnitDigits)||unit.minorUnitDigits<0||unit.minorUnitDigits>9)throw new CardError('FUNDING_UNIT_INVALID',400);environment(unit.environment)}
...
export class CardProviderRegistry{
  ...
  async readRoutedStatus(principal:Principal,productCardId:string,connectors:Readonly<Partial<Record<CardProvider,ProviderReadConnector>>>) {
    ...
    const connector=connectors[binding.provider];if(!connector||connector.provider!==binding.provider||connector.environment!=='TEST')throw new CardError('CARD_PROVIDER_READ_UNAVAILABLE',503);
  ...
  recordEvent(principal:Principal,productCardId:string,input:{externalEventId:string;type:ProviderActivity['type'];status:string;amount?:string;unit?:FundingUnit;occurredAt:string;sourceAsOf:string},now=new Date().toISOString()):ProviderActivity{
    ...
    const amount=input.amount??null,unit=input.unit?assertUnit(input.unit):null;if((amount===null)!==(unit===null)||amount!==null&&!/^(0|[1-9][0-9]{0,38})$/.test(amount))throw new CardError('PROVIDER_EVENT_AMOUNT_INVALID',400);
    return this.store.transaction(ownerKey(principal.owner),empty,state=>{
      ...
      if(unit?.kind==='PROVIDER_TEST_ASSET'||unit?.kind==='CARD_ACCOUNT'){if(unit.provider!==binding.provider||unit.environment!==binding.environment)throw new CardError('PROVIDER_EVENT_UNIT_MISMATCH',400)}
  ...
  doctor(){return {schemaVersion:2,providers:['immersve','lithic'],environment:'TEST',providerWritesEnabled:false,liveIssuingEnabled:false,realCardDataStored:false,ynxtBalanceAuthority:false} as const}
}
