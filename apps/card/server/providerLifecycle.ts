...
export class CardProviderLifecycle {
  ...
  availablePrograms(principal:Principal){if(!principal.scopes.includes('account:read'))throw new CardError('CARD_PERMISSION_DENIED',403);return this.programs.filter(program=>program.enabled&&program.environment==='TEST').map(({provider,programId,environment,cardAccountCurrency,minorUnitDigits,termsVersion,termsHash,riskVersion,riskHash,feeDisclosureVersion,feeDisclosureText,feeDisclosureHash,fundingNetwork,fundingAssetId,decimals})=>({provider,programId,environment,cardAccountCurrency,minorUnitDigits,termsVersion,termsHash,riskVersion,riskHash,feeDisclosureVersion,feeDisclosureText,feeDisclosureHash,fundingNetwork,fundingAssetId,decimals}));}
