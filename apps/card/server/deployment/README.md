...
- The authentication module must be the release's own `server/sharedWalletAuth.ts`, which consumes exact Wallet SDK `a7dad7ec1bc7c06577978bdd5fea8dc9c7a248a9`. The service entry rejects replacement modules. The coordinator-assigned service RPC is `https://rpc.ynxweb4.com`; it is never supplied by an HTTP request.
- Native callers send the restricted `X-YNX-Card-Platform: android` or `ios` header and a fresh SDK-produced V2 proof. This only selects a preconfigured authorizer; it grants no identity. Web calls still require the exact Origin under the shared contract. Same-origin browser GET requests can omit Origin; do not fill it from an untrusted forwarded header. Web integration remains pending a shared accepted request/ingress contract.
