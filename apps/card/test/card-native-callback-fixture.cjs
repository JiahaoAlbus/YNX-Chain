...
async function makeMounted(factory,{initialURL=null,bootstrap=null}={}){
 ...
 const App=execute(path.join(root,'App.tsx'),{'react':observedReact,'react-native':rn,'react-native-safe-area-context':{SafeAreaProvider:'SafeAreaProvider',SafeAreaView:'SafeAreaView'},'expo-status-bar':{StatusBar:'StatusBar'},'expo-local-authentication':{},'lucide-react-native':icons,'./src/api':{state:async()=>{metrics.stateReads++;return null}},'./src/i18n':i18n,'./src/secureState':store,'./src/productWalletRuntime':{createRuntimeCardProductWalletConnection:async input=>wrapActualController(await factory(input))},'./src/providerApplicationClient':{CardProviderClient:class{}},'./src/providerClientRuntime':{createRuntimeProviderClient:async()=>await new Promise(()=>{})},'./src/providerSessionWeb':{restoreCardWebSession:forbidden,beginCardWebSession:forbidden,retryCardWebSession:forbidden,disconnectCardWebSession:forbidden,cardWebProof:forbidden},'./src/standardWalletSdk':{beginStandardWalletOperation:forbidden,discoverWalletProviders:forbidden,isSharedWalletProvider:forbidden,standardWalletConnection:forbidden},'@ynx-chain/wallet-auth':standard,'./src/wallet':wallet,'./src/simulation':{},'./src/GuestExperience':{GuestExperience:Guest}}).default;
