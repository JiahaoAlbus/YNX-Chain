...
function wrapActualController(observer){
 ...
 const composition=execute(path.join(root,'src/productWalletConnection.ts'),{'./productWalletStorage':storage,'@ynx-chain/wallet-auth-card-provider-v2':{ProductSessionGatewayFetchAdapter:class{},RecoverableProductSessionClient:class{async restore(){const result=await observer.restore();return result.sessionState}},WalletConnectionCoordinator:Delegate},'../vendor/product-session-registry-b754ffc42.json':JSON.parse(fs.readFileSync(path.join(root,'vendor/product-session-registry-b754ffc42.json')))});
...
async function makeMounted(factory,{initialURL=null,bootstrap=null,platform='android',webSession={},hostedFactory=null}={}){
 ...
 const rn={Platform:{OS:platform},StyleSheet:{create:x=>x},useColorScheme:()=> 'light',Linking:{getInitialURL:()=>{metrics.initialReads++;return Promise.resolve(initialURL)},addEventListener:(_name,cb)=>{metrics.adds++;event=cb;return{remove:()=>{metrics.removes++;event=null}}},canOpenURL:forbidden,openURL:forbidden},...Object.fromEntries(['ActivityIndicator','Alert','FlatList','Image','Modal','Pressable','ScrollView','Switch','Text','TextInput','View'].map(x=>[x,x]))};
 ...
 const standard={createStandardWalletConnectState:()=>({status:'disconnected',chooserOpen:false,account:null,chainId:null,providerKind:null}),reduceStandardWalletConnectState:(s,e)=>e.type==='OPEN_CHOOSER'?{...s,chooserOpen:true}:e.type==='CLOSE_CHOOSER'?{...s,chooserOpen:false}:e.type==='RESTORE'?{status:'connected',chooserOpen:false,account:e.accounts[0],chainId:e.chainId,providerKind:e.providerKind}:e.type==='DISCONNECT'?{...s,status:'disconnected',account:null,chainId:null,chooserOpen:false}:e.type==='FAIL'?{...s,status:'disconnected',account:null,chainId:null,chooserOpen:true}:s,discoverWalletProviders:forbidden};
 const wallet={classifyCardWalletError:()=>({code:'SYNTHETIC_ERROR',safeMessage:'synthetic unavailable',userAction:'retry'}),watchEip1193Provider:()=>()=>{},restoreEip1193Wallet:async()=>null,YNX_TESTNET_CHAIN_ID:'0x1917'};
 ...
 const App=execute(path.join(root,'App.tsx'),{'react':observedReact,'react-native':rn,'react-native-safe-area-context':{SafeAreaProvider:'SafeAreaProvider',SafeAreaView:'SafeAreaView'},'expo-status-bar':{StatusBar:'StatusBar'},'expo-local-authentication':{},'lucide-react-native':icons,'./src/api':{state:async()=>{metrics.stateReads++;return null}},'./src/i18n':i18n,'./src/secureState':store,'./src/productWalletRuntime':{createRuntimeCardProductWalletConnection:async input=>wrapActualController(await factory(input))},'./src/providerApplicationClient':{CardProviderClient:class{}},'./src/providerClientRuntime':{createRuntimeProviderClient:async()=>await new Promise(()=>{})},'./src/providerSessionWeb':{restoreCardWebSession:async()=>({status:'disconnected'}),beginCardWebSession:forbidden,retryCardWebSession:forbidden,disconnectCardWebSession:forbidden,cardWebProof:forbidden,...webSession},'./src/hostedWalletWeb':{createCardHostedWalletController:hostedFactory??forbidden},'./src/hostedWalletCopy':{hostedWalletText:(locale,key)=>`${locale}:${key}`,hostedWalletNotice:code=>code==='USER_REJECTED'?'rejected':code==='HOSTED_ACCOUNT_CHANGED'?'accountChanged':'unavailable'},'./src/standardWalletSdk':{beginStandardWalletOperation:forbidden,discoverWalletProviders:async()=>[],isSharedWalletProvider:forbidden,peekExactYNXProvider:()=>null,standardWalletConnection:forbidden},'@ynx-chain/wallet-auth':standard,'./src/wallet':wallet,'./src/simulation':{},'./src/GuestExperience':{GuestExperience:Guest}}).default;
