...
async function mountGuest({platform="android",locale="en",width=412,fontScale=2,props={},storageOptions={},layoutOptions={}}={}) {
  const calls={native:0,chooser:0,metamask:0,retry:0,disconnectNative:0,scroll:[],measure:[],reads:0,writes:0};
  ...
  const dimensions={width,height:915,fontScale},layout={y:0,ready:true,defer:false,silent:false,throwMeasure:false,parent:{},...layoutOptions},pendingMeasures=[];
  const sectionNode={measureLayout(inner,success,failure){calls.measure.push({y:layout.y,parentMatches:inner===layout.parent});if(layout.throwMeasure)throw Error("Synthetic native measure failure");if(layout.silent)return;const result={y:layout.y,ready:layout.ready,success,failure};if(layout.defer)pendingMeasures.push(result);else if(result.ready)success(0,result.y,dimensions.width,1000);else failure?.()}};
  ...
  const component=evaluate(path.join(root,"src/GuestExperience.tsx"),{"react":React,"react-native":rn,"lucide-react-native":icons,"./wallet":{METAMASK_CARD_DEEP_LINK:"https://example.invalid/metamask",METAMASK_INSTALL_URL:"https://example.invalid/install"},"./i18n":require("../src/i18n.ts"),"./guestCopy":require("../src/guestCopy.ts"),"./hostedWalletCopy":require("../src/hostedWalletCopy.ts"),"./RegistrationExperience":registration,"./ProviderExperience":{ProviderExperience:()=>null},"@ynx-chain/wallet-auth":{createStandardWalletConnectState:()=>({status:"disconnected",chooserOpen:false})}}).GuestExperience;
