...
async function protectedDevice(){
  ...
  if(!existing){const serialized=JSON.stringify(raw);await SecureStore.setItemAsync(key,serialized);if(await SecureStore.getItemAsync(key)!==serialized)throw new Error("Secure device storage did not preserve the Card identity key.");}
...
function randomBytes(size:number):Uint8Array{return getRandomBytes(size);}
