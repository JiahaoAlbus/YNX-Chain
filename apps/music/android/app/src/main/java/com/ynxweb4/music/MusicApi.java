...
final class MusicApi {
    ...
    String streamAI(String id)throws Exception{String path="/api/ai/proposals/"+id+"/stream";HttpURLConnection x=open(path);x.setRequestMethod("GET");authorize(x,"music.library");int code=x.getResponseCode();String text=new String((code<400?x.getInputStream():x.getErrorStream()).readAllBytes(),StandardCharsets.UTF_8);if(code>=400)throw new IOException(text);return text;}
    ...
    private JSONObject json(String method,String path,JSONObject body,String idempotency)throws Exception{HttpURLConnection x=open(path);x.setRequestMethod(method);String scope=scope(path);if(scope!=null)authorize(x,scope);if(idempotency!=null)x.setRequestProperty("Idempotency-Key",idempotency);if(body!=null){x.setDoOutput(true);x.setRequestProperty("Content-Type","application/json");x.getOutputStream().write(body.toString().getBytes(StandardCharsets.UTF_8));}int code=x.getResponseCode();String text=new String((code<400?x.getInputStream():x.getErrorStream()).readAllBytes(),StandardCharsets.UTF_8);if(code>=400)throw new IOException(new JSONObject(text).optString("error","HTTP "+code));return new JSONObject(text);}
    ...
    JSONObject upload(File wav,String title,String artist,String rightsBasis,String evidence,String provenance)throws Exception{String boundary="YNXMusic"+System.nanoTime();HttpURLConnection x=open("/api/creator/tracks");x.setRequestMethod("POST");authorize(x,"music.creator");x.setDoOutput(true);x.setRequestProperty("Content-Type","multipart/form-data; boundary="+boundary);try(OutputStream out=x.getOutputStream()){field(out,boundary,"title",title);field(out,boundary,"artistName",artist);field(out,boundary,"rightsBasis",rightsBasis);field(out,boundary,"territories","WORLDWIDE");field(out,boundary,"evidenceRef",evidence);field(out,boundary,"audioProvenance",provenance);field(out,boundary,"explicit","false");out.write(("--"+boundary+"\r\nContent-Disposition: form-data; name=\"audio\"; filename=\"owned.wav\"\r\nContent-Type: audio/wav\r\n\r\n").getBytes());Files.copy(wav.toPath(),out);out.write(("\r\n--"+boundary+"--\r\n").getBytes());}int code=x.getResponseCode();String text=new String((code<400?x.getInputStream():x.getErrorStream()).readAllBytes());if(code>=400)throw new IOException(new JSONObject(text).optString("error"));return new JSONObject(text);}
    private HttpURLConnection open(String path)throws Exception{HttpURLConnection x=(HttpURLConnection)new URL(base+path).openConnection();x.setConnectTimeout(8000);x.setReadTimeout(30000);x.setRequestProperty("Accept","application/json");return x;}
    private void authorize(HttpURLConnection connection,String scope)throws Exception{String raw=SecureStore.get(context);if(raw.isEmpty())throw new SecurityException("Connect YNX Wallet");connection.setRequestProperty("X-YNX-Product-Session-Proof",CentralContracts.productSessionProof(new JSONObject(raw),scope,System.currentTimeMillis()));}
    private static String scope(String path){if(path.equals("/api/me")||path.equals("/api/profile")||path.startsWith("/api/cases"))return "music.profile";if(path.startsWith("/api/creator/"))return "music.creator";if(path.startsWith("/api/playback/"))return "music.playback";if(path.startsWith("/api/library")||path.startsWith("/api/playlists")||path.startsWith("/api/ai/"))return "music.library";return null;}
