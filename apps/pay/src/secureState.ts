...
export async function clearRetiredWalletState(){await Promise.all(['session','authorization'].map(key=>SecureStore.deleteItemAsync(PREFIX+key)))}
...
export async function loadLocale(){return SecureStore.getItemAsync(PREFIX+'locale')}
export async function saveLocale(value:string){return SecureStore.setItemAsync(PREFIX+'locale',value)}
export async function loadAILanguage(){return SecureStore.getItemAsync(PREFIX+'ai-language')}
