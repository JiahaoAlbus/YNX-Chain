...
export const PUBLIC_STATUS_SOURCE_SCHEMA='ynx.monitor.public-status-source.v1' as const;
export const PUBLIC_STATUS_SCHEMA='ynx.monitor.public-status.v1' as const;
...
export class PublicStatusReplayGuard {
  ...
  accept(snapshot:PublicStatusSnapshot){
    if(!this.last){this.last={source:snapshot.source,version:snapshot.version,asOf:snapshot.asOf,publishedAt:snapshot.publishedAt};return snapshot;}
...
export function parsePublicStatusSource(raw:unknown,options:{now?:Date;maxAgeSeconds:number;integrityKey:string;expectedSource:string}):PublicStatusSnapshot {
  ...
  const services=boundedArray(input.services,64).map(value=>{
    ...
    exactKeys(item,['id','name','status','asOf','message']);
    return {
  ...
  if(services.some(item=>Date.parse(item.asOf)>publishedAtMillis||now-Date.parse(item.asOf)>options.maxAgeSeconds*1000))throw new PublicStatusError('public_status_stale');
