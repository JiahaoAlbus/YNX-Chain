...
async function transition(base:string,id:string,action:string,token:string,summary:string,evidence:string[]=[]){
  return call(base,`/ops/incidents/${encodeURIComponent(id)}/actions/${action}`,token,{summary,evidence});
}
