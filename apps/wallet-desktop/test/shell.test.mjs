...
test("current Windows upgrade gate binds the exact predecessor version and profile", async () => {
  const workflow = await readFile(new URL("../../../.github/workflows/wallet-desktop-current.yml", import.meta.url), "utf8");
  const helper = await readFile(new URL("../scripts/windows-old-installed-profile.mjs", import.meta.url), "utf8");
  assert.match(helper, /const oldVersion = process\.env\.YNX_OLD_VERSION/);
  assert.match(workflow, /YNX_OLD_VERSION: '0\.6\.8'/);
  assert.match(workflow, /旧钱包 0\.6\.8 QA profile/);
  assert.match(workflow, /windows-password-installed-gate\.mjs restore \$old\.account/);
  assert.doesNotMatch(workflow, /YNX_OLD_ALLOW_CREATE_FAILURE: true/);
});
