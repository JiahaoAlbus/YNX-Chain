...
if (process.platform !== "win32" || process.arch !== "x64" || !["0.6.8", "0.6.9"].includes(oldVersion) || !process.env.YNX_OLD_INSTALLED_EXE || !process.env.RUNNER_TEMP || !process.env.GITHUB_WORKSPACE) throw new Error("OLD_INSTALLED_QA_ENVIRONMENT_UNAVAILABLE");
...
try {
  ...
  outcome = { sourceVersion: oldVersion, account: created.account, ynxAccount: created.ynxAccount, vaultSHA256: createHash("sha256").update(bytes).digest("hex"), installedUIAccountCreated: true, appProcessTerminationIntentional: true, userProfileTouched: false };
} finally {
