...
const allowCreateFailure = process.env.YNX_OLD_ALLOW_CREATE_FAILURE === "true";
if (process.platform !== "win32" || process.arch !== "x64" || !["0.6.8", "0.6.9"].includes(oldVersion) || (allowCreateFailure && oldVersion !== "0.6.8") || !process.env.YNX_OLD_INSTALLED_EXE || !process.env.RUNNER_TEMP || !process.env.GITHUB_WORKSPACE) throw new Error("OLD_INSTALLED_QA_ENVIRONMENT_UNAVAILABLE");
