...
test("browser module is self-contained for the public Studio route",async()=>{const source=await readFile(new URL("./standard-wallet.js",import.meta.url),"utf8");assert.doesNotMatch(source,/\.\.\/\.\.\/packages\//)});
test("missing provider fails with visible recovery metadata",async()=>{await assert.rejects(()=>connectStandardWallet({windowLike:new WindowLike(null),timeoutMs:0}),error=>error.code==="WALLET_NOT_FOUND"&&error.details.installOptions.length===2)});
