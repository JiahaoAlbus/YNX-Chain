...
async function noReplaceMove(source,target){await absent(target);if(process.platform==="linux"){execFileSync("/usr/bin/mv",["-T","--no-clobber",source,target]);await absent(source)}else await rename(source,target)}
async function quarantineOwnedDirectory(path,expected,expectedTree,hooks,label,{suffix=".cleanup-pre-switch"}={}){const quarantine=path+suffix,owned=join(quarantine,"owned");if(!same(await ownedDirectoryIdentity(path),expected)||!same(await treeSnapshot(path),expectedTree))throw Error("OWNED_DIRECTORY_TREE_CHANGED:"+label);if(hooks?.beforeOwnedCleanup)await hooks.beforeOwnedCleanup({path,label,expected,expectedTree,quarantine});await mkdir(quarantine,{mode:0o700});const reservation=await ownedDirectoryIdentity(quarantine);await noReplaceMove(path,owned);if(!same(await ownedDirectoryIdentity(quarantine),reservation)||!same(await ownedDirectoryIdentity(owned),expected)||!same(await treeSnapshot(owned),expectedTree))throw Error("OWNED_DIRECTORY_QUARANTINE_MISMATCH:"+label);await rm(quarantine,{recursive:true});await absent(quarantine);await absent(path)}
...
async function forward(apiPid,viewerPid,preflightPort,hooks){
  ...
  try{
    ...
    const prepared={schemaVersion:"1.0.0",status:"FORWARD_PREPARED_AWAITING_SWITCH",before,material:{candidateSha256:F.candidateSha,rollbackCarrierSha256:F.rollbackCarrierSha,newRelease:created,newReleaseTree,cold}};
    ...
    if(hooks?.failAt==="postSwitch")throw Error("FIXTURE_POST_SWITCH_FAILURE");
...
async function fixture(){
  ...
  try{
    ...
    const collisionQuarantine=F.stage+".cleanup-pre-switch";const noReplaceCollisionRejected=await forward(6493,6494,5188,{...hooks,coldStart:async()=>{throw Error("FIXTURE_COLD_START_FAILURE")},beforeOwnedCleanup:async({label,quarantine})=>{if(label==="candidate-stage")await mkdir(quarantine,{mode:0o700})}}).then(()=>false,()=>true);const noReplaceSourcePreserved=!(await isAbsent(F.stage)),noReplaceReservationPreserved=!(await isAbsent(collisionQuarantine));await rm(F.stage,{recursive:true,force:true});await rm(collisionQuarantine,{recursive:true,force:true});
    const postQuarantine=F.newRelease+".cleanup-post-switch",cleanupPost=async()=>{await rm(postQuarantine,{recursive:true,force:true});await rm(F.newRelease,{recursive:true,force:true});await rm(F.receipt,{force:true});await rm(F.receiptNext,{force:true});await rm(F.currentNext,{force:true})},replaceManifestInode=async path=>{const target=join(path,"creator-studio.manifest.json"),bytes=await readFile(target),original=(await lstat(target)).ino;await rm(target);await writeFile(target,bytes,{mode:0o644});if((await lstat(target)).ino===original)throw Error("FIXTURE_INODE_DID_NOT_CHANGE")};
    const automaticForeignChildRejected=await forward(6493,6494,5188,{...hooks,failAt:"postSwitch",beforeOwnedCleanup:async({path,label})=>{if(label==="candidate-release-post-switch")await writeFile(join(path,"foreign-child.txt"),"foreign")}}).then(()=>false,error=>error.message.includes("AUTOMATIC_ROLLBACK_FAILED:OWNED_DIRECTORY_QUARANTINE_MISMATCH:candidate-release-post-switch"));const automaticForeignForensicPreserved=!(await isAbsent(postQuarantine))&&!(await isAbsent(join(postQuarantine,"owned/foreign-child.txt")));await cleanupPost();
    const automaticNewInodeRejected=await forward(6493,6494,5188,{...hooks,failAt:"postSwitch",beforeOwnedCleanup:async({path,label})=>{if(label==="candidate-release-post-switch")await replaceManifestInode(path)}}).then(()=>false,error=>error.message.includes("AUTOMATIC_ROLLBACK_FAILED:OWNED_DIRECTORY_QUARANTINE_MISMATCH:candidate-release-post-switch"));const automaticNewInodeForensicPreserved=!(await isAbsent(postQuarantine))&&!(await isAbsent(join(postQuarantine,"owned/creator-studio.manifest.json")));await cleanupPost();
    await forward(6493,6494,5188,hooks);const conditionalForeignChildRejected=await rollback(6493,6494,{...hooks,beforeOwnedCleanup:async({path,label})=>{if(label==="candidate-release-post-switch")await writeFile(join(path,"foreign-child.txt"),"foreign")}}).then(()=>false,error=>error.message.includes("OWNED_DIRECTORY_QUARANTINE_MISMATCH:candidate-release-post-switch"));const conditionalForeignForensicPreserved=!(await isAbsent(postQuarantine))&&!(await isAbsent(join(postQuarantine,"owned/foreign-child.txt")));await cleanupPost();
    await forward(6493,6494,5188,hooks);const conditionalNewInodeRejected=await rollback(6493,6494,{...hooks,beforeOwnedCleanup:async({path,label})=>{if(label==="candidate-release-post-switch")await replaceManifestInode(path)}}).then(()=>false,error=>error.message.includes("OWNED_DIRECTORY_QUARANTINE_MISMATCH:candidate-release-post-switch"));const conditionalNewInodeForensicPreserved=!(await isAbsent(postQuarantine))&&!(await isAbsent(join(postQuarantine,"owned/creator-studio.manifest.json")));await cleanupPost();
