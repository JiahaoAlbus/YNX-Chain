...
set -Eeuo pipefail
[[ $EUID == 0 ]]
mode=${1:?Choose creator or viewer}
action=${2:-deploy}
source=b6af671d04a4230bb7a4052cee1ff21d2f5c9c51
...
case "$mode" in
 creator)
  ...
  artifact=/var/tmp/ynx-creator-b6af671d0-runtime.tar.gz
  artifact_sha=bc40e71702aef2282135a03df650a24e2447751242c257a1ecf3bd414189d349
  release=/opt/ynx-creator-studio-wallet/releases/audit-wallet-time-$source
  old_release=/opt/ynx-creator-studio-wallet/releases/audit-v2-29896f10c5e59fc17590dd3697d6e7af3d34a809
  old_dropins="/etc/systemd/system/$unit.d/20260906-v2-consumer.conf /etc/systemd/system/$unit.d/20260906-v3-channel-id.conf"
  old_config=/etc/systemd/system/$unit.d/20260906-v3-channel-id.conf
  old_config_sha=34d6fe89ec14b9941ea567072f992b5f903b90222a390f57244a3022d9ed57e2
  manifest=creator-studio.manifest.json
  neighbors=(ynx-videod.service ynx-video-viewer.service ynx-wallet-gateway.service)
  neighbor_manifest=http://127.0.0.1:6494/runtime-manifest.json
  port=6495
  ;;
 viewer)
  unit=ynx-video-viewer.service
  artifact=/var/tmp/ynx-video-b6af671d0-runtime.tar.gz
  artifact_sha=131229cc7dd9ea1cbabc0b0842d21f438d3689c13d0b4cc460e28d3416f68356
  release=/opt/ynx-video-viewer-wallet/releases/ynx-video-$source
  old_release=/opt/ynx-video-viewer-wallet/releases/ynx-video-48fe3824cc6a38ba944427776e48d076f27f54f5
  old_dropins="/etc/systemd/system/$unit.d/20260906-audit.conf /etc/systemd/system/$unit.d/20260906-library-v2.conf"
  old_config=/etc/systemd/system/$unit.d/20260906-library-v2.conf
  old_config_sha=893afdbdd137c4e38f19d04c505584fd5f1559b829972306487994a0909662a4
...
config=/etc/systemd/system/$unit.d/20260906-wallet-time-ff68.conf
...
echo SELECTED_FRONTEND_AUTHORITY_TIME_DEPLOYED=true
