...
SESSION="$(curl -fsS -X POST "http://127.0.0.1:${PORT}/api/v1/session" -H 'Content-Type: application/json' --data "{\"product\":\"cloud\",\"clientId\":\"com.ynx.cloud.web\",\"bundleId\":\"com.ynx.cloud.web\",\"callback\":\"/cloud/auth/callback\",\"account\":\"ynx1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqp7h6v\",\"chainId\":\"ynx_6423-1\",\"scopes\":[\"files.read\",\"files.write\",\"permissions.manage\",\"audit.read\",\"ai.use\"],\"nonce\":\"smoke\",\"expiresAt\":\"$EXPIRES\",\"devicePublicKey\":\"local-smoke-device\",\"signature\":\"dev-signed\"}" | sed -E 's/.*\"token\":\"([^\"]+)\".*/\1/')"
