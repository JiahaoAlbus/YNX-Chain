...
async function authenticate(product,account="ynx1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqp7h6v",requestedScopes){
  const docs=product==="docs",now=new Date(),client=docs?"ynx-docs-mobile-v1":"ynx-cloud-mobile-v1",bundle=docs?"com.ynxweb4.docs":"com.ynxweb4.cloud",callback=docs?"ynxdocs://wallet-auth/callback":"ynxcloud://wallet-auth/callback",scopes=requestedScopes||(docs?["ai.use","audit.read","comments.write","documents.read","documents.write","sharing.manage"]:["ai.use","audit.read","files.read","files.write","permissions.manage"]);
...
const erasureAccount="ynx1pppppppppppppppppppppppppppppppp5f3cz";
const erasureToken=await authenticate("cloud",erasureAccount,["data.delete","files.read","files.write"]),erasureHeaders={Authorization:`Bearer ${erasureToken}`};
await call("/objects",{method:"POST",headers:erasureHeaders,body:JSON.stringify({kind:"file",name:"portable-before-erasure.txt",mime:"text/plain",content:Buffer.from("portable").toString("base64"),encryption:{clientSide:false}})});
const erasureExport=await call("/export",{headers:erasureHeaders});assert.ok(erasureExport.response.headers.get("x-content-sha256"));
await assert.rejects(()=>call("/account-data",{method:"DELETE",headers:erasureHeaders,body:JSON.stringify({confirm:"DELETE DATA"})}),error=>error.status===400);
const {body:erasure}=await call("/account-data",{method:"DELETE",headers:erasureHeaders,body:JSON.stringify({confirm:"DELETE CLOUD DATA"})});assert.equal(erasure.product,"cloud");assert.equal(erasure.pendingBlobs,0);assert.notEqual(erasure.ownerHash,erasureAccount);
await assert.rejects(()=>call("/account-data/erasures",{headers:erasureHeaders}),error=>error.status===401);
const receiptToken=await authenticate("cloud",erasureAccount,["data.delete"]),receiptHeaders={Authorization:`Bearer ${receiptToken}`};
const {body:receipts}=await call("/account-data/erasures",{headers:receiptHeaders});assert.equal(receipts.at(0).id,erasure.id);assert.ok(!JSON.stringify(receipts).includes(erasureAccount));
