...
test('Cloud uses accepted Standard Wallet, memory-only private state, recovery and explicit AI consent',async()=>{
  const js=await readFile(new URL('../web/app.js',import.meta.url),'utf8')+await readFile(new URL('../web/wallet-login.js',import.meta.url),'utf8');
  ...
  assert.match(js,/createCloudLogin/);assert.doesNotMatch(js,/window\.ynxWallet\?\.authorize|YNX_PRODUCT_SESSION_CHALLENGE_V1|generateKey|session\/challenge/);
  assert.match(js,/YNX_CLOUD_RUNTIME\.apiBase/);assert.doesNotMatch(js,/cloud\/api\/v1/);
  assert.match(html,/Connect YNX Wallet/);assert.match(html,/Connect MetaMask/);assert.match(html,/Download YNX Wallet/);assert.match(html,/Cancel and continue browsing/);
