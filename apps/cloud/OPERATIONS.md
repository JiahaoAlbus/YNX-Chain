...
- Schema migration: stop writers, take a verified recovery backup, start the new binary once, retain the exact versioned migration backup, inspect restricted health for schema 6, verify `/api/v1/usage` does not invent unobserved historical traffic or storage time, verify the erasure receipt map starts empty for v5 input, and run smoke. For rollback, stop writers and use `-rollback-state-v1` with a new destination; never replace current state in place. Validate the rollback with the previous binary before cutover.
