...
This mode still permits canonical Wallet authentication, listing, read/download, quota, usage, audit, portable export, logout, grant/link revocation, upload/AI cancellation, trash, permanent deletion, dedicated-scope product-account erasure, receipt lookup, and pending provider-deletion retry. It rejects new uploads, document edits, restores, stars, shares, access decisions, comments, presence writes, and AI jobs with HTTP 423 plus `X-YNX-Service-Mode: user-exit`. Retention windows and legal holds continue to block erasure; operators must not bypass them. Keep the service and provider online until the announced exit window closes, monitor export/deletion failures, and retain the verified backup according to the disclosed policy. This is a controlled user exit path, not proof that public cessation has occurred.
...
- Product-account erasure: require a fresh canonical session containing only the scopes needed to export and `data.delete`; verify the portable export first if requested; require the exact product-bound confirmation. A 200 receipt proves known provider attempts completed, while 202 proves only logical erasure and retains restricted owner authorization until retry completes. The hashed receipt deliberately remains; opaque nonce digests expire under the security policy. External backups, replicas, and provider sanitization require separate evidence.
...
- Storage lifecycle failure: a missing provider capability, transport failure or response-binding mismatch records `failed` and preserves the prior immutable version class. Keep the object and transition evidence; do not permanently delete the object or erase the product account until an authenticated retry for the same account and product completes. An archive version remains `restore-required` until a provider-bound transition returns it to an immediate-read class.
- Schema migration: stop writers, take a verified recovery backup, start the new binary once, retain the exact versioned migration backup, inspect restricted health for schema 7, verify `/api/v1/usage` does not invent unobserved historical traffic or storage time, verify the erasure receipt map starts empty for v5 input, verify v6 versions normalize to bounded `hot`/`immediate` compatibility without invented provider evidence, verify metadata-only legacy objects remain without lifecycle state, and run smoke. For rollback, stop writers and use `-rollback-state-v1` with a new destination; never replace current state in place. Validate the rollback with the previous binary before cutover.
