...
Current commands: `node apps/cloud/scripts/generate-security-artifacts.mjs <source-commit> --sbom-only` updates the current dependency inventory without rewriting historical APK provenance; omit `--sbom-only` only when a newly verified artifact and all of its source materials are being recorded together. Run `npm --prefix apps/cloud run security` for verification. Historical Android `assembleRelease`/lint and adb install evidence remain local; a fresh reproducible build and production attestation are not claimed.
