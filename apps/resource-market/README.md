...
Configure the central HTTPS Gateway with `YNX_RESOURCE_MARKET_GATEWAY_URL` and the registered client ID. Until the registry and authority routes are merged and deployed, quotes and signed intents fail closed. Sponsorship moves bounded capacity only. A quote, signed intent, or authority acceptance is never reported as asset settlement without separate authoritative evidence. Recovery operators can inspect or restore the atomic `.bak` store with `go run ./apps/resource-market/cmd/recover`.
