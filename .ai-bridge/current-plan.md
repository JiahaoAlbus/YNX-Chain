...
- Optional Envelope v2 `chainCommitmentId` consumes frozen Chain Core contract v1.30.0 / implementation `aab0b6b9fc28b40ef8715214cd95360734db4c48` / contract `3bcae14f9d42fd2fbcce2211a7c51a737f8ed464` from the SHA-256-verified v30 bundle as a read-only external reference and fails closed before storage. Public semantics are unchanged; v30 adds authenticated SafetyStore v2 and requires it for ValidatorVoteSigner. Older validly signed snapshot replay remains possible without an unprovided trusted external monotonic rollback anchor; production signer custody is also unprovided. The contract has 105 vectors and does not establish central integration or deployment.
...
1. Download and hash-verify Run `31838660585` artifacts, bind the exact JSON metrics, and refresh the complete recovery bundle.
