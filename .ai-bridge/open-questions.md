...
| CS-EXT-04 | YNX 13 Monitor + YNX 12 Explorer | Accept health/version/audit evidence and shared request identifiers. | Shared dashboards/public evidence resolve to current source commit. | Local health/version/audit only. |
| CS-EXT-05 | YNX 29 Integration | Freeze the unique cross-product schema/event/error versions and merge order. | Dependency acceptance file is signed off with central commit. | Creator contract remains a candidate. |
...
| CS-EXT-07 | YNX 28 Website | Consume `/creator-studio` metadata and deploy the public product page. | Public page, canonical URL, structured data and source-bound deployment evidence exist. | `websitePublished=false`; runtime and website states remain separate. |
...
No private key, seed, provider secret, signing material or production credential is requested. The product still has substantial autonomous runtime, security, capacity, artifact and Testnet-adapter work remaining.
