...
1. **Wallet/Auth owner:** accept client `ynx-dex-web-v1`, bundle `com.ynxweb4.dex.web`, exact scope `dex:vault:execute`, approval digest and revoke/introspection vectors.
...
7. **GitHub remote evidence:** `gh run list` returns no runs for `codex/final-dex`. The repository Artifact API returned 60 repository-wide artifacts but no DEX artifact appeared in the first 20 results; a full branch-filter retry and both GraphQL/REST Release queries hit repeated TLS handshake timeouts. CI, Release and DEX Artifact evidence therefore remain unverified.

Autonomous work remains: restore drill, supply-chain scans, SLO/capacity, unit economics, 12-language/a11y closure, concentrated liquidity, weighted pool and liquidity bootstrapping.
