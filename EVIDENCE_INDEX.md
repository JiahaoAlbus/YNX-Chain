...
| Protocol-minimum gas suggestions and strict parameter/freeze boundaries | `internal/bftgateway/fee_suggestion_test.go`, `internal/mutationfreeze/middleware.go` | `make bft-evm-fee-suggestion-check` |
| Committed zero-base-fee history, positive consensus max-gas binding, retained-range clamp and fail-closed unavailable/evidence mismatch handling | `internal/bftgateway/evm.go`, `internal/bftgateway/fee_history_test.go`, `scripts/verify/bft-evm-fee-history-check.sh` | `make bft-evm-fee-history-check` |
| Secure bounded-envelope signer and configurable ephemeral fixture funding | `cmd/ynx-consensus-tx`, `cmd/ynx-consensus-lab` | `go test ./cmd/ynx-consensus-tx ./cmd/ynx-consensus-lab` |
| Gateway post-broadcast committed block, AppHash/DataHash, gas and audited-receipt binding | `internal/bftgateway/gateway.go`, `internal/bftgateway/evm.go`, `internal/bftgateway/ethereum_transaction_test.go` | `go test ./internal/bftgateway`; `make consensus-eip1559-commit-check` |
| Four-validator EIP-1559 commit, fault recovery, backup/restore and rollback replay | `scripts/verify/consensus-eip1559-commit-check.sh`, `scripts/verify/consensus-quorum-check.sh` | `make consensus-eip1559-commit-check`; `make consensus-quorum-check` |
...
| ABCI v11 State Sync snapshot export/import and fail-closed restore | `internal/consensus/snapshot.go`, `internal/consensus/snapshot_test.go` | `make consensus-state-sync-check` |
| Local stopped-validator backup, deletion, restore and rollback replay | `scripts/verify/consensus-quorum-check.sh`, `tmp/consensus-quorum-evidence.json` under `backupRestoreRollback` | `make consensus-quorum-check` |
