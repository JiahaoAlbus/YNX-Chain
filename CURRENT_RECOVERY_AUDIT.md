...
- Chain-level fee invariants and some recovery/session management surfaces remain local-only. Smart Account registration, sponsored UserOperation execution, StrategyMandate, StrategyVault, validator delegation/unbonding/withdrawal, and derived native-liability solvency/Merkle proof queries are integrated into branch-local committed-state v11, ABCI execution/query, and BFT Gateway code with local/race evidence; none is deployed on the authoritative public runtime. The solvency API explicitly provides no external reserve attestation or redemption claim.
