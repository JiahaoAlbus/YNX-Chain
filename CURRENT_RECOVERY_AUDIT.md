...
- Chain-level fee invariants and some recovery/session management surfaces remain local-only. Smart Account registration, sponsored UserOperation execution, StrategyMandate, StrategyVault, and validator delegation/unbonding/withdrawal are integrated into branch-local committed-state v11, ABCI execution/query, and BFT Gateway code with local/race evidence; none is deployed on the authoritative public runtime.
