...
The following ingress phase is separately gated by `PUBLIC_BFT_PRODUCTION_INGRESS_APPROVED=yes`. It backs up and checksum-binds only the managed YNX Caddy/Nginx files, enables Gateway runtime readiness, replaces the eight authoritative loopback upstreams with candidate ports, validates/reloads the active ingress, and retains automatic rollback permission. Public verification remains read-only and freeze-preserving; it checks exact builds, height growth, four validators, root-path EVM compatibility, Indexer/Explorer lag, five unchanged signer identities, service health, and mutation HTTP `503`. Failure restores authoritative ingress before candidate/dependency rollback. This is implemented and dry-run/failure-injection verified, not permission to execute it.
