...
The first accepted central integration must additionally execute the vectors in `docs/economics/CROSS_PRODUCT_TEST_VECTORS.json` against the shared Testnet and attach block, transaction, receipt, API, Explorer and Monitor evidence to the exact source commit.
