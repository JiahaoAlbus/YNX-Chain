...
ENV_FILE=.env.deploy make upgrade-source-release-audit
make remote-blocker-report
YNX_UPGRADE_DEPLOY=1 ENV_FILE=.env.deploy make deploy-readiness-gate
YNX_UPGRADE_DEPLOY=1 ENV_FILE=.env.deploy make deploy-testnet
...
For an ordinary upgrade, `upgrade-source-release-audit` must pass immediately before the restricted upgrade gate. It reads all four nodes over strict SSH and proves that the currently running `/status` release, its installed release manifest, and the SHA-256 of `/usr/local/bin/ynx-chaind` agree on one source release while the evidence target is the current local `HEAD`. Upgrade mode may then tolerate only the expected source-to-target release identity differences and the target manifest not existing before installation. It does not tolerate node, SSH, service, peer, block-growth, current-source manifest, or current-source checksum failures. Use `YNX_UPGRADE_DEPLOY=1` for both the gate and the actual ordinary deployment; this flag does not authorize any BFT cutover or transaction phase.
