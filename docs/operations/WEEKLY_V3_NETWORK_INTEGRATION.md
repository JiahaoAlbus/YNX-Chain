...
| Developer/backend modules | The `internal/**/*.go` implementation scan found only the existing Faucet allowed-origin reference to these three public hosts; Developer SDK examples are covered above | Keep service-local upstream configuration and protocol-specific gateways, rather than replacing loopback service addresses with a public alias |
