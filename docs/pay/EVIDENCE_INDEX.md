...
Fresh Testnet transaction/receipt, sponsored UserOperation, authoritative refund transaction, bridge destination confirmation, accepted Quant/Data Fabric verifier key and live evidence stream, Quant Invoice v5 Testnet payment, central integration, staging/public URLs, hosted immutable artifacts, current Android/iOS install and cold launch, production signatures, store releases, CI run URLs, SBOM/provenance, migration/restore drill, load measurements, deployed telemetry and public `/pay` remain unproven.
