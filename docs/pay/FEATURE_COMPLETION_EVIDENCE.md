...
| Tip, Split and recurring draft | partial | Tip is implemented in Invoice v3 across central intent creation, merchant signature and authoritative total matching. Split is now implemented and locally race-tested as a merchant-signed plan with 2–20 immutable shares, canonical Wallet/Gateway claim, one authoritative Invoice v4 per share, expected-payer binding, wrong-payer rejection, idempotency, aggregate state and public payer redaction; central Gateway integration and fresh two-account Testnet proof remain missing. Recurring Draft is persisted, merchant-signed, bounded, idempotent and explicitly non-executing; each occurrence still requires a separate Wallet-approved Invoice, and occurrence generation is not implemented. |
