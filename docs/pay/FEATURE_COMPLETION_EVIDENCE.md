...
| Tip, split and recurring draft | partial | Tip is implemented in invoice v3 across central intent creation, merchant signature, strict Pay parsing, 12-locale review UI and authoritative total matching. Recurring Draft is persisted, merchant-signed, bounded, idempotent and explicitly non-executing; every future occurrence still requires a separate Wallet-approved invoice, and occurrence generation is not implemented. Split remains unavailable pending a central multi-recipient settlement contract |
