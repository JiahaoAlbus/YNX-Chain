...
The Bridge CI build chain also uses the repository Hardhat toolchain. Its transitive `adm-zip` dependency is pinned by npm override to version `0.6.1` under the MIT license and is checked by `make bridge-dependency-audit-check`. This dependency is build-only and is not linked into the Bridge Go binary.
