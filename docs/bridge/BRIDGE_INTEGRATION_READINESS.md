...
This is not a live bridge. External submission is hard-disabled by route validation and reported as false by health and metrics. A remote Testnet coordinator, canonical App Gateway integration, and public TLS read-only evidence surface are recorded, but they provide no public mutation route. No YNX external-chain transaction, mint, burn, native YNXT freeze/seizure, funded bridge account, production relayer custody, approved executable provider route, funded deposit/withdrawal, or stablecoin issuer support has been completed or claimed.
