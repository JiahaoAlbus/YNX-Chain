...
YNX Chain public BFT cutover is an approval-gated transaction. The transaction engine is implemented and locally failure-tested. The production driver currently implements read-only preflight, scoped backup, mutation freeze/unfreeze, read-preserving authoritative pause/resume, final snapshot export, transaction-bound candidate deploy/verify/rollback, parallel candidate dependency startup/continuity/rollback, and recovery verification. Ingress switching and public verification remain unimplemented. Running the plan target does not change services or ingress.
