...
- Last pushed commit known locally before this update: `0bab745 Add persistent EVM log filtering`; local commit `b819ff2 Add contract event log metadata` remains unpushed because GitHub HTTPS returned `LibreSSL SSL_connect: SSL_ERROR_SYSCALL`.
- Chain repo state: `/Users/huangjiahao/Desktop/YNX Chain`, branch `main`, remote `https://github.com/JiahaoAlbus/YNX-Chain.git`, changed in this update in IDE deterministic artifact metadata, simple pure/view runtime calls, EVM `eth_call`, smoke/quickstart checks, API docs, tracker, and state files.
...
- IDE compile now exposes deterministic local artifact metadata with source hash, bytecode hash, artifact hash, ABI, events, functions, compiler mode, runtime mode, verifier mode, and limitations; IDE/EVM calls can execute simple stored pure/view return literals in local devnet.
...
- Add pinned Solidity compiler/verifier integration depth or continue the next deployable service gap while GitHub push and remote deployment remain blocked.
