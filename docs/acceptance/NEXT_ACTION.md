...
- When host-key mismatches exist, `remote-blocker-report` and `deploy-readiness-gate` now also require a fresh `host-key-approval-request` artifact so stale fingerprint requests cannot be used for trusted approval.
...
- `remote-blocker-report` records freshness for the underlying remote-smoke, host-key-audit, and conditional host-key-approval-request evidence, and `deploy-readiness-gate` refuses mutation if any required source is missing or stale.
...
- Current refreshed evidence is `tmp/verify-testnet/remote-evidence.json` generated at `2026-07-09T14:08:00.836Z`, `tmp/verify-testnet/remote-blockers.json` generated at `2026-07-09T14:16:19.034Z`, `tmp/host-key-audit/host-key-audit.txt` modified at `2026-07-09T14:07:45.954Z`, and `tmp/host-key-audit/HOST_KEY_APPROVAL_REQUEST.md` modified at `2026-07-09T14:13:01.277Z`.
