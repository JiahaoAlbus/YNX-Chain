...
Current custody boundary (verified 2026-07-13): validator key/state/node files exist under owner-controlled host-local paths with mode `0600`; no private contents were read during inventory. Five distinct role-specific service signer identities now exist under owner-local mode-`0600` custody with a byte-matched recovery staging copy and non-secret public manifest SHA-256 `274583cc4a6984d795b9e6284a9f2f201b3279f81dface4ba1e4c7d5b6a297bf`. The staging copy is on the same machine and therefore does not prove offline recovery. Remote service signer files do not exist. The freeze-rehearsal approval schema requires a custody reviewer distinct from the transaction approver plus explicit validator recovery, service-signer recovery, owner handover, and rotation attestations. The generated template defaults those fields to false, and the old unapproved packet is not sufficient under the new gate.
