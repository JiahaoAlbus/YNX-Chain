...
Current single action: keep the deliberately unapproved `b638de55c4cd` freeze-rehearsal packet non-mutating while implementing and failure-injecting persistent candidate deployment, BFT-mode dependency startup/continuity, and automatic candidate/dependency rollback against the newly bound final-snapshot contract. Read-preserving authoritative pause/resume and final snapshot export are implemented and fixture/four-role-command-dry-run verified, but are newer than the live control plane. Exercise all new phases only in isolated fixtures and command dry-run until the complete control plane is deployed, read-only rehearsal is rebound, and an independent reviewer returns a completed mode-`0600` approval for the exact transaction. No BFT transaction backup, freeze, pause, snapshot, candidate deployment, dependency transition, or ingress change has run on production.
...
Current local control-plane WIP (verified 2026-07-13): optional marker-driven production pause keeps reads online, resumes height after owned-marker removal, and fails closed on marker access errors. The production driver separately gates pause and final snapshot, requires exact transaction/commit/release and four-role freeze evidence, rejects foreign/tampered evidence, and validates the downloaded migration state. `go test ./cmd/ynx-chaind ./cmd/ynx-consensus-package ./internal/chain`, `make public-bft-production-driver-check`, `make consensus-production-package-check`, and `make deploy-dry-run` pass. This WIP is not deployed and no live pause/snapshot claim is made.
