...
Current single action: keep the deliberately unapproved `b638de55c4cd` freeze-rehearsal packet non-mutating while finishing the full local gate and committing/pushing the now-complete transaction control plane, then deploy only the ordinary authoritative release/control-plane update and rerun four-role convergence plus the non-mutating read-only rehearsal. All named BFT transaction forward/reverse phases are implemented and fixture/four-role-command-dry-run failure-tested, including checksummed ingress switch/restore and public threshold verification, but are newer than the live control plane. Do not execute freeze, pause, snapshot, candidate, dependencies, ingress, or public cutover until the exact deployed control-plane evidence is rebound and an independent reviewer returns a completed mode-`0600` approval. No BFT transaction mutation phase has run on production.
...
Current local control-plane WIP (verified 2026-07-13): all named transaction phases now exist. They bind transaction/commit/release, final snapshot, validator manifest/genesis, candidate package, signer-file inputs, dependency migration continuity, checksummed ingress files, exact public builds, height/four-validator identity, root-path EVM, Indexer/Explorer lag, unchanged signers, service health, freeze state, and automatic rollback evidence. Ten phase failures restore the modeled authoritative baseline; candidate/dependency/ingress command paths pass dry-run and focused failure fixtures. The broader full-suite gates must still be rerun before commit. This WIP is not deployed and no live transaction, ingress, or public BFT claim is made.
...
- ordinary deployment/rehearsal evidence and acceptance state only; do not add new cutover behavior unless a failing gate exposes a real defect.
