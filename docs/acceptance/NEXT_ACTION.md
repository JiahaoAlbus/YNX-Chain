...
Current single action: keep the deliberately unapproved `b638de55c4cd` freeze-rehearsal packet non-mutating while implementing and failure-injecting transaction-bound ingress backup/switch/restore, public continuity verification, and automatic rollback thresholds after the verified parallel candidate dependencies. Read-preserving authoritative pause/resume, final snapshot export, exact-transaction candidate deploy/verify/rollback, and parallel BFT Gateway/Indexer/Explorer/Faucet/AI/Pay/Trust/Resource startup/continuity/rollback are implemented and fixture/four-role-command-dry-run verified, but are newer than the live control plane. Exercise all new phases only in isolated fixtures and command dry-run until the complete control plane is deployed, read-only rehearsal is rebound, and an independent reviewer returns a completed mode-`0600` approval for the exact transaction. No BFT transaction backup, freeze, pause, snapshot, candidate deployment, dependency transition, or ingress change has run on production.
...
Current local control-plane WIP (verified 2026-07-13): optional marker-driven production pause keeps reads online, resumes height after owned-marker removal, and fails closed on marker access errors. The production driver separately gates pause, final snapshot, candidate deployment, and dependency startup; binds the exact transaction/commit/release, snapshot SHA/height/hash/state, approval-selected validator-manifest SHA/genesis time, generated package, signer-file inputs, and automatic rollback evidence; rejects tampered or partial reuse; verifies four-role candidate commands; starts exact-current-build dependencies on private loopback ports; enforces migration-parent and Indexer/Explorer continuity; and makes candidate/dependency rollback idempotent. Focused tests and production-driver/package checks pass for this slice; the broader full-suite gates must still be rerun before commit. This WIP is not deployed and no live pause/snapshot/candidate/dependency claim is made.
...
- production driver and service templates under `scripts/deploy`, `scripts/ops`, and `scripts/verify` for checksummed ingress backup/switch/restore, public verification, and automatic rollback thresholds.
