...
Current single action: after public ingress stabilizes and one fresh read-only evidence package passes, deploy the exact follower-replication, snapshot-v2 integrity, and fault-monitoring release through the ordinary authoritative path, followers first and primary last, then prove v2 persistence, fresh exact convergence, and alertable recovery on all three followers before and after one bounded follower restart.
