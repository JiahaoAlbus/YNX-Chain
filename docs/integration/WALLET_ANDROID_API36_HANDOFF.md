...
Follow-up commit `074c1722089eea90168c841367d4b3a7ec42ff6d` closes the remaining switch-versus-unlock window. A synchronous exclusive gate rejects rapid selection reentry before React renders, the Locked screen disables Unlock and recovery while selection persistence is in flight, and the repository revalidates the exact foreground/lock epoch immediately before writing the selected account. Background, explicit lock or privacy invalidation preserves the prior manifest and remains locked. Verification passed the focused switch/repository 15/15 suite, Wallet 94/94, all Wallet gates and a current-source 2,746-module Hermes export. Device and release booleans remain false.
...
## Generated recovery-key display checkpoint

Commit `119c7fc9e8e863fab25c922b33dc8a3d76de97fa` prevents the Backup Sheet from rendering a newly generated recovery key until its dedicated screenshot-protection promise succeeds and the sheet is still active. Display eligibility is bound to the exact current secret, so a completed protection attempt for an older value cannot reveal a replacement value. Protection failure or dismissal keeps the key hidden and exposes only a close action.
...
## Authorization callback OS-handoff checkpoint

Commit `8bb69ed1269bc3fece4ad024aaf3303bffe7cac7` distinguishes the last active-Wallet check from the irreversible OS callback handoff. Generation, exact account and expiry remain mandatory immediately before `Linking.openURL`; an invalid lifecycle or failed OS open records no returned callback. Once the OS accepts the handoff, the target App may legitimately background Wallet, so `approval-returned` is appended without requiring the now-unmounted Modal. The audit state machine still requires the persisted approval intent and its exact account, and an audit-storage failure does not call the completion callback.

This supersedes only the earlier post-handoff Modal-lifecycle assertion documented in the audit lifecycle checkpoint. Approval intent, explicit rejection, replay persistence and every pre-handoff check remain guarded at their storage or OS linearization point. Verification passed the focused callback/audit 13/13 suite, Wallet 97/97, typecheck, product check, Social harness contract check, release-content check, full-goal coverage and `git diff --check`. A mode-0700 current-source Android Hermes export passed with 2,747 modules and all temporary links and export files were removed. No device command or interaction was performed, so `testedOnDevice`, `installedLocal`, `downloadHosted`, native-app `deployedPublic`, `productionSigned` and `storeReleased` remain false for this exact source.
...
## Android exact-package callback checkpoint

Commit `f6c7e6aaf7787cca74f6a0b48b3fa49ac1c0d57b` prevents Android custom-scheme callback hijacking across the canonical authorization, Exchange, Quant, Developer and DEX return paths. The Wallet revalidates the exact registered callback URL and its sole response parameter, then dispatches a native explicit `ACTION_VIEW` intent constrained with `Intent.setPackage` to the registry-bound Android package. A missing bridge, invalid package identifier, unavailable target package, ambiguous callback or insecure HTTP route fails closed; Android custom schemes never fall back to the generic resolver. HTTPS callbacks and non-Android platforms retain their canonical platform handoff.

Verification passed the focused callback policy 4/4 suite, Wallet 101/101, typecheck, product check, Social harness contract check, release-content check, full-goal coverage and `git diff --check`. `:app:compileDebugKotlin` passed offline with compileSdk/targetSdk 36, and a mode-0700 current-source Android Hermes export passed with 2,748 modules; all temporary links and export files were removed. These are source, compiler and local-test facts only. No device command or interaction was performed, so `testedOnDevice`, `installedLocal`, `downloadHosted`, native-app `deployedPublic`, `productionSigned` and `storeReleased` remain false for this exact source.

## In-flight biometric cancellation checkpoint

Commit `8adb49894c50b423b737408423061a5374ff2330` gives every private-key biometric attempt a process-local generation and native-prompt token. Lifecycle cancellation before hardware/enrollment checks finish prevents the prompt from opening; cancellation while the Android prompt is active invokes Expo's native `cancelAuthenticate`, and the generation check still rejects any late success returned after cancellation. An older prompt's cleanup cannot clear or authorize a newer attempt.

Backgrounding, explicit lock, repository reconstruction, account switch, screenshot-protection failure, every native Modal system dismissal and every Sheet close now cancel the active biometric generation. Verification passed the focused biometric policy 6/6 suite, Wallet 104/104, typecheck, product check, Social harness contract check, release-content check, full-goal coverage and `git diff --check`. A mode-0700 current-source Android Hermes export passed with 2,749 modules and all temporary links and export files were removed. No device command or interaction was performed, so `testedOnDevice`, `installedLocal`, `downloadHosted`, native-app `deployedPublic`, `productionSigned` and `storeReleased` remain false for this exact source.

## Temporary signing-key byte zeroing checkpoint

Commit `c1568cc2585426cda0a7705fec3766581323fc42` routes every mutable 32-byte private-key decode through one synchronous `withSecretBytes` boundary. Native identity, canonical authorization, Signed Intent, native transfer, Exchange, Quant, Developer and DEX signing now overwrite the decoded `Uint8Array` in `finally` after success or exception; an allocated out-of-range key buffer is also wiped before rejection. Tests retain a reference to the temporary buffer and prove all bytes are zero after both a successful operation and a thrown operation.

This checkpoint does not claim that the immutable JavaScript string returned by SecureStore can be overwritten in place. It reduces the lifetime of actual mutable decoded key material without making a false heap-erasure claim. Verification passed the focused signing-domain 22/22 suite, Wallet/Auth 115/115 including the temporarily compiled real Developer Solidity fixture, Wallet current-source 104/104, typecheck, every Wallet gate and `git diff --check`. A mode-0700 current-source Android Hermes export passed with 2,749 modules. The solc 0.8.24 fixture artifact, dependency directory, current-source overlays and all temporary links were removed. No device command or interaction was performed, so exact-source device and release booleans remain false.

Follow-up commit `dd5a4108bab2f926118d3f3147829492328708cc` synchronously cancels any in-flight biometric generation before an active-foreground deep link is parsed or allowed to replace the current review. A replacement authorization/action link, safe launcher or malformed link therefore cannot leave the prior native prompt alive; a late result remains invalid even if parsing the replacement fails. Verification passed the focused lifecycle 4/4 suite, Wallet current-source 104/104, all Wallet gates and a fresh 2,749-module Hermes export. Temporary overlays were removed, no device command was used and exact-source device/release booleans remain false.

## Recoverable action callback checkpoint

Commit `3e98aedc17503329d9e8f3b58bac3cfddcb3c1ea` makes Exchange, Quant, Developer and DEX callback delivery recoverable across OS-open failure and process reconstruction without signing twice. Before the irreversible OS handoff, the Wallet atomically persists the exact signed response as pending together with its action domain, account, bundle, callback and expiry binding. An exact retry resumes that response; binding substitution, expiry, a delivered record or a legacy consumed replay marker fails closed. Successful OS handoff converts the record to delivered and clears the stored response payload, while failed handoff leaves it pending for recovery.

Reconstruction strips expired pending response payloads while retaining terminal replay protection. Verification passed the focused callback/replay 16/16 suite, Wallet current-source 114/114, typecheck, product check, Social harness contract check, release-content check, full-goal coverage and `git diff --check`. A mode-0700 current-source Android Hermes export passed with 2,750 modules and all temporary overlays and export files were removed. No device command or interaction was performed, so `testedOnDevice`, `installedLocal`, `downloadHosted`, native-app `deployedPublic`, `productionSigned` and `storeReleased` remain false for this exact source.

## Recoverable canonical authorization callback checkpoint

Commit `bc2a0712e0dc828e3c157eff0e703e236bed48fc` replaces the canonical authorization flow's consume-before-sign nonce window with a persistent pending-to-delivered callback state machine. The exact signed response is staged once with its request digest, nonce, selected account, bundle, callback and expiry binding. A process restart or failed OS open resumes that response without another secret read or signature; a delivered record, same nonce under another digest, legacy consumed nonce, account drift, route substitution, expiry or storage tamper fails closed.

Approval intent and callback-returned audit transitions are now idempotent for the exact digest and account, allowing recovery at either audit boundary without duplicating or weakening the hash chain. Reconstruction clears expired pending response payloads while preserving terminal replay. Verification passed the focused authorization callback/audit/replay 24/24 suite, Wallet current-source 124/124, typecheck, product check, Social harness contract check, release-content check, full-goal coverage and `git diff --check`. A mode-0700 current-source Android Hermes export passed with 2,750 modules and all temporary overlays and export files were removed. No device command or interaction was performed, so `testedOnDevice`, `installedLocal`, `downloadHosted`, native-app `deployedPublic`, `productionSigned` and `storeReleased` remain false for this exact source.

## Terminal authorization rejection checkpoint

Commit `17241f7771cde88c4ca78ba57db39b68d4850f5f` makes explicit rejection terminal across every recoverable authorization callback boundary. Reject first converts an existing pending response to a delivered tombstone and clears its payload, or creates a response-free tombstone when approval never started. It then idempotently ensures the exact request-rejected audit record. A crash before either boundary can only resume the same rejection; the rejected request cannot sign, reopen or deliver a callback afterward.

Verification passed the focused authorization callback/audit/replay 28/28 suite, Wallet current-source 128/128, typecheck, product check, Social harness contract check, release-content check, full-goal coverage and `git diff --check`. A fresh mode-0700 current-source Android Hermes export passed with 2,750 modules and all temporary overlays and export files were removed. No device command or interaction was performed for this source checkpoint, so device, install, hosted-download, native public-deployment, production-signing and store-release gates remain false pending a separate direct API36 run.
