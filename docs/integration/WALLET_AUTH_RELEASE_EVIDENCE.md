...
The frozen checkpoints are Core `0c747c6030b5a475a1f12dc7e57345555c23055d`, Web `60614bf5e7575c7423a0f8547b5197675f449ffc`, Android `f1edbec46ad4300beec406873b03db2da7c72e4b`, iOS/macOS `7a3a110fb7cd9a33512ce4db87ddd8387cb730d5`, and Desktop/CLI/SDK `2802876f8470264c4a8819f1426e28f957a09289`. Android `9f91b1f5…` is source-only pending: old certificate `40e5ac…ae54` and new certificate `bd03ab…9c24` are discontinuous, so the new APK cannot reuse the old SecureStore/device evidence. Web remains unhosted. iOS recovery remains pending. The macOS arm64 CLI is the sole official YNX-domain download.
...
External Chain Core contract version `1.21.0` is identity-bound at implementation `9468a771b46f50e0e12b7567d7aa51a2f95b4e36` and contract commit `cefb37144517e8f44fd9d0b41119bb5754bdb55d`. Direct Owner object readback verified parent `9468a771…`, tree `cb64ea79…`, contract blob `2ab1e66e…`, and content SHA-256 `94f0fc81…`. This is dependency-contract acceptance only: Wallet/Auth remains Product Session authority, Chain Core remains `dependency-not-accepted` and fail-closed with parallel Auth forbidden, and no source integration or public deployment gate is promoted.

Product Session Router v2 commit `e5932a2eb0e5c01ca31a1ba6e03f9872ccf0ef7f` has remote ref, parent, tree and seven changed blobs read back. Its public probe is intentionally state-free: App Gateway health is HTTP 200, invalid legacy v1 completion reaches the Wallet handler with HTTP 400, while invalid v2 challenge falls through to Chain JSON-RPC with HTTP 200 / `-32601`. The added probe has 6/6 tests, one stable request ID, three bounded transport attempts and a 64 KiB response cap; the official run exits 1 with `UNEXPECTED_HTTP_STATUS`. Thus public reachability is true but v2 handler mount, challenge/session creation, full lifecycle, central integration and public v2 deployment are false. The hosted `d8863f6b…` source package does not contain the current gate and is not runtime deployment evidence.
