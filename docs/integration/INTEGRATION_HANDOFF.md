...
- The accepted dependency baseline locks Hardhat 3.11.1 and `@nomicfoundation/hardhat-ethers` 4.0.15; the current remediation raises `adm-zip` to 0.6.1 and `undici` to 6.28.1. Full and production-only npm audits are clean, so no audit exception is retained. Security/SRE acceptance remains an independent production-release gate.
