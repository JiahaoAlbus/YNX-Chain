...
Core/Auth `bb2d9d4f…` extends the explicit Web-only `wallet-web-companion` registry candidate (`ynx-wallet-web-companion-v1`, bundle `web.ynx.wallet.companion`, exact HTTPS callback) with sender-constrained authoritative revoke before protected-state deletion, replay zero-mutation rejection, Gateway restart restore, expiry fail-close and revoke-outage retention for Retry. Wrong product/bundle/callback/scope and rejection remain fail-closed. The approved/enabled registry row and local 318/318 suite are not deployed truth: migration, public Gateway load, callback route, visible Web/mobile lifecycle, central integration and aggregate public remain false.
