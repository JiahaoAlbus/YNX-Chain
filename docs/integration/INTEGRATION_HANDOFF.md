...
Core/Auth `39c80021…` closes the approval-callback versus disconnect resurrection race for the explicit Web-only `wallet-web-companion`: disconnect waits for the active callback, sender-constrained revokes the issued binding exactly once, clears protected state and finishes disconnected without session resurrection. The inherited callback/disconnect/restart/expiry/replay gates remain local. The approved/enabled registry row and local 322/322 suite are not deployed truth: migration, public Gateway load, callback route, visible Web/mobile lifecycle, central integration and aggregate public remain false.
