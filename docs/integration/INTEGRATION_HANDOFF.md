...
The current consumed checkpoints are Core `0c747c6030b5a475a1f12dc7e57345555c23055d`, Web `60614bf5e7575c7423a0f8547b5197675f449ffc`, Android `f1edbec46ad4300beec406873b03db2da7c72e4b`, iOS/macOS `7a3a110fb7cd9a33512ce4db87ddd8387cb730d5`, and Desktop/CLI/SDK `2802876f8470264c4a8819f1426e28f957a09289`. Android source `9f91b1f5…` is pending: the new APK identity cannot preserve the destroyed one-time-key AVD SecureStore, and no replacement-device validation exists. iOS recovery fix remains in progress. Web preview aggregation remains false and awaits terminal visible-download evidence. macOS arm64 CLI remains the sole official YNX download.

Web `60614bf5…` directly verifies the official Firefox 153.0.4 DMG at 154,770,807 bytes / SHA-256 `792b313c7e2d2b1327f76f455315dd9d68c59c96935aa5e8c67ae42fe6aeea97`, universal x86_64+arm64, Mozilla Developer ID `43AQ936H96`, and notarized Gatekeeper acceptance. In an isolated disposable profile, an unsigned temporary YNX add-on launched twice with the same UUID (PIDs 53579/53723). This advances only extension cold/second launch. Browser signing does not sign the extension; installedLocal, popup DOM/background, Chrome/Edge runtime, provider/account/sign/tx, official hosting/publication and store remain false.
...
Product Session Router v2 Owner commit `e5932a2eb0e5c01ca31a1ba6e03f9872ccf0ef7f` is separately frozen as a negative public routing checkpoint and supersedes `739fca33…`. The legacy v1 Wallet route reaches its canonical handler, but `/v2/product-sessions/challenge` currently returns Chain JSON-RPC `-32601`; the host-neutral `ProductSessionGatewayHttpHandler` is not mounted ahead of the Chain catch-all. Its bounded executable mount probe has 6/6 tests and currently exits 1 with `UNEXPECTED_HTTP_STATUS`; after route mount it must run before lifecycle acceptance, but it proves only canonical handler reachability. Central deployment remains a required external transaction and must prove persisted schema-v2 state, candidate preflight, atomic switch/rollback and the complete challenge→approval/device completion→introspection→replay rejection→revoke→restart/network-loss lifecycle. This Integration slice changes no Gateway/Chain implementation path and claims neither central integration nor public v2 deployment.
