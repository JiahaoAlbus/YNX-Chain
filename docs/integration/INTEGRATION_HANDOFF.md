...
Envelope v2 may carry optional `chainCommitmentId` as an external Chain Core Bulk Data Commitment reference. Data Fabric consumes frozen Chain Core contract v1.29.0, implementation `1571d6889b195daf476151474ce67889acdd9712`, contract commit `f0bc3c98d55976f33525ab6eed32403937b9b9c5`, from the verified 138,646,383-byte v29 bundle with SHA-256 `2543de0458e500880cc5d34f1cb847e7820f285f121e270cc28b7ddda882d8c2`. It accepts only an exact successful `GET /data/commitments/{id}` read from `ynx-consensus-abci` at `abci-state-v14`. Missing, unavailable or contradictory verification rejects before storage. Data Fabric neither computes the commitment ID nor accepts a write owner; Chain Core derives owner from the transaction signer. Raw content, plaintext metadata, recipient lists, access tokens and key material remain off-chain. v29 leaves Bulk Data Commitment, Wallet/Product Session and strategy-action public semantics unchanged and retains the v28 exclusive validator process lease. It adds ValidatorVoteSigner: canonical proposal and exact validator Ed25519 identity validation before durable SafetyStore authorization, persistence before external `crypto.Signer`, and local verification before returning a vote. A signer failure consumes the view and same-view retry fails closed. External signer custody is not owned or claimed. The integration contract advances to 104 vectors. This is not Chain Core real-network partition, packet-loss, DDoS, State Sync, CometBFT/public differential replay, TLC/Apalache, WAN soak, external signer custody, central integration or public promotion evidence; all remain false.
