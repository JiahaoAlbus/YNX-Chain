...
Envelope v2 may carry optional `chainCommitmentId` as an external Chain Core Bulk Data Commitment reference. Data Fabric consumes frozen Chain Core contract v1.31.0, implementation `3af591a2fe110b953da6b109580738bf894a4852`, contract commit `ee45c0700179addadfe1b9c845d2b1b475eea12a`, from the verified 143,353,813-byte v31 bundle with SHA-256 `67aaeed8a0b416996424ee0f553f5bf321ca15447b5474b911181d5e698dc6aa`. It accepts only an exact successful `GET /data/commitments/{id}` read from `ynx-consensus-abci` at `abci-state-v14`; contradiction or unavailability rejects before storage. Data Fabric neither computes the ID nor accepts a write owner, and raw content, recipient lists, tokens, keys and private signer material remain off-chain. v31 leaves Bulk Data Commitment, owner/finality and state/application semantics unchanged. Its external monotonic validator SafetyRollbackAnchor performs CAS of the complete authenticated envelope before local replacement, verifies exact readback and recovers from anchor-ahead crash or older local snapshot replay. Timeout, conflict, tamper and unsafe recovery fail closed. Production anchor authority, remote recovery drill, signer custody, central integration and public deployment remain false/unprovided. The contract has 106 vectors.
