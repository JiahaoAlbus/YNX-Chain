...
Core/Auth `8a7accfe…` extends the explicit Web-only `wallet-web-companion` registry candidate (`ynx-wallet-web-companion-v1`, bundle `web.ynx.wallet.companion`, exact HTTPS callback) with single-flight concurrent disconnect: both callers receive the same disconnected result, exactly one sender-constrained authoritative revoke occurs, and protected state is cleared only after confirmation. The inherited restart restore, expiry fail-close, replay zero-mutation and outage-retention gates remain local. The approved/enabled registry row and local 319/319 suite are not deployed truth: migration, public Gateway load, callback route, visible Web/mobile lifecycle, central integration and aggregate public remain false.
