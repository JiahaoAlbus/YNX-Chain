...
Core/Auth `9e753b33…` extends the explicit Web-only `wallet-web-companion` registry candidate (`ynx-wallet-web-companion-v1`, bundle `web.ynx.wallet.companion`, exact HTTPS callback) with single-flight approval callbacks: identical concurrent callbacks share one challenge/complete/introspect lifecycle, while a distinct callback during verification fails closed with `CONCURRENT_CALLBACK` and cannot join the valid approval. The inherited disconnect/restart/expiry/replay gates remain local. The approved/enabled registry row and local 321/321 suite are not deployed truth: migration, public Gateway load, callback route, visible Web/mobile lifecycle, central integration and aggregate public remain false.
