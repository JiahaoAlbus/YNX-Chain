...
Envelope v2 may carry optional `chainCommitmentId` as an external Chain Core Bulk Data Commitment reference. Data Fabric consumes frozen Chain Core contract v1.23.0, implementation `23f9ce3d32e614aa2f0a80664bbf1d8f8f85e7a5`, contract commit `d26592571e78ae44fc76f2270a21eaf8bd62eaa4`, from the verified 57,861,962-byte v23 bundle with SHA-256 `306b47ce4fc92e3564fbe22b5439c1e1039d6bf0f8f6e81a8dffe4549c89c182`. It accepts only an exact successful `GET /data/commitments/{id}` read from `ynx-consensus-abci` at `abci-state-v14`. Missing, unavailable or contradictory verification rejects before storage. Data Fabric neither computes the commitment ID nor accepts a write owner; Chain Core derives owner from the transaction signer. Raw content, plaintext metadata, recipient lists, access tokens and key material remain off-chain. v23 does not change the Data Commitment ID, owner, version-chain, finality, consensus or v22 strategy-action semantics. It adds Chain Core-owned workspace-isolated reproducible solc 0.8.24 builds for nine Solidity sources and nine selector artifacts, and advances the integration contract to 98 vectors. The bundle's public/deployed/integratedCentral booleans remain false.
