...
Envelope v2 may carry optional `chainCommitmentId` as an external Chain Core Bulk Data Commitment reference. Data Fabric consumes frozen Chain Core contract v1.32.0, implementation `1974dba384a2f0ac1124f4de7025f772fe94bd03`, contract commit `93a95c0005e09ba21f61245f0e001e2acf4a1080`, from the verified 146,122,768-byte v32 bundle with SHA-256 `666118d2fce9d006341142616f03dcf7641748ad3f430d1f3a5f360bc965cbd3`. It accepts only an exact successful `GET /data/commitments/{id}` read from `ynx-consensus-abci` at `abci-state-v14`; contradiction or unavailability rejects before storage. Data Fabric neither computes the ID nor accepts a write owner, and raw content, recipient lists, tokens, keys and private signer material remain off-chain. v32 leaves Bulk Data Commitment, owner/finality and state/application semantics unchanged. Its external monotonic validator SafetyRollbackAnchor keeps the existing complete-envelope CAS/readback/recovery semantics and adds a TLS 1.3 mTLS HTTPS client that rejects environment proxies, redirects, unsafe responses and mismatched committed records. Production endpoint, production CA/client credentials, independent anchor authority deployment, remote recovery drill, signer custody, central integration and public deployment remain false/unprovided. The contract has 107 vectors.
