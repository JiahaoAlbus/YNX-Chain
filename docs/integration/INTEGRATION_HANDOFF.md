...
Wallet/Auth `cdf56db7…` freezes `createProductWalletConnection` as the only product-facing factory. Products cannot inject callback, origin, session, Wallet URL, predictable token factory or clock; 120 concurrent factory nonce/state pairs were unique. `beginLegacyYNX` accepts only a same-product/platform registered legacy callback and opens the registry-derived canonical route. This remains local SDK truth: 0/12 runtimes are migrated, PR CI is not green, 6439 is still undeployed/404, and visible platform, central and aggregate public gates remain false.

Core/Auth `a46d5456…` adds the explicit Web-only `wallet-web-companion` registry candidate (`ynx-wallet-web-companion-v1`, bundle `web.ynx.wallet.companion`, exact HTTPS callback) and a real local handler/fetch-adapter challenge→complete→introspect test. Wrong product/bundle/callback/scope and rejection remain fail-closed. The approved/enabled registry row is not deployed truth: migration, public Gateway load, callback route, visible Web/mobile lifecycle, central integration and aggregate public remain false.
