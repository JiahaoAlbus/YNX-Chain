...
Envelope v2 may carry optional `chainCommitmentId` as an external Chain Core Bulk Data Commitment reference. Data Fabric consumes frozen Chain Core contract v1.34.0, implementation `c3ddcec34234d1ac575a417a03f937c2bff22a4f`, contract commit `1c50fc3ad987b5dd6df54530c330aff9add34e26`, from the verified 1,482,719,301-byte v34 bundle with SHA-256 `d5f2aa10307075c48f6e0a16c3a69d34d7b6aefbce9cada48606f3fb2c1b76ff`. It accepts only an exact successful `GET /data/commitments/{id}` read from `ynx-consensus-abci` at `abci-state-v14`; contradiction or unavailability rejects before storage. Data Fabric neither computes the ID nor accepts a write owner, and raw content, recipient lists, tokens, keys and private signer material remain off-chain. v34 leaves Bulk Data Commitment, owner/finality and state/application semantics unchanged. It adds only verified-client TLS 1.3 safety-anchor health, version and metrics observability, fail-closed redacted dependency failure and fixed-cardinality metrics without validator identity, certificate-fingerprint or dynamic-path leakage. Repository evidence remains local only: real PostgreSQL, production database/certificates, independent deployment, remote recovery, public validator deployment, WAN/soak, central integration and public cutover remain false/unprovided. The contract has 109 vectors.
