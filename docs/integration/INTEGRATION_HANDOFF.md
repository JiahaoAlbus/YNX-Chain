...
Envelope v2 may carry optional `chainCommitmentId` as an external Chain Core Bulk Data Commitment reference. Data Fabric consumes frozen Chain Core contract v1.30.0, implementation `aab0b6b9fc28b40ef8715214cd95360734db4c48`, contract commit `3bcae14f9d42fd2fbcce2211a7c51a737f8ed464`, from the verified 141,977,791-byte v30 bundle with SHA-256 `496032529043ea2f589dff6ad36ab8fb813399562291baca32ccca8358ce8162`. It accepts only an exact successful `GET /data/commitments/{id}` read from `ynx-consensus-abci` at `abci-state-v14`. Missing, unavailable or contradictory verification rejects before storage. Data Fabric neither computes the commitment ID nor accepts a write owner; Chain Core derives owner from the transaction signer. Raw content, plaintext metadata, recipient lists, access tokens and key material remain off-chain. v30 leaves Bulk Data Commitment, Wallet/Product Session and strategy-action public semantics unchanged. Authenticated SafetyStore v2 separates checksum and seal domains; its Ed25519 seal binds Chain ID, exact state, checksum and the configured validator key, and ValidatorVoteSigner requires that store. Key mismatch, legacy unsealed state, checksum recomputation without a valid seal, invalid signer output and seal failure fail closed. An older validly signed snapshot can still replay without a trusted external monotonic rollback anchor; that anchor and production signer custody are not provided. The integration contract has 105 vectors. This is not central integration or public promotion evidence; both remain false.
