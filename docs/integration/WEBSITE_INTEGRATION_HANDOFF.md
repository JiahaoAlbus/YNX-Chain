...
Metadata source: `release/calendar/public-product-metadata.json`
...
Website must consume the exact feature, FAQ, risk, locale, screenshot and structured-data fields from `release/calendar/public-product-metadata.json`. It must not infer that a passing local build is a deployment or that an older preview package represents the current source.
