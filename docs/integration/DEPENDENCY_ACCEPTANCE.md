...
A dependency may change to accepted only when its evidence is bound to a source commit and directly passes the cross-product vectors in `docs/integration/CROSS_PRODUCT_TEST_VECTORS.json`. A provider webhook, manual operator assertion, static page, or mock response is not acceptance evidence.
