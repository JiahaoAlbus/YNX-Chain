...
| YNX Chain Core | Frozen Bulk Data Commitment v1 reference reads plus finalized chain observations | Contract v1.25.0, implementation `503e06d06ca3f45eb189a5eafbfbf17c42df4fa6`, contract commit `8f385d975cd4e64d87fbb02b268e8d969e64d05c`; 84,361,500-byte v25 bundle SHA-256 `ea1f144accefe36844260cf2c6fdb68374e0971c1dbb592a6ea63bd3bdeec8ed` verified; Envelope v2 fail-closed adapter and positive/negative local tests | Contract consumed locally; shared Testnet pending | Run exact committed/unavailable/mismatch vectors against the accepted Gateway and retain owner-issued receipts |
...
The YNX Pay BFT source-to-Ledger path and the Chain Core Bulk Data Commitment reference boundary have executable local evidence. Chain Core v1.25.0 preserves the Bulk Data Commitment, Wallet/Product Session, strategy-action and prior build/quorum semantics while adding candidate-engine-only execution replay across 128 traces, 12,288 generated transactions and 2,048 executions using sequential plus 1/2/4/8/16 workers and three input orders, plus a 100-vector integration contract. Data Fabric records that additive owner evidence but does not redefine it. CometBFT/public differential replay, independent TLC/Apalache, WAN soak, central integration and promotion remain false. The Data Fabric boundary is deliberately read-only: `chainCommitmentId` is an external reference, owner is not accepted from an event, and raw content, recipient lists, access tokens and key material remain off-chain. Neither path is **centrally accepted** or **shared-Testnet verified**.
