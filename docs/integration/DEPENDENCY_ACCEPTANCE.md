...
| YNX Chain Core | Frozen Bulk Data Commitment v1 reference reads plus finalized chain observations | Contract v1.33.0, implementation `78fc2a9df2304cf8509a4c90aa7856cc748e3b57`, contract commit `00f3daa83355b8a482af5d9240e78ebd91dd7995`; 159,859,137-byte v33 bundle SHA-256 `3b31810077b008d3490de91d26a8b70b385d73241db6664dabac5f17a616d772` verified; Envelope v2 fail-closed adapter and positive/negative local tests | Contract consumed locally; shared Testnet pending | Run exact committed/unavailable/mismatch vectors against the accepted Gateway and retain owner-issued receipts |
...
The YNX Pay BFT source-to-Ledger path and the Chain Core Bulk Data Commitment reference boundary have executable local evidence. Chain Core v1.33.0 preserves the Bulk Data Commitment, Wallet/Product Session, strategy-action and authenticated validator store/vote-signer semantics. Its external monotonic SafetyRollbackAnchor protocol and v32 TLS 1.3 mTLS client boundary are unchanged. v33 adds the server-side external authority boundary: verified TLS 1.3 clients, certificate fingerprint binding to one chain/validator, strict bounded HTTP CAS, a single-statement PostgreSQL `INSERT`/`UPDATE RETURNING` CAS repository, deployable command, SQL migration, hardened systemd unit and runbook. Repository tests use a local fake SQL driver: no real PostgreSQL integration, production database or certificates, independently deployed authority, remote recovery drill, public validator deployment, WAN/soak or public cutover is proven. The contract has 108 vectors. Data Fabric records that additive owner evidence but does not redefine it. Chain Core central integration and promotion remain false. The Data Fabric boundary remains read-only and keeps raw content, recipient lists, access tokens and key material off-chain.
