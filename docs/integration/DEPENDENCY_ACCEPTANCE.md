...
|---|---|---|---|---|
| 02 Wallet/Auth | Product registry, exact bundle/callback, device challenge, session verification/introspection, recovery, expiry, revoke | Local remote-verifier adapter and replay/tamper tests pass | Pending owner acceptance | Accepted central commit, deployed health/version, and shared vectors for wrong product/bundle/device/scope, expiry, recovery, and revoke |
...
| 28 Website | Consume `/calendar` metadata, status, downloads, FAQ and structured data | Calendar public package pending in this slice | Pending handoff | Website commit consuming exact metadata plus public route and canonical/status probes |
...
| 30 Security/SRE | Threat, secret/dependency/license scans, SBOM/provenance, artifact/install/public gates | Calendar local boundary and build tests pass; release-grade package missing | Pending acceptance | Current-source artifact set, scan results, SBOM/provenance, immutable hashes, install/cold-start evidence, and no unresolved Critical/High findings |
...
| 13 Monitor | Health/ready/version, metrics, traces, SLO and alerts | Only local health and smoke proof exist | Not started | Structured telemetry, Monitor contract, alert tests, and status linkage |
