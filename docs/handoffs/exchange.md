...
Tournament stages are Baseline, Candidate, Champion, Challenger, Rejected and Retired. Paper Broker state is persistent, fixed point, reconciled and kill-switch protected. Reconciliation mismatch activates a persistent audited kill switch; restart does not clear it. The Testnet Broker uses the Exchange's exact canonical Wallet-signed mandate payload, binds the strategy hash through its nonce domain and enforces market, expiry, capital, position and daily-loss limits. Every order has a separate Wallet signature. The stateless HTTP adapter forwards each user's own ephemeral Exchange session, verifies authoritative response identity/fields and persists a broker proof; concurrent broker calls are tested without weakening exclusive kill/reconcile transitions. Missing Exchange configuration or authorization fails closed. Third-party strategies cannot directly call execution, and live funds stay structurally disabled.
