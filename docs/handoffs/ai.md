...
## 2026-08-02 public Testnet update

- The Web product is publicly available at `https://assistant.ynxweb4.com/` with
  liveness, readiness, and redacted provider status endpoints.
- The deployed Gateway accepts the strict `POST /ai/stream` JSON/SSE contract.
  Its latest bounded provider-backed generation attempt returned HTTP 429
  `provider_rate_limited`; no substitute answer was emitted and
  `generationLive=false` remains authoritative.
- A 25-worker remote check completed 100/100 successful requests for each of
  `/healthz`, `/api/public-status`, and the Web shell. This is bounded public
  route evidence, not a provider-stream or production-scale SLO.
- Canonical Wallet acceptance, hosted native downloads, iOS runtime evidence,
  production signing, store release, and central integration remain pending.
- The current release evidence is recorded in `apps/ai/product-release.json`
  and `apps/ai/evidence-index.json` at source commit
  `16d6d71e2f93418e37fe9d024f58c492ca1baad4`.
...
The repository-level `make ai-gateway-check` was attempted three times during
the earlier local checkpoint. The
...
2. **Central Gateway acceptance:** source commit
   ...
   errors, preserves Provider 429, and redacts upstream bodies. The contract is
   now deployed, but central owner acceptance remains pending; the latest real
   provider generation attempt returned HTTP 429. `integratedCentral` and
   `generationLive` therefore remain false.
