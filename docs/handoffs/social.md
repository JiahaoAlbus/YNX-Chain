...
### 4. 隐私/安全闭环（本地实现与测试完成；部署/法律验收待外部）

- Social 侧已提供完整自有数据导出/删除：
  - `GET /social/v1/privacy/export`
  - `DELETE /social/v1/privacy/delete`
- Chat 新增账户导出、显式确认删除、设备/会话退出、密文与收件人包清理以及重启完整性测试。
- Square 新增账户导出、显式确认删除、资料/互动清理、作者内容安全墓碑化以及重启完整性测试。
- 不可变审计链为安全/争议证据保留既有主体引用；具体保留期限和法定例外仍需司法辖区法律审批。部署后的三服务协调编排仍需中央验收。
...
- **跨产品隐私擦除**：Social、Chat、Square 的本地接口和持久化行为已覆盖；部署后的中央协调调用、失败补偿和法律保留策略仍待验收。
