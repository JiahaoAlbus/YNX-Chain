...
| iOS source/project | Swift frontend parse plus plist/pbxproj lint for both | pass-local source only |
| iOS build/install/cold | `xcodebuild` and `simctl` absent because active developer directory is CommandLineTools | not-run-current-host |
