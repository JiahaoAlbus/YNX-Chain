...
Consume `docs/bridge/consumer-integration-manifest.json`, `docs/bridge/consumer-lifecycle-vectors.json`, `release/integration/ynx-bridge-contract.json`, and `docs/bridge/CROSS_PRODUCT_TEST_VECTORS.json`. Products must fail closed when the provider route is unavailable, when the canonical Wallet/App Gateway session is absent or expired, when the runtime contract is not schema `7` / state machine `ynx.bridge.lifecycle.v1`, or when evidence has not reached `destination_available` with `destinationAssetAvailable=true`.
