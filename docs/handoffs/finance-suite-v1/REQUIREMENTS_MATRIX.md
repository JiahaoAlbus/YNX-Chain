...
| --- | --- | --- | --- |
| DEX | Chain-native swap, add/remove liquidity, LP shares, exact-in/out, fees, slippage, minimum received, approval and explicit Wallet confirmation | proven-public historically; public availability currently unverified | `docs/evidence/dex/native-public-lifecycle-2026-08-11.json`; 2026-08-13 HTTPS and SSH probes timed out, so availability must be re-established before a fresh public claim |
...
| DEX | Multi-hop routing, Strategy Vault, FairFlow, LP protection, kill/revoke/emergency exit | partial / missing-public | Advanced work exists on `codex/final-dex` but diverges from the public native integration branch; port and reconcile without removing native public evidence |
...
| Shared | Multi-user durable storage, rate limits, health/readiness/metrics/logs, retry/reconnect/RPC failover | partial | Product-specific implementations exist with uneven evidence; a cross-product concurrent/restart drill is still required |
