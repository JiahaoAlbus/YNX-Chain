...
| Wallet Auth / Wallet | `codex/ecosystem-wallet-auth` at `bb8ef9924d71c1f991cb1facdfc06ab2d60045c8` | Owned-path review, isolated protocol 13/13, App 13/13, type, product, audit, artifact-hash, and secret checks pass, but total-control protocol review did not accept the candidate | Bind Gateway challenge scopes and expiry exactly to the Wallet approval; make the Android Keystore device-key algorithm/encoding and signature domain match the shared Gateway verifier; then rerun the real cross-App proof before queueing. Central registry/Gateway integration, production signing, native iOS build, physical-device proof, and external audit remain later gates |
