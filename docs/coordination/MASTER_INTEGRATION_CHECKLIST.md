...
The current 23-product gate-by-gate inventory, registered-worktree audit,
platform targets, localization requirements, and rework ownership are maintained
in `docs/coordination/PRODUCT_ACCEPTANCE_MATRIX.md`.
...
| Exchange | `codex/ecosystem-exchange` initially reviewed at `ee1c3f41656be403633d01468ec79af702deb7bf`; clean worktree now at `ff2b6b4ea877a0f708aff5fedc24b3fc7f40762d` pending incremental review | Earlier owned-path review, isolated Exchange Go race, UI 5/5, smoke, and secret checks passed; the newer tip has not yet been promoted and must not be reset to the old candidate | Review the exact increment, then integrate central Wallet Auth and audited custody/indexer controls; keep `YUSD_TEST` non-token and withdrawal broadcast/cross-chain unavailable until real adapters, approvals, deployment, and proof exist |
...
All 15 original product tasks were returned for targeted rework on 2026-07-16.
The rework requires the same original branch, preservation of accepted behavior,
complete product and security states, native/installable platform evidence where
appropriate, exact central integration requests, and a shared auditable
12-language foundation with Arabic RTL. Rework dispatch is not candidate
