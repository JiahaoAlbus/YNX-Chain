...
| Public APIs | `/health`, `/version`, `/prices`, provider registry, replay, and signed ingestion handlers exist; remote evidence proves a limited-source HTTPS deployment that returns degraded 0/3 health and fail-closed prices | Public limited-source control plane verified; authoritative publication pending |
...
| Consumer validation SDKs | Go and TypeScript SDKs reject schema mismatch, unknown fields, stale/future data, unsafe quality, insufficient sources, low confidence/coverage, malformed lineage, unsafe derivations, remote plain HTTP, unbounded responses, and clients without timeouts; both consume canonical acceptance vectors | Implemented and locally tested; immutable package hosting/provenance pending |
| Consumer CLI | `ynx-oracle-cli` binds market/type/version/age/confidence/coverage policy and emits JSON only after strict Go SDK validation | Implemented and race-tested; packaged artifact/provenance pending |
...
| Web/PWA docs and 12-language accessibility | Independent `/oracle` production build, SSR tests, PWA shell, 12 localized runtime/risk vocabularies, Arabic RTL, themes, focus, reduced motion, and 390px responsive rules; real-Chrome keyboard, semantic-label, 200% text and viewport audit | Implemented and locally browser-tested; manual VoiceOver/TalkBack and public-origin audit pending |
