...
`make production-custody-review-packet` reads only the mode-restricted public manifest and ceremony status supplied through `YNX_PRODUCTION_SERVICE_SIGNER_PUBLIC_MANIFEST` and `YNX_PRODUCTION_SERVICE_SIGNER_CEREMONY_STATUS`. It creates a mode-`0600`, default-false independent review template without reading key files. `make production-custody-review-check` proves source-manifest binding, non-overwrite behavior, incomplete-review rejection, and exact `sha256:<digest>` output. A real reviewer must keep evidence fields as non-secret references, set every assertion only after observing the external procedure, and keep the review validity within seven days. The review hash can then be copied into a transaction approval as `custodyEvidence`; free-form text is rejected. The same reviewed file must be supplied as `PUBLIC_BFT_CUSTODY_REVIEW_FILE` during a freeze rehearsal or full cutover. Runtime validation rejects a missing/tampered file or any reviewer, commit, signer-manifest, assertion, expiry, or hash mismatch.
