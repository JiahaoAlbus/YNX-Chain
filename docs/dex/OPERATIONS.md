...
The state-v5/cursor-v6 forward migration and isolated legacy rollback procedure are defined in `MIGRATION_COMPATIBILITY.md`. `ynx-dex-recovery` provides `backup`, `verify`, `restore` and isolated `drill` commands for an exact state/cursor pair; all three HMAC secrets and the verified deployment bindings are required. Never start old and new binaries against the same writable state or cursor paths.
