...
Set `DEX_STRATEGY_VAULT_ADDRESS` and `DEX_FAIRFLOW_ADDRESS` only from the same verified deployment manifest. Cursor v3 binds both addresses and rejects substitution. Enabling either source from a v1 cursor, or enabling FairFlow from a v2 cursor, preserves an exact mode-0600 versioned backup and rewinds to `DEX_INDEXER_START_BLOCK` so earlier actions are not skipped. The versioned `/v1/vault/actions` and `/v1/fairflow/events` APIs expose confirmed logs only and must not be used as substitutes for direct current-state RPC reads. FairFlow clients should pair indexed history with the SDK's fresh authoritative RPC state parser before building a request.
...
The schema-v3 forward migration and isolated v1/v2 rollback procedure are defined in `MIGRATION_COMPATIBILITY.md`. Never start old and new binaries against the same writable state or cursor paths.
