...
The original identities remain in `ynxNativeIdentity.from` and `.to`. The legacy `ynxNativeTransaction` remains exactly four fields: `type`, `amountYNXT`, `feeYNXT`, and `nonce`, so already-installed strict wallet parsers continue to work. The nested `identityProjection` declares version, domain, scheme, which identities are systems, and `systemAddressesAreDisplayOnly=true`. No persisted ledger identifier, transaction hash, account, or balance changes. Native APIs retain their original representation.
