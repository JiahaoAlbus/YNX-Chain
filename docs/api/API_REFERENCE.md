...
Core: `GET /health`, `GET /status`, `GET /metrics`, `GET /blocks/latest`, `GET /txs/{hash}`, `GET /accounts/{address}`, `GET /validators`, `POST /validators/{address}/heartbeat`.

`GET /status` includes `validatorCount`, `readyValidatorCount`, and `validatorPeerReadiness` so node operators and public proof checks can distinguish configured validators from validators with observed local readiness evidence.
