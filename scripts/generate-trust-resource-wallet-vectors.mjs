...
const packageRoot = process.env.YNX_WALLET_AUTH_PACKAGE || path.resolve("../YNX Chain Wallet Auth/packages/wallet-auth");
...
for (const product of products) {
  ...
  await fs.writeFile(path.join(out,"canonical-wallet-v1-test-vector.json"),`${JSON.stringify({testOnly:true,generatedBy:"@ynx-chain/wallet-auth",registryEntry:r,authorizationRequest:parsed,walletApproval,gatewayCompletion,expectedSession},null,2)}\n`);
}
