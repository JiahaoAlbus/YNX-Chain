...
if (acceptance.decision !== "ACCEPTED_FOR_SINGLE_USE_CONTROLLED_V2_RUNTIME_TRANSACTION" || acceptance.candidate?.sourceCommit !== source || acceptance.candidate?.walletAuthTree !== tree || acceptance.candidate?.scopeFiles !== 7) fail("candidate acceptance identity mismatch");
...
if (lease.status !== "ISSUED_NOT_STARTED" || lease.singleUse !== true || lease.authorization?.executionAuthorized !== true || lease.source?.candidateRuntimeCommit !== source || lease.source?.candidateWalletAuthTree !== tree) fail("new lease identity mismatch");
...
if (evidence.sourceCommit !== source || evidence.walletAuthTree !== tree || evidence.changedFiles !== 7 || evidence.review?.fiveV2RoutesMounted !== true || evidence.review?.candidateObservedPublic !== false) fail("review evidence mismatch");
...
console.log(`PASS ${lease.leaseId}: exact ${source.slice(0,12)} seven-file v2 mount is accepted under a fresh unstarted registry-bound lease; all public/client/product aggregate gates remain false`);
