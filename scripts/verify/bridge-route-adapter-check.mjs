...
if (manifest.publicRead?.routes?.path !== "/bridge/routes" || manifest.publicRead?.routes?.source !== "ynx-bridge-route-registry" || manifest.publicRead?.routes?.quotesExecutable !== false || manifest.publicRead?.routes?.deployedPublic !== true || manifest.runtimeTruth?.liveBridge !== false || manifest.runtimeTruth?.externalSubmissionEnabled !== false || manifest.runtimeTruth?.providerStatus !== "connected-live-provider-api-route-execution-disabled" || manifest.runtimeTruth?.providerAPIConnected !== true || manifest.runtimeTruth?.contractStatus !== "unavailable-no-verified-contract-deployment" || manifest.runtimeTruth?.deployedPublic !== false) fail("route catalog handoff conflates public reads with executable Bridge availability");
if (manifest.publicRead?.providers?.supportedExternalProviderAPIConnected !== true || manifest.publicRead?.providers?.credentialsConfigured !== false || manifest.publicRead?.providers?.agreementApproved !== false || manifest.publicRead?.providers?.contractsConfigured !== false || manifest.publicRead?.providers?.routeAvailable !== false || manifest.publicRead?.providers?.deployedPublic !== true) fail("provider catalog handoff overclaims route execution");
if (manifest.publicRead?.assets?.contractMetadataVerified !== false || manifest.publicRead?.assets?.supportedExternalRouteContractMetadataVerified !== true || manifest.publicRead?.assets?.externalExecutionEnabled !== false || manifest.publicRead?.assets?.deployedPublic !== true) fail("asset catalog handoff overclaims YNX contract verification");
if (manifest.publicRead?.status?.externalBridgeAvailable !== false || manifest.publicRead?.status?.providerConnected !== false || manifest.publicRead?.status?.providerAPIConnected !== true || manifest.publicRead?.status?.deployedPublic !== true) fail("status handoff conflates provider API connectivity with Bridge availability");
