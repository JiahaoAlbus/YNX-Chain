...
curl -fsS http://127.0.0.1:6420/node/identity | node -e 'const data=JSON.parse(require("fs").readFileSync(0,"utf8")); const build=data.build || {}; if (!data.peerSyncFreshness || typeof data.expectedValidatorCount !== "number" || typeof data.peerSyncTargetCount !== "number" || typeof build.commit !== "string" || !build.commit || typeof build.release !== "string" || !build.release || typeof build.buildTime !== "string" || !build.buildTime) { console.error(`missing node identity endpoint: ${JSON.stringify(data)}`); process.exit(1); }'
curl -fsS http://127.0.0.1:6420/status | node -e 'const data=JSON.parse(require("fs").readFileSync(0,"utf8")); const build=data.build || {}; if (typeof build.commit !== "string" || !build.commit || typeof build.release !== "string" || !build.release || typeof build.buildTime !== "string" || !build.buildTime) { console.error(`missing status build identity: ${JSON.stringify(data)}`); process.exit(1); }'
