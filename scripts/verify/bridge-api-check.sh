...
gateway_key="bridge-gateway-api-check-key"
...
bridge_env=(
  ...
  YNX_BRIDGE_QUOTE_SEAL_KEY="bridge-api-check-quote-seal-key-000001"
...
curl -fsS "$url/bridge/assets" | node -e 'const d=JSON.parse(require("fs").readFileSync(0,"utf8"));const classes=d.assets?.map(a=>a.assetClass).sort().join(",");if(d.source!=="ynx-bridge-asset-registry"||d.assets?.length!==2||classes!=="testnet-stablecoin,wrapped-test-asset"||d.assets.some(a=>a.availability!=="unavailable"||a.contract!==null||a.decimals!==null||a.contractVerified!==false||a.externalExecutionEnabled!==false||a.allowlistedForCoordinatorIntent!==true))throw new Error(`bad fail-closed asset catalog ${JSON.stringify(d)}`)'
curl -fsS "$url/bridge/status" | node -e 'const d=JSON.parse(require("fs").readFileSync(0,"utf8"));if(d.source!=="ynx-bridge-status"||d.coordinatorState!=="available-local-coordinator"||d.externalBridgeState!=="unavailable"||d.providerConnection!=="not-connected"||d.externalSubmissionEnabled!==false||d.userAssetMovementEnabled!==false||d.officialStablecoinRouteAvailable!==false||d.deployedPublic!==false||d.reconciliation?.independentVerification!==false||d.capabilities?.quoteGeneration!==true||d.capabilities?.quoteExecution!==false||d.capabilities?.walletReviewGeneration!==true||d.capabilities?.refundExecution!==false||d.capabilities?.disputeRecording!==true||d.capabilities?.emergencyExitExecution!==false||d.support?.configured!==false||d.support?.publicStatusUrl!==null)throw new Error(`bad truthful product status ${JSON.stringify(d)}`)'
BRIDGE_URL="$url" node --input-type=module -e 'import {YNXBridgeClient} from "./sdk/bridge/index.js";const c=new YNXBridgeClient({baseURL:process.env.BRIDGE_URL});const [h,t,r,a,s]=await Promise.all([c.getHealth(),c.getTransparency(),c.getRoutes(),c.getAssets(),c.getStatus()]);if(h.liveBridge!==false||t.routes.length!==1||r.routes[0].executable!==false||a.assets.length!==2||s.deployedPublic!==false)process.exit(1)'

quote_body='{"sourceChain":"ethereum-sepolia","sourceAsset":"sepolia-usdc","destinationChain":"ynx_6423-1","destinationAsset":"ynx-usdc","amount":"100","sender":"0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb","recipient":"ynx1recipient000000000000000000000000000001"}'
quote="$(curl -fsS -X POST "$url/bridge/quotes" -H "X-YNX-Bridge-Key: $api_key" -H 'content-type: application/json' -d "$quote_body")"
printf '%s' "$quote" | node -e 'const d=JSON.parse(require("fs").readFileSync(0,"utf8"));if(d.schemaVersion!==1||d.source!=="ynx-bridge-quote-runtime"||!/^sha256:[0-9a-f]{64}$/.test(d.quoteDigest)||!d.id?.startsWith("quote_")||!d.nonce?.startsWith("quote_nonce_")||d.routeId===undefined||d.amount!=="100"||d.availability!=="unavailable"||d.executable!==false||d.failureStatus!=="provider-or-contract-route-unavailable"||d.fees?.status!=="unavailable-no-executable-route"||d.fees?.providerFee!==null||d.fees?.hiddenSpread!==false||d.refund?.available!==false||d.userSigning!=="canonical-wallet-required")throw new Error(`bad bridge quote ${JSON.stringify(d)}`)'
review_body="$(QUOTE="$quote" node -e 'process.stdout.write(JSON.stringify({quote:JSON.parse(process.env.QUOTE)}))')"
session_expires="$(node -e 'process.stdout.write(new Date(Date.now()+30*60*1000).toISOString())')"
review="$(curl -fsS -X POST "$url/bridge/wallet-reviews" -H "X-YNX-Bridge-Gateway-Key: $gateway_key" -H 'X-YNX-App-Gateway: 1' -H 'X-YNX-App-Product: ynx-wallet' -H 'X-YNX-App-Session-ID: session-api-review-001' -H 'X-YNX-App-Session-Account: ynx1walletreview00000000000000000000000000001' -H 'X-YNX-App-Session-Device: device-api-review-001' -H 'X-YNX-Device-ID: device-api-review-001' -H "X-YNX-App-Session-Expires-At: $session_expires" -H 'X-YNX-App-Scope: bridge:review:create' -H 'content-type: application/json' -d "$review_body")"
printf '%s' "$review" | node -e 'const d=JSON.parse(require("fs").readFileSync(0,"utf8"));if(d.schemaVersion!==1||d.source!=="ynx-bridge-wallet-review-runtime"||!/^sha256:[0-9a-f]{64}$/.test(d.reviewDigest)||!d.id?.startsWith("review_")||d.product!=="ynx-wallet"||d.status!=="blocked"||d.approvalAllowed!==false||d.walletSignatureRequired!==false||d.sourceSubmissionAllowed!==false||d.failureStatus!=="provider-or-contract-route-unavailable"||d.credentialBoundary!=="wallet-signs-source-intent-bridge-and-gateway-never-hold-user-private-key")throw new Error(`bad Wallet review ${JSON.stringify(d)}`)'
...
transfer_id="$(printf '%s' "$created" | node -e 'const d=JSON.parse(require("fs").readFileSync(0,"utf8"));const e=d.transfer?.lifecycle?.[0];if(d.replayed||d.transfer?.status!=="pending_attestations"||d.transfer?.phase!=="source_submitted"||d.transfer?.exposureStatus!=="open"||d.transfer?.externalSubmissionEnabled!==false||e?.sequence!==1||e?.phase!=="source_submitted"||e?.source!=="coordinator-source-event"||e?.coverage!=="coordinator-recorded-event-not-independent-chain-proof")throw new Error(`bad create ${JSON.stringify(d)}`);process.stdout.write(d.transfer.id)')"
...
curl -fsS -X POST "$url/bridge/reconciliations" -H "X-YNX-Bridge-Key: $api_key" -H 'content-type: application/json' -d "{\"idempotencyKey\":\"bridge-check-reconcile-002\",\"sourceChain\":\"ethereum-sepolia\",\"destinationChain\":\"ynx_6423-1\",\"sourceAsset\":\"sepolia-usdc\",\"destinationAsset\":\"ynx-usdc\",\"locked\":\"100\",\"burned\":\"0\",\"minted\":\"100\",\"released\":\"0\",\"evidenceRef\":\"fixture:bridge-api-check-balanced\",\"observedAt\":\"$observed_at\"}" | node -e 'const d=JSON.parse(require("fs").readFileSync(0,"utf8"));if(d.replayed||d.reconciliation?.balanced!==true||d.reconciliation?.difference!=="0")throw new Error(`bad balanced reconciliation ${JSON.stringify(d)}`)'
replayed_reconciliation="$(curl -fsS -X POST "$url/bridge/reconciliations" -H "X-YNX-Bridge-Key: $api_key" -H 'content-type: application/json' -d "{\"idempotencyKey\":\"bridge-check-reconcile-001\",\"sourceChain\":\"ethereum-sepolia\",\"destinationChain\":\"ynx_6423-1\",\"sourceAsset\":\"sepolia-usdc\",\"destinationAsset\":\"ynx-usdc\",\"locked\":\"100\",\"burned\":\"0\",\"minted\":\"90\",\"released\":\"0\",\"evidenceRef\":\"fixture:bridge-api-check\",\"observedAt\":\"$observed_at\"}")"
FIRST_RECONCILIATION="$reconciliation" REPLAYED_RECONCILIATION="$replayed_reconciliation" node -e 'const first=JSON.parse(process.env.FIRST_RECONCILIATION);const replay=JSON.parse(process.env.REPLAYED_RECONCILIATION);if(!replay.replayed||JSON.stringify(first.reconciliation)!==JSON.stringify(replay.reconciliation))throw new Error(`reconciliation replay changed ${JSON.stringify(replay)}`)'
curl -fsS "$url/bridge/transparency" | node -e 'const d=JSON.parse(require("fs").readFileSync(0,"utf8"));const r=d.routes?.[0];if(r.coordinatorOutstanding!=="100"||r.lastReconciliation?.difference!=="0"||r.lastReconciliation?.balanced!==true)throw new Error(`bad exposed reconciliation ${JSON.stringify(d)}`)'
...
replayed_reconciliation="$(curl -fsS -X POST "$url/bridge/reconciliations" -H "X-YNX-Bridge-Key: $api_key" -H 'content-type: application/json' -d "{\"idempotencyKey\":\"bridge-check-reconcile-001\",\"sourceChain\":\"ethereum-sepolia\",\"destinationChain\":\"ynx_6423-1\",\"sourceAsset\":\"sepolia-usdc\",\"destinationAsset\":\"ynx-usdc\",\"locked\":\"100\",\"burned\":\"0\",\"minted\":\"90\",\"released\":\"0\",\"evidenceRef\":\"fixture:bridge-api-check\",\"observedAt\":\"$observed_at\"}")"
FIRST_RECONCILIATION="$reconciliation" REPLAYED_RECONCILIATION="$replayed_reconciliation" node -e 'const first=JSON.parse(process.env.FIRST_RECONCILIATION);const replay=JSON.parse(process.env.REPLAYED_RECONCILIATION);if(!replay.replayed||JSON.stringify(first.reconciliation)!==JSON.stringify(replay.reconciliation))throw new Error(`restart reconciliation replay changed ${JSON.stringify(replay)}`)'
...
echo "bridge-api-check passed: truthful product status, fail-closed route and asset catalogs, persistent lifecycle and relayer evidence revalidation, exact reconciliation replay, replay/conflict, limits/delay, pause/resume, reconciliation/transparency, data export/retention hold, auth, tracing, truthful metrics, and mode-0600 state"
