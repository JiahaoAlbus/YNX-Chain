...
BRIDGE_URL="$url" node --input-type=module -e 'import {YNXBridgeClient} from "./sdk/bridge/index.js";const c=new YNXBridgeClient({baseURL:process.env.BRIDGE_URL});const [h,t,r,a]=await Promise.all([c.getHealth(),c.getTransparency(),c.getRoutes(),c.getAssets()]);if(h.liveBridge!==false||t.routes.length!==1||r.routes[0].executable!==false||a.assets.length!==2)process.exit(1)'
