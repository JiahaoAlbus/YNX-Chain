...
if (acceptance.integrationVerification?.walletAuthTestsPassed !== 119 || acceptance.integrationVerification?.walletAuthTestsFailed !== 0 || acceptance.integrationVerification?.focusedTestsPassed !== 4 || acceptance.integrationVerification?.diffCheckPassed !== true || acceptance.integrationVerification?.productionStateCompatibilityPreflightPassed !== false) fail("exact-head verification mismatch");
...
for (const field of ["preflightComplete", "productionStateCompatibilityPreflightComplete", "backupComplete", "deploymentStarted", "deploymentComplete", "rollbackDrillComplete", "runtimeAcceptanceComplete", "installedClientAcceptanceComplete"]) {
...
console.log(`PASS ${acceptance.acceptanceId}: accepted source ${acceptance.candidate.candidateRuntimeCommit.slice(0, 12)}, lease ${lease.leaseId} expires ${lease.expiresAt}, public remains false`);
