  ...
  section(
    ...
    [
      ...
      nodeFailures.some((finding) => finding.classification === "host-key-mismatch")
        ? "- For host-key-mismatch nodes, run `make host-key-repair-plan`, confirm the presented fingerprints from the cloud console or another trusted channel, write the confirmed values to ignored `.host-key-approvals.json`, then require `make host-key-approval-check` to pass before replacing any known_hosts entry."
