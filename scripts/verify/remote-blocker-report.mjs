  ...
  section(
    ...
    [
      ...
      nodeFailures.some((finding) => finding.classification === "host-key-mismatch")
        ? "- For host-key-mismatch nodes, run `make host-key-repair-plan` and `make host-key-approval-request`, confirm the presented fingerprints from the cloud console or another trusted channel, write the confirmed values to ignored `.host-key-approvals.json`, require `make host-key-approval-check` to pass, run `make host-key-approved-repair-dry-run`, and only then run `make host-key-approved-repair`."
