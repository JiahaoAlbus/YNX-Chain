...
for _ in {1..40}; do
  ...
  if printf '%s' "$syncs" | node -e 'const data=JSON.parse(require("fs").readFileSync(0,"utf8")); const sync=data.syncs?.find((item)=>item.source==="ynx_val_primary" && item.target==="ynx_val_secondary" && item.evidence==="peer-rpc-poll:http://127.0.0.1:6461/status"); process.exit(sync?.status === "synced" && sync?.readinessReady === true && sync?.readinessFresh === true && sync?.readinessStatus === "ready" && !sync?.readinessReason && sync?.readinessRecordKey === `${sync.source}->${sync.target}` && Math.abs(Number(sync?.lagBlocks)) <= 1 && typeof sync?.readinessSampledAt === "string" ? 0 : 1);' >/dev/null 2>&1; then
    break
  fi
  sleep 0.25
done
syncs="$(curl -fsS "$YNX_REST_URL/validators/peer-sync")"
printf '%s' "$syncs" | node -e 'const data=JSON.parse(require("fs").readFileSync(0,"utf8")); const sync=data.syncs?.find((item)=>item.source==="ynx_val_primary" && item.target==="ynx_val_secondary"); const samples=new Set((data.syncs||[]).map((item)=>item.readinessSampledAt)); if (!sync || sync.status !== "synced" || sync.readinessReady !== true || sync.readinessFresh !== true || sync.readinessStatus !== "ready" || sync.readinessReason || sync.readinessRecordKey !== `${sync.source}->${sync.target}` || Math.abs(Number(sync.lagBlocks)) > 1 || !sync.evidence?.startsWith("peer-rpc-poll:") || samples.size !== 1) { console.error(`automatic peer sync readiness is not coherent: ${JSON.stringify(data)}`); process.exit(1); }'

status="$(curl -fsS "$YNX_REST_URL/status")"
printf '%s' "$status" | node -e 'const data=JSON.parse(require("fs").readFileSync(0,"utf8")); const sync=data.validatorPeerSync || {}; const ready=data.validatorPeerReadiness||{}; const identityReady=data.nodeIdentity?.validatorReadiness||{}; const freshness=data.nodeIdentity?.peerSyncFreshness||{}; if (data.readyValidatorCount !== 2 || ready.status !== "ready" || ready.ready !== 2 || ready.notReady !== 0 || ready.total !== 2 || !Array.isArray(ready.records) || ready.records.some((record)=>record.ready !== true || record.status !== "ready" || record.reason) || ready.sampledAt !== identityReady.sampledAt || ready.sampledAt !== freshness.generatedAt || data.validatorPeerDiscovery?.expected < 2 || data.validatorPeerDiscovery?.observed < 1 || sync.total < 1 || sync.synced < 1 || sync.lagging !== 0) { console.error(`validator readiness/discovery/sync summary missing or inconsistent: ${JSON.stringify(data)}`); process.exit(1); }'
identity="$(curl -fsS "$YNX_REST_URL/node/identity")"
printf '%s' "$identity" | node -e 'const data=JSON.parse(require("fs").readFileSync(0,"utf8")); const freshness=data.peerSyncFreshness || {}; const ready=data.validatorReadiness||{}; if (data.validatorAddress !== "ynx_val_primary" || data.validatorRole !== "primary validator" || data.expectedValidatorCount !== 2 || data.peerSyncTargetCount !== 1 || !data.blockProductionEnabled || data.replicationMode !== "authoritative_producer" || !Array.isArray(data.peerSyncTargetAddresses) || data.peerSyncTargetAddresses[0] !== "ynx_val_secondary" || freshness.targetCount !== 1 || freshness.missing !== 0 || freshness.stale !== 0 || freshness.future !== 0 || freshness.invalid !== 0 || freshness.fresh !== 1 || freshness.status !== "synced" || ready.status !== "ready" || ready.ready !== 2 || ready.notReady !== 0 || ready.sampledAt !== freshness.generatedAt) { console.error(`node identity/freshness/readiness missing: ${JSON.stringify(data)}`); process.exit(1); }'
printf '%s' "$status" | node -e 'const data=JSON.parse(require("fs").readFileSync(0,"utf8")); const identity=data.nodeIdentity || {}; if (identity.validatorAddress !== "ynx_val_primary" || identity.peerSyncFreshness?.missing !== 0 || identity.peerSyncFreshness?.stale !== 0 || identity.peerSyncFreshness?.future !== 0 || identity.peerSyncFreshness?.invalid !== 0 || identity.validatorReadiness?.status !== "ready" || identity.validatorReadiness?.sampledAt !== data.validatorPeerReadiness?.sampledAt) { console.error(`status node identity/freshness missing: ${JSON.stringify(data)}`); process.exit(1); }'
