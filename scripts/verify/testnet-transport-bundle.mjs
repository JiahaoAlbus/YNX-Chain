...
export function makeBundle({events, summary, inputSHA256}) {
  ...
  return {schema:"ynx-transport-shareable-bundle/v1",runId:start.runId,vantage:start.vantage,pathMode,
    ...
    sourceSHA256:Object.fromEntries(Object.entries(start.sourceSHA256 ?? {}).filter(([k,v]) => ["testnet-transport-monitor.mjs","testnet-transport-host-snapshot.py","testnet-alias-preflight.mjs","testnet-transport-client-path.mjs"].includes(k) && hash(v))),
    clientPath:start.clientPath ? {routeReadable:start.clientPath.routeReadable===true,
      interfaceFamily:["utun","tun","tap","wg","ipsec","ppp","bridge","en","eth","lo","other"].includes(start.clientPath.interfaceFamily)?start.clientPath.interfaceFamily:null,
      interfaceMTU:integer(start.clientPath.interfaceMTU),routeUsesKnownTunnelInterface:typeof start.clientPath.routeUsesKnownTunnelInterface==="boolean"?start.clientPath.routeUsesKnownTunnelInterface:null,
      environmentProxyPresent:start.clientPath.environmentProxyPresent===true,systemProxyReadable:start.clientPath.systemProxyReadable===true,
      systemProxyFlags:start.clientPath.systemProxyFlags?Object.fromEntries(["HTTPEnable","HTTPSEnable","SOCKSEnable","ProxyAutoConfigEnable","ProxyAutoDiscoveryEnable"].map(k=>[k,start.clientPath.systemProxyFlags[k]===true])):null,
      independentVantageProven:false,tunnelAbsenceProven:false} : null,
