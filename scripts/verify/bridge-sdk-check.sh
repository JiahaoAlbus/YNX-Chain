...
node -e 'const p=require("./sdk/bridge/package.json");if(p.private!==true||p.name!=="@ynx-chain/bridge-sdk"||p.version!=="0.2.0"||p.types!=="./index.d.ts"||p.files.join(",")!=="README.md,index.js,index.d.ts"||p.exports?.["."]?.types!=="./index.d.ts")process.exit(1)'
node -e 'const fs=require("node:fs");const d=fs.readFileSync("sdk/bridge/index.d.ts","utf8");for(const required of ["destination_mint_release_confirmed","destination_available","destinationAssetAvailable","getVersion","getStateMachine"])if(!d.includes(required))process.exit(1)'
