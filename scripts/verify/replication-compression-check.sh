...
go test ./internal/chain -run 'TestReplicationBatch(Converges|CatchesUp|Checkpoints|Rejects)' -count=1
grep -Fq 'gzip.NewWriterLevel' internal/api/server.go
grep -Fq 'MaxReplicationBatchBlocks = 4096' internal/chain/replication.go
grep -Fq 'operationalCheckpointInterval = 5 * time.Minute' internal/chain/replication.go
