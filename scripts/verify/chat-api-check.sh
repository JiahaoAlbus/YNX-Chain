...
printf '%s' "$health" | node -e 'const d=JSON.parse(require("fs").readFileSync(0,"utf8"));if(!d.ok||d.service!=="ynx-chatd"||d.nativeAddressDefault!==true||d.plaintextStored!==false||d.persistence!=="atomic-json-mode-0600"||d.rotationCount!==0||d.truthfulStatus!=="local-bounded-chat-core-not-remote-deployed")throw new Error(`bad chat health ${JSON.stringify(d)}`)'
...
grep -Fq 'ynx_chat_device_rotations 0' <<<"$metrics"
grep -Fq 'ynx_chat_remote_deployed 0' <<<"$metrics"
[[ "$(stat -f %Lp "$state" 2>/dev/null || stat -c %a "$state")" == 600 ]]
! grep -Fq "$api_key" "$state" "$log"

echo "chat-api-check passed: signed ynx1 devices, exact multi-device envelope fan-out, per-device acknowledgement, bounded rotation/recovery, replay/conflict rejection, restart persistence, truthful health, and mode-0600 ciphertext-only state"
