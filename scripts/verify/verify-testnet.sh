...
check_replication_convergence() {
  ...
  check_replica() {
    ...
    printf '%s' "$identity" | node -e 'const x=JSON.parse(require("fs").readFileSync(0,"utf8")),r=x.replication||{}; if (x.blockProductionEnabled !== false || x.replicationMode !== "authoritative_follower" || !String(x.replicationSource||"").includes("'"$PRIMARY_NODE_HOST"':6420") || !r.configured || r.status!=="synced" || r.catchingUp!==false || r.fresh!==true || r.localHeight!==Number("'"$target_height"'") || r.sourceHeight!==Number("'"$target_height"'") || r.localBlockHash!=="'"$target_hash"'" || r.sourceBlockHash!=="'"$target_hash"'") process.exit(1);' || return 1
