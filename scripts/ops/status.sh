...
status_node() {
  ...
  if [[ "$kind" == "full" ]]; then
    endpoints="$endpoints; curl -fsS http://127.0.0.1:6426/health || true; curl -fsS http://127.0.0.1:6427/health || true; curl -fsS http://127.0.0.1:6428/health || true; curl -fsS http://127.0.0.1:6429/health || true; curl -fsS http://127.0.0.1:6430/health || true; curl -fsS http://127.0.0.1:6431/health || true; curl -fsS http://127.0.0.1:6432/health || true"
  fi
