...
backup_node() {
  ...
  if [[ "$kind" == "full" ]]; then
    ...
    extra_paths="$extra_paths /var/lib/ynx-chain/app-gateway /var/lib/ynx-chain/wallet-gateway /etc/ynx/ynx-wallet-gatewayd.env /etc/systemd/system/ynx-wallet-gatewayd.service"
  fi
