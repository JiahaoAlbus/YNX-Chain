...
rollback_node() {
  ...
  if [[ "$kind" == "full" ]]; then
    checks="$checks && test -x '/opt/ynx-chain/releases/$release/bin/ynx-indexerd' && test -x '/opt/ynx-chain/releases/$release/bin/ynx-explorerd' && test -x '/opt/ynx-chain/releases/$release/bin/ynx-faucetd' && test -x '/opt/ynx-chain/releases/$release/bin/ynx-ai-gatewayd'"
    installs="$installs && sudo install -m 0755 '/opt/ynx-chain/releases/$release/bin/ynx-indexerd' /usr/local/bin/ynx-indexerd && sudo install -m 0755 '/opt/ynx-chain/releases/$release/bin/ynx-explorerd' /usr/local/bin/ynx-explorerd && sudo install -m 0755 '/opt/ynx-chain/releases/$release/bin/ynx-faucetd' /usr/local/bin/ynx-faucetd && sudo install -m 0755 '/opt/ynx-chain/releases/$release/bin/ynx-ai-gatewayd' /usr/local/bin/ynx-ai-gatewayd"
  fi
