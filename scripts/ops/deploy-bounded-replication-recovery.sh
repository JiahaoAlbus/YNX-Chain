...
preflight_node() {
  ...
  remote_exec "$role" "$user" "$host" "$key" "set -euo pipefail; systemctl is-active --quiet ynx-chaind; test -x /usr/local/bin/ynx-chaind; sudo test -r /etc/ynx/ynx-chaind.env; command -v tar >/dev/null; command -v sha256sum >/dev/null || command -v shasum >/dev/null; curl -fsS --max-time 8 http://127.0.0.1:6420/status >/dev/null; echo 'ynx-chaind preflight passed'"
}
