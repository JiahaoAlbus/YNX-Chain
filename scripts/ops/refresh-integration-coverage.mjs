...
const globals = [
  ...
  ["INT-SEC-001", "security-supply-chain", "Verify threat model, boundaries, secret/dependency/license/SAST/DAST/artifact/container scans, SBOM, provenance, reproducibility and build-script allowlist.", "externalBlocked", "Continue autonomous scans and time-bounded controls while requesting the missing Security/SRE owner branch through the central operator-input process.", { evidence: ["release/integration/security/npm-audit-policy.json"], tests: ["npm-audit-policy-check", "npm-audit-policy-check --self-test", "secret-scan", "static-check"], blockedBy: ["codex/final-security-sre and 30-security-sre were not observed"] }],
  ["INT-NPM-AUDIT-001", "security-supply-chain", "Constrain the unfixed Hardhat adm-zip advisory to an exact, development-only, non-runtime graph with zero production vulnerabilities and an automatically expiring exception.", "implementedLocal", "Bind the protected commit, rerun the exact audit policy and obtain Security/SRE acceptance before any production release.", { evidence: ["release/integration/security/npm-audit-policy.json"], tests: ["integration-npm-audit-policy-check", "integration-npm-audit-policy-check-test"], artifact: "release/integration/security/npm-audit-policy.json", blockedBy: ["30-security-sre review before production release"] }],
