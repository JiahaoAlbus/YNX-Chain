...
preflight_key_role() {
  ...
  if [[ "$mode" == "create" ]]; then
    ...
    ynx_ops_ssh "$role" "$user" "$host" "$key" "id -u ynx >/dev/null && sudo test -s '$key_dir/priv_validator_key.json' && sudo test -s '$key_dir/node_key.json' && systemctl is-active ynx-chaind >/dev/null"
  fi
}
