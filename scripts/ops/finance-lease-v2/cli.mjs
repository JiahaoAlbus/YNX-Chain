...
// OpenSSH expands tokens/environment references and parses a whitespace list
// in UserKnownHostsFile. Accept only a single canonical ASCII literal pathname.
function literalSshPath(p){
 need(typeof p==='string'&&/^\/[A-Za-z0-9_.-]+(?:\/[A-Za-z0-9_.-]+)*$/.test(p)&&p.split('/').every(x=>x!=='.'&&x!=='..'),'SSH_LITERAL_PATH');
 return p;
}
export function run(argv){const [mode,...rest]=argv;need(['doctor','verify','issue','transport-plan'].includes(mode),'COMMAND');const o={};for(let i=0;i<rest.length;i+=2){need(['--lease','--trust-root','--context','--approved-payload-sha256','--private-key-file','--output','--known-hosts','--identity-file'].includes(rest[i])&&rest[i+1]&&!Object.hasOwn(o,rest[i]),'ARGUMENTS');o[rest[i]]=rest[i+1];}
 ...
 if(mode==='transport-plan'){
  ...
  const p=literalSshPath(o['--known-hosts']);literalSshPath(o['--identity-file']);const st=fs.lstatSync(p);need(fs.realpathSync(p)===p,'SSH_CANONICAL_PATH');need(st.isFile()&&!st.isSymbolicLink()&&(st.mode&0o022)===0&&st.uid===process.getuid(),'KNOWN_HOSTS_PERMISSIONS');
  ...
  return {status:'LOCALLY_VERIFIED_REMOTE_REVERIFY_REQUIRED',leaseId:verified.leaseId,argv:['/usr/bin/ssh','-F','/dev/null','-T','-p','22','-i',o['--identity-file'],'-o','PermitLocalCommand=no','-o','ProxyCommand=none','-o','ProxyJump=none','-o','ControlMaster=no','-o','ControlPath=none','-o','ForwardAgent=no','-o','ForwardX11=no','-o','ConnectTimeout=10','-o','ServerAliveInterval=15','-o','ServerAliveCountMax=2','-o','BatchMode=yes','-o','IdentitiesOnly=yes','-o','StrictHostKeyChecking=yes','-o','UpdateHostKeys=no','-o','HostKeyAlgorithms=ssh-ed25519','-o','GlobalKnownHostsFile=/dev/null','-o','UserKnownHostsFile='+p,'--','ubuntu@'+verified.target.host,'/usr/bin/sudo','-n','/usr/bin/python3','/opt/ynx/finance-release-v2/executor.py','execute','--lease-id',verified.leaseId],executed:false};
 }
