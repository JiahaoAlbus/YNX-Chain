...

{
  ...
  echo "- Run \`make host-key-approval-template\` to create a blank approval template, fill confirmed fingerprints into ignored \`.host-key-approvals.json\`, require \`make host-key-approval-check\` to pass, then use \`make host-key-approved-repair-dry-run\` before \`make host-key-approved-repair\`."
