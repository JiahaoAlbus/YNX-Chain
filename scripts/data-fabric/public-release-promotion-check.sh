...
jq -n \
  ...
  '{schema:"ynx-data-fabric-cold-start-evidence/v1",commit:$commit,release:$release,target:{os:"linux",architecture:"amd64"},status:"verified",archiveSha256:$archiveSha256,verificationScope:"promotion-contract-test"}' \
  >"$cold_start"
...
run_promotion() {
  ...
  YNX_DATA_FABRIC_COLD_START_EVIDENCE_FILE="$cold_start" \
...
printf '{"status":"verified","commit":"%s","release":"%s","boundaries":["approval","signature","hosting-receipt","https-backread","object-inventory"]}\n' "$commit" "$release"
