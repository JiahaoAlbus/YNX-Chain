...
YNX_WALLET_GATEWAY_DEPLOY_MODE="${YNX_WALLET_GATEWAY_DEPLOY_MODE:-dry-run}"
YNX_WALLET_GATEWAY_DEPLOY_COMMIT="${YNX_WALLET_GATEWAY_DEPLOY_COMMIT:-}"
case "$YNX_WALLET_GATEWAY_DEPLOY_MODE" in
  dry-run | rollback-drill | deploy) ;;
  *) echo "YNX_WALLET_GATEWAY_DEPLOY_MODE must be dry-run, rollback-drill or deploy" >&2; exit 1 ;;
esac
[[ -r "$PRIMARY_NODE_SSH_KEY" ]] || { echo "Primary Testnet SSH key is not readable" >&2; exit 1; }
ynx_require_clean_worktree
source_commit="$(git rev-parse HEAD)"
[[ "$YNX_WALLET_GATEWAY_DEPLOY_COMMIT" == "$source_commit" ]] || { echo "YNX_WALLET_GATEWAY_DEPLOY_COMMIT must equal the exact clean HEAD" >&2; exit 1; }
...
install -m 0755 scripts/deploy/install-wallet-gateway-testnet-remote.sh "$release_root/install-wallet-gateway-testnet-remote.sh"
(
...
chmod 0755 "$release_root/install-wallet-gateway-testnet-remote.sh" "$release_root/wallet-auth/scripts/ynx-wallet-gatewayd.mjs" "$release_root/wallet-auth/scripts/probe-wallet-authorization-rejection.mjs"
...
printf 'walletGatewayPackage=passed\nmode=%s\nrelease=%s\nsourceCommit=%s\nregistryFileSha256=%s\nregistryRuntimeSha256=%s\narchiveSha256=%s\n' "$YNX_WALLET_GATEWAY_DEPLOY_MODE" "$release" "$source_commit" "$registry_file_sha" "$registry_runtime_sha" "$tarball_sha"

if [[ "$YNX_WALLET_GATEWAY_DEPLOY_MODE" == "dry-run" ]]; then exit 0; fi
...
sudo bash '$remote_release_dir/install-wallet-gateway-testnet-remote.sh' '$remote_release_dir' '$source_commit' '$release' '$build_time' '$registry_file_sha' '$registry_runtime_sha' '$YNX_WALLET_GATEWAY_DEPLOY_MODE'
rm -f '$remote_tarball'"
