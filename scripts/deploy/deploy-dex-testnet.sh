...
release="ynx-dex-$short_commit"
build_time="$(node -e 'process.stdout.write(new Date().toISOString())')"
work="$(mktemp -d "${TMPDIR:-/tmp}/ynx-dex-testnet.XXXXXX")"
trap 'rm -rf "$work"' EXIT
release_root="$work/$release"
mkdir -p "$release_root/web" "$release_root/token-lists"
...
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath \
  ...
  -o "$release_root/ynx-dex-indexerd" ./cmd/ynx-dex-indexerd
cp -R apps/dex/dist/. "$release_root/web/"
cp token-lists/dex-testnet.json "$release_root/token-lists/"
install -m 0644 scripts/deploy/install-dex-testnet-remote.sh "$release_root/install-dex-testnet-remote.sh"
...
tar -tzf "$tarball" | grep -Fq "$release/ynx-dex-indexerd"
tar -tzf "$tarball" | grep -Fq "$release/web/index.html"
printf 'dexPackage=passed\nrelease=%s\nsourceCommit=%s\nbinarySha256=%s\nwebTreeSha256=%s\narchiveSha256=%s\n' \
  "$release" "$source_commit" "$binary_sha" "$web_sha" "$tarball_sha"

if [[ "$YNX_DEX_TESTNET_DRY_RUN" == "1" ]]; then
  echo "dexDeploy=dry-run"
  exit 0
fi

remote_tarball="/tmp/$release.tar.gz"
remote_release_dir="/opt/ynx/releases/dex/$release"
remote="$PRIMARY_NODE_USER@$PRIMARY_NODE_HOST"
ynx_transport_scp "dex-primary" "$PRIMARY_NODE_SSH_KEY" "$tarball" "$remote" "$remote_tarball"
remote_command="set -e
chmod 0600 '$remote_tarball'
printf '%s  %s\\n' '$tarball_sha' '$remote_tarball' | sha256sum -c -
sudo install -d -o root -g root /opt/ynx/releases/dex
