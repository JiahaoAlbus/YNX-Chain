...
bash -c 'set -a; . /etc/ynx/ynx-payd.env; set +a; exec runuser -u ynx -- "$1" --http 127.0.0.1:17430 --audit-log "/tmp/$2-preflight-audit.jsonl"' _ "$candidate" "$release" >"$backup/preflight.log" 2>&1 &
preflight=$!
